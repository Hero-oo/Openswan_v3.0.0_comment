...
/* MD5 block update operation. Continues an MD5 message-digest
  ...
  context.
 */
void osMD5Update(vcontext, input, inputLen) void *vcontext;
...
long unsigned int inputLen; /* length of input block */
{
	...
	if (inputLen >= partLen) {
		MD5_memcpy((POINTER)&context->buffer[myindex],
			   (CONSTPOINTER)input, partLen);
	...
	MD5_memcpy((POINTER)&context->buffer[myindex], (CONSTPOINTER)&input[i],
		   inputLen - i);
#endif
}
 ...
 */
void osMD5Final(digest, vcontext) void *vcontext;
