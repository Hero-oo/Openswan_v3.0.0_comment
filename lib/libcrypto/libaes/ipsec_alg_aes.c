...
static int _aes_set_key(struct ipsec_alg_enc *alg, __u8 *key_e, const __u8 *key,
			size_t keysize)
{
	int ret;
...
static int _aes_cbc_encrypt(struct ipsec_alg_enc *alg, __u8 *key_e, __u8 *in,
...
#if CONFIG_KLIPS_ENC_AES_MAC
static int _aes_mac_set_key(struct ipsec_alg_auth *alg, __u8 *key_a,
...
#endif
{
	...
	if (keyminbits)
		ipsec_alg_AES.ixt_common.ixt_support.ias_keyminbits =
			keyminbits;
	if (keymaxbits) {
		ipsec_alg_AES.ixt_common.ixt_support.ias_keymaxbits =
			keymaxbits;
		if (keymaxbits * 8 >
		    ipsec_alg_AES.ixt_common.ixt_support.ias_keymaxbits)
	...
	if (excl_aes)
		ipsec_alg_AES.ixt_common.ixt_state |= IPSEC_ALG_ST_EXCL;
	...
	printk("ipsec_aes_init(alg_type=%d alg_id=%d name=%s): ret=%d\n",
	       ...
	       ipsec_alg_AES.ixt_common.ixt_name, ret);
	if (ret == 0 && test_aes) {
		test_ret = ipsec_alg_test(
			ipsec_alg_AES.ixt_common.ixt_support.ias_exttype,
			ipsec_alg_AES.ixt_common.ixt_support.ias_id, test_aes);
		printk("ipsec_aes_init(alg_type=%d alg_id=%d): test_ret=%d\n",
		       ipsec_alg_AES.ixt_common.ixt_support.ias_exttype,
		       ipsec_alg_AES.ixt_common.ixt_support.ias_id, test_ret);
	}
#if CONFIG_KLIPS_ENC_AES_MAC
	if (auth_id != 0) {
		int ret;
		...
		printk("ipsec_aes_init(alg_type=%d alg_id=%d name=%s): ret=%d\n",
		       ...
		       ipsec_alg_AES_MAC.ixt_common.ixt_name, ret);
		if (ret == 0 && test_aes) {
			test_ret = ipsec_alg_test(
				ipsec_alg_AES_MAC.ixt_common.ixt_support
					.ias_exttype,
				ipsec_alg_AES_MAC.ixt_common.ixt_support.ias_id,
				test_aes);
			printk("ipsec_aes_init(alg_type=%d alg_id=%d): test_ret=%d\n",
		...
		printk(KERN_DEBUG
		       "klips_debug: experimental ipsec_alg_AES_MAC not registered [Ok] (auth_id=%d)\n",
		       auth_id);
	}
#endif /* CONFIG_KLIPS_ENC_AES_MAC */
	return ret;
