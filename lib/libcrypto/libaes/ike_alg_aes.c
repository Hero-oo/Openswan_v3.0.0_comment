...
#ifdef HAVE_LIBNSS
static void do_aes_libnss(u_int8_t *buf, size_t buf_len, u_int8_t *key,
			  size_t key_size, u_int8_t *iv, bool enc)
{
	...
	enccontext = PK11_CreateContextBySymKey(
		ciphermech, enc ? CKA_ENCRYPT : CKA_DECRYPT, symkey, secparam);
	if (enccontext == NULL) {
		loglog(RC_LOG_SERIOUS,
