...
int serpent_set_key(serpent_context *cx, const unsigned char *key, int key_len)
{
	...
	for (i = 0; i < 132; ++i) {
		lk = l_key[i] ^ l_key[i + 3] ^ l_key[i + 5] ^ l_key[i + 7] ^
		     0x9e3779b9 ^ i;
...
int serpent_encrypt(serpent_context *cx, const u8 *in, u8 *out)
{
	u32 *l_key = cx->keyinfo;
...
int serpent_decrypt(serpent_context *cx, const u8 *in, u8 *out)
{
	u32 *l_key = cx->keyinfo;
	const u32 *in_blk = (const u32 *)in;
	u32 *out_blk = (u32 *)out;
