...
#endif
#define HMAC_SET_KEY_IMPL(func_name, hctx_t, blocksize, func_init,    \
			  func_update)                                \
...
#define HMAC_HASH_IMPL(func_name, hctx_t, ctx_t, ahlen, func_update, \
		       func_result)                                  \
	void func_name(hctx_t *hctx, const u_int8_t *dat, int len,   \
		       u_int8_t *hash, int hashlen)                  \
	{                                                            \
		ctx_t ctx;                                           \
		ctx = hctx->ictx;                                    \
		if (dat)                                             \
			func_update(&ctx, dat, len);                 \
		if (hash) {                                          \
