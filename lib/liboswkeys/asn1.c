...
time_t asn1totime(const chunk_t *utctime, asn1_t type)
{
	...
	if ((eot = memchr(utctime->ptr, 'Z', utctime->len)) != NULL) {
		...
		tz_offset = 3600 * tz_hour +
			    60 * tz_min; /* positive time zone offset */
	} else if ((eot = memchr(utctime->ptr, '-', utctime->len)) != NULL) {
		int tz_hour, tz_min;
		...
		tz_offset = -3600 * tz_hour -
			    60 * tz_min; /* negative time zone offset */
	} else {
	...
	{
		const char *format = (type == ASN1_UTCTIME) ?
					     "%2d%2d%2d%2d%2d" :
					     "%4d%2d%2d%2d%2d";

		if (sscanf((const char *)utctime->ptr, format, &t.tm_year,
 ...
 */
bool extract_object(asn1Object_t const *objects, u_int *objectID,
		    chunk_t *object, u_int *level, asn1_ctx_t *ctx)
{
	...
	*object = empty_chunk;
	...
	{
		if (ctx->loopAddr[obj.level] &&
		    ctx->blobs[obj.level + 1].len > 0) {
			*objectID =
				ctx->loopAddr[obj.level]; /* another iteration */
	...
	*level = ctx->level0 + obj.level;
	...
	if ((obj.flags & ASN1_DEF) &&
		...
		/* field is missing */
		DBG(DBG_PARSING, DBG_log("L%d - %s:", *level, obj.name);)
	...
	if (obj.flags & ASN1_OBJ) {
		object->ptr = start_ptr;
		object->len = (size_t)(blob->ptr - start_ptr);
		DBG(ctx->cond, DBG_dump_chunk("", *object);)
		...
		*object = *blob1;
		...
		case ASN1_GENERALIZEDTIME:
			DBG(DBG_PARSING,
			    ...
			    DBG_log("  '%s'",
				    timetoa(&timep, TRUE, tbuf, sizeof(tbuf)));)
			return TRUE;
...
bool is_asn1(chunk_t blob)
{
	...
	if (tag != ASN1_SEQUENCE && tag != ASN1_SET) {
		DBG(DBG_PARSING, DBG_log("  file content is not binary ASN.1");)
		return FALSE;
	}
