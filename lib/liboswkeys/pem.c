...
static bool find_boundary(const char *tag, chunk_t *line)
{
	...
	while (line->len > 0) {
		if (present("-----", line)) {
			DBG(DBG_PARSING, DBG_log("  -----%s %.*s-----", tag,
 ...
 */
static bool pem_decrypt_3des(chunk_t *blob, chunk_t *iv, const char *passphrase)
{
	MD5_CTX context;
	...
	osMD5Init(&context);
	osMD5Update(&context, (const unsigned char *)passphrase,
		    strlen(passphrase));
	...
	oswcrypto.des_ede3_cbc_encrypt((des_cblock *)blob->ptr,
				       (des_cblock *)blob->ptr, blob->len,
				       ks[0], ks[1], ks[2],
				       (des_cblock *)des_iv, FALSE);

	/* determine amount of padding */
	last_padding_pos = blob->ptr + blob->len - 1;
	padding = *last_padding_pos;
	first_padding_pos = (padding > blob->len) ? blob->ptr :
						    last_padding_pos - padding;
	...
	if (pass->prompt && pass->fd != NULL_FD) {
		...
		for (i = 0; i < MAX_PROMPT_PASS_TRIALS; i++) {
			...
			if (pem_decrypt_3des(blob, iv, pass->secret)) {
				pass->prompt(
					RC_SUCCESS,
					"valid passphrase, private key loaded successfully");
				pfree(blob_copy.ptr);
				return NULL;
			}
			...
			pfree(blob->ptr);
			clonetochunk(*blob, blob_copy.ptr, blob_copy.len,
				     "blob copy");
		}
		pass->prompt(RC_LOG_SERIOUS, "%s", ugh);
		pfree(blob_copy.ptr);
...
err_t pemtobin(chunk_t *blob, prompt_pass_t *pass, const char *label, bool *pgp)
{
	...
	while (fetchline(&src, &line)) {
		if (state == PEM_PRE) {
			...
			if (state == PEM_MSG) {
				state = (memchr(line.ptr, ':', line.len) ==
					 NULL) ?
						PEM_BODY :
						PEM_HEADER;
			}
