...
static void RSA_show_key_field(const char *name, MP_INT *num)
{
	int sz;
...
void RSA_show_key_fields(struct private_key_stuff *pks)
{
...
err_t unpack_RSA_public_key(struct RSA_public_key *rsa, const chunk_t *pubkey)
{
	...
	if (pubkey->len - (exponent.ptr - pubkey->ptr) <
	    exponent.len + RSA_MIN_OCTETS_RFC)
...
void calculate_rsa_ckaid(osw_public_key *pub)
{
	if (pub->alg == PUBKEY_ALG_RSA) {
		...
		if (rsa->key_rfc3110.len == 0) {
			...
			rsa->key_rfc3110.ptr = alloc_bytes(
				key3110len,
				"rfc3110 format of public key [created]");
		...
		datatot(pub->key_ckaid, sizeof(pub->key_ckaid), 'G',
			pub->key_ckaid_print_buf,
			sizeof(pub->key_ckaid_print_buf));
	}
}

err_t str2pubkey(const unsigned char *key1, enum pubkey_alg kind,
		 osw_public_key *opk)
{
	err_t ugh = NULL;
	chunk_t pubkey;
	...
	case PUBKEY_ALG_RSA:
		opk->alg = kind;
		ugh = atobytes((const char *)key1, 0, (char *)pubkey.ptr,
