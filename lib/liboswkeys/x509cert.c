...
static const asn1Object_t generalNameObjects[] = {
	...
	{ 0, "uniformResourceIdentifier", ASN1_CONTEXT_S_6,
	  ASN1_OPT | ASN1_BODY }, /* 12 */
...
static const asn1Object_t timeObjects[] = {
	...
	{ 0, "generalizeTime", ASN1_GENERALIZEDTIME,
	  ASN1_OPT | ASN1_BODY }, /*  2 */
...
static const asn1Object_t authorityKeyIdentifierObjects[] = {
	...
	{ 1, "authorityCertIssuer", ASN1_CONTEXT_C_1,
	  ASN1_OPT | ASN1_OBJ }, /*  3 */
	{ 1, "end opt", ASN1_EOC, ASN1_END }, /*  4 */
	{ 1, "authorityCertSerialNumber", ASN1_CONTEXT_S_2,
...
static const asn1Object_t crlDistributionPointsObjects[] = {
	...
	{ 3, "nameRelativeToCRLIssuer", ASN1_CONTEXT_C_1,
	  ASN1_OPT | ASN1_BODY }, /*  5 */
	...
	if (end_id->kind != ID_NONE) /* check for matching subjectAltName */
	{
		...
		while (gn != NULL) {
			...
			if (same_id(&id, end_id)) {
				copy_subject_dn =
					FALSE; /* take subjectAltName instead */
				break;
			}
			gn = gn->next;
		}
			...
			chunk_t *digest)
{
	...
	case OID_SHA224_WITH_RSA: {
		...
		publicKey = (SECKEYPublicKey *)PORT_ArenaZAlloc(
		...
		skip = (issuer_cert->modulus.len > 0 &&
			issuer_cert->modulus.ptr[0] == 0x00) ?
			       1 :
			       0;
		if (skip != 1) {
			DBG(DBG_X509 | DBG_CONTROL,
		...
		if (retVal == SECSuccess) {
			retVal = SECITEM_CopyItem(
				arena, &publicKey->u.rsa.publicExponent,
				&nss_e);
		}
		...
		DBG(DBG_X509 | DBG_CONTROL,
		    ...
		    DBG_dump("NSS adjusted digest sig: ",
			     dsig.data + dsig.len - digest->len, digest->len);
			...
			chunk_t *digest)
{
	...
	case OID_SHA512_WITH_RSA: {
		...
		n_to_mpz(e, issuer_cert->publicExponent.ptr,
			 issuer_cert->publicExponent.len);
		n_to_mpz(n, issuer_cert->modulus.ptr, issuer_cert->modulus.len);
		...
		DBG(DBG_CRYPT,
		    DBG_dump_chunk("decrypt_sig() decrypted signature: ",
				   decrypted))
		     ...
		     const x509cert_t *issuer_cert)
{
	...
	if (algorithm != OID_UNKNOWN) {
		DBG(DBG_X509 | DBG_PARSING, DBG_log("signature algorithm: '%s'",
	...
	DBG(DBG_PARSING, DBG_dump_chunk("  digest:", digest))
...
static bool parse_basicConstraints(chunk_t blob, int level0)
{
	...
	while (objectID < BASIC_CONSTRAINTS_ROOF) {
		if (!extract_object(basicConstraintsObjects, &objectID, &object,
				    &level, &ctx))
			break;
...
static generalName_t *parse_generalName(chunk_t blob, int level0)
{
	...
	while (objectID < GN_OBJ_ROOF) {
		...
		case GN_OBJ_DIRECTORY_NAME:
			DBG(DBG_PARSING, u_char buf[ASN1_BUF_LEN];
...
static time_t parse_time(chunk_t blob, int level0)
{
	...
	while (objectID < X509_TIME_ROOF) {
		...
		if (objectID == X509_TIME_UTC ||
		    objectID == X509_TIME_GENERALIZED) {
			return asn1totime(&object,
					  (objectID == X509_TIME_UTC) ?
						  ASN1_UTCTIME :
 ...
 */
void parse_authorityKeyIdentifier(chunk_t blob, int level0, chunk_t *authKeyID,
				  chunk_t *authKeySerialNumber)
{
	...
	while (objectID < AUTH_KEY_ID_ROOF) {
		...
		case AUTH_KEY_ID_KEY_ID:
			*authKeyID =
				parse_keyIdentifier(object, level + 1, TRUE);
			break;
		case AUTH_KEY_ID_CERT_ISSUER: {
			generalName_t *gn =
				parse_generalNames(object, level + 1, TRUE);
...
static bool parse_extendedKeyUsage(chunk_t blob, int level0)
{
	...
	while (objectID < EXT_KEY_USAGE_ROOF) {
		if (!extract_object(extendedKeyUsageObjects, &objectID, &object,
				    &level, &ctx))
			return FALSE;
...
bool parse_x509cert(chunk_t blob, u_int level0, x509cert_t *cert)
{
	...
	while (objectID < X509_OBJ_ROOF) {
		...
		case X509_OBJ_VERSION:
			...
			DBG(DBG_PARSING, DBG_log("  v%d", cert->version);)
			break;
		...
		case X509_OBJ_MODULUS:
			...
			if (object.len >
			    RSA_MAX_OCTETS + (size_t)(*object.ptr == 0x00)) {
		...
		case X509_OBJ_EXTN_VALUE: {
			...
			case OID_SUBJECT_KEY_ID:
				cert->subjectKeyID = parse_keyIdentifier(
					object, level, FALSE);
				break;
			case OID_SUBJECT_ALT_NAME:
				cert->subjectAltName = parse_generalNames(
					object, level, FALSE);
				break;
			...
			case OID_AUTHORITY_KEY_ID:
				parse_authorityKeyIdentifier(
					object, level, &cert->authKeyID,
					&cert->authKeySerialNumber);
				break;
			case OID_AUTHORITY_INFO_ACCESS:
				parse_authorityInfoAccess(
					object, level, &cert->accessLocation);
				break;
			case OID_EXTENDED_KEY_USAGE:
				cert->isOcspSigner =
					parse_extendedKeyUsage(object, level);
				break;
			default:
				break;
			}
...
bool parse_x509crl(chunk_t blob, u_int level0, x509crl_t *crl)
{
	...
	while (objectID < CRL_OBJ_ROOF) {
		...
		case CRL_OBJ_VERSION:
			crl->version = (object.len) ? (1 + (u_int)*object.ptr) :
						      1;
			DBG(DBG_PARSING, DBG_log("  v%d", crl->version);)
			break;
		...
		case CRL_OBJ_REVOCATION_DATE: {
		   ...
		   with revocedCertificates pointing to the first revoked certificate */
			...
			revokedCert_t *revokedCert =
				alloc_thing(revokedCert_t, "revokedCert");
