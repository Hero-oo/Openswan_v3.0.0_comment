 ...
 */
bool load_coded_file(const char *filename, prompt_pass_t *pass, int verbose,
		     const char *type, chunk_t *blob, bool *pgp)
{
	...
	if (fd) {
		...
		if (verbose) {
			openswan_log("  loaded %s file '%s' (%zu bytes)", type,
				     filename, bytes);
		}
 ...
 */
rsa_privkey_t *load_rsa_private_key(const char *filename, int verbose,
				    prompt_pass_t *pass)
{
	bool pgp = FALSE;
	...
	if (*filename == '/') {
		...
		snprintf(path, sizeof(path), "%s/%s", oco->private_dir,
			 filename);
 ...
 */
bool load_cert(bool forcedtype, const char *filename, int verbose,
	       const char *label, cert_t *cert)
{
	...
	if (!forcedtype) {
		if (load_coded_file(filename, NULL, verbose, label, &blob,
				    &pgp)) {
			if (pgp) {
				pgpcert_t *pgpcert =
					alloc_thing(pgpcert_t, "pgpcert");
				*pgpcert = empty_pgpcert;
				...
				x509cert_t *x509cert =
					alloc_thing(x509cert_t, "x509cert");
 ...
 */
bool load_host_cert(enum ipsec_cert_type certtype, const char *filename,
...
void load_authcerts_from_nss(const char *type, u_char auth_flags)
{
	...
	if (list) {
		for (node = CERT_LIST_HEAD(list); !CERT_LIST_END(node, list);
			...
			cert_t cert;
			if (load_cert_from_nss(
				    CERT_NONE, node->cert->nickname,
