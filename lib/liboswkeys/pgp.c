...
static const char *const pgp_sym_alg_name[] = {
	"Plaintext", "IDEA", "3DES",	"CAST5",   "Blowfish", "SAFER",
	"DES",	     "AES",  "AES-192", "AES-256", "Twofish"
};
...
static const char *const pgp_rsa_privkey_name[] = { "private exponent",
						    "prime1", "prime2" };
...
static u_char pgp_version(chunk_t *blob)
{
	...
	blob->len--;
	DBG(DBG_PARSING, DBG_log("L3 - version:"); DBG_log("  V%d", version))
	return version;
}
...
static bool parse_pgp_pubkey_packet(chunk_t *packet, pgpcert_t *cert)
{
	...
	DBG(DBG_PARSING, char tbuf[TIMETOA_BUF]; DBG_log("L3 - created:");
	...
	case PGP_PUBKEY_ALG_RSA_SIGN_ONLY:
		...
		DBG(DBG_PARSING, DBG_log("L3 - modulus:"))
		...
		DBG(DBG_PARSING, DBG_log("L3 - public exponent:"))
	...
	case PGP_PUBKEY_ALG_DSA:
		...
		DBG(DBG_PARSING, DBG_log("  DSA"))
		openswan_log("  DSA public keys not supported");
		return FALSE;
	default:
		cert->pubkeyAlg = 0;
		DBG(DBG_PARSING, DBG_log("  other"))
...
static bool parse_pgp_secretkey_packet(chunk_t *packet, rsa_privkey_t *key)
{
	...
	DBG(DBG_PARSING, DBG_log("L3 - string-to-key:  %d", s2k))
	...
	DBG(DBG_PARSING, DBG_log("  %s", pgp_sym_alg_name[s2k]))
...
static bool parse_pgp_signature_packet(chunk_t *packet, pgpcert_t *cert)
{
	...
	DBG(DBG_PARSING, DBG_log("L3 - signature type:  0x%2x", sig_type))
...
{
	DBG(DBG_PARSING, DBG_log("L0 - PGP file:"))
