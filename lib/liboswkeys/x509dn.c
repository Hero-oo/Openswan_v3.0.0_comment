...
static u_char oid_E[] = { 0x2A, 0x86, 0x48, 0x86, 0xF7, 0x0D, 0x01, 0x09, 0x01 };
 ...
 */
err_t init_rdn(chunk_t dn, chunk_t *rdn, chunk_t *attribute, bool *next)
{
	*rdn = empty_chunk;
 ...
 */
err_t get_next_rdn(chunk_t *rdn, chunk_t *attribute, chunk_t *oid,
...
int dntoa(char *dst, size_t dstlen, chunk_t dn)
{
	...
	{
		DBG(DBG_PARSING, DBG_log("error in DN parsing: %s", ugh));
		str.ptr = (unsigned char *)dst;
		str.len = dstlen;
		hex_str(dn, &str);
	}
...
} rtab[] = { { 0, "cn=John Doe,dc=example,dc=com,ou=Xelerance",
