...
const char *const cert_status_names[] = { "good", "revoked", "unknown",
					  "undefined" };
...
const char ASN1_sha1_id[] = { 0x30, 0x09, 0x06, 0x05, 0x2B, 0x0E,
			      0x03, 0x02, 0x1A, 0x05, 0x00 };

const char ASN1_sha1WithRSA_id[] = { 0x30, 0x0D, 0x06, 0x09, 0x2A,
				     0x86, 0x48, 0x86, 0xF7, 0x0D,
				     0x01, 0x01, 0x05, 0x05, 0x00 };

const char ASN1_nonce_oid[] = { 0x06, 0x09, 0x2B, 0x06, 0x01, 0x05,
				0x05, 0x07, 0x30, 0x01, 0x02 };

const char ASN1_response_oid[] = { 0x06, 0x09, 0x2B, 0x06, 0x01, 0x05,
				   0x05, 0x07, 0x30, 0x01, 0x04 };

const char ASN1_response_content[] = { 0x04, 0x0D, 0x30, 0x0B, 0x06,
				       0x09, 0x2B, 0x06, 0x01, 0x05,
...
static const asn1Object_t basicResponseObjects[] = {
	...
	{ 2, "versionContext", ASN1_CONTEXT_C_0,
	  ASN1_NONE | ASN1_DEF }, /*  2 */
	...
	{ 5, "critical", ASN1_BOOLEAN, ASN1_BODY | ASN1_DEF }, /* 16 */
...
static const asn1Object_t singleResponseObjects[] = {
	...
	{ 4, "critical", ASN1_BOOLEAN, ASN1_BODY | ASN1_DEF }, /* 24 */
 ...
 */
static bool build_ocsp_location(const x509cert_t *cert,
 ...
 */
static bool same_ocsp_location(const ocsp_location_t *a,
			       const ocsp_location_t *b)
{
	return ((a->authKeyID.ptr != NULL) ?
			same_keyid(a->authKeyID, b->authKeyID) :
			(same_dn(a->issuer, b->issuer) &&
			 same_serial(a->authKeySerialNumber,
				     b->authKeySerialNumber))) &&
 ...
 */
ocsp_location_t *get_ocsp_location(const ocsp_location_t *loc,
...
static cert_status_t get_ocsp_status(const ocsp_location_t *loc,
				     chunk_t serialNumber, time_t *nextUpdate)
{
 ...
 */
bool verify_by_ocsp(/*const*/ x509cert_t *cert, bool strict, time_t *until)
{
	u_char status;
	ocsp_location_t location;
	time_t nextUpdate = 0;
	...
	switch (status) {
	case CERT_GOOD:
		DBG(DBG_CONTROL, DBG_log("certificate is good"))
...
#ifdef HAVE_THREADS
				DBG(DBG_CONTROL, char buf[BUF_LEN];
 ...
 */
void ocsp_set_default_uri(char *uri)
{
 ...
 */
static void mv_chunk(u_char **pos, chunk_t content)
{
	if (content.len > 0) {
...
static bool get_ocsp_requestor_cert(ocsp_location_t *location)
{
	...
	for (;;) {
		...
		cert = get_x509cert(location->issuer,
				    location->authKeySerialNumber,
		...
		if (pks != NULL) {
			DBG(DBG_CONTROL, DBG_log("matching private key found"))
...
static chunk_t generate_signature(chunk_t digest,
				  const struct private_key_stuff *pks)
{
	chunk_t sigdata;
	u_char *pos;
...
static chunk_t build_signature(chunk_t tbsRequest)
{
     ...
     */
	pos = build_asn1_object(&digest, ASN1_OCTET_STRING, digest_raw.len);
	...
	sigdata = generate_signature(digest_info, ocsp_requestor_pks);
	...
	pos = build_asn1_explicit_object(&certs, ASN1_CONTEXT_C_0,
					 ASN1_SEQUENCE,
					 ocsp_requestor_cert->certificate.len);
	chunkcpy(pos, ocsp_requestor_cert->certificate);

	/* build signature comprising algorithm, signature and cert */
	pos = build_asn1_explicit_object(
		&signature, ASN1_CONTEXT_C_0, ASN1_SEQUENCE,
 ...
 */
static chunk_t build_req_cert(ocsp_location_t *location,
			      ocsp_certinfo_t *certinfo)
{
	chunk_t reqCert;
	...
	pos = build_asn1_object(&reqCert, ASN1_SEQUENCE,
				sizeof(ASN1_sha1_id) + issuerNameHash.len +
					issuerKeyHash.len + serialNumber.len);
	constcpy(pos, ASN1_sha1_id);
 ...
 */
static chunk_t build_request(ocsp_location_t *location,
			     ocsp_certinfo_t *certinfo)
{
	...
	pos = build_asn1_object(&request, ASN1_SEQUENCE, reqCert.len);
...
static chunk_t build_request_list(ocsp_location_t *location)
{
	 ...
	 */
		request_list_t *req =
			alloc_thing(request_list_t, "ocsp request");
	...
	pos = build_asn1_object(&requestList, ASN1_SEQUENCE, datalen);
...
static chunk_t build_requestor_name(void)
{
	...
	pos = build_asn1_explicit_object(&reqName, ASN1_CONTEXT_C_1,
					 ASN1_CONTEXT_C_4,
					 ocsp_requestor_cert->subject.len);
...
static chunk_t build_nonce_extension(ocsp_location_t *location)
{
	...
	pos = build_asn1_object(&content, ASN1_OCTET_STRING, NONCE_LENGTH);
 ...
 */
static chunk_t build_accept_extension(void)
{
     ...
     */
	clonetochunk(content, ASN1_response_content,
		     sizeof(ASN1_response_content), "clone");
 ...
 */
static chunk_t build_tbs_request(ocsp_location_t *location,
				 bool has_requestor_cert)
{
	...
	pos = build_asn1_object(&tbsRequest, ASN1_SEQUENCE,
				requestList.len + requestorName.len +
					requestExt.len);
	...
	DBG(
		...
		if (location->authKeyID.ptr != NULL) {
			datatot(location->authKeyID.ptr,
				location->authKeyID.len, ':', buf, BUF_LEN);
...
static bool valid_ocsp_response(response_t *res)
{
	...
	DBG(DBG_CONTROL, DBG_log("ocsp signer cert found"))
	...
	DBG(DBG_CONTROL, DBG_log("signature of ocsp response is valid"))
	...
	for (pathlen = 0; pathlen < MAX_CA_PATH_LEN; pathlen++) {
		...
		DBG(
			...
			if (cert->authKeyID.ptr != NULL) {
				datatot(cert->authKeyID.ptr,
					cert->authKeyID.len, ':', buf, BUF_LEN);
				DBG_log("authkey:  %s", buf);
			})
		...
		DBG(DBG_CONTROL, DBG_log("issuer cacert found"))
		...
		DBG(DBG_CONTROL, DBG_log("certificate signature is valid"))

		/* check if cert is self-signed */
		if (same_dn(cert->issuer, cert->subject)) {
			DBG(DBG_CONTROL, DBG_log("reached self-signed root ca"))
			unlock_authcert_list("valid_ocsp_response");
			return TRUE;
		}
	}
...
static bool parse_basic_ocsp_response(chunk_t blob, int level0, response_t *res)
{
	...
	while (objectID < BASIC_RESPONSE_ROOF) {
		if (!extract_object(basicResponseObjects, &objectID, &object,
				    &level, &ctx))
			return FALSE;
		...
		case BASIC_RESPONSE_VERSION:
			...
			if (version != OCSP_BASIC_RESPONSE_VERSION) {
				plog("wrong ocsp basic response version (version= %i)",
				     version);
				return FALSE;
			}
			break;
		case BASIC_RESPONSE_ID_BY_NAME:
			...
			DBG(DBG_PARSING, char buf[BUF_LEN];
			    dntoa(buf, BUF_LEN, object); DBG_log("  '%s'", buf))
			break;
		...
		case BASIC_RESPONSE_ALGORITHM:
			res->algorithm =
				parse_algorithmIdentifier(object, level + 1);
			break;
		...
		case BASIC_RESPONSE_CERTIFICATE: {
			...
			clonetochunk(blob2, object.ptr, object.len,
				     "ocspcert blob");
			*cert = empty_x509cert;
...
static response_status parse_ocsp_response(chunk_t blob, response_t *res)
{
	...
	while (objectID < OCSP_RESPONSE_ROOF) {
		if (!extract_object(ocspResponseObjects, &objectID, &object,
				    &level, &ctx))
		...
		case OCSP_RESPONSE: {
			...
			case OID_BASIC:
				if (!parse_basic_ocsp_response(object,
							       level + 1, res))
					return STATUS_INTERNALERROR;
				break;
			default:
				DBG(DBG_CONTROL,
				    ...
				    DBG_dump_chunk("ocsp response OID: ",
						   object);)
				return STATUS_INTERNALERROR;
			}
		} break;
				       ...
				       single_response_t *sres)
{
	...
	while (objectID < SINGLE_RESPONSE_ROOF) {
		if (!extract_object(singleResponseObjects, &objectID, &object,
				    &level, &ctx))
			return FALSE;
...
{
	ocsp_location_t *location =
		alloc_thing(ocsp_location_t, "ocsp location");

	/* unshare location fields */
	clonetochunk(location->issuer, loc->issuer.ptr, loc->issuer.len,
		     "ocsp issuer");

	clonetochunk(location->authNameID, loc->authNameID.ptr,
		     loc->authNameID.len, "ocsp authNameID");

	if (loc->authKeyID.ptr == NULL)
		location->authKeyID = empty_chunk;
	else
		clonetochunk(location->authKeyID, loc->authKeyID.ptr,
			     loc->authKeyID.len, "ocsp authKeyID");

	if (loc->authKeySerialNumber.ptr == NULL)
		location->authKeySerialNumber = empty_chunk;
	else
		clonetochunk(location->authKeySerialNumber,
			     loc->authKeySerialNumber.ptr,
			     loc->authKeySerialNumber.len,
			     "ocsp authKeySerialNumber");

	clonetochunk(location->uri, loc->uri.ptr, loc->uri.len, "ocsp uri");
	...
	*chain = location;
	...
	DBG(DBG_CONTROL, DBG_log("new ocsp location added"))
		  ...
		  ocsp_location_t **chain, bool request)
{
	...
	if (cmp != 0) {
		...
		ocsp_certinfo_t *cnew =
			alloc_thing(ocsp_certinfo_t, "ocsp certinfo");
	...
	DBG(DBG_CONTROL, char buf[BUF_LEN];
	    datatot(info->serialNumber.ptr, info->serialNumber.len, ':', buf,
		    BUF_LEN);
	    DBG_log("ocsp %s for serial %s %s",
		    request ? "fetch request" : "certinfo", buf,
		    (cmp == 0) ? (request ? "already exists" : "updated") :
				 "added"))
	...
	if (request) {
		...
		certinfo->nextUpdate =
			(certinfo->once) ? (tnow + OCSP_DEFAULT_VALID_TIME) :
					   info->nextUpdate;
	}
}
...
void parse_ocsp(ocsp_location_t *location, chunk_t blob)
{
	...
	DBG(DBG_CONTROL, DBG_log("valid ocsp response"))
	...
	/* now parse the single responses one at a time */
	{
		...
		while (objectID < RESPONSES_ROOF) {
			if (!extract_object(responsesObjects, &objectID,
					    &object, &level, &ctx))
				return;
			...
			if (objectID == RESPONSES_SINGLE_RESPONSE) {
				...
				if (parse_ocsp_single_response(
					    object, level + 1, &sres)) {
					process_single_response(location,
								&sres);
				}
			}
			objectID++;
