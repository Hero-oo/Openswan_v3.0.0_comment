...
int sign_hash(const struct private_key_stuff *pks, const u_char *hash_val,
	      size_t hash_len, u_char *sig_val, size_t sig_len)
{
	...
	if (privateKey == NULL) {
		...
		if (pks->pub->nssCert != NULL) {
			privateKey = PK11_FindKeyByAnyCert(
				pks->pub->nssCert,
				osw_return_nss_password_file_info());
			if (privateKey == NULL) {
				loglog(RC_LOG_SERIOUS,
			 ...
			 size_t hash_len, const u_char *sig_val, size_t sig_len)
{
	...
	if (arena == NULL) {
		...
		return "10"
		       "NSS error: Not enough memory to create arena";
	}
	...
	if (publicKey == NULL) {
		...
		PORT_SetError(SEC_ERROR_NO_MEMORY);
		return "11"
		       "NSS error: Not enough memory to create publicKey";
	}
	...
	chunk_t n = mpz_to_n_autosize(
		&k->n); /* chunk_t n = chunk_clone(k->n, "n"); */
	chunk_t e = mpz_to_n_autosize(
		&k->e); /* chunk_t e = chunk_clone(k->e, "e"); */
	...
	if (retVal == SECSuccess) {
		retVal = SECITEM_CopyItem(
			arena, &publicKey->u.rsa.publicExponent, &nss_e);
	}
	...
	if (PK11_VerifyRecover(publicKey, &signature, &data,
			       osw_return_nss_password_file_info()) ==
	    SECSuccess) {
		DBG(DBG_CRYPT, DBG_dump("NSS RSA verify: decrypted sig: ",
					data.data, data.len));
	} else {
		DBG(DBG_CRYPT,
		    DBG_log("NSS RSA verify: decrypting signature is failed"));
		ret = "13"
		      "NSS error: Not able to decrypt";
		goto out;
	}
	...
	if (hash_len > s_max_octets) {
		loglog(RC_LOG_SERIOUS,
		       "RSA Signature NOT verified: hash too big for buffer");
		ret = "15"
		      "NSS error: hash too big for buffer";
		goto out;
	}
