			 ...
			 x509cert_t *chain)
{
	...
	while (cert != NULL) {
		if ((keyid.ptr != NULL) ?
			    same_keyid(keyid, cert->authKeyID) :
...
void store_x509certs(x509cert_t **firstcert, bool strict)
{
	...
	while (*pp != NULL) {
		...
		if (verify_x509cert(cert, strict, &valid_until)) {
			...
			add_x509_public_key_to_list(&pluto_pubkeys, NULL, cert,
...
bool insert_crl(chunk_t blob, chunk_t crl_uri)
{
	...
	*crl = empty_x509crl;
	...
	if (parse_x509crl(blob, 0, crl)) {
		...
		if (issuer_cert == NULL) {
			...
			strncat(distpoint,
				...
				(crl->distributionPoints->name.len < PATH_MAX ?
					 crl->distributionPoints->name.len :
					 PATH_MAX));
		...
		DBG(DBG_X509, DBG_log("crl issuer cacert found"))
		...
		DBG(DBG_X509, DBG_log("valid crl signature"))
...
void load_crls(void)
{
	...
	if (chdir(oco->crls_dir) == -1) {
		openswan_log("Could not change to directory '%s': %d %s",
			     oco->crls_dir, errno, strerror(errno));
	} else {
		n = scandir(oco->crls_dir, &filelist, (void *)file_select,
			    alphasort);
		DBG(DBG_CONTROL, DBG_log("Found %d items in directory '%s'", n,
					 oco->crls_dir));
			  ...
			  time_t *until)
{
	...
	if (crl == NULL) {
		...
		openswan_log("no crl from issuer \"%s\" found (strict=%s)",
			     ibuf, strict ? "yes" : "no");
		...
		DBG(DBG_X509, DBG_log("issuer crl \"%s\" found", ibuf))
	     ...
	     */
			...
			revoked_crl =
				x509_check_revocation(crl, cert->serialNumber);
			...
			if (expired_crl) {
				...
				openswan_log(
					...
					cbuf,
					timetoa(&crl->nextUpdate, TRUE, tbuf,
						sizeof(tbuf)));
...
bool verify_x509cert(/*const*/ x509cert_t *cert, bool strict, time_t *until)
{
	...
	*until = cert->notAfter;
	...
	for (pathlen = 0; pathlen < MAX_CA_PATH_LEN; pathlen++) {
		...
		if (ugh != NULL) {
			openswan_log("checking validity of \"%s\": %s", sbuf,
				     ugh);
			return FALSE;
		}

		DBG(DBG_X509, DBG_log("valid certificate for \"%s\"", sbuf))
		...
		DBG(DBG_X509, DBG_log("issuer cacert \"%s\" found", ibuf))
		...
		DBG(DBG_X509, DBG_log("valid certificate signature (%s -> %s)",
				      ibuf, sbuf);)
		unlock_authcert_list("verify_x509cert");
	...
	openswan_log("maximum ca path length of %d levels exceeded",
		     MAX_CA_PATH_LEN);
	return FALSE;
}
 ...
 */
void add_x509_altnames_to_list(struct pubkey_list **pl, const generalName_t *gn,
			       ...
			       enum dns_auth_level dns_auth_level)
{
	...
	if (id.kind != ID_NONE) {
		...
		pk->dns_auth_level = dns_auth_level;
		pk->until_time = until;
		clonereplacechunk(pk->issuer, cert->issuer.ptr,
				  cert->issuer.len, "issuer");
 ...
 * insert it into a pubkeyrec
 */
void add_x509_public_key_to_list(struct pubkey_list **pl, struct id *keyid,
				 x509cert_t *cert, time_t until,
				 enum dns_auth_level dns_auth_level,
				 struct pubkey **p_key)
{
	...
	{
		add_x509_altnames_to_list(&pluto_pubkeys, gn, cert, until,
					  dns_auth_level);
		gn = gn->next;
	}
