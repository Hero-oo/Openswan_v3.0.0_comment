...
static const char *const kern_interface_name[] = {
	"none", "auto-pick", "klips", "netkey", "win2k", "mastklips", "bsdkame"
};
enum_names kern_interface_names = { NO_KERNEL, USE_BSDKAME, kern_interface_name,
				    NULL };
...
enum_names dpd_action_names = { EVENT_NULL, DPD_ACTION_RESTART_BY_PEER,
				dpd_action_name, NULL };

/* Timer events */
static const char *const timer_event_name[] = { "EVENT_NULL",
						"EVENT_REINIT_SECRET",
						"EVENT_SHUNT_SCAN",
						"EVENT_SO_DISCARD",
						"EVENT_RETRANSMIT",
...
enum_names timer_event_names = { EVENT_NULL, EVENT_SA_DELETE, timer_event_name,
				 NULL };

/* State of exchanges */
static const char *const state_name[] = {
	"STATE_UNDEFINED", "STATE_DELETING", "OPPO_ACQUIRE",
	"OPPO_GW_DISCOVERED",

	"STATE_MAIN_R0", "STATE_MAIN_I1", "STATE_MAIN_R1", "STATE_MAIN_I2",
	"STATE_MAIN_R2", "STATE_MAIN_I3", "STATE_MAIN_R3", "STATE_MAIN_I4",

	"STATE_AGGR_R0", "STATE_AGGR_I1", "STATE_AGGR_R1", "STATE_AGGR_I2",
	"STATE_AGGR_R2",

	"STATE_QUICK_R0", "STATE_QUICK_I1", "STATE_QUICK_R1", "STATE_QUICK_I2",
	"STATE_QUICK_R2",

	"STATE_INFO", "STATE_INFO_PROTECTED",

	"STATE_XAUTH_R0", "STATE_XAUTH_R1", "STATE_MODE_CFG_R0",
	...
	/* v2 */
	"STATE_IKEv2_START", "STATE_PARENT_I1", "STATE_PARENT_I2",
	"STATE_PARENT_I3", "STATE_PARENT_R1", "STATE_PARENT_R2",

	"STATE_CHILD_C0_KEYING", "STATE_CHILD_C1_KEYED", "STATE_CHILD_C1_REKEY",

	"STATE_IKESA_DEL", "STATE_CHILDSA_DEL",

	"STATE_IKEv2_ROOF"

};

enum_names state_names = { STATE_UNDEFINED, STATE_IKEv2_ROOF - 1, state_name,
			   NULL };
...
enum_names state_stories = { STATE_MAIN_R0, STATE_IKEv2_ROOF - 1, state_story,
			     NULL };
...
enum_names pluto_cryptoop_names = { pcr_build_kenonce, pcr_compute_dh_v2,
				    pluto_cryptoop_strings, NULL };
...
static const char *const pluto_cryptoimportance_strings[] = {
	"import:not set",	    "import:respond to stranger",
	"import:respond to friend", "import:ongoing calculation",
	"import:local rekey",	    "import:admin initiate"
};

enum_names pluto_cryptoimportance_names = { pcim_notset_crypto,
					    pcim_demand_crypto,
					    pluto_cryptoimportance_strings,
					    NULL };
...
enum_names routing_story = { RT_UNROUTED, RT_ROUTED_TUNNEL,
			     routing_story_strings, NULL };

static const char *const stfstatus_names[] = {
	"STF_IGNORE", "STF_INLINE",	    "STF_SUSPEND",
	"STF_OK",     "STF_INTERNAL_ERROR", "STF_TOOMUCHCRYPTO",
	"STF_FATAL",  "STF_STOLEN",	    "STF_FAIL"
};
enum_names stfstatus_name = { STF_IGNORE, STF_FAIL, stfstatus_names, NULL };
...
const char *stf_status_name(stf_status code)
{
	...
	snprintf(stf_status_buffer, sizeof(stf_status_buffer), "STF_FAIL%+d",
		 code - STF_FAIL);
	return stf_status_buffer;
}

/* Timer events */
static const char *const dns_auth_level_name[] = { "unsigned", "notsec",
						   "dndsec",   "trustca",
						   "localf",   "localc" };

enum_names dns_auth_level_names = { DAL_UNSIGNED, DAL_CERTFILE,
				    dns_auth_level_name, NULL };

/* Goal BITs for establishing an SA
 * Note: we drop the POLICY_ prefix so that logs are more concise.
 */

const char *const sa_policy_bit_names[] = { "PSK",
					    "RSASIG",
					    "ENCRYPT",
					    "AUTHENTICATE",
					    "COMPRESS",
...
const char *prettypolicy(lset_t policy)
{
	...
	const char *bn =
		bitnamesofb(sa_policy_bit_names,
	...
	if (bn != pbitnamesbuf)
		pbitnamesbuf[0] = '\0';
	snprintf(buf, sizeof(buf), "%s%s%s%s%s%s%s", pbitnamesbuf,
		 shunt != 0 ? "+" : "",
