						   ...
						   struct spd_route *sr)
{
	...
	DBG(DBG_CONTROLMORE, DBG_log("pick_if looking for port: %u, family: %u",
				     desired_port, family));
	...
	for (ifp = iflist; ifp != NULL; ifp = ifp->next) {
		...
		DBG(DBG_CONTROLMORE,
		    DBG_log("  considering %s %s port: %u, family: %u, best: %s/%u %d",
			    ifp->ip_dev->id_rname, ifp->addrname, ifp->port,
			    ifp->ip_addr.u.v4.sin_family,
			    best_ifp ? best_ifp->ip_dev->id_rname : "<none>",
			    best_score, isloopbackaddr(&iflist->ip_addr)));

		/* the port must always match, not a best case */
		if (ifp->port != desired_port)
			continue;

		/* the family MUST match, unless it is zero */
		if (ifp->ip_addr.u.v4.sin_family != family && family != 0)
			continue;
		...
		case AF_INET:
			...
			if (ifp->ip_addr.u.v4.sin_addr.s_addr ==
				    e1->host_addr.u.v4.sin_addr.s_addr &&
			    e1->host_addr.u.v4.sin_addr.s_addr != 0) {
				score += 20;
			}
			break;
		...
		case AF_INET6:
			...
			if ((ifp->ip_addr.u.v6.sin6_addr.s6_addr[0] & 0xfe) !=
			    0xfc)
				score += 10;
             ...
             */
			if (memcmp(&ifp->ip_addr.u.v6.sin6_addr,
				   &e1->host_addr.u.v6.sin6_addr, 16) == 0) {
				score += 128;
			}
	...
	DBG(DBG_CONTROLMORE,
	    DBG_log("  picking maching interface for family[%u,%u]: %s resulted in: %s",
		    family, family2, afi ? afi->name : "<family:0>",
		    best_ifp ? best_ifp->addrname : "none"));
...
bool orient(struct connection *c, unsigned int pluto_port)
{
	struct spd_route *sr;
	bool result;
	...
	if (!oriented(*c)) {
	     ...
	     */
			for (p = interfaces; p != NULL; p = p->next) {
				DBG(DBG_CONTROLMORE,
				    DBG_log("orient %s checking against if: %s (%s:%s:%u)",
					    c->name, p->ip_dev->id_rname,
					    p->socktypename, p->addrname,
					    p->port));
#ifdef NAT_TRAVERSAL
				if (p->ike_float)
					continue;
				...
				if (c->loopback && sameaddr(&sr->this.host_addr,
							    &p->ip_addr)) {
					DBG(DBG_CONTROLMORE,
			...
			if (!oriented(*c)) {
				bool this_has_private_key =
					osw_end_has_private_key(&sr->this);
				bool that_has_private_key =
				...
				if ((sr->this.host_type == KH_DEFAULTROUTE ||
                     ...
                     */
					DBG(DBG_CONTROLMORE,
					    DBG_log("  orient %s matched on this having private key",
						    c->name));

					/* take the family from the other end */
					c->interface =
						pick_matching_interfacebyfamily(
							interfaces, pluto_port,
							family, sr);
					c->ip_oriented = FALSE;

				} else if ((sr->that.host_type ==
						    KH_DEFAULTROUTE ||
					    sr->that.host_type == KH_ANY) &&
					   that_has_private_key) {
					DBG(DBG_CONTROLMORE,
					    DBG_log("  orient %s matched on that having private key",
						    c->name));
					...
					DBG(DBG_CONTROLMORE,
					    DBG_log("  orient %s matched on this being defaultroute, and that lacking private key",
						    c->name));
					...
					DBG(DBG_CONTROLMORE,
					    DBG_log("  orient %s matched on that being defaultroute, and this lacking private key",
						    c->name));
					swap_ends(sr);
	...
	result = oriented(*c);
	DBG(DBG_CONTROLMORE,
	    DBG_log("  orient %s finished with: %u [%s]", c->name, result,
		    c->interface ? c->interface->addrname : "none"));
