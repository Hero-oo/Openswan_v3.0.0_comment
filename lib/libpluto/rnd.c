 ...
 */
static u_char get_rnd_byte(void)
{
...
void init_rnd_pool(void)
{
	...
	for (i = 0; random_fd == -1 && i < max_rnd_devices; i++) {
		...
		if (random_fd == -1) {
			openswan_log("WARNING: open of %s failed: %s",
				     random_devices[i], strerror(errno));
		}
	}

	if (random_fd == -1 || i == max_rnd_devices) {
		openswan_log(
			"Failed to open any source of random. Unable to start any connections.");
		return;
	}
