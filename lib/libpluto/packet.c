...
struct_desc isakmp_hdr_desc = { "ISAKMP Message", isa_fields,
...
struct_desc isakmp_generic_desc = { "ISAKMP Generic Payload", isag_fields,
...
struct_desc isakmp_oakley_attribute_desc = { "ISAKMP Oakley attribute",
					     isaat_fields_oakley,
...
struct_desc isakmp_ipsec_attribute_desc = { "ISAKMP IPsec DOI attribute",
					    isaat_fields_ipsec,
					    sizeof(struct isakmp_attribute) };
...
struct_desc isakmp_xauth_attribute_desc = { "ISAKMP ModeCfg attribute",
					    isaat_fields_xauth,
					    sizeof(struct isakmp_attribute) };
...
struct_desc isakmp_sa_desc = { "ISAKMP Security Association Payload",
			       isasa_fields, sizeof(struct isakmp_sa) };

static field_desc ipsec_sit_field[] = { { ft_set, 32 / BITS_PER_BYTE,
					  "IPsec DOI SIT", &sit_bit_names },
					{ ft_end, 0, NULL, NULL } };

struct_desc ipsec_sit_desc = { "IPsec DOI SIT", ipsec_sit_field,
			       sizeof(u_int32_t) };
...
struct_desc isakmp_proposal_desc = { "ISAKMP Proposal Payload", isap_fields,
...
struct_desc isakmp_isakmp_transform_desc = {
	"ISAKMP Transform Payload (ISAKMP)", isat_fields_isakmp,
...
static field_desc isat_fields_ah[] = {
	...
	{ ft_len, 16 / BITS_PER_BYTE, "length", NULL },
	{ ft_nat, 8 / BITS_PER_BYTE, "transform number", NULL },
	{ ft_enum, 8 / BITS_PER_BYTE, "AH transform ID",
	  &ah_transformid_names },
	{ ft_mbz, 16 / BITS_PER_BYTE, NULL, NULL },
	{ ft_end, 0, NULL, NULL }
};

struct_desc isakmp_ah_transform_desc = { "ISAKMP Transform Payload (AH)",
					 isat_fields_ah,
					 sizeof(struct isakmp_transform) };
...
static field_desc isat_fields_esp[] = {
	...
	{ ft_len, 16 / BITS_PER_BYTE, "length", NULL },
	{ ft_nat, 8 / BITS_PER_BYTE, "transform number", NULL },
	{ ft_enum, 8 / BITS_PER_BYTE, "ESP transform ID",
	  &esp_transformid_names },
	{ ft_mbz, 16 / BITS_PER_BYTE, NULL, NULL },
	{ ft_end, 0, NULL, NULL }
};

struct_desc isakmp_esp_transform_desc = { "ISAKMP Transform Payload (ESP)",
					  isat_fields_esp,
					  sizeof(struct isakmp_transform) };
...
struct_desc isakmp_ipcomp_transform_desc = { "ISAKMP Transform Payload (COMP)",
					     isat_fields_ipcomp,
					     sizeof(struct isakmp_transform) };
 ...
 * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 */
struct_desc isakmp_keyex_desc = { "ISAKMP Key Exchange Payload", isag_fields,
				  sizeof(struct isakmp_generic) };
...
static field_desc isaid_fields[] = {
	...
	{ ft_mbz, 8 / BITS_PER_BYTE, NULL, NULL },
	{ ft_len, 16 / BITS_PER_BYTE, "length", NULL },
	{ ft_enum, 8 / BITS_PER_BYTE, "ID type",
	  &ident_names }, /* ??? depends on DOI? */
	{ ft_nat, 8 / BITS_PER_BYTE, "DOI specific A",
	  NULL }, /* ??? depends on DOI? */
	{ ft_nat, 16 / BITS_PER_BYTE, "DOI specific B",
	  NULL }, /* ??? depends on DOI? */
	{ ft_end, 0, NULL, NULL }
};

struct_desc isakmp_identification_desc = { "ISAKMP Identification Payload",
					   isaid_fields,
...
static field_desc isaiid_fields[] = {
	...
	{ ft_nat, 8 / BITS_PER_BYTE, "Protocol ID",
	  NULL }, /* ??? UDP/TCP or 0? */
 ...
 */
struct_desc isakmp_ipsec_certificate_desc = { "ISAKMP Certificate Payload",
					      isacert_fields,
					      ISAKMP_CERT_SIZE };
 ...
 */
struct_desc isakmp_ipsec_cert_req_desc = { "ISAKMP Certificate RequestPayload",
					   isacr_fields, ISAKMP_CR_SIZE };
 ...
 * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 */
struct_desc isakmp_hash_desc = { "ISAKMP Hash Payload", isag_fields,
				 sizeof(struct isakmp_generic) };
 ...
 * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 */
struct_desc isakmp_signature_desc = { "ISAKMP Signature Payload", isag_fields,
				      sizeof(struct isakmp_generic) };
 ...
 * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 */
struct_desc isakmp_nonce_desc = { "ISAKMP Nonce Payload", isag_fields,
				  sizeof(struct isakmp_generic) };
...
static field_desc isan_fields[] = {
	...
	{ ft_enum, 32 / BITS_PER_BYTE, "DOI", &doi_names },
	{ ft_nat, 8 / BITS_PER_BYTE, "protocol ID",
	  NULL }, /* ??? really enum: ISAKMP, IPSEC, ESP, ... */
	{ ft_nat, 8 / BITS_PER_BYTE, "SPI size", NULL },
...
struct_desc isakmp_notification_desc = { "ISAKMP Notification Payload",
					 isan_fields,
...
static field_desc isad_fields[] = {
	...
	{ ft_enum, 32 / BITS_PER_BYTE, "DOI", &doi_names },
	{ ft_nat, 8 / BITS_PER_BYTE, "protocol ID",
	  NULL }, /* ??? really enum: ISAKMP, IPSEC */
	{ ft_nat, 8 / BITS_PER_BYTE, "SPI size", NULL },
	{ ft_nat, 16 / BITS_PER_BYTE, "number of SPIs", NULL },
	{ ft_end, 0, NULL, NULL }
};

struct_desc isakmp_delete_desc = { "ISAKMP Delete Payload", isad_fields,
				   sizeof(struct isakmp_delete) };
 ...
 * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 */
struct_desc isakmp_vendor_id_desc = { "ISAKMP Vendor ID Payload", isag_fields,
				      sizeof(struct isakmp_generic) };
...
struct_desc isakmp_attr_desc = { "ISAKMP Mode Attribute", isaattr_fields,
 ...
 * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 */
struct_desc isakmp_nat_d = { "ISAKMP NAT-D Payload", isag_fields,
			     sizeof(struct isakmp_generic) };
...
struct_desc isakmp_nat_oa = { "ISAKMP NAT-OA Payload", isanat_oa_fields,
...
struct_desc ikev2_generic_desc = { "IKEv2 Generic Payload", ikev2generic_fields,
...
struct_desc ikev2_trans_attr_desc = { "IKEv2 Attribute Substructure Payload",
				      ikev2_trans_attr_fields,
				      sizeof(struct ikev2_trans_attr) };
...
struct_desc ikev2_ke_desc = { "IKEv2 Key Exchange Payload", ikev2ke_fields,
			      sizeof(struct ikev2_ke) };
...
struct_desc ikev2_id_desc = { "IKEv2 Identification Payload", ikev2id_fields,
...
static field_desc ikev2_cert_fields[] = {
	...
	{ ft_set, 8 / BITS_PER_BYTE, "critical bit", critical_names },
	{ ft_len, 16 / BITS_PER_BYTE, "length", NULL },
	{ ft_loose_enum, 8 / BITS_PER_BYTE, "ikev2 cert encoding",
	  &ikev2_cert_type_names },
	{ ft_end, 0, NULL, NULL }
};

struct_desc ikev2_certificate_desc = { "IKEv2 Certificate Payload",
				       ikev2_cert_fields, IKEV2_CERT_SIZE };
...
struct_desc ikev2_certificate_req_desc = { "IKEv2 Certificate Request Payload",
					   ikev2_cert_req_fields,
					   IKEV2_CERTREQ_SIZE };
...
struct_desc ikev2_a_desc = { "IKEv2 Authentication Payload", ikev2a_fields,
			     sizeof(struct ikev2_a) };
 ...
 */
struct_desc ikev2_nonce_desc = { "IKEv2 Nonce Payload", ikev2generic_fields,
				 sizeof(struct ikev2_generic) };
...
struct_desc ikev2_delete_desc = { "IKEv2 Delete Payload", ikev2_delete_fields,
...
struct_desc ikev2_ts_desc = { "IKEv2 Traffic Selector Payload", ikev2ts_fields,
			      sizeof(struct ikev2_ts) };
...
struct_desc ikev2_ts1_desc = { "IKEv2 Traffic Selector", ikev2ts1_fields,
			       sizeof(struct ikev2_ts1) };
...
struct_desc ikev2_e_desc = { "IKEv2 Encryption Payload", ikev2e_fields,
			     sizeof(struct ikev2_generic) };
...
void init_sub_pbs(pb_stream *parent_pbs, pb_stream *child_pbs, const char *name)
{
	init_pbs(child_pbs, parent_pbs->cur, parent_pbs->roof - parent_pbs->cur,
		 name);
...
static void DBG_prefix_print_struct(const pb_stream *pbs, const char *label,
				    const void *struct_ptr, struct_desc *sd,
				    bool len_meaningful)
{
...
bool in_struct(void *struct_ptr, struct_desc *sd, pb_stream *ins,
	       pb_stream *obj_pbs)
{
	err_t ugh = NULL;
	...
	if (ins->roof - cur < (ptrdiff_t)sd->size) {
		...
		u_int8_t *roof =
			cur + sd->size; /* may be changed by a length field */
		...
		, (int) (cur - ins->cur), fp->name == NULL? "" : fp->name));
			...
			case ft_mbz: /* must be zero */
				for (; i != 0; i--) {
					if (*cur++ != 0) {
						ugh = builddiag(
							"byte %d of %s must be zero, but is not",
							(int)(cur - ins->cur),
							sd->name);
						break;
					}
			...
			case ft_zig: /* should be zero, ignore if not */
				for (; i != 0; i--) {
					if (*cur++ != 0) {
						openswan_log(
							"byte %d of %s should have been zero, but was not",
							(int)(cur - ins->cur),
							sd->name);
						/*
			...
			case ft_set: /* bits representing set */
			{
				...
				case ft_af_enum: /* Attribute Format + value from an enumeration */
					...
					/* FALL THROUGH */
				case ft_enum: /* value from an enumeration */
					if (enum_name(fp->desc, actual_n) ==
					    NULL) {
						ugh = builddiag(
							"%s of %s has an unknown in value: %lu",
							fp->name, sd->name,
							(unsigned long)actual_n);
					}
					/* FALL THROUGH */
				case ft_loose_enum: /* value from an enumeration with only some names known */
					break;
			...
			case ft_end: /* end of field list */
				...
				ins->cur = roof;
				DBG(DBG_PARSING, DBG_prefix_print_struct(
							 ins, "parse ",
							 struct_ptr, sd, TRUE));
				return TRUE;
...
bool in_raw(void *bytes, size_t len, pb_stream *ins, const char *name)
{
	if (pbs_left(ins) < len) {
		openswan_loglog(RC_LOG_SERIOUS,
				"not enough bytes left to get %s from %s", name,
				ins->name);
		return FALSE;
	} else {
		if (bytes == NULL) {
...
bool out_struct(const void *struct_ptr, struct_desc *sd, pb_stream *outs,
		pb_stream *obj_pbs)
{
	err_t ugh = NULL;
	const u_int8_t *inp = struct_ptr;
	...
	DBG(DBG_EMITTING, DBG_prefix_print_struct(outs, "emit ", struct_ptr, sd,
						  obj_pbs == NULL));
	...
	if (outs->roof - cur < (ptrdiff_t)sd->size) {
	    ...
	    DBG_log("out_struct: %d %s"
		     ...
		     */
				...
				case ft_enum: /* value from an enumeration */
					if (enum_name(fp->desc, actual_n) ==
					    NULL) {
						ugh = builddiag(
							"%s of %s has an unknown out value: %lu",
							fp->name, sd->name,
							(unsigned long)actual_n);
					}
					break;
			...
			case ft_end: /* end of field list */
				...
				if (obj_pbs == NULL) {
					close_output_pbs(
						&obj); /* fill in length field, if any */
				} else {
	...
	loglog(RC_LOG_SERIOUS, "%s",
	       ugh); /* ??? serious, but errno not relevant */
	return FALSE;
}
...
void pbs_set_np(pb_stream *outs, u_int8_t np)
{
	...
	DBG(DBG_EMITTING,
	    DBG_log("   next-payload: %s [@%ld=0x%2x]",
		    enum_show(&payload_names, np),
		    (long)(outs->next_payload_pointer - outs->start), np));

	*outs->next_payload_pointer = np;
}

bool out_generic(u_int8_t np, struct_desc *sd, pb_stream *outs,
		 pb_stream *obj_pbs)
{
...
bool out_generic_raw(u_int8_t np, struct_desc *sd, pb_stream *outs,
		     const void *bytes, size_t len, const char *name)
{
	pb_stream pbs;
...
bool out_raw(const void *bytes, size_t len, pb_stream *outs, const char *name)
{
	if (pbs_left(outs) < len) {
		...
		DBG(DBG_EMITTING, DBG_log("emitting %u raw bytes of %s into %s",
...
{
	if (pbs_left(outs) < len) {
		loglog(RC_LOG_SERIOUS, "not enough room left to place %s in %s",
		       name, outs->name);
		return FALSE;
	} else {
		DBG(DBG_EMITTING,
