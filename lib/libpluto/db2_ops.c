...
struct db_v2_context *db2_prop_new(int max_conj, int max_trans, int max_attrs)
{
	...
	new_db2 = alloc_bytes(sizeof(struct db_v2_context), "db_context");

	if (new_db2 &&
	    db2_prop_init(new_db2, max_conj, max_trans, max_attrs) < 0) {
		if (new_db2)
			pfree(new_db2);
		return NULL;
	}
...
static int db2_prop_expand(struct db_v2_context *ctx, int delta_conj)
{
	...
	if (!new_conj)
		goto out;
	memcpy(new_conj, old_conj,
	       ctx->max_conj * sizeof(struct db_v2_prop_conj));
...
static void db2_trans_increment(struct db_v2_context *ctx)
{
...
/*	Expand storage for transforms by number delta_trans */
static int db2_trans_expand(struct db_v2_context *ctx, int delta_trans)
{
	int ret = -1;
	...
	if (!new_trans)
		goto out;
	memcpy(new_trans, old_trans,
	       ctx->max_trans * sizeof(struct db_v2_trans));
	...
	for (pc = ctx->prop.props, pi = 0; pi < ctx->prop.prop_cnt;
	     pc++, pi++) {
 ...
 */
static int db2_attrs_expand(struct db_v2_context *ctx, int delta_attrs)
{
	int ret = -1;
...
static void db2_prop_print(struct db_v2_prop_conj *p)
{
	...
	DBG_log("%u:  protoid=\"%s\" [trans: %u]", p->propnum,
		enum_name(&protocol_names, p->protoid), p->trans_cnt);
	for (ti = 0, t = p->trans; ti < p->trans_cnt; ti++, t++) {
		...
		DBG_log("    %s value=\"%s\" [attrs: %u]",
			...
			enum_name(n, t->value), t->attr_cnt);
...
void db2_print(struct db_v2_context *ctx)
{
	...
	if (ctx == NULL)
		return;
	DBG_log("proposals: cnt=%u (next=%u)", ctx->prop.prop_cnt,
		ctx->prop.conjnum);
...
void sa_v2_print(struct db_sa *sa)
{
	...
	if (sa == NULL)
		return;
	DBG_log("proposals: cnt=%u", sa->prop_disj_cnt);
...
struct db_sa *sa_copy_sa_first(struct db_sa *sa)
{
	...
	pc->prop_cnt = 1;
	pc->props = clone_bytes(pc->props, sizeof(pc->props[0]),
	...
	p->trans_cnt = 1;
	p->trans = clone_bytes(p->trans, sizeof(p->trans[0]),
...
struct db_sa *sa_merge_proposals(struct db_sa *a, struct db_sa *b)
{
	...
	n->prop_conjs = clone_bytes(n->prop_conjs,
	...
	for (i = 0; i < n->prop_conj_cnt; i++) {
		...
		for (j = 0; j < pca->prop_cnt; j++) {
			...
			memcpy(t, pa->trans,
			       (pa->trans_cnt) * sizeof(pa->trans[0]));
			memcpy(t + (pa->trans_cnt), pb->trans,
