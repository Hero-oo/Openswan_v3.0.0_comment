...
void print_sa_attr_oakley(struct db_attr *at)
{
	...
	DBG_log("        type: %u(%s) val: %u(%s)\n", at->type.oakley,
		enum_name(&oakley_attr_names, at->type.oakley), at->val,
...
void print_sa_attr_ipsec(struct db_attr *at)
{
	...
	DBG_log("        type: %u(%s) val: %u(%s)\n", at->type.ipsec,
		enum_name(&ipsec_attr_names, at->type.ipsec), at->val,
		en ? enum_name(en, at->val) : "unknown");
}

void print_sa_trans(bool parentSA, struct db_trans *tr)
{
	unsigned int i;
...
void print_sa_prop(bool parentSA, struct db_prop *dp)
{
	unsigned int i;
	DBG_log("    protoid: %u (%s) cnt: %u\n", dp->protoid,
		enum_name(&protocol_names, dp->protoid), dp->trans_cnt);
	if (!dp->trans) {
		if (dp->trans_cnt)
...
void print_sa_prop_conj(bool parentSA, struct db_prop_conj *pc)
{
	unsigned int i;
