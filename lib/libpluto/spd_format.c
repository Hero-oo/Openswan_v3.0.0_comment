		  ...
		  const struct end *that, bool is_left, lset_t policy)
{
	...
	if (this->modecfg_server || this->modecfg_client ||
		...
		if (this->modecfg_server) {
			strncat(endopts, "MS",
				sizeof(endopts) - strlen(endopts) - 1);
		}

		if (this->modecfg_client) {
			strncat(endopts, plus,
				sizeof(endopts) - strlen(endopts) - 1);
			strncat(endopts, "MC",
				sizeof(endopts) - strlen(endopts) - 1);
		}
	...
	if (is_left)
		snprintf(buf, buf_len, "%s%s%s%s%s%s%s%s%s%s%s%s%s%s",
			 ...
			 host, host_port, id_obrackets, host_id, id_comma,
			 endopts, id_cbrackets, protoport, hop_sep, hop);
	else
		snprintf(buf, buf_len, "%s%s%s%s%s%s%s%s%s%s%s%s%s%s", hop,
			 hop_sep, host, host_port, id_obrackets, host_id,
			 id_comma, endopts, id_cbrackets, protoport, client_sep,
 ...
 */
size_t format_connection(char *buf, size_t buf_len, const struct connection *c,
			 struct spd_route *sr)
{
	size_t w = format_end(buf, buf_len, &sr->this, &sr->that, TRUE, LEMPTY);
	snprintf(buf + w, buf_len - w, "...");
	w += strlen(buf + w);
	return w + format_end(buf + w, buf_len - w, &sr->that, &sr->this, FALSE,
			      c->policy);
}
