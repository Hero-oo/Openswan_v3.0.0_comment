...
int alg_enum_search_ppfix(enum_names *ed, const char *prefix,
			  const char *postfix, const char *str, int str_len)
{
	char buf[64];
	char *ptr;
	int ret;
 ...
 */
enum ikev2_trans_type_encr ealg_getbyname(const char *const str, int len,
					  unsigned int *auxp)
{
	...
	search_ret =
		enum_search_nocase(ikev2_encr_names.official_names, str, len);
	if (search_ret != -1) {
		ret = search_ret;
		goto out;
	}
 ...
 */
enum ikev2_trans_type_integ aalg_getbyname(const char *const str, int len,
					   unsigned int *auxp)
{
	int search_ret = -1;
	...
	/* look for the name by literal name, upcasing first */
	search_ret =
		enum_search_nocase(ikev2_integ_names.official_names, str, len);
	if (search_ret >= 0)
		goto out;
 ...
 */
enum ikev2_trans_type_dh modp_getbyname(const char *const str, int len,
					unsigned int *auxp)
{
	int search_ret = -1;
...
consider_prf_modp:
		if ((p_ctx->modp_getbyname) && (p_ctx->prfalg_getbyname) &&
		    isalpha(ch)) {
...
static int parser_alg_info_add(struct parser_context *p_ctx,
			       ...
			       const struct oakley_group_desc *(*lookup_group)(
				       enum ikev2_trans_type_dh group))
{
	...
	if (p_ctx->ealg_permit && *p_ctx->ealg_buf) {
		auxinfo = 0;
		ealg_id = p_ctx->ealg_getbyname(
			p_ctx->ealg_buf, strlen(p_ctx->ealg_buf), &auxinfo);
		...
		if (ealg_id == IKEv2_ENCR_AES_GCM_8 ||
			...
			if (p_ctx->eklen != 128 && p_ctx->eklen != 192 &&
			    p_ctx->eklen != 256) {
				p_ctx->err =
	...
	if (p_ctx->aalg_permit && *p_ctx->aalg_buf) {
		auxinfo = 0;
		aalg_id = p_ctx->aalg_getbyname(
			p_ctx->aalg_buf, strlen(p_ctx->aalg_buf), &auxinfo);
...
#ifdef HAVE_LIBNSS
		if (Pluto_IsFIPS() &&
		    ((aalg_id == IKEv2_AUTH_HMAC_SHA2_256_128) ||
		     (aalg_id == IKEv2_AUTH_HMAC_SHA2_384_192) ||
		     (aalg_id == IKEv2_AUTH_HMAC_SHA2_512_256))) {
	...
	if (p_ctx->prfalg_getbyname && *p_ctx->prfalg_buf) {
		auxinfo = 0;
		prfalg_id = p_ctx->prfalg_getbyname(
			p_ctx->prfalg_buf, strlen(p_ctx->prfalg_buf), &auxinfo);
	...
	if (modp_id == OAKLEY_INVALID_GROUP && p_ctx->modp_getbyname &&
	    *p_ctx->modp_buf) {
		auxinfo = 0;
		modp_id = p_ctx->modp_getbyname(
			p_ctx->modp_buf, strlen(p_ctx->modp_buf), &auxinfo);
	...
	if (modp_id != OAKLEY_INVALID_GROUP && lookup_group &&
	    !lookup_group(modp_id)) {
		p_ctx->err = "found modp group id, but not supported";
		goto out;
	}

	(*alg_info_add)(alg_info, ealg_id, p_ctx->eklen, aalg_id, p_ctx->aklen,
			prfalg_id, modp_id);
	return 0;
out:
	return -1;
...
int alg_info_parse_str(struct alg_info *alg_info, const char *alg_str,
		       const char **err_p,
	...
	*err_buf = 0;
	...
	for (ret = 0, ptr = alg_str; ret < ST_EOF;) {
		...
		case ST_EOF:
			...
			if (parser_alg_info_add(&ctx, alg_info, alg_info_add,
						lookup_group) < 0) {
		...
		case ST_ERR:
			snprintf(err_buf, sizeof(err_buf),
				 ...
				 "just after \"%.*s\""
				 " (old_state=%s)",
				 ctx.err, (int)(ptr - alg_str - 1), alg_str,
						  ...
						  const char **err_p)
{
	...
	ret = alg_info_parse_str((struct alg_info *)alg_info_esp, alg_str,
				 err_p, parser_init_esp, alg_info_esp_add,
				 NULL);
						 ...
						 const char **err_p)
{
	...
	ret = alg_info_parse_str((struct alg_info *)alg_info_esp, alg_str,
				 err_p, parser_init_ah, alg_info_ah_add, NULL);
...
int alg_info_snprint(char *buf, int buflen, struct alg_info *alg_info)
{
	...
	case PROTO_ISAKMP:
		ALG_INFO_IKE_FOREACH((struct alg_info_ike *)alg_info, ike_info,
				     cnt)
		{
			int np;
			alg_info_snprint_ike2(ike_info, ike_info->ike_eklen,
					      ike_info->ike_hklen, &np, ptr,
					      buflen);
			ptr += np;
			buflen -= np;
			if (cnt > 0) {
