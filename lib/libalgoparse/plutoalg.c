...
prfalg_getbyname_ike(const char *const str, const int len, unsigned int *auxp)
{
	...
	if (strncasecmp(str, "prf", 3) == 0) {
		search_ret =
			keyword_search(&ikev2_prf_alg_names.aliases, str + 3);
		if (search_ret >= 0)
			return search_ret;
	} else {
		...
		if (prfname) {
			...
			search_ret = enum_search_nocase(
				&ikev2_prf_names, prfname, strlen(prfname));
			if (search_ret >= 0)
				return search_ret;
		}
	}
 ...
 */
int modp_getbyname_ike(const char *const str, int len, unsigned int *auxp)
{
	int ret = -1;
	if (!str || !*str)
		return ret;
...
static void __alg_info_ike_add(struct alg_info_ike *alg_info, int ealg_id,
			       ...
			       int prfalg_id, unsigned int modp_id)
{
...
static enum ikev2_trans_type_encr default_cipher_algs[] = {
	IKEv2_ENCR_AES_CBC, IKEv2_ENCR_AES_GCM_8, /* IoT SHOULD */
};
	...
	for (i_group = 0; i_group < n_groups; i_group++) {
		...
		for (i_prf = 0; i_prf < n_prfs; i_prf++) {
			...
			for (i_integ = 0; i_integ < n_integs; i_integ++) {
				...
				for (i_cipher = 0; i_cipher < n_ciphers;
				     i_cipher++) {
					int x_cipher = ciphers[i_cipher];

					if (ek_bits == 0 &&
					    ciphers == default_cipher_algs) {
						struct ike_encr_desc *edesc =
							ike_alg_get_encr(
								x_cipher);
						if (edesc)
							ek_bits =
							...
							DBG(DBG_CRYPT,
							    DBG_log("warning: unable to get cipher default keylen, "
								    "ike_alg_get_encr(%d) == NULL",
								    x_cipher));
					}

					__alg_info_ike_add(
						(struct alg_info_ike *)alg_info,
						x_cipher, ek_bits, x_integ,
						ak_bits, x_prf_id, x_modp_id);
				}
			}
		}
	}
				 ...
				 struct alg_info_esp *alg_info)
{
	...
	ALG_INFO_ESP_FOREACH(alg_info, esp_info, cnt)
	{
		...
		if (!eklen)
			eklen = kernel_alg_esp_enc_keylen(
					esp_info->esp_ealg_id) *
				BITS_PER_BYTE;
		...
		ret = snprintf(ptr, buflen, "%s%s(%d)_%03d-%s(%d)_%03d", sep,
				...
				struct alg_info_esp *alg_info)
{
	...
	ALG_INFO_ESP_FOREACH(alg_info, esp_info, cnt)
	{
		...
		ret = snprintf(ptr, buflen, "%s%s(%d)_%03d", sep,
...
{
	const char *modpname =
		enum_name(ikev2_group_names.official_names, modp_id);
...
char *alg_info_snprint_ike2(struct ike_info *ike_info, int eklen, int aklen,
			    int *usedsize, char *buf, int buflen)
{
	int ret;
	char *curbuf = buf;
	const int totlen = buflen;
	const char *prfname = enum_name(ikev2_prf_alg_names.official_names,
					ike_info->ike_prfalg);
	const char *modpname = alg_info_modp_shortname(ike_info->ike_modp);
	const char *encname =
		enum_name(ikev2_encr_names.official_names, ike_info->ike_ealg);
	const char *hashname =
		enum_name(ikev2_integ_names.official_names, ike_info->ike_halg);
	assert(prfname != NULL);
	assert(ike_info != NULL);
	if (eklen == 0) {
		ret = snprintf(curbuf, buflen - 1, "%s(%d)", encname,
			       ike_info->ike_ealg);
	} else {
		ret = snprintf(curbuf, buflen - 1, "%s(%d)_%03d", encname,
			       ike_info->ike_ealg, eklen);
	}
	if (ret <= 0)
		return "invalid ikeinfo";
	curbuf += ret;
	buflen -= ret;

	if (aklen == 0) {
		ret = snprintf(curbuf, buflen - 1, "-%s(%d)", hashname,
			       ike_info->ike_halg);
	} else {
		ret = snprintf(curbuf, buflen - 1, "-%s(%d)_%03d", hashname,
	...
	ret = snprintf(curbuf, buflen - 1, "-%s(%d)-%s(%d)", prfname,
		       ike_info->ike_prfalg, modpname, ike_info->ike_modp);
	buflen -= ret;

	if (usedsize)
		*usedsize = (totlen - buflen);
	return buf;
}
			  ...
			  struct alg_info_ike *alg_info)
{
	...
	ALG_INFO_IKE_FOREACH(alg_info, ike_info, cnt)
	{
		if (ike_alg_enc_present(ike_info->ike_ealg,
					ike_info->ike_eklen) &&
		    (ikev2_alg_integ_present(ike_info->ike_halg,
					     ike_info->ike_hklen)) &&
			...
			if (ret >= 0 && ret <= buflen) {
				ptr += ret;
				buflen -= ret;
				alg_info_snprint_ike2(ike_info, eklen, aklen,
						      &ret, ptr, buflen);
			}
...
struct alg_info_ike *alg_info_ike_defaults(void)
{
	...
	alg_info_ike_add(IKETOINFO(ike_info), IKEv2_ENCR_INVALID, 0,
			 IKEv2_AUTH_INVALID, 0, IKEv2_PRF_INVALID,
			 OAKLEY_INVALID_GROUP);
						  ...
						  const char **err_p)
{
	...
	if (alg_info_parse_str((struct alg_info *)alg_info_ike, alg_str, err_p,
			       parser_init_ike, alg_info_ike_add,
			       lookup_group) < 0) {
			     ...
			     struct alg_info_esp *alg_info)
{
	...
	if (ealg_insecure ||
		...
		openswan_log(
			...
			ealg_insecure ?
				"insecure key_len and enc. alg. not listed in \"esp\" string" :
				"strict flag");
		return FALSE;
	}
	return TRUE;
