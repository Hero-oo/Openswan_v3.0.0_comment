...
kernelalg2ikev2(enum ipsec_authentication_algo kernel_integ)
{
...
static struct pluto_sadb_alg *sadb_alg_ptr(
	int satype UNUSED, int exttype, int alg_id, int rw,
...
)
{
	...
	case SADB_EXT_SUPPORTED_AUTH:
		...
		if (extname)
			*extname = "integ";
		if (alg_names)
			*alg_names = &trans_type_integ_names;
		if (p_ikev2_id)
			*p_ikev2_id = v2_auth_id;
	...
	case SADB_EXT_SUPPORTED_ENCRYPT:
		if (extname)
			*extname = "encr";
		if (alg_names)
			*alg_names = &trans_type_encr_names;
		if (p_ikev2_id)
			*p_ikev2_id = alg_id;
...
const struct pluto_sadb_alg *kernel_alg_sadb_alg_get(int satype, int exttype,
						     int alg_id)
{
...
int kernel_alg_add(int satype, int exttype, const struct sadb_alg *sadb_alg)
{
	...
	if (!(alg_p = sadb_alg_ptr(satype, exttype, alg_id, 1, &extname, &en,
				   &ikev2num))) {
	...
	if (alg_p->kernel_sadb_alg.sadb_alg_minbits == 0) {
		openswan_log(
			"registered kernel %s algorithm %s [%u, %u<=key<=%u]",
			extname, enum_show(en, ikev2num), ikev2num,
			sadb_alg->sadb_alg_minbits, sadb_alg->sadb_alg_maxbits);
	}
			    ...
			    struct alg_info_esp *alg_info)
{
	...
	if (alg_id == ESP_AES_GCM_8 || alg_id == ESP_AES_GCM_12 ||
	    alg_id == ESP_AES_GCM_16) {
		if (key_len != 128 && key_len != 192 && key_len != 256) {
			ugh = builddiag(
				"kernel_alg_db_add() key_len is incorrect: alg_id=%d, "
     ...
     */
	if ((key_len) &&
	    ((key_len < alg_p->kernel_sadb_alg.sadb_alg_minbits) ||
	     (key_len > alg_p->kernel_sadb_alg.sadb_alg_maxbits))) {
		ugh = builddiag(
			"kernel_alg_db_add() key_len not in range: alg_id=%d, "
			"key_len=%d, alg_minbits=%d, alg_maxbits=%d",
			alg_id, key_len,
			alg_p->kernel_sadb_alg.sadb_alg_minbits,
			alg_p->kernel_sadb_alg.sadb_alg_maxbits);
	...
	if (!ugh && alg_p != NULL) {
		DBG(DBG_KLIPS,
		    DBG_log("kernel_alg_esp_enc_ok(%d,%d): "
			    ...
			    "res=%d, ret=%d",
			    alg_id, key_len, alg_p->kernel_sadb_alg.sadb_alg_id,
			    alg_p->kernel_sadb_alg.sadb_alg_ivlen,
			    alg_p->kernel_sadb_alg.sadb_alg_minbits,
			    alg_p->kernel_sadb_alg.sadb_alg_maxbits,
			    alg_p->kernel_sadb_alg.sadb_alg_reserved, ret););
	} else {
		DBG(DBG_KLIPS, DBG_log("kernel_alg_esp_enc_ok(%d,%d): NO",
				       alg_id, key_len););
	}
	return ugh;
}
...
int kernel_alg_esp_enc_keylen(int alg_id)
{
	...
	if (!ESP_EALG_PRESENT(alg_id))
		goto none;
	keylen = esp_ealg[alg_id].kernel_sadb_alg.sadb_alg_maxbits /
		 BITS_PER_BYTE;
	switch (alg_id) {
	/*
                     * this is veryUgly[TM]
...
struct pluto_sadb_alg *kernel_alg_esp_sadb_alg(int alg_id)
{
...
int kernel_alg_esp_auth_keylen(enum ikev2_trans_type_integ authnum)
{
...
int kernel_alg_ah_auth_keylen(enum ikev2_trans_type_integ authnum)
{
	return kernel_alg_esp_auth_keylen(authnum);
}

bool kernel_alg_esp_info(struct esp_info *ei,
			 enum ikev2_trans_type_encr sadb_ealg, u_int16_t keylen,
			 enum ikev2_trans_type_integ sadb_aalg)
{
	...
	if (keylen == 0) {
		if (ei) {
			ei->enckeylen =
				ei->encr_info->kernel_sadb_alg.sadb_alg_minbits /
				BITS_PER_BYTE;
		}
		...
		openswan_log(
			"ealg %s proposed keylen=%u is invalid, not %u<X<%u ",
			enum_name(&trans_type_encr_names, sadb_ealg), keylen,
	...
	if (ei) {
		ei->authkeylen =
			ei->auth_info->kernel_sadb_alg.sadb_alg_maxbits /
			BITS_PER_BYTE;
		DBG(DBG_PARSING,
		    DBG_log("kernel_alg_esp_info():"
			    "transid=%d, auth=%d, "
			    "enckeylen=%d, authkeylen=%d",
			    sadb_ealg, sadb_aalg, (int)ei->enckeylen,
			    (int)ei->authkeylen));
	}
	return TRUE;
}
