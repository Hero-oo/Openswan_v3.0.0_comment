 ...
 * so we leave it off.
 */
const char *const debug_bit_names[] = { "raw",
					"crypt",
					"parsing",
					"emitting",
					"control",
...
enum_names connection_kind_names = { CK_UNSET, CK_GOING_AWAY,
				     connection_kind_name, NULL };

/* Payload types (RFC 2408 "ISAKMP" section 3.1) */

const char *const payload_name[] = {
	"ISAKMP_NEXT_NONE",   "ISAKMP_NEXT_SA", /* 1 */
	"ISAKMP_NEXT_P",      "ISAKMP_NEXT_T",
	"ISAKMP_NEXT_KE",     "ISAKMP_NEXT_ID", /* 5 */
	"ISAKMP_NEXT_CERT",   "ISAKMP_NEXT_CR",
	"ISAKMP_NEXT_HASH",   "ISAKMP_NEXT_SIG",
	"ISAKMP_NEXT_NONCE", /* 10 */
	"ISAKMP_NEXT_N",      "ISAKMP_NEXT_D",
	"ISAKMP_NEXT_VID",    "ISAKMP_NEXT_MODECFG", /* 14 */
	"ISAKMP_NEXT_NAT-D",  "ISAKMP_NEXT_16",
	"ISAKMP_NEXT_17",     "ISAKMP_NEXT_18",
	"ISAKMP_NEXT_19",     "ISAKMP_NEXT_NAT-D",
...
const char *const payload_name_ikev2_main[] = {
	"ISAKMP_NEXT_v2SA", /* 33 */
	"ISAKMP_NEXT_v2KE",	 "ISAKMP_NEXT_v2IDi",
	"ISAKMP_NEXT_v2IDr",	 "ISAKMP_NEXT_v2CERT",
	"ISAKMP_NEXT_v2CERTREQ", "ISAKMP_NEXT_v2AUTH",
	"ISAKMP_NEXT_v2Ni",	 "ISAKMP_NEXT_v2N",
	"ISAKMP_NEXT_v2D",	 "ISAKMP_NEXT_v2V",
	"ISAKMP_NEXT_v2TSi",	 "ISAKMP_NEXT_v2TSr",
	"ISAKMP_NEXT_v2E",	 "ISAKMP_NEXT_v2CP",
	"ISAKMP_NEXT_v2EAP",	 NULL /* termination for bitnamesof() */
};

const char *const payload_name_nat_d[] = { "ISAKMP_NEXT_NAT-D",
					   "ISAKMP_NEXT_NAT-OA", NULL };

static enum_names payload_names_nat_d = { ISAKMP_NEXT_NATD_DRAFTS,
					  ISAKMP_NEXT_NATOA_DRAFTS,
					  payload_name_nat_d, NULL };

static enum_names payload_names_ikev2_main = { ISAKMP_NEXT_v2SA,
					       ISAKMP_NEXT_v2EAP,
...
enum_names payload_names_ikev2 = { ISAKMP_NEXT_NONE, ISAKMP_NEXT_NONE,
				   payload_name_ikev2,
				   &payload_names_ikev2_main };

enum_names payload_names = { ISAKMP_NEXT_NONE, ISAKMP_NEXT_NATOA_RFC,
			     payload_name, &payload_names_ikev2_main };

/* Exchange types (note: two discontinuous ranges) */

static const char *const exchange_name[] = {
	"ISAKMP_XCHG_NONE",	"ISAKMP_XCHG_BASE", "ISAKMP_XCHG_IDPROT",
	"ISAKMP_XCHG_AO",	"ISAKMP_XCHG_AGGR", "ISAKMP_XCHG_INFO",
	"ISAKMP_XCHG_MODE_CFG",
};

static const char *const exchange_name2[] = {
	"ISAKMP_XCHG_QUICK", "ISAKMP_XCHG_NGRP",   "ISAKMP_v2_SA_INIT",
	"ISAKMP_v2_AUTH",    "ISAKMP_v2_CHILD_SA", "ISAKMP_v2_INFORMATIONAL",
};

static enum_names exchange_desc2 = { ISAKMP_XCHG_QUICK, ISAKMP_v2_INFORMATIONAL,
				     exchange_name2, NULL };

enum_names exchange_names = { ISAKMP_XCHG_NONE, ISAKMP_XCHG_MODE_CFG,
			      exchange_name, &exchange_desc2 };
/* Flag BITS */
const char *const flag_bit_names[] = { "ISAKMP_FLAG_ENCRYPTION", /* bit 0 */
...
const char *const sit_bit_names[] = { "SIT_IDENTITY_ONLY", "SIT_SECRECY",
				      "SIT_INTEGRITY", NULL };

/* Protocol IDs (RFC 2407 "IPsec DOI" section 4.4.1) */

static const char *const protocol_name[] = {
	"PROTO_RESERVED",  "PROTO_ISAKMP", "PROTO_IPSEC_AH",
	"PROTO_IPSEC_ESP", "PROTO_IPCOMP",
};

enum_names protocol_names = { PROTO_RESERVED, PROTO_IPCOMP, protocol_name,
			      NULL };
...
enum_names ikev2_protocol_names = { 0, PROTO_IPSEC_ESP, ikev2_protocol_name,
				    NULL };
...
enum_names isakmp_transformid_names = { KEY_IKE, KEY_IKE, isakmp_transform_name,
					NULL };
...
enum_names ah_transformid_names_private_use = { AH_NULL, AH_SHA2_256_TRUNC,
						ah_transform_name_private_use,
						NULL };

static const char *const ah_transform_name[] = {
	/* 0-1 RESERVED */
	"AH_MD5",	   "AH_SHA",	      "AH_DES",	   "AH_SHA2_256",
	"AH_SHA2_384",	   "AH_SHA2_512",     "AH_RIPEMD", "AH_AES_XCBC_MAC",
	"AH_RSA",	   "AH_AES_128_GMAC", /* RFC4543 Errata1821  */
...
enum_names ah_transformid_names = { AH_MD5, AH_AES_256_GMAC, ah_transform_name,
...
static const char *const esp_transform_name_private_use[] = {
	/* id=249 */
	"ESP_MARS",    "ESP_RC6",   "ESP_KAME_NULL", "ESP_SERPENT",
	"ESP_TWOFISH", "ESP_ID254", "ESP_ID255",
};

enum_names esp_transformid_names_private_use = { ESP_MARS, ESP_ID255,
...
static const char *const esp_transform_name[] = {
	...
	"ESP_DES",	  "ESP_3DES",
	"ESP_RC5",	  "ESP_IDEA",
	"ESP_CAST",	  "ESP_BLOWFISH",
	"ESP_3IDEA",	  "ESP_DES_IV32",
	"ESP_RC4",	  "ESP_NULL",
	"ESP_AES",	  "ESP_AES_CTR",
	"ESP_AES_CCM_8",  "ESP_AES_CCM_12",
	"ESP_AES_CCM_16", "ESP_UNASSIGNED_ID17",
	"ESP_AES_GCM_8",  "ESP_AES_GCM_12",
	"ESP_AES_GCM_16", "ESP_SEED_CBC",
	"ESP_CAMELLIA",	  "ESP_NULL_AUTH_AES_GMAC", /* RFC4543 [Errata1821] */
	/* 24-248    Unassigned */
	/* 249-255   Reserved for private use */
};

enum_names esp_transformid_names = { ESP_DES_IV64, ESP_NULL_AUTH_AES_GMAC,
				     esp_transform_name,
...
enum_names ipcomp_transformid_names = { IPCOMP_OUI, IPCOMP_V42BIS,
					ipcomp_transform_name, NULL };

/* Identification type values */

static const char *const ident_name[] = {
	"ID_IPV4_ADDR",	      "ID_FQDN",
	"ID_USER_FQDN",	      "ID_IPV4_ADDR_SUBNET",
	"ID_IPV6_ADDR",	      "ID_IPV6_ADDR_SUBNET",
	"ID_IPV4_ADDR_RANGE", "ID_IPV6_ADDR_RANGE",
	"ID_DER_ASN1_DN",     "ID_DER_ASN1_GN",
	"ID_KEY_ID",
};

enum_names ident_names = { ID_IPV4_ADDR, ID_KEY_ID, ident_name, NULL };
...
enum_names cert_type_names = { CERT_NONE, CERT_X509_ATTRIBUTE, cert_type_name,
			       NULL };
...
enum_names ikev2_cert_type_names = { CERT_NONE, CERT_RAW_RSA,
				     ikev2_cert_type_name, NULL };

/*
 * certificate request payload policy
 */
static const char *const certpolicy_type_name[] = { "CERT_NEVERSEND",
						    "CERT_SENDIFASKED",
						    "CERT_ALWAYSSEND",
						    "CERT_FORCEDTYPE" };

enum_names certpolicy_type_names = { cert_neversend, cert_alwayssend,
				     certpolicy_type_name, NULL };
...
const char *const oakley_attr_bit_names[] = { "OAKLEY_TRANSFORM_ZERO",
...
static const char *const ipsec_attr_name[] = {
	"SA_LIFE_TYPE",	      "SA_LIFE_DURATION",   "GROUP_DESCRIPTION",
	"ENCAPSULATION_MODE", "AUTH_ALGORITHM",	    "KEY_LENGTH",
	"KEY_ROUNDS",	      "COMPRESS_DICT_SIZE", "COMPRESS_PRIVATE_ALG",
	"ECN_TUNNEL",
};
...
enum_names ipsec_private_attr_names = { SECCTX, SECCTX, ipsec_private_attr_name,
					NULL };

enum_names ipsec_attr_names = { SA_LIFE_TYPE, ECN_TUNNEL, ipsec_attr_name,
...
enum_names sa_lifetime_names = { SA_LIFE_TYPE_SECONDS, SA_LIFE_TYPE_KBYTES,
				 sa_lifetime_name, NULL };
...
static enum_names enc_rfc_mode_names = { ENCAPSULATION_MODE_TUNNEL,
					 ENCAPSULATION_MODE_UDP_TRANSPORT_RFC,
					 enc_rfc_mode_name, NULL };

enum_names enc_mode_names = { ENCAPSULATION_MODE_UDP_TUNNEL_DRAFTS,
			      ENCAPSULATION_MODE_UDP_TRANSPORT_DRAFTS,
			      enc_draft_mode_name, &enc_rfc_mode_names };
...
enum_names auth_alg_names_stolen_use = { AUTH_ALGORITHM_NULL_KAME,
					 AUTH_ALGORITHM_NULL_KAME,
					 auth_alg_name_stolen_use, NULL };

static const char *const auth_alg_name[] = {
	"AUTH_ALGORITHM_NONE", /* our own value, not standard */
	"AUTH_ALGORITHM_HMAC_MD5",	"AUTH_ALGORITHM_HMAC_SHA1",
	"AUTH_ALGORITHM_DES_MAC",	"AUTH_ALGORITHM_KPDK",
	"AUTH_ALGORITHM_HMAC_SHA2_256", "AUTH_ALGORITHM_HMAC_SHA2_384",
	"AUTH_ALGORITHM_HMAC_SHA2_512", "AUTH_ALGORITHM_HMAC_RIPEMD",
	"AUTH_ALGORITHM_AES_CBC",	"AUTH_ALGORITHM_SIG_RSA", /* RFC4359 */
...
enum_names auth_alg_names = { AUTH_ALGORITHM_NONE, AUTH_ALGORITHM_AES_CBC,
			      auth_alg_name, &auth_alg_names_stolen_use };

const char *const modecfg_cisco_attr_name[] = {
	"CISCO_BANNER",	       "CISCO_SAVE_PW",	      "CISCO_DEF_DOMAIN",
	"CISCO_SPLIT_DNS",     "CISCO_SPLIT_INC",     "CISCO_UDP_ENCAP_PORT",
	"CISCO_UNKNOWN",       "CISCO_DO_PFS",	      "CISCO_FW_TYPE",
	"CISCO_BACKUP_SERVER", "CISCO_DDNS_HOSTNAME", NULL
};

enum_names modecfg_cisco_attr_names_tv = { CISCO_BANNER + ISAKMP_ATTR_AF_TV,
					   CISCO_DDNS_HOSTNAME +
						   ISAKMP_ATTR_AF_TV,
					   modecfg_cisco_attr_name, NULL };

enum_names modecfg_cisco_attr_names = { CISCO_BANNER, CISCO_DDNS_HOSTNAME,
					modecfg_cisco_attr_name,
					&modecfg_cisco_attr_names_tv };

/* From draft-beaulieu-ike-xauth */
const char *const xauth_attr_name[] = { "XAUTH-TYPE",
					"XAUTH-USER-NAME",
					"XAUTH-USER-PASSWORD",
					"XAUTH-PASSCODE",
					"XAUTH-MESSAGE",
...
enum_names xauth_attr_names_tv = { XAUTH_TYPE + ISAKMP_ATTR_AF_TV,
				   XAUTH_ANSWER + ISAKMP_ATTR_AF_TV,
				   xauth_attr_name, &modecfg_cisco_attr_names };

enum_names xauth_attr_names = { XAUTH_TYPE, XAUTH_ANSWER, xauth_attr_name,
				&xauth_attr_names_tv };

/* for XAUTH-TYPE attribute */
const char *const xauth_type_name[] = { "Generic", "RADIUS-CHAP", "OTP",
					"S/KEY", NULL };
enum_names xauth_type_names = { XAUTH_TYPE_GENERIC, XAUTH_TYPE_SKEY,
				xauth_type_name, NULL };

const char *const modecfg_attr_name[] = {
	"INTERNAL_IP4_ADDRESS",	   "INTERNAL_IP4_NETMASK",
	"INTERNAL_IP4_DNS",	   "INTERNAL_IP4_NBNS",
	"INTERNAL_ADDRESS_EXPIRY", "INTERNAL_IP4_DHCP",
	"APPLICATION_VERSION",	   "INTERNAL_IP6_ADDRESS",
	"INTERNAL_IP6_NETMASK",	   "INTERNAL_IP6_DNS",
	"INTERNAL_IP6_NBNS",	   "INTERNAL_IP6_DHCP",
	"INTERNAL_IP4_SUBNET",	   "SUPPORTED_ATTRIBUTES",
	"INTERNAL_IP6_SUBNET",	   NULL
};

enum_names modecfg_attr_names_tv = { INTERNAL_IP4_ADDRESS + ISAKMP_ATTR_AF_TV,
				     INTERNAL_IP6_SUBNET + ISAKMP_ATTR_AF_TV,
				     modecfg_attr_name, &xauth_attr_names };

enum_names modecfg_attr_names = { INTERNAL_IP4_ADDRESS, INTERNAL_IP6_SUBNET,
				  modecfg_attr_name, &modecfg_attr_names_tv };
...
enum_names oakley_lifetime_names = { OAKLEY_LIFE_SECONDS, OAKLEY_LIFE_KILOBYTES,
				     oakley_lifetime_name, NULL };

/* IKEv2 PRF attribute (none defined) */
static const char *const ikev2_prf_name[] = { "prfmd5",	     "prfsha1",
					      "prftiger",    "prfaes128xcbc",
					      "prfsha2_256", "prfsha2_384",
					      "prfsha2_512", "prfaes128cmac" };

enum_names ikev2_prf_names = { IKEv2_PRF_HMAC_MD5, IKEv2_PRF_AES128_CMAC,
			       ikev2_prf_name, NULL };
...
static const char *const oakley_enc_name[] = {
	"OAKLEY_DES_CBC",	  "OAKLEY_IDEA_CBC", "OAKLEY_BLOWFISH_CBC",
	"OAKLEY_RC5_R16_B64_CBC", "OAKLEY_3DES_CBC", "OAKLEY_CAST_CBC",
	"OAKLEY_AES_CBC",
};

enum_names oakley_enc_names = { OAKLEY_DES_CBC, OAKLEY_AES_CBC, oakley_enc_name,
				NULL };
...
enum_names oakley_hash_names2 = { OAKLEY_SHA, OAKLEY_SHA, oakley_hash_name2,
				  NULL };

static const char *const oakley_hash_name[] = {
	"OAKLEY_MD5",	   "OAKLEY_SHA1",     "OAKLEY_TIGER",
	"OAKLEY_SHA2_256", "OAKLEY_SHA2_384", "OAKLEY_SHA2_512",
};

enum_names oakley_hash_names = { OAKLEY_MD5, OAKLEY_SHA2_512, oakley_hash_name,
				 &oakley_hash_names2 };

/* Oakley Authentication Method attribute */

static const char *const oakley_auth_name1[] = {
	"OAKLEY_PRESHARED_KEY",	  "OAKLEY_DSS_SIG",	"OAKLEY_RSA_SIG",
	"OAKLEY_RSA_ENC",	  "OAKLEY_RSA_ENC_REV", "OAKLEY_ELGAMAL_ENC",
...
static enum_names oakley_auth_names1 = { OAKLEY_PRESHARED_KEY,
					 OAKLEY_ELGAMAL_ENC_REV,
					 oakley_auth_name1, NULL };

static enum_names oakley_auth_names2 = { HybridInitRSA, HybridRespDSS,
					 oakley_auth_name2,
					 &oakley_auth_names1 };

enum_names oakley_auth_names = { XAUTHInitPreShared,
				 XAUTHRespRSARevisedEncryption,
...
enum_names ikev2_auth_names = { v2_AUTH_RSA, v2_AUTH_DSA, ikev2_auth_strings,
				NULL };

/* Oakley Group Description attribute */

static const char *const oakley_group_name[] = {
	"OAKLEY_GROUP_MODP768", "OAKLEY_GROUP_MODP1024", "OAKLEY_GROUP_GP155",
	"OAKLEY_GROUP_GP185",	"OAKLEY_GROUP_MODP1536",
};

/* from rfc3526, rfc5114 and rfc5903 */
static const char *const oakley_group_name_rfc3526_rfc5114_rfc5903[] = {
	"OAKLEY_GROUP_MODP2048", "OAKLEY_GROUP_MODP3072",
	"OAKLEY_GROUP_MODP4096", "OAKLEY_GROUP_MODP6144",
	"OAKLEY_GROUP_MODP8192", "OAKLEY_GROUP_ECP256",
	"OAKLEY_GROUP_ECP384",	 "OAKLEY_GROUP_ECP512",
	"OAKLEY_GROUP_DH22",	 "OAKLEY_GROUP_DH23",
	"OAKLEY_GROUP_DH24"
};

/* from rfc8031 -- EdDSA curves */
static const char *const oakley_group_name_rfc8031[] = { "OAKLEY_GROUP_X25519",
							 "OAKLEY_GROUP_X448" };

enum_names oakley_group_names_rfc8031 = { OAKLEY_GROUP_X25519,
					  OAKLEY_GROUP_X448,
...
enum_names oakley_group_names = { OAKLEY_GROUP_MODP768, OAKLEY_GROUP_MODP1536,
				  oakley_group_name,
				  &oakley_group_names_rfc3526_rfc5114_rfc5903 };
...
enum_and_keyword_names ikev2_group_names = {
	...
	aliases:
		{ ikev2_group_name_aliases, elemsof(ikev2_group_name_aliases) },
};
...
enum_names oakley_group_type_names = { OAKLEY_GROUP_TYPE_MODP,
				       OAKLEY_GROUP_TYPE_EC2N,
				       oakley_group_type_name, NULL };
...
enum_names notification_juniper_names = { NETSCREEN_NHTB_INFORM,
					  NETSCREEN_NHTB_INFORM,
					  notification_juniper_name, NULL };

enum_names notification_cisco_more_names = { ISAKMP_N_CISCO_LOAD_BALANCE,
					     ISAKMP_N_CISCO_PRESHARED_KEY_HASH,
					     notification_cisco_more_name,
					     &notification_juniper_names };

enum_names notification_ios_alives_names = { ISAKMP_N_IOS_KEEP_ALIVE_REQ,
					     ISAKMP_N_IOS_KEEP_ALIVE_ACK,
					     notification_ios_alives_name,
					     &notification_cisco_more_names };
...
enum_names notification_dpd_names = { R_U_THERE, R_U_THERE_ACK,
				      notification_dpd_name,
				      &notification_cisco_chatter_names };

enum_names notification_names = { INVALID_PAYLOAD_TYPE, AUTHORIZATION_FAILED,
				  notification_name, &notification_dpd_names };

enum_names notification_status_names = { CONNECTED, CONNECTED,
					 notification_status_name,
					 &notification_names };

enum_names ipsec_notification_names = { IPSEC_RESPONDER_LIFETIME,
					IPSEC_INITIAL_CONTACT,
					ipsec_notification_name,
...
enum_names ikev2_notify_names_16384 = { v2N_INITIAL_CONTACT,
					v2N_SECURE_PASSWORD_METHODS,
					ikev2_notify_name_16384, NULL };

enum_names ikev2_notify_names = { 0, v2N_CHILD_SA_NOT_FOUND, ikev2_notify_name,
...
enum_names ikev2_ts_type_names = { IKEv2_TS_IPV4_ADDR_RANGE,
				   IKEv2_TS_FC_ADDR_RANGE, ikev2_ts_type_name,
				   NULL };
...
const char *const attr_msg_type_name[] = {
	"ISAKMP_CFG_RESERVED", "ISAKMP_CFG_REQUEST", "ISAKMP_CFG_REPLY",
	"ISAKMP_CFG_SET",      "ISAKMP_CFG_ACK",     NULL
};

enum_names attr_msg_type_names = { 0, ISAKMP_CFG_ACK, attr_msg_type_name,
				   NULL };
...
const char *const trans_type_encr_name[] = {
	"des_iv64",	 "des",		  "3des",	"rc5",
	"idea",		 "cast",	  "blowfish",	"3idea",
	"des_iv32",	 "res10",	  "null",	"aes_cbc",
	"aes_ctr",	 "aes_ccm_8",	  "aes_ccm_12", "aes_ccm_16",
	"unassigned_17", "aes_gcm_8",	  "aes_gcm_12", "aes_gcm_16",
	"null_aes_gmac", "p1619_xts_aes",
};
enum_names trans_type_encr_names = { IKEv2_ENCR_DES_IV64,
				     IKEv2_IEEE_P1619_XTS_AES,
				     trans_type_encr_name, NULL };
...
enum_names trans_type_prf_names = { IKEv2_PRF_HMAC_MD5, IKEv2_PRF_HMAC_SHA2_512,
				    trans_type_prf_name, NULL };
...
enum_names trans_type_integ_names = { IKEv2_AUTH_NONE,
				      IKEv2_AUTH_HMAC_SHA2_512_256,
				      trans_type_integ_name, NULL };
...
enum_and_keyword_names ikev2_integ_names = {
	...
	aliases:
		{ ikev2_integ_name_aliases, elemsof(ikev2_integ_name_aliases) },
};
...
enum_names trans_type_esn_names = { IKEv2_ESN_DISABLED, IKEv2_ESN_ENABLED,
				    trans_type_esn_name, NULL };

/* Transform Type */
const char *const trans_type_name[] = { "trans-type-encr", "trans-type-prf",
					"trans-type-integ", "trans-type-dh",
					"trans-type-esn" };
enum_names trans_type_names = { IKEv2_TRANS_TYPE_ENCR, IKEv2_TRANS_TYPE_ESN,
...
enum_names ikev2_trans_attr_descs = { IKEv2_KEY_LENGTH, IKEv2_KEY_LENGTH,
...
const unsigned int ikev2_trans_attr_val_descs_size =
	elemsof(ikev2_trans_attr_val_descs);
...
enum_names rr_qtype_names = { ns_t_tkey, ns_t_any, rr_qtype_name,
			      &rr_type_names };

static const char *const rr_class_name[] = { "C_IN", /* 1 the arpa internet */
					     NULL };

enum_names rr_class_names = { ns_c_in, ns_c_in, rr_class_name, NULL };

static const char *const ppk_name[] = { "PPK_PSK", "PPK_DSS",	"PPK_RSA",
					"PPK_PIN", "PPK_XAUTH", NULL };
...
struct keyword_enum_values kw_host_list = {
	kw_host_values,
	sizeof(kw_host_values) / sizeof(struct keyword_enum_value)
};

/* look up enum names in an enum_names */

const char *enum_name_default(enum_names *ed, unsigned long val,
			      const char *def)
{
	enum_names *p;
...
const struct keyword_enum_value *
keyword_search_aux(const struct keyword_enum_values *kevs, const char *str)
{
	...
	for (kevcount = kevs->valuesize, kev = kevs->values;
	     kevcount > 0 && strcasecmp(str, kev->name) != 0; kev++, kevcount--)
		;
...
int keyword_search(const struct keyword_enum_values *kevs, const char *str)
{
...
const char *keyword_name(const struct keyword_enum_values *kevs,
			 unsigned int value, char namebuf[KEYWORD_NAME_BUFLEN])
{
	int kevcount;
	const struct keyword_enum_value *kev;

	for (kevcount = kevs->valuesize, kev = kevs->values;
	     kevcount > 0 && kev->value != value; kev++, kevcount--)
		;
...
const char *end_type_name(enum keyword_host host_type, ip_address *host_addr,
			  char *outbuf, size_t outbuf_len)
{
	if (host_type != KH_IPADDR) {
		if (outbuf_len < KEYWORD_NAME_BUFLEN)
			return "truncated";
...
int enum_search_cmp(enum_names *ed, const char *str, size_t len, strcmpfunc cmp)
{
	enum_names *p;
	const char *ptr;
	unsigned en;
...
int enum_search_nocase(enum_names *ed, const char *str, size_t len)
{
 ...
 */
const char *bitnamesofb(const char *const table[], lset_t val, char *b,
			size_t blen)
{
	char *p = b;
	lset_t bit;
	...
	for (tp = table, bit = 01; val != 0; bit <<= 1) {
		if (val & bit) {
			...
			if (n == NULL || *n == '\0') {
				...
				snprintf(flagbuf, sizeof(flagbuf), "0x%llx",
					 bit);
				n = flagbuf;
			}
