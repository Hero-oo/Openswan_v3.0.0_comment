...
const char * /* NULL on success, else literal */
	ttodata(src, srclen, base, dst, dstlen, lenp) const char *src;
size_t srclen; /* 0 means apply strlen() */
int base; /* 0 means figure it out */
char *dst; /* need not be valid if dstlen is 0 */
size_t dstlen;
...
size_t /* 0 for failure, true length for success */
	atodata(src, srclen, dst, dstlen) const char *src;
size_t srclen;
char *dst;
size_t dstlen;
{
 ...
 */
const char *atobytes(src, srclen, dst, dstlen, lenp) const char *src;
size_t srclen;
char *dst;
size_t dstlen;
size_t *lenp;
...
static int /* number of result bytes, or error code */
	unhex(src, dst, dstlen) const char *src; /* known to be full length */
char *dst;
 ...
 */
static int /* number of result bytes, or error code */
	unb64(src, dst, dstlen) const char *src; /* known to be full length */
char *dst;
size_t dstlen;
{
	char *p;
 ...
 */
static int /* number of result bytes, or error code */
	untext(src, dst, dstlen) const char *src; /* known to be full length */
char *dst;
size_t dstlen; /* not large enough is a failure */
{
	if (dstlen < 1)
...
static const char * /* literal or errp */
	badch(src, errcode, errp, errlen) const char *src;
int errcode;
 ...
 */
err_t ckaidhex2ckaid(const char *key_ckaid_hex,
		     unsigned char ckaid[CKAID_BUFSIZE])
{
	return ttodatav(key_ckaid_hex, 0, 16, (char *)ckaid, CKAID_BUFSIZE,
			NULL, NULL, 0, TTODATAV_IGNORESPACE);
}
 ...
 */
static void hexout(s, len, f) const char *s;
size_t len;
FILE *f;
{
	size_t i;
...
#define IGNORESPACE_BIAS 1000
	...
	{ 16, "2A3A 92D4 E0FA 5CD7 8DE1 D133 0C62 6985 2B6E D701",
	  "\x2A\x3A\x92\xD4\xE0\xFA\x5C\xD7\x8D\xE1\xD1\x33\x0C\x62\x69\x85\x2B\x6E\xD7\x01",
	  TTODATAV_IGNORESPACE },
	{
		0,
		"0s",
 ...
 */
static void check(r, buf, n, oops, status) struct artab *r;
char *buf;
size_t n;
err_t oops;
int *status;
{
	if (oops != NULL && r->data == NULL) {
		...
		printf("`%s' gave error `%s', expecting %zu `", r->ascii, oops,
		       strlen(r->data));
	...
	regress(pgm) char *pgm;
{
	...
	for (r = atodatatab; r->ascii != NULL; r++) {
		...
		if (base >= IGNORESPACE_BIAS) {
			...
			check(r, buf, n,
			      ttodatav(r->ascii, 0, base, buf, sizeof(buf), &n,
				       NULL, 0, TTODATAV_IGNORESPACE),
			      &status);
			if (xbase != 0)
				check(r, buf, n,
