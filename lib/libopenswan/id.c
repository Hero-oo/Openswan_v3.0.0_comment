...
{
	if (canary[0] != 'a' || canary[1] != 'b' || canary[2] != 'c' ||
	    canary[3] != 'd') {
		return FALSE;
	}
	return TRUE;
...
void unshare_id_content(struct id *id)
{
	...
	case ID_KEY_ID:
		id->name.ptr =
			clone_bytes(id->name.ptr, id->name.len, "keep id name");
...
bool same_exact_id(const struct id *a, const struct id *b)
{
	 ...
	 */
		{
			size_t al = a->name.len, bl = b->name.len;
...
int id_count_wildcards(const struct id *id)
{
	...
	DBG(DBG_CONTROL,
	    DBG_log("counting wild cards for %s is %d", idbuf, count));
