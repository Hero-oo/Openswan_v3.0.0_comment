...
int starter_whack_read_reply(int sock, char xauthname[XAUTH_MAX_NAME_LENGTH],
			     char xauthpass[XAUTH_MAX_PASS_LENGTH],
			     int xauthnamelen, int xauthpasslen)
{
		...
		*be = '\0';
		...
		for (;;) {
			...
			if (write(STDOUT_FILENO, ls, le - ls) == -1) {
				...
				starter_log(
					LOG_LEVEL_ERR,
					"whack: write() failed (%d %s), and ignored.\n",
					e, strerror(e));
			}
...
static int send_whack_msg(struct starter_config *cfg, struct whack_message *msg)
{
	if (cfg->send_whack_msg) {
		...
		starter_log(LOG_LEVEL_ERR,
			    "no send_whack_msg function defined");
		return -1;
	}
}

static int send_whack_msg_to_socket(struct starter_config *cfg,
...
static char *split_dns_hostname(struct starter_conn *conn, char *lr,
				char *dnsname, ip_address *host_addr)
{
		...
		*slash = '\0';
		...
		if (!e) {
			...
			*host_addr = tmp;
			...
			starter_log(
				LOG_LEVEL_DEBUG,
				"conn %s, %s= %dns hint(%s) failed to parse as IPv4/IPv6 address, ignored",
...
static int set_whack_end(struct starter_config *cfg, struct starter_conn *conn,
			 char *lr, struct whack_end *w, struct starter_end *l)
{
	w->id = l->id;
	...
	case KH_IPHOSTNAME:
		...
		w->host_addr_name = split_dns_hostname(
			conn, lr, l->strings[KSCF_IP], &w->host_addr);
		break;
...
int starter_whack_build_pkmsg(struct starter_config *cfg,
			      ...
			      struct starter_end *end, unsigned int keynum,
			      enum pubkey_source key_type,
			      unsigned char *rsakey, char *ckaid_buf,
			      size_t ckaid_buf_len, const char *lr)
{
	...
	if (end->id && rsakey) {
		...
		case PUBKEY_CERTIFICATE:
			starter_log(LOG_LEVEL_DEBUG,
				    "conn %s/%s has key%u from certificate",
				    connection_name(conn), lr, keynum);
			if (ckaid_buf) {
				snprintf(ckaid_buf, ckaid_buf_len,
					 "certificate");
			}
			break;
		...
		case PUBKEY_PREEXCHANGED:
			err = atobytes((char *)rsakey, 0, (char *)keyspace,
				       sizeof(keyspace), &keylen);

			if (ckaid_buf) {
				calc_ckaid(ckaid_buf, ckaid_buf_len, keyspace,
					   keylen);
			}
			...
			if (err) {
				starter_log(
					LOG_LEVEL_ERR,
					"conn %s/%s: rsakey%u malformed [%s]",
					connection_name(conn), lr, keynum, err);
				return 1;
			} else {
				clonereplacechunk(msg->keyval, keyspace, keylen,
						  "rsakey");
				return 0;
			}
		}
				    ...
				    struct starter_end *end, const char *lr)
{
	...
	init_whack_msg(&msg);
	ret = starter_whack_build_pkmsg(cfg, &msg, conn, end, 1,
					end->rsakey1_type, end->rsakey1,
					ckaid_print_buf,
					sizeof(ckaid_print_buf), lr);
	starter_log(LOG_LEVEL_DEBUG, "   looking for key1, result=%d", ret);

	if (ret == 0) {
		starter_log(LOG_LEVEL_DEBUG, "   sending pubkey 1: %s",
			    ckaid_print_buf);
		end->rsakey1_ckaid =
			clone_str(ckaid_print_buf, "pubkey 1 ckaid");
	...
	init_whack_msg(&msg);
	ret = starter_whack_build_pkmsg(cfg, &msg, conn, end, 2,
					end->rsakey2_type, end->rsakey2,
					ckaid_print_buf,
					sizeof(ckaid_print_buf), lr);
	starter_log(LOG_LEVEL_DEBUG, "   looking for key2, result=%d", ret);

	if (ret == 0) {
		starter_log(LOG_LEVEL_DEBUG, "   sending pubkey 2: %s",
			    ckaid_print_buf);
		end->rsakey2_ckaid =
			clone_str(ckaid_print_buf, "pubkey 2 ckaid");
		ret = send_whack_msg(cfg, &msg);
		if (ret != 0)
			return ret;
	}
				   ...
				   struct starter_conn *conn)
{
	...
	starter_log(
		...
		conn->name, msg->end_addr_family, msg->tunnel_addr_family);
	...
	starter_log(LOG_LEVEL_INFO, "conn: \"%s\" loopback=%d", conn->name,
		    msg->loopback);

	if (conn->options_set[KBF_LABELED_IPSEC]) {
		msg->labeled_ipsec = conn->options[KBF_LABELED_IPSEC];
	}
	starter_log(LOG_LEVEL_INFO, "conn: \"%s\" labeled_ipsec=%d", conn->name,
		    msg->labeled_ipsec);

	msg->policy_label = conn->policy_label;
	starter_log(LOG_LEVEL_INFO, "conn: \"%s\" policy_label=%d", conn->name,
		    msg->policy_label);
#endif
					...
					struct starter_conn *conn)
{
	...
	if (conn->policy & POLICY_RSASIG) {
		starter_log(LOG_LEVEL_DEBUG,
			    "conn: \"%s\" sending RSA keys for left",
			    conn->name);
		r = starter_whack_add_pubkey(cfg, conn, &conn->left, "left");
		if (r == 0) {
			starter_log(LOG_LEVEL_DEBUG,
				    "conn: \"%s\" sending RSA keys for right",
				    conn->name);
...
bool one_subnet_from_string(struct starter_conn *conn, char **psubnets, int af,
			    ip_subnet *sn, char *lr)
{
	char *eln;
			    ...
			    struct starter_conn *conn)
{
	...
	if (conn->left.strings_set[KSCF_SUBNET]) {
		...
		one_subnet_from_string(conn, &leftnets,
				       conn->tunnel_addr_family, &lnet, "left");
		lc = 1;
	}
			   ...
			   struct starter_conn *conn)
{
	...
	return starter_permutate_conns(starter_whack_basic_add_conn, cfg, conn);
}
			   ...
			   struct starter_conn *conn)
{
	...
	return starter_permutate_conns(starter_whack_basic_del_conn, cfg, conn);
}
			     ...
			     struct starter_conn *conn)
{
	...
	return starter_permutate_conns(starter_whack_basic_route_conn, cfg,
				       conn);
}
