...
void confwrite_int(FILE *out, char *side, unsigned int context,
		   unsigned int keying_context, knf options,
		   int_set options_set, ksf strings)
{
...
void confwrite_str(FILE *out, char *side, int context, int keying_context,
		   ksf strings, str_set strings_set)
{
	struct keyword_def *k;
	...
	for (k = ipsec_conf_keywords_v2; k->keyname != NULL; k++) {
		...
		if (strings_set[k->field]) {
			...
			fprintf(out, "\t%s%s=%s%s%s\n", side, k->keyname, quote,
				strings[k->field], quote);
		}
	}
}

void confwrite_side(FILE *out, struct starter_conn *conn,
		    struct starter_end *end, char *side)
{
	...
	confwrite_int(out, side, kv_conn | kv_leftright, keyingtype,
		      end->options, end->options_set, end->strings);
	confwrite_str(out, side, kv_conn | kv_leftright, keyingtype,
...
void confwrite_comments(FILE *out, struct starter_conn *conn)
{
	...
	for (sc = conn->comments.tqh_first; sc != NULL; sc = scnext) {
...
void confwrite_conn(FILE *out, struct starter_conn *conn)
{
	...
	confwrite_int(out, "", kv_conn, keyingtype, conn->options,
		      conn->options_set, conn->strings);
	confwrite_str(out, "", kv_conn, keyingtype, conn->strings,
		      conn->strings_set);
	...
	if (conn->end_addr_family) {
		...
		fprintf(out, "\tendaddrfamily=%s\n",
			keyword_name(&kw_connaddrfamily_list,
				     conn->end_addr_family, extra));
	}
	if (conn->tunnel_addr_family) {
		char extra[KEYWORD_NAME_BUFLEN];
		fprintf(out, "\ttunneladdrfamily=%s\n",
			keyword_name(&kw_connaddrfamily_list,
				     conn->tunnel_addr_family, extra));
	}
...
*/
	...
	confwrite_int(out, "", kv_config, 0, cfg->setup.options,
		      cfg->setup.options_set, cfg->setup.strings);
	confwrite_str(out, "", kv_config, 0, cfg->setup.strings,
		      cfg->setup.strings_set);
