...
#define POLICY_ONLY_CONN(conn)                            \
	if (conn->options[KBF_AUTO] > STARTUP_ROUTE) {    \
...
static char **new_list(char *value)
{
	...
	nlist = (char **)alloc_bytes((count + 1) * sizeof(char *),
				     "new_list nlist");
	for (b = val, count = 0; b < end;) {
		e = b + strlen(b);
		if (e != b) {
 ...
 */
static bool load_setup(struct starter_config *cfg, struct config_parsed *cfgp)
{
	bool err = FALSE;
	struct kw_list *kw;
 ...
 */
static bool validate_end(struct starter_conn *conn_st, struct starter_end *end,
			 bool left, bool resolvip UNUSED, err_t *perr)
{
	err_t er = NULL;
	char *err_str = NULL;
...
#define ERR_FOUND(args...)                             \
	...
	case KH_IPADDR:
		...
		if (end->strings[KSCF_IP][0] == '%') {
			if (end->iface)
				pfree(end->iface);
			end->iface = clone_str(end->strings[KSCF_IP] + 1,
					       "KH_IPADDR end->iface");
		...
		er = ttoaddr_num(end->strings[KNCF_IP], 0, AF_INET6,
				 &(end->addr));
		if (er == NULL) { /* no error! */
			newfamily = AF_INET6;
		} else { /* error */
			er = ttoaddr_num(end->strings[KNCF_IP], 0, AF_INET,
					 &(end->addr));
			if (er == NULL) {
				newfamily = AF_INET;
			}
	...
	if (end->strings_set[KSCF_SUBNET]) {
		...
		if (er)
			ERR_FOUND("bad subnet %ssubnet=%s [%s] family=%s",
				  leftright, value, er, family2str(family));
	...
	if (end->strings_set[KSCF_NEXTHOP]) {
		...
		if (strcasecmp(value, "%defaultroute") == 0) {
			...
			if (tnatoaddr(value, strlen(value), AF_INET,
				...
				if (er)
					ERR_FOUND("bad addr %snexthop=%s [%s]",
						  leftright, value, er);
			}
	...
	if (end->options_set[KSCF_RSAKEY1]) {
		...
		case PUBKEY_PREEXCHANGED:
			...
			if (end->strings_set[KSCF_RSAKEY1]) {
				...
				end->rsakey1 = (unsigned char *)clone_str(
					value, "end->rsakey1");
				if (str2pubkey(end->rsakey1, PUBKEY_ALG_RSA,
					       &opk1) == NULL) {
					end->rsakey1_ckaid = clone_str(
						opk1.key_ckaid_print_buf,
						"end->rsakey1_ckaid");
			...
			if (end->strings_set[KSCF_RSAKEY2]) {
				...
				end->rsakey2 = (unsigned char *)clone_str(
					value, "end->rsakey2");
				if (str2pubkey(end->rsakey2, PUBKEY_ALG_RSA,
					       &opk2) == NULL) {
					end->rsakey2_ckaid = clone_str(
						opk2.key_ckaid_print_buf,
						"end->rsakey2_ckaid");
	...
	if (end->strings_set[KSCF_SOURCEIP]) {
		...
		if (tnatoaddr(value, strlen(value), AF_INET,
			...
			if (er)
				ERR_FOUND(
					"bad numerical addr %ssourceip=%s [%s]",
					leftright, value, er);
		}

		if (!end->has_client) {
			starter_log(LOG_LEVEL_INFO,
				    "defaulting %ssubnet to %s\n", leftright,
				    value);
			er = addrtosubnet(&end->sourceip, &end->subnet);
			if (er)
				ERR_FOUND(
					"attempt to default %ssubnet from %s failed: %s",
					leftright, value, er);
			end->has_client = TRUE;
	...
	if (end->strings_set[KSCF_UPDOWN]) {
			...
			clone_str(end->strings[KSCF_UPDOWN], "KSCF_UPDOWN");
	}
 ...
 */
bool translate_conn(struct starter_conn *conn, struct section_list *sl,
		    enum keyword_set assigned_value, err_t *error,
		    bool alsoflip)
{
	...
	for (; kw; kw = kw->next) {
		...
		if (kw->keyword.keydef->validity & kv_leftright) {
			...
			if (kw->keyword.keyleft) {
				snprintf(keyname, sizeof(keyname), "left%s",
					 kw->keyword.keydef->keyname);
				...
				snprintf(keyname, sizeof(keyname), "right%s",
					 kw->keyword.keydef->keyname);
		...
		starter_log(LOG_LEVEL_DEBUG, "#analyzing %s[%d] kwtype=%d\n",
			    keyname, field, kw->keyword.keydef->type);
#endif
		...
		case kt_appendlist:
			...
			if (!(*the_strings)[field]) {
				(*the_strings)[field] = clone_str(
					kw->keyword.string,
					"kt_appendlist kw->keyword.string");
			} else {
				...
				n = alloc_bytes(old_len + 1 + new_len + 1,
						"kt_appendlist");
				memcpy(n, s, old_len);
				n[old_len] = ' ';
				memcpy(n + old_len + 1, kw->keyword.string,
				       new_len + 1); /* includes '\0' */
				(*the_strings)[field] = n;
				pfree(s);
			}
		...
		case kt_loose_enumarg:
			...
			if (kw->number == LOOSE_ENUM_OTHER) {
				...
				(*the_strings)[field] = clone_str(
					...
					"kt_loose_enum kw->keyword.string");
				(*set_strings)[field] = TRUE;
			} else if (kw->keyword.keydef->type ==
					   kt_loose_enumarg &&
				   kw->argument != NULL) {
				pfreeany((*the_strings)[field]);
				(*the_strings)[field] = clone_str(
					kw->argument,
					"kt_loose_enum kw->keyword.argument");
				(*set_strings)[field] = TRUE;
			}
		       ...
		       struct starter_comments_list *from)
{
	...
	for (sc = from->tqh_first; sc != NULL; sc = scnext) {
...
static int load_conn_basic(struct starter_conn *conn, struct section_list *sl,
			   enum keyword_set assigned_value, err_t *perr)
{
	int err;
...
char **process_alsos(struct starter_config *cfg, struct starter_conn *conn,
		     struct config_parsed *cfgp, char **alsos, int alsosize,
		     bool alsoflip, err_t *perr)
{
	int alsoplace;
	unsigned int err;
	alsoplace = 0;
	...
	while (alsoplace < alsosize && alsos[alsoplace] != NULL &&
		...
		for (sl1 = cfgp->sections.tqh_first;
		     sl1 != NULL &&
		     strcasecmp(alsos[alsoplace], sl1->name) != 0;
		...
		if (sl1 && !sl1->beenhere) {
			...
			if (conn->strings[KSF_ALSO]) {
				...
				if (newalsos && newalsos[0] != NULL) {
					...
					/* count them */
					for (newalsoplace = 0;
					     newalsos[newalsoplace] != NULL;
					     newalsoplace++)
						;

					/* extend conn->alsos */
					ra = alloc_bytes(
						(alsosize + newalsoplace + 1) *
							sizeof(char *),
						"conn->alsos");
					memcpy(ra, alsos,
					       alsosize * sizeof(char *));
					pfree(alsos);
					alsos = ra;
		...
		if (alsoplace >= ALSO_LIMIT) {
			starter_log(
				...
				conn->name, conn->alsos[alsoplace], ALSO_LIMIT);
			*perr = "too many also";
			return NULL;
		}
	}
...
static int validate_family_consistency(const char *connname,
				       const char *addrtype, unsigned int left,
				       unsigned int right, unsigned int family)
{
     ...
     */
	if (family != 0 && (left == 0 || left == family) &&
     ...
     */
	if (left != 0 && family == 0 && right != 0 && left == right) {
		nfamily = right;
	}
...
static int load_conn(struct starter_config *cfg, struct starter_conn *conn,
		     struct config_parsed *cfgp, struct section_list *sl,
		     bool alsoprocessing, bool defaultconn, bool resolvip,
		     err_t *perr)
{
	unsigned int err;
	err = 0;
	...
	if (conn->strings[KSF_ALSO] != NULL && !alsoprocessing) {
		starter_log(LOG_LEVEL_INFO,
			    "also= is not valid in section '%s'", sl->name);
		return 1;
	}
	...
	if (alsoprocessing && conn->alsos) {
		...
		starter_log(LOG_LEVEL_DEBUG, "# conn %s processing alsos",
			    conn->name);
		conn->alsos = process_alsos(cfg, conn, cfgp, alsos, alsosize,
					    FALSE, perr);
		...
		if (conn->strings[KSF_ALSOFLIP]) {
			...
			starter_log(LOG_LEVEL_DEBUG,
				    "# conn %s processing alsoflips",
				    conn->name);
			conn->also_flips = process_alsos(cfg, conn, cfgp, alsos,
							 alsosize, TRUE, perr);
		}
	}
	...
	if (conn->options_set[KBF_AUTHBY]) {
		...
		starter_log(LOG_LEVEL_DEBUG,
			    ...
			    conn->name, (unsigned int)conn->policy,
	...
	if (conn->strings_set[KSF_POLICY_LABEL]) {
		conn->policy_label = clone_str(conn->strings[KSF_POLICY_LABEL],
					       "KSF_POLICY_LABEL");
	}
	...
	if (conn->strings_set[KSF_CONNALIAS]) {
		conn->connalias = clone_str(conn->strings[KSF_CONNALIAS],
					    "KSF_CONNALIAS");
	}
	...
	if (!defaultconn) {
		...
		conn->end_addr_family = validate_family_consistency(
			conn->name, "end", conn->left.end_addr_family,
			conn->right.end_addr_family, conn->end_addr_family);

		conn->tunnel_addr_family = validate_family_consistency(
			conn->name, "tunnel", conn->left.tunnel_addr_family,
...
void conn_default(char *n, struct starter_conn *conn, struct starter_conn *def)
{
	int i;
...
#define CONN_STR2(v, T) \
	if (v)          \
	v = (T)clone_str((char *)v, #v)
#define CONN_STR(v) \
	if (v)      \
	v = clone_str((char *)v, #v)
				    ...
				    err_t *perr)
{
	...
	conn = (struct starter_conn *)alloc_bytes(sizeof(struct starter_conn),
...
int init_load_conn(struct starter_config *cfg, struct config_parsed *cfgp,
		   struct section_list *sconn, bool alsoprocessing,
		   bool defaultconn, bool resolvip, err_t *perr)
{
	int connerr;
	struct starter_conn *conn;
	...
	connerr = load_conn(cfg, conn, cfgp, sconn, TRUE, defaultconn, resolvip,
			    perr);
				     ...
				     bool setuponly)
{
	...
	cfg = (struct starter_config *)alloc_bytes(
		sizeof(struct starter_config), "starter_config cfg");
	    ...
	    */
		for (sconn = cfgp->sections.tqh_first; (!err) && sconn != NULL;
			...
			if (strcmp(sconn->name, "%default") == 0) {
				...
				err += load_conn(cfg, &cfg->conn_default, cfgp,
						 sconn, FALSE,
						 /*default conn*/ TRUE,
						 resolvip, perr);
			}
