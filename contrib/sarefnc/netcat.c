...
unsigned int pnum;
{
   ...
   more frequent if we're scanning */
     ...
     * down I'll reconsider. */
		...
		if (servent) {
			...
			if (x != y) /* "never happen" */
				holler("Warning: port-bynum mismatch, %d != %d",
				       x, y);
   ...
   you're at 3com and have some company-internal services defined... */
	if (pstring) {
		...
		if (x)
			return (getportpoop(
				NULL, x)); /* recurse for numeric-string-arg */
...
doexec(fd) int fd;
{
	...
	if (doexec_use_sh) {
		...
		bail("exec %s failed",
		     pr00gie); /* this gets sent out.  Hmm... */
	}
...
/* If you want to screw with RCVBUF/SNDBUF, do it here.  Liudvikas Bukys at
  ...
  rr = setsockopt(nnetfd, SOL_SOCKET, SO_SNDBUF, &o_sndbuf, sizeof o_sndbuf);
	...
	if (rr)
		bail("Can't grab %s:%d with bind", inet_ntoa(lclend->sin_addr),
		     lp);
...
/* don't even bother compiling if we can't do IP options here! */
		...
		if (!optbuf) { /* and don't already *have* a srcrt set */
			...
			*opp++ = (char)(((gatesidx + 1) * sizeof(IA)) + 3) &
				 0xff; /* length */
	...
	*addrlen = msgh.msg_namelen;
	...
	for (cmsg = CMSG_FIRSTHDR(&msgh); cmsg != NULL;
		...
		if (cmsg->cmsg_level == IPPROTO_IP &&
			...
			holler("UDP saref me=%04x him=%04x", refp[0], refp[1]);
...
/* If we can, look for any IP options.  Useful for testing the receiving end of
   ...
   any machines I've tested, but feel free to surprise me. */
	...
	{ /* we've got options, lessee em... */
		...
		char *pp =
			&bigbuf_net[128]; /* get random space farther out... */
		memset(bigbuf_net, 0, 256); /* clear it all first */
		while (x > 0) {
			sprintf(pp, "%2.2x ",
				*q); /* clumsy, but works: turn into hex */
   ...
   other words, we need a TCP MSG_PEEK. */
	...
	if (rp)
		if (z != rp)
			x = 1;
	if (x) /* guilty! */
		bail("invalid connection to [%s] from %s [%s] %d", cp,
...
int n;
{
	...
	*op = ' ';
	...
	while (bc) { /* for chunk-o-data ... */
		...
		if (bc < x) {
			soc = soc - 16 +
			      bc; /* fiddle for however much is left */
			x = (bc * 3) +
			    11; /* 2 digits + space per, after D & offset */
			op = &stage[x];
			x = 16 - bc;
			while (x) {
...
int fd;
{
   ...
   either find it or do your own bit-bashing: *ding1 |= (1 << fd), etc... */
	...
	if (insaved) {
		...
		if (Single) /* if not scanning, this is a one-off first */
			insaved =
				0; /* buffer left over from argv construction, */
		else {
			FD_CLR(0,
			       ding1); /* OR we've already got our repeat chunk, */
   ...
   we create a expendable copy and give *that* to select.  *Fuck* me ... */
		...
		rr = select(fd + 1, ding2, 0, 0,
			    timer2); /* here it is, kiddies */
		if (rr < 0) {
   ...
   us the right info, and none of the examples I found bothered. */
		...
		if (FD_ISSET(fd, ding2)) { /* net: ding! */
			...
			if (rr <= 0) {
				FD_CLR(fd,
				       ding1); /* net closed, we'll finish up... */
				rzleft =
   ...
   open TCP port or every UDP attempt, so save its size and clean up stdin */
				if (!Single) { /* we might be scanning... */
					insaved = rr; /* save len */
					FD_CLR(0,
					       ding1); /* disable further junk from stdin */
   ...
   not sure if the order of this matters, but write net -> stdout first. */
		...
		if (rnleft) {
			...
			if (rr > 0) {
				...
				rnleft -=
					rr; /* will get sanity-checked above */
		...
		if (rzleft) {
			if (o_interval) /* in "slowly" mode ?? */
				...
				rr = rzleft;
			rr = write(fd, zp,
				   rr); /* one line, or the whole buffer */
			if (rr > 0) {
				if (o_wfile)
					oprint(0, zp,
					       rr); /* log what got sent */
				zp += rr;
				rzleft -= rr;
		...
		if ((rzleft) ||
		    (rnleft)) { /* shovel that shit till they ain't */
...
char *start;
{
	...
	while (next = strstr(next + 1, "\\-")) {
		p = next;
		while (p <
		       end) /* copy string back one char, overwriting backslash */
			*(p++) = *(p + 1);
		end--;
	}
...
/* and suppress others... */
   ...
   anything left over to readwrite(). */
	if (argc == 1) {
		...
		insaved = read(0, cp,
			       BIGSIZ - 1); /* we're gonna fake fgets() here */
		cp[BIGSIZ - 1] = '\0';
		if (insaved <= 0)
			bail("wrong");
	...
	while ((x = getopt(argc, argv,
			   "abc:e:g:G:hi:klno:p:q:rs:S:T:tuvw:z")) != EOF) {
		...
		case 'G': /* srcrt gateways pointer val */
			...
			if ((x) &&
			    (x ==
			     (x & 0x1c))) /* mask off bits of fukt values */
				gatesptr = x;
			else
   ...
   be useful, so we'll still pass o_nflag here instead of forcing numeric.  */
		...
		case 'w': /* wait time */
			...
			timer1 = (struct timeval *)Hmalloc(
				sizeof(struct timeval));
			timer2 = (struct timeval *)Hmalloc(
				sizeof(struct timeval));
			timer1->tv_sec =
				o_wait; /* we need two.  see readwrite()... */
			break;
#ifdef IP_TOS
		case 'T':
   ...
   unless we finally implement -a, that is. */
		if (argv[optind]) whereto = gethostpoop(argv[optind], o_nflag);
   ...
   argument, so we can control the pattern somewhat. */
	while (argv[optind]) {
       ...
       it do all of the dash code if there aren't any dashes at all */
		...
		{
			if ((!o_lport) &&
			    (o_random)) { /* -p overrides random local-port */
				ourport =
					(RAND() &
					 0xffff); /* random local-bind -- well above */
				if (ourport <
				    8192) /* resv and any likely listeners??? */
					ourport +=
						8192; /* if it *still* conflicts, use -s. */
			}
