...
{
	NSString *selected =
		[NSString stringWithString:[sender titleOfSelectedItem]];
	Connection *selectedConn = [[self connections]
		objectAtIndex:[selConn indexOfSelectedItem]];

	if ([selected isEqualToString:@"Raw RSA"]) {
		NSLog(@"user selected option Raw RSA");
		NSMutableString *tmp2 =
			[NSMutableString stringWithString:@"RSA Sig Key"];
		...
		if ([selected isEqualToString:@"X.509"]) {
			...
			NSMutableString *tmp = [NSMutableString
				stringWithString:@"RSA Sig Key"];
			...
			NSMutableString *tmp1 =
				[NSMutableString stringWithString:@"Secret"];
