...
static OSStatus DoConnect(AuthorizationRef auth, const void *userData,
			  CFDictionaryRef request,
			  CFMutableDictionaryRef response, aslclient asl,
			  aslmsg aslMsg)
...
#pragma mark ipsec
	...
	err2 = asl_log(asl, aslMsg, ASL_LEVEL_DEBUG,
		       "Run ipsec --version. ret: %d", ret);

	CFStringRef nameString;
	nameString =
		(CFStringRef)CFDictionaryGetValue(request, CFSTR("connName"));
	char command[100];
	char thisConnName[50];
	CFStringGetCString(nameString, thisConnName, 50,
			   CFStringGetSystemEncoding());
	sprintf(command, "/usr/local/sbin/ipsec auto --up %s", thisConnName);

	err2 = asl_log(asl, aslMsg, ASL_LEVEL_DEBUG, command);

	CFStringRef testString = CFStringCreateWithCString(
		NULL, command, CFStringGetSystemEncoding());

	if (retval == noErr) {
		CFDictionaryAddValue(response, CFSTR(kBASTestString),
				     testString);
	}
...
static const BASCommandProc kCommandProcs[] = { DoConnect, NULL };
