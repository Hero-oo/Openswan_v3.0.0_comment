...
- (IBAction)showAdvMenu:(id)sender
{
	...
	if (!advMenuController) {
		...
		[advMenuController
			setSelItemIndex:[[self selConn] indexOfSelectedItem]];
	} else {
		[[advMenuController selConn]
			selectItemAtIndex:[[self selConn] indexOfSelectedItem]];
	}
...
// returns three file descriptors that are bound to low-numbered TCP ports.
{
	...
	request = CFDictionaryCreate(NULL, (const void **)keys,
				     (const void **)values, keyCount,
	...
	err = BASExecuteRequestInHelperTool(gAuth, kCommandSet, bundleID,
					    request, &response);
	...
	if ((err != noErr) && (err != userCanceledErr)) {
		...
		alertResult = NSRunAlertPanel(@"Needs Install",
					      @"BAS needs to install",
					      @"Install", @"Cancel", NULL);

		if (alertResult == NSAlertDefaultReturn) {
			// Try to fix things.

			err = BASFixFailure(gAuth, (CFStringRef)bundleID,
					    CFSTR("InstallTool"),
					    CFSTR("HelperTool"), failCode);

			// If the fix went OK, retry the request.

			if (err == noErr) {
				err = BASExecuteRequestInHelperTool(
					gAuth, kCommandSet, bundleID, request,
					&response);
			}
		} else {
			err = userCanceledErr;
	...
	if (err == noErr) {
		...
		returnString = (CFStringRef)CFDictionaryGetValue(
			response, CFSTR(kBASTestString));
...
- (IBAction)connect:(id)sender
{
	if ([self timer] == nil) {
		...
		[self setConnDurationPrint:[NSString
						   stringWithString:@"0:0:0"]];
	}
	if ([sender state] == NSOnState) {
		[connView setHidden:YES];
		[discView setHidden:NO];
		...
		Connection *conn = [[[ConnectionsDB sharedInstance] connDB]
			objectAtIndex:[selConn indexOfSelectedItem]];

		char connName[100];
		[[conn connName] getCString:connName
				  maxLength:100
				   encoding:NSMacOSRomanStringEncoding];

		CFStringRef reqConnName = CFStringCreateWithCString(
			NULL, connName, CFStringGetSystemEncoding());
...
- (void)updateConnDuration:(NSTimer *)aTimer
{
	...
	[self setConnDurationPrint:[NSString stringWithFormat:@"%d:%d:%d",
							      hours, mins,
							      secs]];
}
...
- (NSDictionary *)registrationDictionaryForGrowl
{
	...
	notifications =
		[NSArray arrayWithObject:@"Openswan Growl Notification"];

	NSDictionary *dict;
	dict = [NSDictionary
		dictionaryWithObjectsAndKeys:notifications,
...
int main(int argc, char *argv[])
{
	...
	BASSetDefaultRules(gAuth, kCommandSet,
