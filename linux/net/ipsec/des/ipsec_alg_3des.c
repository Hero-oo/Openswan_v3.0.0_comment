...
static int _3des_set_key(struct ipsec_alg_enc *alg, __u8 *key_e,
...
static int _3des_cbc_encrypt(struct ipsec_alg_enc *alg, __u8 *key_e, __u8 *in,
			     int ilen, __u8 *iv, int encrypt)
{
	...
	des_ede3_cbc_encrypt((des_cblock *)in, (des_cblock *)in, ilen, ctx->s1,
			     ctx->s2, ctx->s3, &miv, encrypt);
	return 1;
}
...
#endif
{
	...
	if (excl_3des)
		ipsec_alg_3DES.ixt_common.ixt_state |= IPSEC_ALG_ST_EXCL;
	...
	printk("ipsec_3des_init(alg_type=%d alg_id=%d name=%s): ret=%d\n",
	       ...
	       ipsec_alg_3DES.ixt_common.ixt_name, ret);
	if (ret == 0 && test_3des) {
		...
		printk("ipsec_3des_init(alg_type=%d alg_id=%d): test_ret=%d\n",
		       ipsec_alg_3DES.ixt_common.ixt_support.ias_exttype,
		       ipsec_alg_3DES.ixt_common.ixt_support.ias_id, test_ret);
	}
	return ret;
}
