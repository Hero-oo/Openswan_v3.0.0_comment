...
int pfkey_msg_hdr_build(struct sadb_ext **pfkey_ext, uint8_t msg_type,
			uint8_t satype, uint8_t msg_errno, uint32_t seq,
			uint32_t pid4msg)
{
	int error = 0;
	struct sadb_msg *pfkey_msg = (struct sadb_msg *)*pfkey_ext;

	DEBUGGING(PF_KEY_DEBUG_BUILD, "pfkey_msg_hdr_build:\n");
	DEBUGGING(PF_KEY_DEBUG_BUILD,
		  "pfkey_msg_hdr_build: "
...
int pfkey_sa_builds(struct sadb_ext **pfkey_ext, struct sadb_builds sab)
{
	int error = 0;
	...
	DEBUGGING(
		...
		sab.sa_base.sadb_sa_replay, sab.sa_base.sadb_sa_state,
		sab.sa_base.sadb_sa_auth, sab.sa_base.sadb_sa_encrypt,
	...
	if (sab.sa_base.sadb_sa_auth > SADB_AALG_MAX) {
		DEBUGGING(PF_KEY_DEBUG_BUILD,
			  ...
			  sab.sa_base.sadb_sa_auth, SADB_AALG_MAX);
		SENDERR(EINVAL);
	}
	...
	if (sab.sa_base.sadb_sa_encrypt > K_SADB_EALG_MAX) {
		DEBUGGING(PF_KEY_DEBUG_BUILD,
			  ...
			  sab.sa_base.sadb_sa_encrypt, K_SADB_EALG_MAX);
		SENDERR(EINVAL);
	}
#endif
	...
	if (sab.sa_base.sadb_sa_state > K_SADB_SASTATE_MAX) {
		DEBUGGING(PF_KEY_DEBUG_BUILD,
			  ...
			  sab.sa_base.sadb_sa_state, K_SADB_SASTATE_MAX);
		SENDERR(EINVAL);
	}
	...
	if (sab.sa_base.sadb_sa_state == K_SADB_SASTATE_DEAD) {
		DEBUGGING(PF_KEY_DEBUG_BUILD,
			  ...
			  sab.sa_base.sadb_sa_state, K_SADB_SASTATE_DEAD);
		SENDERR(EINVAL);
	}
	...
	*pfkey_ext = (struct sadb_ext *)pfkey_sa;
	...
	if (pfkey_sa == NULL) {
		DEBUGGING(PF_KEY_DEBUG_BUILD, "pfkey_sa_build: "
					      "memory allocation failed\n");
		SENDERR(ENOMEM);
	}
...
int pfkey_sa_build(struct sadb_ext **pfkey_ext, uint16_t exttype, uint32_t spi,
		   uint8_t replay_window, uint8_t sa_state, uint8_t auth,
		   uint8_t encrypt, uint32_t flags)
{
...
int pfkey_lifetime_build(struct sadb_ext **pfkey_ext, uint16_t exttype,
			 uint32_t allocations, uint64_t bytes, uint64_t addtime,
			 uint64_t usetime, uint32_t packets)
{
	int error = 0;
...
int pfkey_address_build(struct sadb_ext **pfkey_ext, uint16_t exttype,
			uint8_t proto, uint8_t prefixlen,
			struct sockaddr *address)
{
	...
	case AF_INET:
		...
		if (len > 0 && len < sizeof(ipaddr_txt))
			snprintf(&ipaddr_txt[len - 1], sizeof(ipaddr_txt) - len,
				 ":%d",
	...
	pfkey_address = (struct sadb_address *)MALLOC(ALIGN_N(
		sizeof(struct sadb_address) + saddr_len, IPSEC_PFKEYv2_ALIGN));
	*pfkey_ext = (struct sadb_ext *)pfkey_address;
	...
	memcpy((char *)pfkey_address + sizeof(struct sadb_address), address,
	       saddr_len);
...
int pfkey_key_build(struct sadb_ext **pfkey_ext, uint16_t exttype,
		    uint16_t key_bits, unsigned char *key)
{
	int error = 0;
	struct sadb_key *pfkey_key = (struct sadb_key *)*pfkey_ext;

	DEBUGGING(PF_KEY_DEBUG_BUILD, "pfkey_key_build:\n");
	/* sanity checks... */
	if (pfkey_key) {
	...
	pfkey_key = (struct sadb_key *)MALLOC(sizeof(struct sadb_key) +
					      DIVUP(key_bits, 64) *
						      IPSEC_PFKEYv2_ALIGN);
	...
	*pfkey_ext = (struct sadb_ext *)pfkey_key;
	...
	pfkey_key->sadb_key_len = DIVUP(
		sizeof(struct sadb_key) * IPSEC_PFKEYv2_ALIGN + key_bits, 64);
	pfkey_key->sadb_key_exttype = exttype;
	pfkey_key->sadb_key_bits = key_bits;
	pfkey_key->sadb_key_reserved = 0;
	memcpy((char *)pfkey_key + sizeof(struct sadb_key), key,
	       DIVUP(key_bits, 8));
...
int pfkey_ident_build(struct sadb_ext **pfkey_ext, uint16_t exttype,
		      uint16_t ident_type, uint64_t ident_id, uint8_t ident_len,
		      char *ident_string)
{
	int error = 0;
	struct sadb_ident *pfkey_ident = (struct sadb_ident *)*pfkey_ext;
	int data_len =
		ident_len * IPSEC_PFKEYv2_ALIGN - sizeof(struct sadb_ident);
	...
	pfkey_ident =
		(struct sadb_ident *)MALLOC(ident_len * IPSEC_PFKEYv2_ALIGN);
	...
	*pfkey_ext = (struct sadb_ext *)pfkey_ident;
	...
	memcpy((char *)pfkey_ident + sizeof(struct sadb_ident), ident_string,
	       data_len);
...
int pfkey_sens_build(struct sadb_ext **pfkey_ext, uint32_t dpd,
		     uint8_t sens_level, uint8_t sens_len,
		     uint64_t *sens_bitmap, uint8_t integ_level,
		     uint8_t integ_len, uint64_t *integ_bitmap)
{
	int error = 0;
	...
	DEBUGGING(PF_KEY_DEBUG_BUILD, "pfkey_sens_build:\n");
	/* sanity checks... */
	if (pfkey_sens) {
	...
	pfkey_sens = (struct sadb_sens *)MALLOC(sizeof(struct sadb_sens) +
						(sens_len + integ_len) *
							sizeof(uint64_t));
...
int pfkey_prop_build(struct sadb_ext **pfkey_ext, uint8_t replay,
		     unsigned int comb_num, struct sadb_comb *comb)
{
	int error = 0;
	int i;
	...
	DEBUGGING(PF_KEY_DEBUG_BUILD, "pfkey_prop_build:\n");
	/* sanity checks... */
	if (pfkey_prop) {
	...
	pfkey_prop = (struct sadb_prop *)MALLOC(
		sizeof(struct sadb_prop) + comb_num * sizeof(struct sadb_comb));
	...
	*pfkey_ext = (struct sadb_ext *)pfkey_prop;
	...
	combp = (struct sadb_comb *)((char *)*pfkey_ext +
...
int pfkey_supported_build(struct sadb_ext **pfkey_ext, uint16_t exttype,
			  unsigned int alg_num, struct sadb_alg *alg)
{
	int error = 0;
	unsigned int i;
	...
	pfkey_supported = (struct sadb_supported *)MALLOC(
	...
	*pfkey_ext = (struct sadb_ext *)pfkey_supported;
	...
	if (pfkey_supported == NULL) {
		DEBUGGING(PF_KEY_DEBUG_BUILD, "pfkey_supported_build: "
					      "memory allocation failed\n");
		SENDERR(ENOMEM);
	}
	memset(pfkey_supported, 0,
	       sizeof(struct sadb_supported) +
		       alg_num * sizeof(struct sadb_alg));
	...
	pfkey_alg = (struct sadb_alg *)((char *)pfkey_supported +
					sizeof(struct sadb_supported));
	for (i = 0; i < alg_num; i++) {
...
int pfkey_spirange_build(struct sadb_ext **pfkey_ext, uint16_t exttype,
			 ...
			 uint32_t max) /* in network order */
{
	...
	pfkey_spirange =
		(struct sadb_spirange *)MALLOC(sizeof(struct sadb_spirange));

	*pfkey_ext = (struct sadb_ext *)pfkey_spirange;

	if (pfkey_spirange == NULL) {
		DEBUGGING(PF_KEY_DEBUG_BUILD, "pfkey_spirange_build: "
					      "memory allocation failed\n");
		SENDERR(ENOMEM);
	}
	memset(pfkey_spirange, 0, sizeof(struct sadb_spirange));
...
int pfkey_x_kmprivate_build(struct sadb_ext **pfkey_ext)
{
	...
	pfkey_x_kmprivate = (struct sadb_x_kmprivate *)MALLOC(
		sizeof(struct sadb_x_kmprivate));

	*pfkey_ext = (struct sadb_ext *)pfkey_x_kmprivate;

	if (pfkey_x_kmprivate == NULL) {
		DEBUGGING(PF_KEY_DEBUG_BUILD, "pfkey_x_kmprivate_build: "
					      "memory allocation failed\n");
		SENDERR(ENOMEM);
	}
	memset(pfkey_x_kmprivate, 0, sizeof(struct sadb_x_kmprivate));
...
int pfkey_x_satype_build(struct sadb_ext **pfkey_ext, uint8_t satype)
{
	int error = 0;
	int i;
	...
	pfkey_x_satype =
		(struct sadb_x_satype *)MALLOC(sizeof(struct sadb_x_satype));
	...
	*pfkey_ext = (struct sadb_ext *)pfkey_x_satype;
	...
	memset(pfkey_x_satype, 0, sizeof(struct sadb_x_satype));
...
int pfkey_x_debug_build(struct sadb_ext **pfkey_ext, uint32_t tunnel,
			uint32_t netlink, uint32_t xform, uint32_t eroute,
			uint32_t spi, uint32_t radij, uint32_t esp, uint32_t ah,
			uint32_t rcv, uint32_t pfkey, uint32_t ipcomp,
			uint32_t verbose)
{
	int error = 0;
	int i;
	...
	pfkey_x_debug =
		(struct sadb_x_debug *)MALLOC(sizeof(struct sadb_x_debug));
...
int pfkey_x_nat_t_type_build(struct sadb_ext **pfkey_ext, uint8_t type)
{
	int error = 0;
	int i;
	...
	pfkey_x_nat_t_type = (struct sadb_x_nat_t_type *)MALLOC(
...
int pfkey_x_nat_t_port_build(struct sadb_ext **pfkey_ext, uint16_t exttype,
			     uint16_t port)
{
	int error = 0;
	...
	DEBUGGING(PF_KEY_DEBUG_BUILD, "pfkey_x_nat_t_port_build:\n");
	/* sanity checks... */
	...
	pfkey_x_nat_t_port = (struct sadb_x_nat_t_port *)MALLOC(
...
int pfkey_x_protocol_build(struct sadb_ext **pfkey_ext, uint8_t protocol)
{
	int error = 0;
...
int pfkey_outif_build(struct sadb_ext **pfkey_ext, uint16_t outif)
{
	int error = 0;
...
int pfkey_saref_build(struct sadb_ext **pfkey_ext, IPsecSAref_t in,
		      IPsecSAref_t out)
{
	int error = 0;
...
int (*ext_default_builders[K_SADB_EXT_MAX + 1])(
	struct sadb_msg *, struct sadb_ext *) = { NULL, /* pfkey_msg_build, */
						  pfkey_sa_build,
						  pfkey_lifetime_build,
						  pfkey_lifetime_build,
						  pfkey_lifetime_build,
		    ...
		    int dir)
{
	...
	*pfkey_msg =
	...
	DEBUGGING(PF_KEY_DEBUG_BUILD,
		  ...
		  *pfkey_msg, (unsigned long)(total_size * IPSEC_PFKEYv2_ALIGN),
		  &(extensions[0]));
	...
	for (ext = 1; ext <= K_SADB_EXT_MAX; ext++) {
		...
		if (extensions[ext]) {
			...
			if (!pfkey_permitted_extension(
				    dir, (*pfkey_msg)->sadb_msg_type, ext)) {
				ERROR("ext type %d not permitted for %d/%d (build)\n",
				      ext, dir, (*pfkey_msg)->sadb_msg_type);
				SENDERR(EINVAL);
			}
			...
			DEBUGGING(
				...
				(unsigned long)(extensions[ext]->sadb_ext_len *
						IPSEC_PFKEYv2_ALIGN),
				ext, extensions[ext]->sadb_ext_type);

			memcpy(pfkey_ext, extensions[ext],
			       (extensions[ext])->sadb_ext_len *
				       IPSEC_PFKEYv2_ALIGN);
			{
				char *pfkey_ext_c = (char *)pfkey_ext;

				pfkey_ext_c += (extensions[ext])->sadb_ext_len *
					       IPSEC_PFKEYv2_ALIGN;
 ...
 *
 */
	if ((error = pfkey_msg_parse(*pfkey_msg, NULL, extensions_check,
				     dir))) {
		ERROR("pfkey_msg_build: "
