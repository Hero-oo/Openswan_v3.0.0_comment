...
#define barf_out(fmt, args...)                                                 \
	do {                                                                   \
		struct ipsec_alg *ixtc = (struct ipsec_alg *)ixt;              \
		printk(KERN_ERR "%s: (%s) " fmt, __FUNCTION__, ixtc->ixt_name, \
		       ##args);                                                \
		goto out;                                                      \
	} while (0)
					  ...
					  struct list_head *head)
{
	...
	for (p = head->next; p != head; p = p->next) {
		...
		if (ixt->ixt_alg_type == alg_type &&
		    ixt->ixt_alg_id == alg_id) {
			goto out;
		}
	}
	ixt = NULL;
...
static int ipsec_alg_insert(struct ipsec_alg *ixt)
{
	...
	if (ixt_cur &&
		...
		barf_out("ipsec_alg for alg_type=%d, alg_id=%d already exist. "
			 "Not loaded (ret=%d).\n",
			 ixt->ixt_alg_type, ixt->ixt_alg_id, ret = -EEXIST);
	}
 ...
 */
int ipsec_alg_esp_encrypt(struct ipsec_sa *sa_p, __u8 *idat, int ilen, __u8 *iv,
			  int encrypt)
{
	int ret;
	struct ipsec_alg_enc *ixt_e = sa_p->ips_alg_enc;
	int debug_flag =
		(encrypt == IPSEC_ALG_ENCRYPT ? debug_tunnel : debug_rcv);
	...
	if (ixt_e == NULL) {
		KLIPS_ERROR(debug_flag, "klips_debug:ipsec_alg_esp_encrypt: "
	...
	KLIPS_PRINT(debug_flag,
		    ...
		    sa_p->ips_encalg, sa_p->ips_key_e, idat, ilen, iv, encrypt);
	ret = ixt_e->ixt_e_cbc_encrypt(ixt_e, sa_p->ips_key_e, idat, ilen, iv,
				       encrypt);
	KLIPS_PRINT(debug_flag,
		    "klips_debug:ipsec_alg_esp_encrypt: "
		    "returned ret=%d\n",
...
int ipsec_alg_enc_key_create(struct ipsec_sa *sa_p)
{
	...
	struct ipsec_alg_enc *ixt_e = (struct ipsec_alg_enc *)ipsec_alg_get(
		IPSEC_ALG_TYPE_ENCRYPT, sa_p->ips_encalg);
	...
	if (ixt_e->ixt_e_new_key) {
		...
		if ((ekp = ixt_e->ixt_e_new_key(ixt_e, sa_p->ips_key_e,
		...
		KLIPS_PRINT(debug_pfkey,
			    ...
			    (caddr_t)sa_p->ips_key_e, sa_p->ips_key_bits_e / 8);

		ret = ixt_e->ixt_e_set_key(ixt_e, ekp, (caddr_t)sa_p->ips_key_e,
					   sa_p->ips_key_bits_e / 8);
		if (ret < 0) {
			kfree(ekp);
			goto ixt_out;
...
int ipsec_alg_auth_key_create(struct ipsec_sa *sa_p)
{
	...
	int keyminbits, keymaxbits;
	caddr_t akp = NULL;
	struct ipsec_alg_auth *ixt_a = (struct ipsec_alg_auth *)ipsec_alg_get(
		IPSEC_ALG_TYPE_AUTH, sa_p->ips_authalg);
	...
	if ((akp = (caddr_t)kmalloc(ixt_a->ixt_a_ctx_size, GFP_ATOMIC)) ==
	    NULL) {
		ret = -ENOMEM;
		goto ixt_out;
	}
	...
	if (ixt_a->ixt_a_hmac_set_key) {
		ret = ixt_a->ixt_a_hmac_set_key(ixt_a, akp, sa_p->ips_key_a,
						sa_p->ips_key_bits_a /
							8); /* XXX XXX */
		if (ret < 0) {
			kfree(akp);
			  ...
			  int len, __u8 *hash, int hashlen)
{
	...
	if (!ixt_a) {
		KLIPS_PRINT(debug_pfkey, "klips_debug:ipsec_sa_esp_hash: "
					 "NULL ipsec_alg_auth object\n");
		return -EPROTO;
	}
	KLIPS_PRINT(debug_tunnel | debug_rcv,
		    ...
		    espp, len, hash, hashlen);
	ixt_a->ixt_a_hmac_hash(ixt_a, sa_p->ips_key_a, espp, len, hash,
			       hashlen);
	return 0;
}
...
{
	int ret = -EINVAL;
	if (ixt->ixt_common.ixt_blocksize ==
	    0) /*  || ixt->ixt_common.ixt_blocksize%2) need for ESP_NULL */
...
zero_key_ok:
	if (ixt->ixt_e_ctx_size == 0 && ixt->ixt_e_new_key == NULL)
		barf_out(KERN_ERR
			 "invalid key_e_size=%d and ixt_e_new_key=NULL\n",
			 ixt->ixt_e_ctx_size);
...
static int check_auth(struct ipsec_alg_auth *ixt)
{
	...
	if (ixt->ixt_common.ixt_blocksize > AH_BLKLEN_MAX)
		barf_out(KERN_ERR
			 ...
			 ixt->ixt_common.ixt_blocksize, AH_BLKLEN_MAX);
	if (ixt->ixt_common.ixt_support.ias_keyminbits == 0 &&
	    ixt->ixt_common.ixt_support.ias_keymaxbits == 0 &&
	    ixt->ixt_a_keylen == 0)
		goto zero_key_ok;
	...
	if (ixt->ixt_common.ixt_support.ias_keymaxbits !=
	    ixt->ixt_common.ixt_support.ias_keyminbits)
		barf_out(KERN_ERR
			 "keymaxbits must equal keyminbits (not sure).\n");
	...
	case IPSEC_ALG_TYPE_ENCRYPT:
		...
		/*
			 ...
			 * fixed NULL support by David De Reu <DeReu@tComLabs.com>
 			 */
		if (ixt->ixt_support.ias_ivlen == 0 && ixt->ixt_blocksize > 1) {
...
#define test_size (BUFSZ * 3 + key_e_size + iv_size + keysize + MARGIN * 7)
	...
	ret = ixt_e->ixt_e_cbc_encrypt(ixt_e, tmp_key_e, test_enc, BUFSZ,
				       test_iv, 1);
	printk(KERN_INFO "klips_info: ipsec_alg_test_encrypt: "
			 "cbc_encrypt=1 ret=%d\n",
	       ret);
	ret = memcmp(test_enc, test_tmp, BUFSZ);
	printk(KERN_INFO "klips_info: ipsec_alg_test_encrypt: "
			 "memcmp(enc, tmp) ret=%d: %s\n",
	       ret, ret != 0 ? "OK. (encr->DIFFers)" : "FAIL! (encr->SAME)");
	...
	printk(KERN_INFO "klips_info: ipsec_alg_test_encrypt: "
	       ...
	       ret,
	       ret == 0 ? "OK. (encr->decr->SAME)" :
			  "FAIL! (encr->decr->DIFFers)");
	{
		...
		for (encrypt = 0; encrypt < 2; encrypt++) {
			for (i = 0; i < 5; i++) {
				...
				while (jiffies == now) {
					mb();
					ixt_e->ixt_e_cbc_encrypt(
						ixt_e, tmp_key_e, test_tmp,
...
#define test_size (BUFSZ + key_a_size + keysize + AHHMAC_HASHLEN + MARGIN * 4)
	...
	printk(KERN_INFO "klips_info: ipsec_alg_test_auth: "
			 "ret=%d\n",
	       ret);
	{
		/*	Shamelessly taken from drivers/md sources  O:)  */
		...
		printk(KERN_INFO "klips_info: ipsec_alg_test_auth: "
				 "%s hash speed=%d KB/s\n",
		       ixt_a->ixt_common.ixt_name, speed);
	}
out:
	if (buf)
		kfree(buf);
...
{
	KLIPS_PRINT(1,
		    "klips_info:ipsec_alg_init: "
		    "KLIPS alg v=%d.%d.%d-%d (EALG_MAX=%d, AALG_MAX=%d)\n",
		    IPSEC_ALG_VERSION_QUAD(IPSEC_ALG_VERSION), K_SADB_EALG_MAX,
		    K_SADB_AALG_MAX);
	/*	Initialize tables */
	write_lock_bh(&ipsec_alg_lock);
	ipsec_alg_hash_init();
...
{
	int i;
	struct alg_walkstate *aws =
		kmalloc(sizeof(struct alg_walkstate), GFP_KERNEL);
	struct list_head *head;
	struct ipsec_alg *ixt;
	...
	for (i = 0, head = ipsec_alg_hash_table; i < IPSEC_ALG_HASHSZ;
	     i++, head++) {
		struct list_head *p;
		for (p = head->next; p != head; p = p->next) {
			aws->alg_total++;
	...
	for (i = 0, head = ipsec_alg_hash_table;
	     i < IPSEC_ALG_HASHSZ && (aws->alg_offset <= *pos); i++, head++) {
		struct list_head *p;
		aws->alg_hash_num = i;
		for (p = head->next; p != head && (aws->alg_offset <= *pos);
		     p = p->next) {
...
static void *proc_alg_next(struct seq_file *m, void *v, loff_t *pos)
{
	...
	for (i = aws->alg_hash_num,
	    head = &ipsec_alg_hash_table[aws->alg_hash_num];
	     i < IPSEC_ALG_HASHSZ && (aws->alg_offset <= *pos); i++, head++) {
		struct list_head *p;
...
static int proc_alg_show(struct seq_file *m, void *v)
{
	...
	seq_printf(
		...
		ixt->ixt_state, ixt->ixt_blocksize, ixt->ixt_support.ias_ivlen,
	...
	case IPSEC_ALG_TYPE_ENCRYPT: {
		...
		seq_printf(m, "KEYLEN=%d CTXSIZE=%d ", enc->ixt_e_keylen,
			   enc->ixt_e_ctx_size);
		break;
	}
	}
...
static struct seq_operations proc_alg_op = { .start = proc_alg_start,
					     .next = proc_alg_next,
					     .stop = proc_alg_stop,
					     .show = proc_alg_show };

int proc_alg_open(struct inode *inode, struct file *file)
{
