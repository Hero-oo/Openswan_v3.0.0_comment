...
DEBUG_NO_STATIC int pfkey_sa_parse(struct sadb_ext *pfkey_ext)
{
	int error = 0;
	...
	if (pfkey_sa->sadb_sa_encrypt > K_SADB_EALG_MAX) {
		ERROR("pfkey_sa_parse: "
		      ...
		      pfkey_sa->sadb_sa_encrypt, K_SADB_EALG_MAX);
		SENDERR(EINVAL);
	}
#endif
	...
	if (pfkey_sa->sadb_sa_auth > K_SADB_AALG_MAX) {
		ERROR("pfkey_sa_parse: "
		      ...
		      pfkey_sa->sadb_sa_auth, K_SADB_AALG_MAX);
		SENDERR(EINVAL);
	}
#endif
	...
	if (pfkey_sa->sadb_sa_state > K_SADB_SASTATE_MAX) {
		ERROR("pfkey_sa_parse: "
		      ...
		      pfkey_sa->sadb_sa_state, K_SADB_SASTATE_MAX);
		SENDERR(EINVAL);
	}
#endif
	...
	if (pfkey_sa->sadb_sa_state == K_SADB_SASTATE_DEAD) {
		ERROR("pfkey_sa_parse: "
		      "state=%d is DEAD=%d.\n",
		      pfkey_sa->sadb_sa_state, K_SADB_SASTATE_DEAD);
		SENDERR(EINVAL);
	}
	...
	if (!((pfkey_sa->sadb_sa_exttype == K_SADB_EXT_SA) ||
		...
		ERROR("pfkey_sa_parse: "
		      ...
		      pfkey_sa->sadb_sa_exttype, K_SADB_EXT_SA,
		      K_SADB_X_EXT_SA2);
		SENDERR(EINVAL);
	}
	...
	DEBUGGING(
		...
		pfkey_sa->sadb_sa_replay, pfkey_sa->sadb_sa_state,
		pfkey_sa->sadb_sa_auth, pfkey_sa->sadb_sa_encrypt,
		pfkey_sa->sadb_sa_flags, pfkey_sa->sadb_x_sa_ref);

errlab:
	return error;
}

DEBUG_NO_STATIC int pfkey_lifetime_parse(struct sadb_ext *pfkey_ext)
{
	int error = 0;
...
DEBUG_NO_STATIC int pfkey_address_parse(struct sadb_ext *pfkey_ext)
{
	int error = 0;
	int saddr_len = 0;
	...
	if (pfkey_address->sadb_address_len !=
		...
		ERROR("pfkey_address_parse: "
		      ...
		      pfkey_address->sadb_address_len,
		      (int)sizeof(struct sadb_address), saddr_len);
		SENDERR(EINVAL);
	}
...
DEBUG_NO_STATIC int pfkey_key_parse(struct sadb_ext *pfkey_ext)
{
	int error = 0;
	...
	if (pfkey_key->sadb_key_len <
	    sizeof(struct sadb_key) / IPSEC_PFKEYv2_ALIGN) {
		ERROR("pfkey_key_parse: "
		      "size wrong ext_len=%d, key_ext_len=%d.\n",
		      pfkey_key->sadb_key_len, (int)sizeof(struct sadb_key));
		SENDERR(EINVAL);
	}
	...
	if (pfkey_key->sadb_key_len !=
		...
		ERROR("pfkey_key_parse: "
		      ...
		      pfkey_key->sadb_key_bits, pfkey_key->sadb_key_len);
		SENDERR(EINVAL);
	}
	...
	DEBUGGING(PF_KEY_DEBUG_PARSE_STRUCT,
		  ...
		  pfkey_key->sadb_key_len, pfkey_key->sadb_key_exttype,
		  pfkey_v2_sadb_ext_string(pfkey_key->sadb_key_exttype),
		  pfkey_key->sadb_key_bits, pfkey_key->sadb_key_reserved);

errlab:
	return error;
}

DEBUG_NO_STATIC int pfkey_ident_parse(struct sadb_ext *pfkey_ext)
{
	int error = 0;
	struct sadb_ident *pfkey_ident = (struct sadb_ident *)pfkey_ext;

	/* sanity checks... */
	if (pfkey_ident->sadb_ident_len <
	...
	if (pfkey_ident->sadb_ident_type > K_SADB_IDENTTYPE_MAX) {
		ERROR("pfkey_ident_parse: "
		      ...
		      pfkey_ident->sadb_ident_type, K_SADB_IDENTTYPE_MAX);
		SENDERR(EINVAL);
	}
...
DEBUG_NO_STATIC int pfkey_sens_parse(struct sadb_ext *pfkey_ext)
{
	int error = 0;
	struct sadb_sens *pfkey_sens = (struct sadb_sens *)pfkey_ext;

	/* sanity checks... */
	if (pfkey_sens->sadb_sens_len <
	    sizeof(struct sadb_sens) / IPSEC_PFKEYv2_ALIGN) {
		DEBUGGING(PF_KEY_DEBUG_PARSE_PROBLEM,
			  "pfkey_sens_parse: "
			  "size wrong ext_len=%d, key_ext_len=%d.\n",
...
DEBUG_NO_STATIC int pfkey_prop_parse(struct sadb_ext *pfkey_ext)
{
	int error = 0;
	int i, num_comb;
	...
	/* sanity checks... */
	if ((pfkey_prop->sadb_prop_len <
	     sizeof(struct sadb_prop) / IPSEC_PFKEYv2_ALIGN) ||
	...
	num_comb = ((pfkey_prop->sadb_prop_len * IPSEC_PFKEYv2_ALIGN) -
		    sizeof(struct sadb_prop)) /
		   sizeof(struct sadb_comb);
	...
	for (i = 0; i < num_comb; i++) {
		...
		if (pfkey_comb->sadb_comb_auth > K_SADB_AALG_MAX) {
			DEBUGGING(
				...
				i, pfkey_comb->sadb_comb_auth, K_SADB_AALG_MAX);
			SENDERR(EINVAL);
		}
		...
		if (pfkey_comb->sadb_comb_auth) {
			...
			if (pfkey_comb->sadb_comb_auth_minbits >
				...
				DEBUGGING(
					...
					i, pfkey_comb->sadb_comb_auth_minbits,
					pfkey_comb->sadb_comb_auth_maxbits);
				SENDERR(EINVAL);
			}
		} else {
			...
			if (pfkey_comb->sadb_comb_auth_maxbits) {
				DEBUGGING(
					...
					i, pfkey_comb->sadb_comb_auth_maxbits);
				SENDERR(EINVAL);
			}
		}
		...
		if (pfkey_comb->sadb_comb_encrypt > K_SADB_EALG_MAX) {
			DEBUGGING(
				...
				i, pfkey_comb->sadb_comb_encrypt,
				K_SADB_EALG_MAX);
			SENDERR(EINVAL);
		}
#endif
		...
		if (pfkey_comb->sadb_comb_hard_allocations &&
			...
			DEBUGGING(
				...
				i, pfkey_comb->sadb_comb_soft_allocations,
				pfkey_comb->sadb_comb_hard_allocations);
			SENDERR(EINVAL);
		}

		if (pfkey_comb->sadb_comb_hard_bytes &&
		...
		if (pfkey_comb->sadb_comb_hard_addtime &&
		...
		if (pfkey_comb->sadb_comb_hard_usetime &&
...
#ifdef COMB_PACKETS
		if (pfkey_comb->sadb_x_comb_hard_packets &&
			...
			DEBUGGING(
				...
				i, k_pfkey_comb->sadb_x_comb_soft_packets,
...
DEBUG_NO_STATIC int pfkey_supported_parse(struct sadb_ext *pfkey_ext)
{
	int error = 0;
	...
	num_alg = ((pfkey_supported->sadb_supported_len * IPSEC_PFKEYv2_ALIGN) -
		   sizeof(struct sadb_supported)) /
		  sizeof(struct sadb_alg);
...
DEBUG_NO_STATIC int pfkey_spirange_parse(struct sadb_ext *pfkey_ext)
{
	int error = 0;
	...
	if (ntohl(pfkey_spirange->sadb_spirange_max) <
...
DEBUG_NO_STATIC int pfkey_x_kmprivate_parse(struct sadb_ext *pfkey_ext)
{
	int error = 0;
...
DEBUG_NO_STATIC int pfkey_x_satype_parse(struct sadb_ext *pfkey_ext)
{
	int error = 0;
	int i;
	...
	DEBUGGING(PF_KEY_DEBUG_PARSE_FLOW, "pfkey_x_satype_parse: enter\n");
	/* sanity checks... */
...
DEBUG_NO_STATIC int pfkey_x_ext_debug_parse(struct sadb_ext *pfkey_ext)
{
	int error = 0;
	int i;
	struct sadb_x_debug *pfkey_x_debug = (struct sadb_x_debug *)pfkey_ext;

	DEBUGGING(PF_KEY_DEBUG_PARSE_FLOW, "pfkey_x_debug_parse: enter\n");
	/* sanity checks... */
...
DEBUG_NO_STATIC int pfkey_x_ext_protocol_parse(struct sadb_ext *pfkey_ext)
{
	int error = 0;
...
#ifdef NAT_TRAVERSAL
DEBUG_NO_STATIC int pfkey_x_ext_nat_t_type_parse(struct sadb_ext *pfkey_ext)
{
	return 0;
}
DEBUG_NO_STATIC int pfkey_x_ext_nat_t_port_parse(struct sadb_ext *pfkey_ext)
{
	return 0;
}
#endif

DEBUG_NO_STATIC int pfkey_x_ext_outif_parse(struct sadb_ext *pfkey_ext)
{
	int error = 0;
...
DEBUG_NO_STATIC int pfkey_x_ext_saref_parse(struct sadb_ext *pfkey_ext)
{
	int error = 0;
...
#define DEFINEPARSER(NAME) \
	static struct pf_key_ext_parsers_def NAME##_def = { NAME, #NAME };
...
int pfkey_msg_parse(struct sadb_msg *pfkey_msg,
		    struct pf_key_ext_parsers_def *ext_parsers[],
		    struct sadb_ext *extensions[], int dir)
{
	int error = 0;
	int remain;
	...
	if (ext_parsers == NULL)
		ext_parsers = ext_default_parsers;
	...
	if (pfkey_msg->sadb_msg_type > K_SADB_MAX) {
		ERROR("pfkey_msg_parse: "
		      "msg type=%d > max=%d.\n",
		      pfkey_msg->sadb_msg_type, K_SADB_MAX);
		SENDERR(EINVAL);
	}
	...
	if ((dir == EXT_BITS_IN) &&
	    (pfkey_msg->sadb_msg_type != K_SADB_ACQUIRE) &&
	...
	while ((remain * IPSEC_PFKEYv2_ALIGN) >= sizeof(struct sadb_ext)) {
		...
		if ((pfkey_ext->sadb_ext_type > K_SADB_EXT_MAX) ||
		    (!pfkey_ext->sadb_ext_type)) {
			DEBUGGING(
				PF_KEY_DEBUG_PARSE_PROBLEM,
				"pfkey_msg_parse: "
		...
		if (!pfkey_permitted_extension(dir, pfkey_msg->sadb_msg_type,
					       pfkey_ext->sadb_ext_type)) {
		...
		DEBUGGING(
			...
			remain, pfkey_ext->sadb_ext_type,
			pfkey_v2_sadb_ext_string(pfkey_ext->sadb_ext_type),
			pfkey_ext->sadb_ext_len, pfkey_ext,
		...
		pfkey_mark_extension(pfkey_ext->sadb_ext_type,
				     &extensions_seen);
...
next_ext:
	...
	if ((dir == EXT_BITS_IN) &&
	    ...
	    (((extensions_seen & (1 << SADB_EXT_SA)) != (1 << SADB_EXT_SA)) ||
	     ((((struct k_sadb_sa *)extensions[SADB_EXT_SA])->sadb_sa_flags &
	       SADB_X_SAFLAGS_CLEARFLOW) != SADB_X_SAFLAGS_CLEARFLOW))) {
		DEBUGGING(
			PF_KEY_DEBUG_PARSE_PROBLEM,
			"pfkey_msg_parse: "
	...
	case K_SADB_UPDATE:
		...
		if (ntohl(((struct k_sadb_sa *)(extensions[SADB_EXT_SA]))
				  ->sadb_sa_spi) <= 255) {
			DEBUGGING(PF_KEY_DEBUG_PARSE_PROBLEM,
				  "pfkey_msg_parse: "
				  "spi=%08x must be > 255.\n",
				  ntohl(((struct k_sadb_sa
						  *)(extensions[SADB_EXT_SA]))
						->sadb_sa_spi));
			SENDERR(EINVAL);
		}
