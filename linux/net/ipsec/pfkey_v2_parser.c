				       ...
				       struct pfkey_extracted_data *extr)
{
	...
	uint8_t satype = ((struct sadb_msg *)extensions[K_SADB_EXT_RESERVED])
				 ->sadb_msg_satype;
				   ...
				   ((spi_diff < (2 ^ 8)) ?
					    ...
					    ((spi_diff < (2 ^ 16)) ?
						     ...
						     ((spi_diff < (2 ^ 24)) ?
							      3 :
							      4))));
			extr->ips->ips_said.spi = htonl(
				ntohl(minspi) + (rand_val % (spi_diff + 1)));
			i++;
			spin_lock_bh(&tdb_lock);
	...
	if (ip_address_family(&extr->ips->ips_said.dst) == AF_INET6 &&
	    ip6_chk_addr(&extr->ips->ips_said.dst.u.v6.sin6_addr) ==
		    IS_MYADDR) {
		extr->ips->ips_flags |= EMT_INBOUND;
	} else
#endif
		if (ip_chk_addr((unsigned long)extr->ips->ips_said.dst.u.v4
					.sin_addr.s_addr) == IS_MYADDR) {
		extr->ips->ips_flags |= EMT_INBOUND;
	}
	...
	if (!(pfkey_safe_build(error = pfkey_msg_hdr_build(
				       &extensions_reply[0], K_SADB_GETSPI,
				       satype, 0,
				       ((struct sadb_msg *)
						extensions[K_SADB_EXT_RESERVED])
					       ->sadb_msg_seq,
				       ((struct sadb_msg *)
						extensions[K_SADB_EXT_RESERVED])
					       ->sadb_msg_pid),
			       extensions_reply) &&
	      pfkey_safe_build(error = pfkey_sa_build(
				       &extensions_reply[K_SADB_EXT_SA],
				       K_SADB_EXT_SA, extr->ips->ips_said.spi,
				       0, K_SADB_SASTATE_LARVAL, 0, 0, 0),
			       extensions_reply)

	      && pfkey_safe_build(
			 error = pfkey_address_build(
				 &extensions_reply[K_SADB_EXT_ADDRESS_SRC],
				 K_SADB_EXT_ADDRESS_SRC,
				 0, /*extr->ips->ips_said.proto,*/
				 0, extr->ips->ips_addr_s),
			 extensions_reply) &&
	      pfkey_safe_build(error = pfkey_address_build(
				       &extensions_reply[K_SADB_EXT_ADDRESS_DST],
				       K_SADB_EXT_ADDRESS_DST,
				       0, /*extr->ips->ips_said.proto,*/
				       0, extr->ips->ips_addr_d),
			       extensions_reply))) {
		KLIPS_PRINT(
			debug_pfkey,
			"klips_debug:pfkey_getspi_parse: "
	...
	for (pfkey_socketsp = pfkey_open_sockets; pfkey_socketsp;
		...
		KLIPS_PRINT(
			...
			satype, satype2name(satype), pfkey_socketsp->socketp);
	}
				       ...
				       struct pfkey_extracted_data *extr)
{
	...
	if (((struct sadb_sa *)extensions[K_SADB_EXT_SA])->sadb_sa_state !=
		...
		KLIPS_PRINT(debug_pfkey,
			    ...
			    ((struct sadb_sa *)extensions[K_SADB_EXT_SA])
				    ->sadb_sa_state,
			    K_SADB_SASTATE_MATURE);
		SENDERR(EINVAL);
	}
	...
	if (!(pfkey_safe_build(error = pfkey_msg_hdr_build(
				       &extensions_reply[0], K_SADB_UPDATE,
				       satype, 0,
				       ((struct sadb_msg *)
						extensions[K_SADB_EXT_RESERVED])
					       ->sadb_msg_seq,
	      ...
	      pfkey_safe_build(
		      error = pfkey_sa_build(
			      &extensions_reply[K_SADB_EXT_SA], K_SADB_EXT_SA,
			      extr->ips->ips_said.spi, extr->ips->ips_replaywin,
			      extr->ips->ips_state, extr->ips->ips_authalg,
			      extr->ips->ips_encalg, extr->ips->ips_flags),
		      extensions_reply)
	      ...
	      (extensions[K_SADB_EXT_LIFETIME_HARD] ?
		       pfkey_safe_build(
			       error = pfkey_lifetime_build(
				       &extensions_reply[K_SADB_EXT_LIFETIME_HARD],
	      ...
	      (extensions[K_SADB_EXT_LIFETIME_SOFT] ?
		       pfkey_safe_build(
			       error = pfkey_lifetime_build(
				       &extensions_reply[K_SADB_EXT_LIFETIME_SOFT],
	      ...
	      (extr->ips->ips_ident_s.data ?
		       pfkey_safe_build(
			       error = pfkey_ident_build(
				       &extensions_reply[K_SADB_EXT_IDENTITY_SRC],
	      ...
	      (extr->ips->ips_ident_d.data ?
		       pfkey_safe_build(
			       error = pfkey_ident_build(
				       &extensions_reply[K_SADB_EXT_IDENTITY_DST],
				    ...
				    struct pfkey_extracted_data *extr)
{
	...
	if (!(pfkey_safe_build(error = pfkey_msg_hdr_build(
				       &extensions_reply[0], K_SADB_ADD, satype,
				       0,
				       ((struct sadb_msg *)
						extensions[K_SADB_EXT_RESERVED])
					       ->sadb_msg_seq,
	      ...
	      pfkey_safe_build(error = pfkey_saref_build(
				       &extensions_reply[K_SADB_X_EXT_SAREF],
				       extr->ips->ips_ref,
				       extr->ips->ips_refhim),
			       extensions_reply)
	      /* The 3 lifetime extentions should only be sent if non-zero. */
	...
	if (extr->outif != 0 && extr->outif != -1) {
		...
		extr->ips->ips_transport_direct =
			ipsec_mast_is_transport(extr->outif);
	}
				       ...
				       struct pfkey_extracted_data *extr)
{
	...
	if (!(pfkey_safe_build(error = pfkey_msg_hdr_build(
				       &extensions_reply[0], K_SADB_DELETE,
				       satype, 0,
				       ((struct sadb_msg *)
						extensions[K_SADB_EXT_RESERVED])
					       ->sadb_msg_seq,
				       ((struct sadb_msg *)
						extensions[K_SADB_EXT_RESERVED])
					       ->sadb_msg_pid),
			       extensions_reply) &&
	      pfkey_safe_build(error = pfkey_sa_builds(
				       &extensions_reply[K_SADB_EXT_SA], sab),
			       extensions_reply) &&
	      pfkey_safe_build(error = pfkey_address_build(
				       &extensions_reply[K_SADB_EXT_ADDRESS_SRC],
				       K_SADB_EXT_ADDRESS_SRC,
				    ...
				    struct pfkey_extracted_data *extr)
{
	...
	if (!(pfkey_safe_build(error = pfkey_msg_hdr_build(
	      ...
	      (extr->ips->ips_addr_p ?
		       pfkey_safe_build(
			       error = pfkey_address_build(
				       &extensions_reply[K_SADB_EXT_ADDRESS_PROXY],
				       K_SADB_EXT_ADDRESS_PROXY,
				       0, /*extr->ips->ips_said.proto,*/
				       0, extr->ips->ips_addr_p),
			       extensions_reply) :
		       1)
#if 0
					...
					struct pfkey_extracted_data *extr)
{
	...
	for (pfkey_socketsp = pfkey_registered_sockets[satype]; pfkey_socketsp;
	     pfkey_socketsp = pfkey_socketsp->next) {
		if ((error = pfkey_upmsg(
					 ...
					 struct pfkey_extracted_data *extr)
{
	...
	error = pfkey_register_reply(
		satype, (struct sadb_msg *)extensions[K_SADB_EXT_RESERVED]);
errlab:
	return error;
}
...
int pfkey_register_reply(int satype, struct sadb_msg *sadb_msg)
{
	...
	KLIPS_PRINT(debug_pfkey,
		    ...
		    satype, pfkey_supported_list[satype]);
	...
	if (!(pfkey_safe_build(error = pfkey_msg_hdr_build(
				       &extensions_reply[0], K_SADB_REGISTER,
				       satype, 0,
	      ...
	      (alg_num_a ? pfkey_safe_build(
				   error = pfkey_supported_build(
					   &extensions_reply
						   [K_SADB_EXT_SUPPORTED_AUTH],
					   K_SADB_EXT_SUPPORTED_AUTH, alg_num_a,
					   alg_a),
				   extensions_reply) :
			   1) &&
	      (alg_num_e ?
		       pfkey_safe_build(
			       error = pfkey_supported_build(
				       &extensions_reply
					       [K_SADB_EXT_SUPPORTED_ENCRYPT],
				       K_SADB_EXT_SUPPORTED_ENCRYPT, alg_num_e,
				       alg_e),
			       extensions_reply) :
		       1))) {
		KLIPS_PRINT(
		...
		for (pfkey_socketsp = pfkey_open_sockets; pfkey_socketsp;
			...
			if ((error = pfkey_upmsg(
				...
				KLIPS_PRINT(
					...
					satype, satype2name(satype), proto,
					pfkey_socketsp->socketp, error);
				SENDERR(-error);
			}
			KLIPS_PRINT(
					...
					struct pfkey_extracted_data *extr)
{
	 ...
	 */
	if (!(pfkey_safe_build(error = pfkey_msg_hdr_build(
				       &extensions_reply[0], K_SADB_X_GRPSA,
				       satype, 0,
				       ((struct sadb_msg *)
						extensions[K_SADB_EXT_RESERVED])
					       ->sadb_msg_seq,
		       ...
		       (pfkey_safe_build(
				error = pfkey_x_satype_build(
					...
					((struct sadb_x_satype *)
						 extensions[K_SADB_X_EXT_SATYPE2])
						->sadb_x_satype_satype
					/* proto2satype(extr->ips2->ips_said.proto) */),
				extensions_reply) &&
			pfkey_safe_build(
				error = pfkey_sa_build(
					&extensions_reply[K_SADB_X_EXT_SA2],
					K_SADB_X_EXT_SA2,
					  ...
					  struct pfkey_extracted_data *extr)
{
	...
	if (extr->ips->ips_flags & SADB_X_SAFLAGS_INFLOW ||
		...
		if ((error = ipsec_makeroute(
			     &(extr->eroute->er_eaddr),
			     &(extr->eroute->er_emask), extr->ips->ips_said,
			     ((struct sadb_msg *)extensions[K_SADB_EXT_RESERVED])
				     ->sadb_msg_pid,
			     NULL, &(extr->ips->ips_ident_s),
	...
	if (!(pfkey_safe_build(error = pfkey_msg_hdr_build(
				       &extensions_reply[0], K_SADB_X_ADDFLOW,
				       satype, 0,
				       ((struct sadb_msg *)
						extensions[K_SADB_EXT_RESERVED])
					       ->sadb_msg_seq,
	...
	for (pfkey_socketsp = pfkey_open_sockets; pfkey_socketsp;
		...
		KLIPS_PRINT(
			...
			satype, satype2name(satype), extr->ips->ips_said.proto,
			pfkey_socketsp->socketp);
	}
					  ...
					  struct pfkey_extracted_data *extr)
{
	...
	error = pfkey_msg_hdr_build(
		&extensions_reply[0], K_SADB_X_DELFLOW, satype, 0,
		((struct sadb_msg *)extensions[K_SADB_EXT_RESERVED])
			->sadb_msg_seq,
		((struct sadb_msg *)extensions[K_SADB_EXT_RESERVED])
			->sadb_msg_pid);
...
int pfkey_expire(struct ipsec_sa *ipsp, int hard)
{
	...
	if (!(pfkey_safe_build(error = pfkey_msg_hdr_build(
				       &extensions[0], K_SADB_EXPIRE, satype, 0,
				       ++pfkey_msg_seq, 0),
			       extensions) &&
	      pfkey_safe_build(
		      error = pfkey_sa_build(&extensions[K_SADB_EXT_SA],
					     K_SADB_EXT_SA, ipsp->ips_said.spi,
					     ipsp->ips_replaywin,
					     ipsp->ips_state, ipsp->ips_authalg,
					     ipsp->ips_encalg, ipsp->ips_flags),
		      extensions) &&
	...
	for (pfkey_socketsp = pfkey_open_sockets; pfkey_socketsp;
		...
		KLIPS_PRINT(
			...
			satype, satype2name(satype), ipsp->ips_said.proto,
			pfkey_socketsp->socketp);
	}
...
int pfkey_acquire(struct ipsec_sa *ipsp)
{
	...
	struct sadb_comb comb[] = {
		...
		{ K_SADB_AALG_MD5HMAC, K_SADB_EALG_3DESCBC, SADB_SAFLAGS_PFS,
		  128, 128, 168, 168, 0, 0, 0, 0, 0, 57600, 86400, 57600,
		  86400 },
		{ K_SADB_AALG_SHA1HMAC, K_SADB_EALG_3DESCBC, SADB_SAFLAGS_PFS,
		  160, 160, 168, 168, 0, 0, 0, 0, 0, 57600, 86400, 57600,
		  86400 },
	};
	...
	if (!(pfkey_safe_build(error = pfkey_msg_hdr_build(
				       &extensions[0], K_SADB_ACQUIRE, satype,
				       0, ++pfkey_msg_seq, 0),
			       extensions) &&
	      pfkey_safe_build(error = pfkey_address_build(
				       &extensions[K_SADB_EXT_ADDRESS_SRC],
				       K_SADB_EXT_ADDRESS_SRC,
				       ipsp->ips_transport_protocol, 0,
				       ipsp->ips_addr_s),
			       extensions) &&
	      pfkey_safe_build(error = pfkey_address_build(
				       &extensions[K_SADB_EXT_ADDRESS_DST],
	      ...
	      && (ipsp->ips_ident_s.type != SADB_IDENTTYPE_RESERVED ?
			  pfkey_safe_build(
				  error = pfkey_ident_build(
					  &extensions[SADB_EXT_IDENTITY_SRC],
					  K_SADB_EXT_IDENTITY_SRC,
	      ...
	      && (ipsp->ips_ident_d.type != SADB_IDENTTYPE_RESERVED ?
			  pfkey_safe_build(
				  error = pfkey_ident_build(
					  &extensions[K_SADB_EXT_IDENTITY_DST],
					  K_SADB_EXT_IDENTITY_DST,
...
#endif
	      &&
	      pfkey_safe_build(error = pfkey_prop_build(
				       &extensions[K_SADB_EXT_PROPOSAL],
				       64, /* replay */
			    ...
			    __u16 sport)
{
	...
	if ((satype == 0) || (satype > K_SADB_SATYPE_MAX)) {
		KLIPS_PRINT(debug_pfkey,
			    "klips_debug:pfkey_nat_t_new_mapping: "
			    "SAtype=%d unspecified or unknown.\n",
			    satype);
		SENDERR(EINVAL);
	}
	...
	if (!(pfkey_safe_build(error = pfkey_msg_hdr_build(
				       &extensions[0],
				       K_SADB_X_NAT_T_NEW_MAPPING, satype, 0,
				       ++pfkey_msg_seq, 0),
			       extensions)
	      /* SA */
	      && pfkey_safe_build(error = pfkey_sa_build(
					  &extensions[K_SADB_EXT_SA],
					  K_SADB_EXT_SA, ipsp->ips_said.spi, 0,
					  0, 0, 0, 0),
				  extensions)
	      ...
	      && pfkey_safe_build(error = pfkey_x_nat_t_port_build(
	      ...
	      && pfkey_safe_build(error = pfkey_address_build(
					  &extensions[K_SADB_EXT_ADDRESS_DST],
					  K_SADB_EXT_ADDRESS_DST,
					  ipsp->ips_said.proto, 0, ipaddr),
				  extensions)
	      /* NAT_T_DPORT = new port */
	      && pfkey_safe_build(error = pfkey_x_nat_t_port_build(
					  &extensions[K_SADB_X_EXT_NAT_T_DPORT],
					  K_SADB_X_EXT_NAT_T_DPORT, sport),
				  extensions))) {
		KLIPS_PRINT(
			debug_pfkey,
			"klips_debug:pfkey_nat_t_new_mapping: "
...
DEBUG_NO_STATIC int
pfkey_x_nat_t_new_mapping_parse(struct sock *sk, struct sadb_ext **extensions,
				struct pfkey_extracted_data *extr)
{
...
DEBUG_NO_STATIC int (*ext_processors[K_SADB_EXT_MAX + 1])(
	struct sadb_ext *pfkey_ext, struct pfkey_extracted_data *extr) = {
...
DEBUG_NO_STATIC int pfkey_x_simple_reply(struct sock *sk,
					 struct sadb_ext *extensions[], int err)
{
	struct sadb_msg *pfkey_reply = NULL;
	int error = 0;
	struct sadb_msg *m =
...
DEBUG_NO_STATIC int (*msg_parsers[K_SADB_MAX + 1])(
	struct sock *sk, struct sadb_ext *extensions[],
	struct pfkey_extracted_data *extr) = {
	NULL, /* RESERVED */
	pfkey_getspi_parse,
	pfkey_update_parse,
		      ...
		      struct sadb_msg **pfkey_reply)
{
	...
	error = pfkey_safe_build(
		pfkey_msg_hdr_build(&extensions[0], msg_type,
				    proto2satype(extr->ips->ips_said.proto), 0,
				    seq, pfkey_msg->sadb_msg_pid),
		extensions);

	if (!error &&
	    pfkey_required_extension(EXT_BITS_OUT, msg_type, K_SADB_EXT_SA)) {
		error = pfkey_sa_build(
			&extensions[K_SADB_EXT_SA], K_SADB_EXT_SA,
			extr->ips->ips_said.spi, extr->ips->ips_replaywin,
			extr->ips->ips_state, extr->ips->ips_authalg,
			extr->ips->ips_encalg, extr->ips->ips_flags);
	...
	if (!error && pfkey_required_extension(EXT_BITS_OUT, msg_type,
					       K_SADB_X_EXT_SAREF)) {
...
int pfkey_msg_interp(struct sock *sk, struct sadb_msg *pfkey_msg)
{
	...
	KLIPS_PRINT(
		...
		pfkey_msg->sadb_msg_version, pfkey_msg->sadb_msg_type,
		pfkey_msg->sadb_msg_errno, pfkey_msg->sadb_msg_satype,
		satype2name(pfkey_msg->sadb_msg_satype),
		pfkey_msg->sadb_msg_len, pfkey_msg->sadb_msg_reserved,
		pfkey_msg->sadb_msg_seq, pfkey_msg->sadb_msg_pid);
