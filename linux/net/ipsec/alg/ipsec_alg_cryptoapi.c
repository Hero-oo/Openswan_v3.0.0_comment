...
static __u8 *_capi_new_key(struct ipsec_alg_enc *alg, const __u8 *key,
			   size_t keylen);
static int _capi_cbc_encrypt(struct ipsec_alg_enc *alg, __u8 *key_e, __u8 *in,
...
static int setup_ipsec_alg_capi_cipher(struct ipsec_alg_capi_cipher *cptr)
{
	...
	if (excl)
		cptr->alg.ixt_state |= IPSEC_ALG_ST_EXCL;
	...
	printk("setup_ipsec_alg_capi_cipher(): "
	       ...
	       cptr->alg.ixt_alg_type, cptr->alg.ixt_alg_id, cptr->alg.ixt_name,
	       cptr->alg.ixt_keyminbits, cptr->alg.ixt_keymaxbits, ret);
	return ret;
}
/*
...
static int setup_cipher_list(struct ipsec_alg_capi_cipher *clist)
{
	...
	for (cptr = clist; cptr->ciphername; cptr++) {
		...
		if (cptr->parm[0] == 0 || (noauto && cptr->parm[0] < 0)) {
			if (debug > 0)
				printk(KERN_INFO
				       ...
				       cptr->ciphername, noauto, cptr->parm[0],
				       cptr->parm[1]);
			continue;
		}
		/*
		...
		if (setup_cipher(cptr->ciphername)) {
			...
			if (debug > 0)
				printk(KERN_INFO
				       "setup_cipher_list(): lookup for ciphername=%s: not found \n",
				       cptr->ciphername);
		}
	}
	return 0;
...
static int test_cipher_list(struct ipsec_alg_capi_cipher *clist)
{
	...
	for (cptr = clist; cptr->ciphername; cptr++) {
		if (cptr->alg.ixt_state & IPSEC_ALG_ST_REGISTERED) {
			test_ret = ipsec_alg_test(cptr->alg.ixt_alg_type,
						  cptr->alg.ixt_alg_id, test);
			printk("test_cipher_list(alg_type=%d alg_id=%d): test_ret=%d\n",
			       cptr->alg.ixt_alg_type, cptr->alg.ixt_alg_id,
			       test_ret);
		}
	}
	return 0;
