...
static voidpf my_zcalloc(voidpf opaque, uInt items, uInt size)
{
			     ...
			     unsigned int *flags)
{
	...
	KLIPS_PRINT(sysctl_ipsec_debug_ipcomp, "klips_debug:skb_compress: .\n");
	...
	if (iph->version == 6) {
		...
		nexthdroff = ipsec_ipv6_skip_exthdr(
			skb, ((void *)(iph6 + 1)) - (void *)skb->data, &nexthdr,
			&frag_off);
	...
	((struct ipcomphdr *)(((char *)iph) + iphlen))->ipcomp_cpi =
		htons((__u16)(ntohl(ips->ips_said.spi) & 0x0000ffff));
	KLIPS_PRINT(
		sysctl_ipsec_debug_ipcomp,
		"klips_debug:skb_compress: "
		"spi=%08x, spi&0xffff=%04x, cpi=%04x, payload size: raw=%d, comp=%d.\n",
		ntohl(ips->ips_said.spi), ntohl(ips->ips_said.spi) & 0x0000ffff,
		ntohs(((struct ipcomphdr *)(((char *)iph) + iphlen))
			      ->ipcomp_cpi),
		pyldsz, cpyldsz);
	...
	if (iph->version == 6) {
		...
		iph6->payload_len = htons(iphlen + sizeof(struct ipcomphdr) +
					  cpyldsz - sizeof(struct ipv6hdr));
	} else
#endif
	{
		iph->protocol = IPPROTO_COMP;
		iph->tot_len =
			htons(iphlen + sizeof(struct ipcomphdr) + cpyldsz);
	...
	memcpy((char *)iph + iphlen + sizeof(struct ipcomphdr), buffer,
	       cpyldsz);
	kfree(buffer);
			       ...
			       unsigned int *flags)
{
	...
	if (oiph->version == 6) {
		...
		nexthdroff = ipsec_ipv6_skip_exthdr(
			skb, ((void *)(oiph6 + 1)) - (void *)skb->data,
			&nexthdr, &frag_off);
	...
	if ((((struct ipcomphdr *)((char *)oiph + iphlen))->ipcomp_flags !=
		...
		KLIPS_PRINT(
			...
			ntohs(((struct ipcomphdr *)((char *)oiph + iphlen))
				      ->ipcomp_flags),
			ntohs(((struct ipcomphdr *)((char *)oiph + iphlen))
				      ->ipcomp_cpi),
