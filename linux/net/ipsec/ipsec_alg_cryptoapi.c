...
#ifdef CONFIG_KLIPS_ENC_1DES
	{ CIPHERNAME_1DES, 8, 64, 64, des_ede1, {
		 ixt_common: {
			 ixt_support: {
				 ias_id: ESP_DES,
			 }
			   ...
			   size_t keylen)
{
	 ...
	 *	alloc tfm
	 */
	tfm = crypto_blkcipher_tfm(
		crypto_alloc_blkcipher(cptr->ciphername, 0, 0));
	if (!tfm) {
		printk(KERN_ERR "_capi_new_key(): "
	...
	if (crypto_blkcipher_setkey(crypto_blkcipher_cast(tfm), key, keylen) <
	    0) {
		printk(KERN_ERR
		       "_capi_new_key(): "
...
static int setup_cipher_list(struct ipsec_alg_capi_cipher *clist)
{
	...
	for (cptr = clist; cptr->ciphername; cptr++) {
		...
		if (cptr->parm[0] == 0 || (noauto && cptr->parm[0] < 0)) {
			...
			if (debug_crypto > 0)
				printk(KERN_INFO
				       "setup_cipher_list(): going to init ciphername=%s: noauto=%d parm[0]=%d parm[1]=%d\n",
				       cptr->ciphername, noauto, cptr->parm[0],
				       cptr->parm[1]);
		}
		/*
		...
		if (setup_cipher(cptr->ciphername)) {
			...
			printk(KERN_INFO
			       "KLIPS: lookup for ciphername=%s: not found \n",
			       cptr->ciphername);
		}
	}
	return 0;
