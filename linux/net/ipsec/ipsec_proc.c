...
extern int ipsec_xform_get_info(char *buffer, char **start, off_t offset,
...
static void *proc_eroute_start(struct seq_file *m, loff_t *pos)
{
	struct rj_walkstate *rjws =
		kmalloc(sizeof(struct rj_walkstate), GFP_KERNEL);
	if (!rjws)
		return NULL;
...
static void *proc_eroute_next(struct seq_file *m, void *v, loff_t *pos)
{
	...
	spin_unlock_bh(&eroute_lock);
	if (rjws->walkonce_control == WALK_DONE)
		return NULL;
...
static int proc_eroute_show(struct seq_file *m, void *v)
{
	...
	if (rjws->current_node && !(rjws->current_node->rj_flags & RJF_ROOT)) {
		ipsec_rj_walker_procprint(m, rjws->current_node);
	}
	return 0;
}

static struct seq_operations proc_eroute_op = { .start = proc_eroute_start,
						.next = proc_eroute_next,
						.stop = proc_eroute_stop,
						.show = proc_eroute_show };

static int proc_eroute_open(struct inode *inode, struct file *file)
{
...
{
	int i;
	struct spi_walk_state *sws =
		kmalloc(sizeof(struct spi_walk_state), GFP_KERNEL);
...
static void proc_spi_stop(struct seq_file *m, void *v)
{
	if (v) {
		...
		if (sws->spi_current)
			ipsec_sa_put(sws->spi_current, IPSEC_REFPROC);
		sws->spi_current = NULL;
		kfree(sws);
	}
}
...
static void *proc_spi_next(struct seq_file *m, void *v, loff_t *pos)
{
	...
	for (i = sws->spi_hash_num;
	     i < SADB_HASHMOD && (sws->spi_offset <= *pos); i++) {
		sws->spi_hash_num = i;
		for (sa_p = sws->spi_current; sa_p && (sws->spi_offset <= *pos);
		     sa_p = sa_p->ips_hnext) {
			if (sws->spi_offset == *pos) {
				sws->spi_current =
					ipsec_sa_get(sa_p, IPSEC_REFPROC);
 ...
 */
static void ipsec_lifetime_format(struct seq_file *m, char *lifename,
				  ...
				  struct ipsec_lifetime64 *lifetime)
{
	...
	if (lifetime->ipl_count > 1 || lifetime->ipl_soft ||
	    lifetime->ipl_hard) {
		seq_printf(m, "%s(%Lu,%Lu,%Lu)", lifename, count,
...
static int proc_spi_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "%s%s%s", IPS_XFORM_NAME(sa_p));
	seq_printf(m, ": dir=%s",
		   (sa_p->ips_flags & EMT_INBOUND) ? "in " : "out");
	...
	if ((sa_p->ips_said.proto == IPPROTO_IPIP) &&
		...
		if (sa_p->ips_flow_s.u.v4.sin_family == AF_INET) {
			subnettoa(sa_p->ips_flow_s.u.v4.sin_addr,
				  sa_p->ips_mask_s.u.v4.sin_addr, 0, buf_s,
				  sizeof(buf_s));

			subnettoa(sa_p->ips_flow_d.u.v4.sin_addr,
				  sa_p->ips_mask_d.u.v4.sin_addr, 0, buf_d,
				  sizeof(buf_d));
		} else {
			subnet6toa(&sa_p->ips_flow_s.u.v6.sin6_addr,
				   &sa_p->ips_mask_s.u.v6.sin6_addr, 0, buf_s,
				   sizeof(buf_s));

			subnet6toa(&sa_p->ips_flow_d.u.v6.sin6_addr,
				   &sa_p->ips_mask_d.u.v6.sin6_addr, 0, buf_d,
				   sizeof(buf_d));
		}
	...
	if (sa_p->ips_iv_bits) {
		int j;
		seq_printf(m, " iv_bits=%dbits iv=0x", sa_p->ips_iv_bits);
	...
	if (sa_p->ips_encalg || sa_p->ips_authalg) {
		if (sa_p->ips_replaywin) {
			seq_printf(m, " ooowin=%d", sa_p->ips_replaywin);
		}
		...
		if (sa_p->ips_replaywin_lastseq) {
			seq_printf(m, " seq=%d", sa_p->ips_replaywin_lastseq);
		}
		if (sa_p->ips_replaywin_bitmap) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 3, 0)
			seq_printf(m, " bit=0x%Lx", sa_p->ips_replaywin_bitmap);
#else
	...
	if (sa_p->ips_flags & ~EMT_INBOUND) {
		seq_printf(m, " flags=0x%x", sa_p->ips_flags & ~EMT_INBOUND);
		seq_printf(m, "<");
		/* flag printing goes here */
		seq_printf(m, ">");
	}
	if (sa_p->ips_auth_bits) {
		seq_printf(m, " alen=%d", sa_p->ips_auth_bits);
	}
	if (sa_p->ips_key_bits_a) {
		seq_printf(m, " aklen=%d", sa_p->ips_key_bits_a);
	}
	if (sa_p->ips_errs.ips_auth_errs) {
		seq_printf(m, " auth_errs=%d", sa_p->ips_errs.ips_auth_errs);
	}
	if (sa_p->ips_key_bits_e) {
		seq_printf(m, " eklen=%d", sa_p->ips_key_bits_e);
	}
	...
	ipsec_lifetime_format(m, "alloc", ipsec_life_countbased,
			      &sa_p->ips_life.ipl_allocations);

	ipsec_lifetime_format(m, "bytes", ipsec_life_countbased,
			      &sa_p->ips_life.ipl_bytes);

	ipsec_lifetime_format(m, "addtime", ipsec_life_timebased,
			      &sa_p->ips_life.ipl_addtime);

	ipsec_lifetime_format(m, "usetime", ipsec_life_timebased,
			      &sa_p->ips_life.ipl_usetime);

	ipsec_lifetime_format(m, "packets", ipsec_life_countbased,
			      &sa_p->ips_life.ipl_packets);

	if (sa_p->ips_life.ipl_usetime.ipl_last) { /* XXX-MCR should be last? */
		seq_printf(m, " idle=%Ld",
			   ipsec_jiffieshz_elapsed(
				   jiffies / HZ,
				   sa_p->ips_life.ipl_usetime.ipl_last));
	}

#ifdef CONFIG_KLIPS_IPCOMP
	if (sa_p->ips_said.proto == IPPROTO_COMP &&
	    (sa_p->ips_comp_ratio_dbytes || sa_p->ips_comp_ratio_cbytes)) {
		seq_printf(m, " ratio=%Ld:%Ld", sa_p->ips_comp_ratio_dbytes,
...
#ifdef NAT_TRAVERSAL
	{
		...
		seq_printf(m, " natencap=%s", natttype_name);

		seq_printf(m, " natsport=%d", sa_p->ips_natt_sport);

		seq_printf(m, " natdport=%d", sa_p->ips_natt_dport);
	}
#else
	...
	seq_printf(m, " refcount=%d", atomic_read(&sa_p->ips_refcount) - 1);

	seq_printf(m, " ref=%d", sa_p->ips_ref);
	seq_printf(m, " refhim=%d", sa_p->ips_refhim);

	if (sa_p->ips_out) {
		seq_printf(m, " outif=%s:%d", sa_p->ips_out->name,
...
static struct seq_operations proc_spi_op = { .start = proc_spi_start,
					     .next = proc_spi_next,
					     .stop = proc_spi_stop,
					     .show = proc_spi_show };

static int proc_spi_open(struct inode *inode, struct file *file)
{
...
static struct seq_operations proc_spigrp_op = { .start = proc_spi_start,
						.next = proc_spi_next,
						.stop = proc_spi_stop,
						.show = proc_spigrp_show };

static int proc_spigrp_open(struct inode *inode, struct file *file)
{
...
static int proc_tncfg_show(struct seq_file *m, void *v)
{
	...
	if (dev) {
		...
		if (priv) {
			...
			seq_printf(m, " mtu=%d(%d) -> %d", dev->mtu, priv->mtu,
				   privdev ? privdev->mtu : 0);
		} else {
			KLIPS_PRINT(
				debug_tunnel & DB_TN_PROCFS,
...
static struct seq_operations proc_tncfg_op = { .start = proc_tncfg_start,
...
struct ipsec_proc_list ipsec_proc_stats = { .name = "stats",
					    .parent = &proc_net_ipsec_dir,
					    .dir = &proc_stats_dir };
