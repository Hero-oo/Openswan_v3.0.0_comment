...
DEBUG_NO_STATIC int ipsec_updatereplaywindow(struct ipsec_sa *ipsp, __u32 seq)
{
	...
	if (seq > ipsp->ips_replaywin_lastseq) {
		...
		if (seq - ipsp->ips_replaywin_lastseq - 1 >
			...
			ipsp->ips_replaywin_maxdiff =
				seq - ipsp->ips_replaywin_lastseq - 1;
		}
...
struct auth_alg ipsec_rcv_md5[] = { { osMD5Init, osMD5Update, osMD5Final,
				      AHMD596_ALEN } };

#endif /* CONFIG_KLIPS_AUTH_HMAC_MD5 */

#ifdef CONFIG_KLIPS_AUTH_HMAC_SHA1
struct auth_alg ipsec_rcv_sha1[] = { { SHA1Init, SHA1Update, SHA1Final,
				       AHSHA196_ALEN } };
#endif /* CONFIG_KLIPS_AUTH_HMAC_MD5 */
...
static inline void ipsec_rcv_redodebug(struct ipsec_rcv_state *irs)
{
	if (osw_ip_hdr_version(irs) == 4) {
		inet_addrtot(AF_INET, &osw_ip4_hdr(irs)->saddr, 0,
			     irs->ipsaddr_txt, sizeof(irs->ipsaddr_txt));
		inet_addrtot(AF_INET, &osw_ip4_hdr(irs)->daddr, 0,
			     irs->ipdaddr_txt, sizeof(irs->ipdaddr_txt));
	} else if (osw_ip_hdr_version(irs) == 6) {
		inet_addrtot(AF_INET6, &osw_ip6_hdr(irs)->saddr, 0,
			     irs->ipsaddr_txt, sizeof(irs->ipsaddr_txt));
		inet_addrtot(AF_INET6, &osw_ip6_hdr(irs)->daddr, 0,
			     irs->ipdaddr_txt, sizeof(irs->ipdaddr_txt));
	} else {
...
DEBUG_NO_STATIC char *ipsec_rcv_err(int err)
{
	static char tmp[32];
	switch ((int)err) {
	case IPSEC_RCV_PENDING:
		return ("IPSEC_RCV_PENDING");
	case IPSEC_RCV_LASTPROTO:
		return ("IPSEC_RCV_LASTPROTO");
	case IPSEC_RCV_OK:
		return ("IPSEC_RCV_OK");
	case IPSEC_RCV_BADPROTO:
		return ("IPSEC_RCV_BADPROTO");
	case IPSEC_RCV_BADLEN:
		return ("IPSEC_RCV_BADLEN");
	case IPSEC_RCV_ESP_BADALG:
		return ("IPSEC_RCV_ESP_BADALG");
	case IPSEC_RCV_3DES_BADBLOCKING:
		return ("IPSEC_RCV_3DES_BADBLOCKING");
	case IPSEC_RCV_ESP_DECAPFAIL:
		return ("IPSEC_RCV_ESP_DECAPFAIL");
	case IPSEC_RCV_DECAPFAIL:
		return ("IPSEC_RCV_DECAPFAIL");
	case IPSEC_RCV_SAIDNOTFOUND:
		return ("IPSEC_RCV_SAIDNOTFOUND");
	case IPSEC_RCV_IPCOMPALONE:
		return ("IPSEC_RCV_IPCOMPALONE");
	case IPSEC_RCV_IPCOMPFAILED:
		return ("IPSEC_RCV_IPCOMPFAILED");
	case IPSEC_RCV_SAIDNOTLIVE:
		return ("IPSEC_RCV_SAIDNOTLIVE");
	case IPSEC_RCV_FAILEDINBOUND:
		return ("IPSEC_RCV_FAILEDINBOUND");
	case IPSEC_RCV_LIFETIMEFAILED:
		return ("IPSEC_RCV_LIFETIMEFAILED");
	case IPSEC_RCV_BADAUTH:
		return ("IPSEC_RCV_BADAUTH");
	case IPSEC_RCV_REPLAYFAILED:
		return ("IPSEC_RCV_REPLAYFAILED");
	case IPSEC_RCV_AUTHFAILED:
		return ("IPSEC_RCV_AUTHFAILED");
	case IPSEC_RCV_REPLAYROLLED:
		return ("IPSEC_RCV_REPLAYROLLED");
	case IPSEC_RCV_BAD_DECRYPT:
		return ("IPSEC_RCV_BAD_DECRYPT");
	case IPSEC_RCV_REALLYBAD:
		return ("IPSEC_RCV_REALLYBAD");
	case IPSEC_RCV_ERRMEMALLOC:
...
struct {
	...
	[IPSEC_RSM_DECAP_INIT] = { ipsec_rcv_decap_init,
				   IPSEC_RSM_DECAP_LOOKUP },
	[IPSEC_RSM_DECAP_LOOKUP] = { ipsec_rcv_decap_lookup,
				     IPSEC_RSM_AUTH_INIT },
	...
	if (skb != NULL && irs->ipsp->ips_out) {
		if (skb->dev != irs->ipsp->ips_out) {
			KLIPS_PRINT(
				...
				skb->dev->name, irs->ipsp->ips_out->name);
		}
...
static enum ipsec_rcv_value ipsec_rcv_decap_ipip(struct ipsec_rcv_state *irs)
{
	...
	if (ipp->version == 4 && ipp->protocol != IPPROTO_IPIP &&
	    ...
	    ipp->protocol !=
		    IPPROTO_ATT_HEARTBEAT) { /* AT&T heartbeats to SIG/GIG */
		KLIPS_PRINT(
			debug_rcv,
	...
	if (sysctl_ipsec_inbound_policy_check) {
		...
		if ((ipsnext = ipsp->ips_next)) {
			...
			size_t sa_len2;
			if (debug_rcv)
				sa_len2 = satot(&ipsnext->ips_said, 0, sa2,
						sizeof(sa2));
			else
				sa_len2 = 0;
			KLIPS_PRINT(debug_rcv,
		...
		} else if (((struct sockaddr_in *)(ipsp->ips_addr_s))
				   ->sin_family == AF_INET6) {
			psin = (struct sockaddr_in *)(ipsp->ips_addr_s);
		}
		if ((psin && ipp->saddr != psin->sin_addr.s_addr)
#ifdef CONFIG_KLIPS_IPV6
		    || (psin6 && memcmp(&ipp6->saddr, &psin6->sin6_addr,
					sizeof(ipp6->saddr)) != 0)
#endif
		) {
	...
	if ((ipp->version == 4 || ipp->version == 6) &&
		...
		skb_set_network_header(
			skb, ipsec_skb_offset(skb, skb_transport_header(skb)));
		...
		if (ipp->version == 6) {
			...
			nexthdroff = ipsec_ipv6_skip_exthdr(
				skb, ipsec_skb_offset(skb, ipp6 + 1), &nexthdr,
				&frag_off);
			skb_set_transport_header(
				skb, ipsec_skb_offset(skb, (void *)skb->data +
								   nexthdroff));
...
#endif /* CONFIG_KLIPS_IPV6 */
		{
			skb_set_transport_header(
				skb,
				ipsec_skb_offset(skb, skb_network_header(skb) +
							      (ipp->ihl << 2)));
			...
			if (ip_address_family(&ipsp->ips_flow_s) != AF_INET6) {
				...
				failed_inbound_check = 1;
				     ...
				     ipsp->ips_mask_s.u.v6.sin6_addr
					     .s6_addr32[0]) ^
				    ipsp->ips_flow_s.u.v6.sin6_addr
					    .s6_addr32[0]) |
				   ((ipp6->daddr.s6_addr32[0] &
				     ipsp->ips_mask_d.u.v6.sin6_addr
					     .s6_addr32[0]) ^
				    ipsp->ips_flow_d.u.v6.sin6_addr
					    .s6_addr32[0])) {
				failed_inbound_check = 1;
	...
	if (failed_inbound_check) {
		...
		if (ipsp->ips_flow_s.u.v4.sin_family == AF_INET6) {
			subnet6toa(&ipsp->ips_flow_s.u.v6.sin6_addr,
				   &ipsp->ips_mask_s.u.v6.sin6_addr, 0,
				   sflow_txt, sizeof(sflow_txt));
			subnet6toa(&ipsp->ips_flow_d.u.v6.sin6_addr,
				   &ipsp->ips_mask_d.u.v6.sin6_addr, 0,
				   dflow_txt, sizeof(dflow_txt));
		} else
#endif
		{
			subnettoa(ipsp->ips_flow_s.u.v4.sin_addr,
				  ipsp->ips_mask_s.u.v4.sin_addr, 0, sflow_txt,
				  sizeof(sflow_txt));
			subnettoa(ipsp->ips_flow_d.u.v4.sin_addr,
				  ipsp->ips_mask_d.u.v4.sin_addr, 0, dflow_txt,
				  sizeof(dflow_txt));
		}
		KLIPS_PRINT(
			debug_rcv,
			...
			irs->sa_len ? irs->sa : " (error)", sflow_txt,
			dflow_txt, irs->ipsaddr_txt, irs->ipdaddr_txt);
		if (irs->stats) {
			irs->stats->rx_dropped++;
		}
		goto rcvleave;
	...
	*/
	...
	if (irs->natt_len) {
		...
		osw_ip4_hdr(irs)->tot_len =
			htons(ntohs(osw_ip4_hdr(irs)->tot_len) - irs->natt_len);
...
static enum ipsec_rcv_value ipsec_rcv_auth_init(struct ipsec_rcv_state *irs)
{
	...
	if (sysctl_ipsec_inbound_policy_check) {
		struct sockaddr_in *psin = NULL;
		struct sockaddr_in6 *psin6 = NULL;
		char sa_saddr_txt[ADDRTOA_BUF] = {
			0,
		};

		if (((struct sockaddr_in6 *)(newipsp->ips_addr_s))
		...
		if ((psin &&
		     osw_ip4_hdr(irs)->saddr != psin->sin_addr.s_addr) ||
		    (psin6 &&
		     memcmp(&osw_ip6_hdr(irs)->saddr, &psin6->sin6_addr,
			    sizeof(osw_ip6_hdr(irs)->saddr)) != 0)) {
			if (debug_rcv) {
				/* generate SA->saddr */
				if (psin) {
		...
		KLIPS_PRINT(
			...
			irs->sa_len ? irs->sa : " (error)", irs->ipsaddr_txt);
...
static enum ipsec_rcv_value ipsec_rcv_auth_decap(struct ipsec_rcv_state *irs)
{
	...
	if (irs->ipsp->ocf_in_use) {
		...
		if ((irs->ixt_a = irs->ipsp->ips_alg_auth)) {
			...
			KLIPS_PRINT(debug_rcv,
				    "klips_debug:ipsec_rcv_auth_decap: "
				    "authalg=%d authlen=%d\n",
				    irs->ipsp->ips_authalg, irs->authlen);
		} else
#endif /* CONFIG_KLIPS_ALG */
	...
	irs->ilen = ((irs->skb->data + irs->skb->len) -
...
static enum ipsec_rcv_value ipsec_rcv_auth_calc(struct ipsec_rcv_state *irs)
{
	...
	if (irs->authfuncs ||
		...
		KLIPS_PRINT(debug_rcv,
			    "klips_debug:ipsec_rcv_auth_calc: "
			    "encalg = %d, authalg = %d.\n",
			    irs->ipsp->ips_encalg, irs->ipsp->ips_authalg);
...
static enum ipsec_rcv_value ipsec_rcv_decap_cont(struct ipsec_rcv_state *irs)
{
	...
	if (osw_ip_hdr_version(irs) == 6) {
		...
		irs->iphlen = nexthdroff - (irs->iph - (void *)irs->skb->data);
		skb_set_transport_header(
			skb, ipsec_skb_offset(skb, skb_network_header(skb) +
							   irs->iphlen));
	} else
#endif /* CONFIG_KLIPS_IPV6 */
	{
	...
	if (osw_ip_hdr_version(irs) == 6) {
		...
		osw_ip4_hdr(irs)->check =
			0; /* NOTE: this will be included in checksum */
		osw_ip4_hdr(irs)->check = ip_fast_csum(
			(unsigned char *)ip_hdr(skb), irs->iphlen >> 2);
		skb->protocol = htons(ETH_P_IP);
	}
	...
	if (sysctl_ipsec_inbound_policy_check) {
		if (ipsnext) {
			if (irs->next_header != IPPROTO_AH &&
			    ...
			    irs->next_header !=
				    IPPROTO_ATT_HEARTBEAT /* heartbeats to AT&T SIG/GIG */
			) {
				KLIPS_PRINT(
					debug_rcv,
	...
	if ((irs->natt_type) && (irs->proto != IPPROTO_IPIP)) {
		...
		*/
		...
		if (natt_oa != 0) {
			...
			osw_ip4_hdr(irs)->check =
				ip_fast_csum((unsigned char *)irs->iph,
					     osw_ip4_hdr(irs)->ihl);
...
#ifdef NET_26
	irs->skb->data =
		skb_push(irs->skb, skb_transport_header(irs->skb) -
...
{
	if (irs == NULL) {
		KLIPS_PRINT(debug_rcv, "klips_debug:ipsec_rsm: "
				       "irs == NULL.\n");
		return;
	}
...
int klips26_rcv_encap(struct sk_buff *skb, __u16 encap_type)
{
	...
	default:
		if (printk_ratelimit()) {
			printk(KERN_INFO
			       "KLIPS received unknown UDP-ESP encap type %u\n",
			       encap_type);
		}
		goto rcvleave;
	}
...
#else
	irs_cache_allocator = kmem_cache_create(
		"ipsec_irs", sizeof(struct ipsec_rcv_state), 0, 0, NULL, NULL);
#endif
	if (!irs_cache_allocator)
		return -ENOMEM;
