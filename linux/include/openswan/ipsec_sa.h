...
#define IPsecSAref2SA(x) \
	(ipsec_sadb.refTable[IPsecSAref2table(x)]->entry[IPsecSAref2entry(x)])
...
struct ipsec_sa {
	...
	struct xform_functions
		*ips_xformfuncs; /* pointer to routines to process this SA */
...
extern struct ipsec_sa *
ipsec_sa_alloc(int *error); /* pass in error var by pointer */
extern int ipsec_sa_free(struct ipsec_sa *ips);

#define ipsec_sa_get(ips, type) \
	__ipsec_sa_get(ips, __FUNCTION__, __LINE__, type)
extern struct ipsec_sa *__ipsec_sa_get(struct ipsec_sa *ips, const char *func,
				       int line, int type);

#define ipsec_sa_put(ips, type) \
	__ipsec_sa_put(ips, __FUNCTION__, __LINE__, type)
...
enum ipsec_direction { ipsec_incoming = 1, ipsec_outgoing = 2 };
