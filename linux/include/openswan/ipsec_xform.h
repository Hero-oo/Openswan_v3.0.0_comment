...
#define XF_ESP3DESMD596 \
	10 /* triple DES, HMAC-MD-5, 96-bits of authentication */
#define XF_ESPNULLMD596 12 /* NULL, HMAC-MD-5 with 96-bits of authentication */
#define XF_ESPNULLSHA196 \
	13 /* NULL, HMAC-SHA-1 with 96-bits of authentication */
#define XF_ESP3DESSHA196 \
	14 /* triple DES, HMAC-SHA-1, 96-bits of authentication */
...
#define AH_NULL 251
#define AH_MAX \
	255 /* note Authentication Algorithm (Value 5) Reference: [RFC2407] has \
...
#define IPS_XFORM_NAME(x)                                                 \
	PROTO2TXT((x)->ips_said.proto),                                   \
		(x)->ips_said.proto == IPPROTO_COMP ?                     \
			((x)->ips_encalg == SADB_X_CALG_NONE ?            \
				 "_NONE" :                                \
			 (x)->ips_encalg == SADB_X_CALG_DEFLATE ?         \
				 "_DEFLATE" :                             \
			 (x)->ips_encalg == SADB_X_CALG_LZS ?             \
				 "_LZS" :                                 \
				 "_UNKNOWN_comp") :                       \
		(x)->ips_encalg == ESP_NONE ?                             \
			"" :                                              \
		(x)->ips_encalg == ESP_NULL ?                             \
			"_NULL" :                                         \
		(x)->ips_encalg == ESP_3DES ?                             \
			"_3DES" :                                         \
		(x)->ips_encalg == ESP_AES ?                              \
			"_AES" :                                          \
		(x)->ips_encalg == ESP_SERPENT ?                          \
			"_SERPENT" :                                      \
		(x)->ips_encalg == ESP_TWOFISH ?                          \
			"_TWOFISH" :                                      \
			enc_name_id(x->ips_encalg) /* "_UNKNOWN_encr" */, \
		(x)->ips_authalg == AH_NONE ?                             \
			"" :                                              \
		(x)->ips_authalg == AH_MD5 ?                              \
			"_HMAC_MD5" :                                     \
		(x)->ips_authalg == AH_SHA ?                              \
			"_HMAC_SHA1" :                                    \
		(x)->ips_authalg == AH_SHA2_256 ?                         \
			"_HMAC_SHA2_256" :                                \
		(x)->ips_authalg == AH_SHA2_384 ?                         \
			"_HMAC_SHA2_384" :                                \
		(x)->ips_authalg == AH_SHA2_512 ?                         \
			"_HMAC_SHA2_512" :                                \
