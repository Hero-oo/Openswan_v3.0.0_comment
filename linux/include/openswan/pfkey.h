...
enum pfkey_ext_required { EXT_BITS_IN = 0, EXT_BITS_OUT = 1 };

enum pfkey_ext_perm { EXT_BITS_PERM = 0, EXT_BITS_REQ = 1 };
...
int pfkey_sa_ref_build(struct sadb_ext **pfkey_ext, uint16_t exttype,
...
int pfkey_sa_build(struct sadb_ext **pfkey_ext, uint16_t exttype,
		   uint32_t spi, /* in network order */
		   uint8_t replay_window, uint8_t sa_state, uint8_t auth,
		   uint8_t encrypt, uint32_t flags);

extern int pfkey_saref_build(struct sadb_ext **pfkey_ext, IPsecSAref_t in,
			     IPsecSAref_t out);
...
extern int pfkey_outif_build(struct sadb_ext **pfkey_ext, uint16_t outif);
