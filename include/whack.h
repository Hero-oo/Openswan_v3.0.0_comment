...
#define WHACK_MAGIC_BASE \
	(u_int32_t)(((((('o' << 8) + 'h') << 8) + 'k') << 8) + 39UL)
...
enum whack_opt_set {
	...
	WHACK_STARTWHACKRECORD =
		2, /* string1 contains file to write options to */
...
#define LIST_ALL              \
	LRANGES(LIST_PUBKEYS, \
		LIST_PSKS) /* all list options: omits: events/hostpairs */
