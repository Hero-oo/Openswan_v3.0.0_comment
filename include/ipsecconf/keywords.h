...
#define KEY_STRINGS_MAX \
	((int)KSF_MAX > (int)KSCF_MAX ? (int)KSF_MAX : (int)KSCF_MAX) + 1
#define KEY_NUMERIC_MAX \
	((int)KBF_MAX > (int)KNCF_MAX ? (int)KBF_MAX : (int)KNCF_MAX) + 1
...
enum keyword_valid {
	...
	kv_policy =
		LELEM(6), /* is a policy affecting verb, processed specially */
	kv_processed =
		LELEM(7), /* is processed, do not output literal string */
	kv_duplicateok =
		LELEM(8), /* it is okay if also= items are duplicated */
	kv_obsolete = LELEM(
		9), /* option that is obsoleted, allow keyword but warn and ignore */
...
struct keyword_def {
	...
	unsigned int
		loose_enum_value; /* what value to use if the for a literal type */
...
struct config_parsed {
	...
	TAILQ_HEAD(sectionhead, section_list)
	sections;
...
extern unsigned int parser_loose_enum_arg(struct keyword *k, const char *s,
					  char **rest);
