...
extern void openswan_loglog(int mess_no, const char *message, ...)
	PRINTF_LIKE(2);
...
#define DBG_cond_dump_chunk(cond, label, ch) \
	DBG(cond, DBG_dump_chunk(label, ch))
...
extern char
	diag_space[LOG_WIDTH]; /* output buffer, but can be occupied at call */
...
#define log_errno(a)                          \
	{                                     \
		int e = errno;                \
		openswan_log_errno_routine a; \
	}
extern void openswan_log_errno_routine(int e, const char *message, ...)
	PRINTF_LIKE(2);
#define exit_log_errno(a)                          \
	{                                          \
		int e = errno;                     \
		openswan_exit_log_errno_routine a; \
	}
extern void openswan_exit_log_errno_routine(int e, const char *message, ...)
	PRINTF_LIKE(2) NEVER_RETURNS NEVER_RETURNS;
