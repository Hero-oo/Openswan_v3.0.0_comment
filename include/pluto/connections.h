...
struct end {
	...
	ip_address host_addr, host_nexthop, host_srcip;
	ip_address
		saved_hint_addr; /* the address we got from the cfg file if IPHOSTNAME */
	...
	struct pubkey *key1,
		*key2; /* references to the public key to be used to authenticate this connection */
...
struct connection {
	...
	CIRCLEQ_ENTRY(connection)
	log_link; /* linked list of open conns {} */
	...
	struct IPhost_pair
		*IPhost_pair; /* opaque from connections.c/hostpair.c */
	struct IDhost_pair
		*IDhost_pair; /* opaque from connections.c/hostpair.c */
	...
	generalName_t *
		ikev1_requested_ca_names; /* ikev1 collected certificate requests */
	generalName_t *
		ikev2_requested_ca_hashes; /* concatenated SHA1 hashes acceptable CA keys */
#ifdef XAUTH_USEPAM
...
extern struct iface_port *
pick_matching_interfacebyfamily(struct iface_port *iflist, int pluto_port,
...
extern void initiate_connection(const char *name, int whackfd, lset_t moredebug,
...
extern int initiate_ondemand(const ip_address *our_client,
			     ...
			     struct xfrm_user_sec_ctx_ike *uctx, err_t why);
...
extern struct connection *con_by_name(const char *nm, bool strict);

#define find_host_connection(exact, me, my_port, histype, him, his_port,      \
			     policy_set, policy_clear, pPolicy_hint)          \
	find_host_connection2(__FUNCTION__, exact, me, my_port, histype, him, \
			      his_port, policy_set, policy_clear,             \
			      pPolicy_hint)
extern struct connection *find_host_connection2(
...
extern struct cnnection *
ikev2_ts_instantiate(struct connection *c, const ip_address *our_client,
		     const u_int16_t our_port, const ip_address *peer_client,
		     const u_int16_t peer_port, const u_int8_t protocol);
...
extern int add_pending(int whack_sock, struct state *isakmp_sa,
...
extern char *fmt_connection_inst_name(struct connection *c, char *instname,
