...
struct trans_attrs {
	...
	enum ikev2_trans_type_esn
		esn; /* if Extended Sequence Numbers are enabled */
	...
	const struct ike_encr_desc
		*encrypter; /* package of encryption routines */
	const struct ike_prf_desc
		*prf_hasher; /* pseudo-random function => hash */
	const struct ike_integ_desc
		*integ_hasher; /* package of hashing routines */
...
#define assert_suspended(_st, _md)                                             \
	do {                                                                   \
		passert((_st));                                                \
		...
		if ((_md) && (_md)->st != (_st)) {                             \
			DBG_log("%s:%u md=%p->st=%p != st=%p", __func__,       \
				__LINE__, (_md), (_md)->st, (_st));            \
			impossible();                                          \
		}                                                              \
	} while (0)

/* assign or clear (md==NULL) async crypto operation */
#define set_suspended(_st, _md)                                                                                                               \
	do {                                                                                                                                  \
		...
		if (_md) { /* we are about to suspend the md */                                                                               \
			if (had_md) {                                                                                                         \
				DBG_log("%s:%u set_suspended() called on #%lu with md=%p, already claimed by md=%p (at %s:%u)",               \
					__func__, __LINE__,                                                                                   \
					(_st)->st_serialno, (_md), had_md,                                                                    \
					(_st)->st_suspended_md_func,                                                                          \
					(_st)->st_suspended_md_line);                                                                         \
				impossible();                                                                                                 \
			}                                                                                                                     \
...
#define IKEv2_ORIG_INITIATOR_FLAG(st) \
	(IKEv2_IS_ORIG_INITIATOR(st) ? ISAKMP_FLAGS_I : 0)

/* map state->st_ikev2_orig_initiator to INITIATOR vs RESPONDER as per enum phase1_role */
#define IKEv2_ORIGINAL_ROLE(st) \
	(IKEv2_IS_ORIG_INITIATOR(st) ? INITIATOR : RESPONDER)
...
extern struct state *duplicate_state(struct state *st),
	*find_state_ikev1(const u_char *icookie, const u_char *rcookie,
...
extern void initialize_new_state(struct state *st, struct connection *c,
				 lset_t policy, int try, int whack_sock,
...
extern void fmt_state(struct state *st, const time_t n, char *state_buf,
		      const size_t state_buf_len, char *state_buf2,
		      const size_t state_buf_len2);
extern void delete_states_by_peer(ip_address *peer);
extern void replace_states_by_peer(ip_address *peer);

extern void set_state_ike_endpoints(struct state *st, struct connection *c);
