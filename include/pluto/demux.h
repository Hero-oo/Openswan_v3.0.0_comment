...
struct msg_digest {
	...
	const struct state_v2_microcode
		*svm; /* microcode for initial state (v2)*/
	bool new_iv_set;
...
extern void fmt_ipsec_sa_established(struct state *st, char *sadetails,
				     int sad_len);
extern void fmt_isakmp_sa_established(struct state *st, char *sadetails,
				      int sad_len);
