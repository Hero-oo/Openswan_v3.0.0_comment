...
struct kernel_ops {
	...
	void (*set_debug)(int, openswan_keying_debug_func_t debug_func,
	...
	bool (*raw_eroute)(const ip_address *this_host,
			   ...
			   unsigned int proto, unsigned int transport_proto,
			   enum eroute_type satype,
			   const struct pfkey_proto_info *proto_info,
			   time_t use_lifetime, enum pluto_sadb_operations op,
			   const char *text_said, char *policy_label);
	bool (*shunt_eroute)(struct connection *c, const struct spd_route *sr,
	...
	void (*remove_orphaned_holds)(int transportproto, const ip_subnet *ours,
				      const ip_subnet *his);
	...
	ipsec_spi_t (*get_spi)(const ip_address *src, const ip_address *dst,
			       int proto, bool tunnel_mode, unsigned reqid,
			       ipsec_spi_t min, ipsec_spi_t max,
			       const char *text_said);
	bool (*docommand)(struct connection *c, const struct spd_route *sr,
...
extern bool replace_bare_shunt(const ip_address *src, const ip_address *dst,
			       ...
			       ,
			       bool repl, int transport_proto, const char *why);

extern bool delete_bare_shunt_ptr(struct bare_shunt **bs_pp, const char *why);

extern bool assign_hold(struct connection *c, struct spd_route *sr,
...
extern bool get_ipsec_spi(struct ipsec_proto_info *pi, int proto,
			  struct state *st, bool tunnel_mode);
...
extern bool install_ipsec_sa(struct state *parent_st, struct state *st,
			     bool inbound_also);
extern void delete_ipsec_sa(struct state *st, bool inbound_only);
extern bool route_and_eroute(struct connection *c, const struct spd_route *sr,
			     struct spd_route *orig_sr, struct state *st);
...
static inline bool compatible_overlapping_connections(struct connection *a,
						      struct connection *b)
{
	return kernel_ops->overlap_supported && a && b && a != b &&
