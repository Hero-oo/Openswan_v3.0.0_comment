...
#define NAT_T_WITH_PORT_FLOATING                                          \
	(LELEM(NAT_TRAVERSAL_IETF_02_03) | LELEM(NAT_TRAVERSAL_IETF_05) | \
	 LELEM(NAT_TRAVERSAL_RFC))

/**
 * NAT-Traversal methods which use officials values (RFC)
 */
#define NAT_T_WITH_RFC_VALUES (LELEM(NAT_TRAVERSAL_RFC))

/**
 * NAT-Traversal methods which use officials values (RFC) for encapsulation
 */
#define NAT_T_WITH_ENCAPSULATION_RFC_VALUES (LELEM(NAT_TRAVERSAL_RFC))
 ...
 * IKE port floating
 */
bool nat_traversal_port_float(struct state *st, struct msg_digest *md, bool in);
