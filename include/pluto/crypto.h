...
extern const struct oakley_group_desc *
...
void crypto_cbc_encrypt(const struct ike_encr_desc *e, bool enc, u_int8_t *buf,
			size_t size, struct state *st);

#define update_iv(st)                                    \
	passert(st->st_new_iv_len <= sizeof(st->st_iv)); \
	memcpy((st)->st_iv, (st)->st_new_iv,             \
	       (st)->st_iv_len = (st)->st_new_iv_len)

#define iv_min(x, y) ((x) < (y) ? (x) : (y))
#define init_new_iv(st)                                      \
	passert(st->st_new_iv_len <= sizeof(st->st_new_iv)); \
	memcpy((st)->st_new_iv, (st)->st_iv, (st)->st_new_iv_len);
#define save_iv(st, tmp)                           \
	passert((st)->st_iv_len <= sizeof((tmp))); \
	memcpy((tmp), (st)->st_iv, iv_min((st)->st_iv_len, sizeof((tmp))));
#define save_new_iv(st, tmp)                           \
	passert((st)->st_new_iv_len <= sizeof((tmp))); \
	memcpy((tmp), (st)->st_new_iv, (st)->st_new_iv_len);
#define set_iv(st, tmp)                                  \
	passert((st)->st_iv_len <= sizeof((st)->st_iv)); \
	memcpy((st)->st_iv, (tmp),                       \
	       iv_min((st)->st_iv_len, sizeof((st)->st_iv)));
#define set_new_iv(st, iv)                                       \
	passert((st)->st_new_iv_len <= sizeof((st)->st_new_iv)); \
	memcpy((st)->st_new_iv, (iv),                            \
	       iv_min((st)->st_new_iv_len, sizeof((st)->st_new_iv)));
#define set_ph1_iv(st, iv)                                       \
	passert((st)->st_ph1_iv_len <= sizeof((st)->st_ph1_iv)); \
	memcpy((st)->st_ph1_iv, (iv),                            \
	       iv_min((st)->st_ph1_iv_len, sizeof((st)->st_ph1_iv)));
...
extern void hmac_init(struct hmac_ctx *ctx, const struct ike_integ_desc *h,
...
extern void hmac_update(struct hmac_ctx *ctx, const u_char *data,
			size_t data_len);
...
nss_key_derivation_mech(const struct ike_integ_desc *hasher);
extern void nss_symkey_log(PK11SymKey *key, const char *msg);
extern chunk_t hmac_pads(u_char val, unsigned int len);
extern PK11SymKey *
pk11_derive_wrapper_osw(PK11SymKey *base, CK_MECHANISM_TYPE mechanism,
			chunk_t data, CK_MECHANISM_TYPE target,
			CK_ATTRIBUTE_TYPE operation, int keySize);
extern PK11SymKey *PK11_Derive_osw(PK11SymKey *base,
