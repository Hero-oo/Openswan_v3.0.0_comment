...
extern void sign_hash(const struct private_key_stuff *pks,
		      const u_char *hash_val, size_t hash_len, u_char *sig_val,
		      size_t sig_len);

/* decrypt(verify) a signature to recover the contained hash */
extern err_t
verify_signed_hash(const struct RSA_public_key *k, u_char *s,
		   unsigned int s_max_octets /* working space: RSA_MAX_OCTETS */
		   ,
...
osw_find_secret_by_public_key(struct secret *secrets,
			      struct pubkey *my_public_key, int kind);

extern struct secret *osw_get_xauthsecret(const struct connection *c UNUSED,
					  char *xauthname);

#define osw_asymmetric_key(cert)                                          \
	(cert.type == CERT_PKCS7_WRAPPED_X509 || cert.type == CERT_PGP || \
	 cert.type == CERT_DNS_SIGNED_KEY ||                              \
	 cert.type == CERT_X509_SIGNATURE || cert.type == CERT_RAW_RSA)
...
extern stf_status check_signature_gen(
	struct connection *c, struct state *st,
	const u_char hash_val[MAX_DIGEST_LEN], size_t hash_len,
