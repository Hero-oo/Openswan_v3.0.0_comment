...
#define iv_size enc_blocksize
	...
	void (*do_crypt)(u_int8_t *dat /* encrypts and decrypts *INPLACE */
			 ,
			 size_t datasize, u_int8_t *key, size_t key_size,
...
extern bool ikev1_alg_enc_ok(int ealg, unsigned key_len,
			     struct alg_info_ike *alg_info_ike, const char **,
			     char *, size_t);
...
extern struct ike_alg *ike_alg_ikev1_find(enum ikev2_trans_type algo_type,
					  unsigned algo_id, unsigned keysize);
static inline struct ike_encr_desc *ikev1_alg_get_encr(int alg)
{
	return (struct ike_encr_desc *)ike_alg_ikev1_find(IKEv2_TRANS_TYPE_ENCR,
							  alg, 0);
}

static inline struct ike_integ_desc *ikev1_crypto_get_hasher(unsigned int alg)
{
	return (struct ike_integ_desc *)ike_alg_ikev1_find(
		IKEv2_TRANS_TYPE_INTEG, alg, 0);
}

static inline struct ike_prf_desc *ikev1_crypto_get_prf(unsigned int alg)
{
	return (struct ike_prf_desc *)ike_alg_ikev1_find(IKEv2_TRANS_TYPE_PRF,
							 alg, 0);
}
#endif
...
{
	return (struct ike_encr_desc *)ike_alg_ikev2_find(IKEv2_TRANS_TYPE_ENCR,
							  alg, 0);
}
static __inline__ struct ike_integ_desc *
ike_alg_get_integ(enum ikev2_trans_type_integ halg)
{
	return (struct ike_integ_desc *)ike_alg_ikev2_find(
		IKEv2_TRANS_TYPE_INTEG, halg, 0);
}
static __inline__ struct ike_prf_desc *
ike_alg_get_prf(enum ikev2_trans_type_prf prfalg)
{
	return (struct ike_prf_desc *)ike_alg_ikev2_find(IKEv2_TRANS_TYPE_PRF,
							 prfalg, 0);
}
static __inline__ struct ike_dh_desc *ike_alg_get_dh(int alg)
{
	return (struct ike_dh_desc *)ike_alg_ikev2_find(IKEv2_TRANS_TYPE_DH,
							alg, 0);
}
...
extern struct db_sa *
ikev2_kernel_alg_makedb(lset_t policy, struct alg_info_esp *ei, bool logit);

extern struct db_sa *kernel_alg_makedb(lset_t policy, struct alg_info_esp *ei,
...
extern enum ikev2_trans_type_integ
ikev1toikev2integ(enum ikev1_auth_attribute num);
