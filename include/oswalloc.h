...
extern void *clone_bytes2(const void *orig, size_t size, const char *name,
			  int leak_detective);
...
#define clone_bytes(orig, size, name) \
	(clone_bytes2(orig, size, name, leak_detective))

#define alloc_thing(thing, name) (alloc_bytes(sizeof(thing), (name)))

#define clone_thing(orig, name) \
	clone_bytes((const void *)&(orig), sizeof(orig), (name))
#define clone_str(str, name) \
	((str) == NULL ?     \
		 NULL :      \
		 clone_bytes((str), strlen((const char *)(str)) + 1, (name)))
...
#define strtochunk(ch, str, name)                    \
	{                                            \
		(ch).len = strlen(str) + 1;          \
		clonetochunk(ch, str, ch.len, name); \
	}

#define chunk_clone(OLD, NAME)                                  \
	(chunk_t)                                               \
	{                                                       \
...
#define clonereplacechunk(ch, addr, size, name)     \
	{                                           \
		pfreeany((ch).ptr);                 \
		clonetochunk(ch, addr, size, name); \
	}
