...
extern const struct pluto_sadb_alg *
...
#define ESP_EALG_PRESENT(algo)   \
	(ESP_EALG_VALID(algo) && \
	 (esp_ealg[(algo)].kernel_sadb_alg.sadb_alg_id != 0))
...
#define ESP_AALG_PRESENT(algo)   \
	(ESP_AALG_VALID(algo) && \
	 (esp_aalg[(algo)].kernel_sadb_alg.sadb_alg_id != 0))
