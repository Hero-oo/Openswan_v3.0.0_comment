...
struct esp_info {
	...
	enum ikev2_trans_type_encr
		transid; /* ESP transform (AES, 3DES, etc.) (in IKEv2 terms )*/
...
extern char *alg_info_snprint_ike2(struct ike_info *ike_info, int eklen,
				   int aklen, int *usedsize, char *buf,
				   int buflen);
...
extern int alg_info_parse_str(struct alg_info *alg_info, const char *alg_str,
			      const char **err_p,
