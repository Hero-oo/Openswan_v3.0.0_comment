...
enum pluto_crypto_requests {
	...
	pcr_compute_dh_v2 =
		9, /* perform IKEv2 PARENT SA calculation, create SKEYSEED */
};
...
#define DBG_OPPOINFO \
	LELEM(14) /* log various informational things about oppo/%trap-keying */
...
#define IMPAIR_DELAY_ADNS_KEY_ANSWER \
	LELEM(IMPAIR0 + 0) /* sleep before answering */
#define IMPAIR_DELAY_ADNS_TXT_ANSWER \
	LELEM(IMPAIR0 + 1) /* sleep before answering */
...
#define IMPAIR_DIE_ONINFO \
	LELEM(IMPAIR0 +   \
	      5) /* cause state to be deleted upon receipt of information payload */
#define IMPAIR_JACOB_TWO_TWO \
	LELEM(IMPAIR0 + 6) /* cause pluto to send all messages twice. */
/* cause pluto to send all messages twice. */
#define IMPAIR_MAJOR_VERSION_BUMP \
	LELEM(IMPAIR0 +           \
	      7) /* cause pluto to send an IKE major version that's higher then we support. */
#define IMPAIR_MINOR_VERSION_BUMP \
	LELEM(IMPAIR0 +           \
	      8) /* cause pluto to send an IKE minor version that's higher then we support. */
#define IMPAIR_RETRANSMITS \
	LELEM(IMPAIR0 + 9) /* cause pluto to never retransmit */
#define IMPAIR_SEND_BOGUS_ISAKMP_FLAG \
	LELEM(IMPAIR0 +               \
	      10) /* causes pluto to set a RESERVED ISAKMP flag to test ignoring/zeroing it */

#define DBG_NONE 0 /* no options on, including impairments */
#define DBG_ALL          \
	LRANGES(DBG_RAW, \
		DBG_OPPOINFO) /* all logging options on EXCEPT DBG_PRIVATE and DBG_WHACKWATCH */
#endif
...
enum phase1_role { INITIATOR = 1, RESPONDER = 2 };
...
#define PHASE1_INITIATOR_STATES                                               \
	(LELEM(STATE_MAIN_I1) | LELEM(STATE_MAIN_I2) | LELEM(STATE_MAIN_I3) | \
	 LELEM(STATE_MAIN_I4) | LELEM(STATE_AGGR_I1) | LELEM(STATE_AGGR_I2) | \
	 LELEM(STATE_XAUTH_I0) | LELEM(STATE_XAUTH_I1) |                      \
	 LELEM(STATE_MODE_CFG_I1) | LELEM(STATE_PARENT_I1))
#define IS_PHASE1_INIT(s)                                  \
	((s) == STATE_MAIN_I1 || (s) == STATE_MAIN_I2 ||   \
	 (s) == STATE_MAIN_I3 || (s) == STATE_MAIN_I4 ||   \
	 (s) == STATE_AGGR_I1 || (s) == STATE_AGGR_I2 ||   \
...
#define IS_ISAKMP_ENCRYPTED(s)                           \
	(STATE_MAIN_R2 <= (s) && STATE_AGGR_R0 != (s) && \
	 STATE_AGGR_I1 != (s) && STATE_INFO != (s))
#define IS_ISAKMP_AUTHENTICATED(s)                        \
	((STATE_MAIN_R3 <= (s) && STATE_AGGR_R0 != (s) && \
	  STATE_AGGR_I1 != (s)) ||                        \
	 STATE_PARENT_I3 == (s) || STATE_PARENT_R2)
#define IS_ISAKMP_SA_ESTABLISHED(s) LHAS(ISAKMP_SA_ESTABLISHED_STATES, (s))
#define ISAKMP_SA_ESTABLISHED_STATES                                          \
	(LELEM(STATE_MAIN_R3) | LELEM(STATE_MAIN_I4) | LELEM(STATE_AGGR_I2) | \
	 LELEM(STATE_AGGR_R2) | LELEM(STATE_XAUTH_R0) |                       \
	 LELEM(STATE_XAUTH_R1) | LELEM(STATE_PARENT_I3) |                     \
	 LELEM(STATE_PARENT_R2) | LELEM(STATE_MODE_CFG_R0) |                  \
	 LELEM(STATE_MODE_CFG_R1) | LELEM(STATE_MODE_CFG_R2) |                \
	 LELEM(STATE_MODE_CFG_I1) | LELEM(STATE_XAUTH_I0) |                   \
	 LELEM(STATE_XAUTH_I1))

#define IS_IPSEC_SA_ESTABLISHED(s) \
	((s) == STATE_QUICK_I2 || (s) == STATE_QUICK_R2)
#define IS_ONLY_INBOUND_IPSEC_SA_ESTABLISHED(s) ((s) == STATE_QUICK_R1)
#ifdef MODECFG
#define IS_MODE_CFG_ESTABLISHED(s) ((s) == STATE_MODE_CFG_R2)
#endif

#define IS_PARENT_SA_HALFOPEN(s) \
	((s) == STATE_PARENT_I1 || (s) == STATE_PARENT_R1)

#define IS_PARENT_SA_ESTABLISHED(s)                          \
	((s) == STATE_PARENT_I3 || (s) == STATE_PARENT_I2 || \
	 (s) == STATE_PARENT_R2 || (s) == STATE_IKESA_DEL)
/*
...
#define IS_CHILD_SA_ESTABLISHED(st)              \
	((((st->st_state == STATE_PARENT_I3) ||  \
	 ...
	 (st->st_state == STATE_CHILD_C1_KEYED))
...
#define POLICY_IPSEC_SHIFT 2 /* log2(POLICY_ENCRYPT) */
	...
	POLICY_SHUNT_TRAP =
		(0ul << POLICY_SHUNT_SHIFT), /* default: negotiate */
   ...
   */
	...
	POLICY_PERHOST =
		LELEM(20), /* should we specialize the policy to the host? */
	POLICY_SUBHOST = LELEM(
		21), /* if the policy applies below the host level (TCP/UDP/SCTP ports), */
	POLICY_PERPROTO = LELEM(
		22), /* should we specialize the policy to the protocol? */
	POLICY_OVERLAPIP = LELEM(
		23), /* can two conns that have subnet=vhost: declare the same IP? */
   ...
   */
	...
	POLICY_IKEV2_ALLOW_NARROWING =
		LELEM(27), /* Allow RFC-5669 section 2.9? 0x0800 0000 */

	POLICY_MODECFGDNS1 = LELEM(28), /* should we offer a DNS server IP */
	POLICY_MODECFGDNS2 =
		LELEM(29), /* should we offer another DNS server IP */
	POLICY_MODECFGWINS1 = LELEM(30), /* should we offer a WINS server IP */
	POLICY_MODECFGWINS2 =
		LELEM(31), /* should we offer another WINS server IP */

	POLICY_SAREF_TRACK = LELEM(32), /* Saref tracking via _updown */
	POLICY_SAREF_TRACK_CONNTRACK =
		LELEM(33), /* use conntrack optimization */

	/* additional misc information */

	POLICY_INVALID_CONFIG = LELEM(
		34), /* detected invalid/incompatible 'conn' ipsec.conf, should not negotiate */
...
extern const char *end_type_name(enum keyword_host host_type,
				 ip_address *host_addr, char *outbuf,
				 size_t outbuf_len);
