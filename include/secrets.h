 ...
 */
typedef int (*secret_eval)(struct secret *secret, struct private_key_stuff *pks,
			   void *uservoid);
...
struct pubkey {
	...
	char *dns_sig;
	time_t installed_time, last_tried_time, last_worked_time, until_time;
	chunk_t issuer;

	unsigned char key_ckaid
		[CKAID_BUFSIZE]; /* typically, 20 bytes, presented in hex */
	char key_ckaid_print_buf[CKAID_BUFSIZE * 2 + (CKAID_BUFSIZE / 2) +
				 2]; /* a buffer for above, produced by datatot */
...
extern void delete_public_keys(struct pubkey_list **head, const struct id *id,
			       enum pubkey_alg alg);
...
extern void form_keyid_from_nss(SECItem e, SECItem n, char *keyid,
				unsigned *keysize);
extern err_t
extract_and_add_secret_from_nss_cert_file(struct private_key_stuff *pks,
...
extern void osw_load_preshared_secrets(struct secret **psecrets, int verbose,
...
osw_find_secret_by_id(struct secret *secrets, enum PrivateKeyKind kind,
		      const struct id *my_id, osw_public_key *key1,
		      osw_public_key *key2, const struct id *his_id, bool asym);
