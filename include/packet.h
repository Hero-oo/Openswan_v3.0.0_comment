...
struct packet_byte_stream {
	...
	u_int8_t *start, *cur, /* current position in stream */
...
extern void init_pbs(pb_stream *pbs, u_int8_t *start, size_t len,
		     const char *name);
extern void init_sub_pbs(pb_stream *parent_pbs, pb_stream *child_pbs,
			 const char *name);

extern bool in_struct(void *struct_ptr, struct_desc *sd, pb_stream *ins,
		      pb_stream *obj_pbs);
extern bool in_raw(void *bytes, size_t len, pb_stream *ins, const char *name);

extern bool out_struct(const void *struct_ptr, struct_desc *sd, pb_stream *outs,
		       pb_stream *obj_pbs);
extern void pbs_set_np(pb_stream *outs, u_int8_t np);
extern void pbs_copy_np(pb_stream *from, pb_stream *to);

extern bool out_generic(u_int8_t np, struct_desc *sd, pb_stream *outs,
			pb_stream *obj_pbs);
extern bool out_generic_raw(u_int8_t np, struct_desc *sd, pb_stream *outs,
...
#define NOFFSETOF_isag_length \
	2 /* on-the-wire offset of isag_length (two octets, network order */
...
extern struct_desc isakmp_oakley_attribute_desc, isakmp_ipsec_attribute_desc,
...
extern struct_desc isakmp_isakmp_transform_desc, isakmp_ah_transform_desc,
	isakmp_esp_transform_desc, isakmp_ipcomp_transform_desc;
