...
void verify_signature(const char *keyname, unsigned int keysize_bits)
{
...

	{
		...
		char ckaid_print_buf[CKAID_BUFSIZE * 2 + (CKAID_BUFSIZE / 2) +
				     2];

		if (keyname) {
			snprintf(pubkey_file_name, sizeof(pubkey_file_name),
				 "pubkey-%s.pubkey", keyname);
		} else {
			snprintf(pubkey_file_name, sizeof(pubkey_file_name),
				 "pubkey-%04d.pubkey", keysize_bits);
		}
		...
		pubkey_space_len = 0;
		if (getline(&pubkey_space, &pubkey_space_len, pubkey_file) <
		    0) {
			perror("getline");
			exit(13);
		}
		...
		e = ttodatav(pubkey_space, 0, 0, pubkey_bin_space,
			     sizeof(pubkey_bin_space), &pubkey_bin_space_len,
		...
		datatot(pk.key_ckaid, sizeof(pk.key_ckaid), 'G',
			ckaid_print_buf, sizeof(ckaid_print_buf));
		printf("ckaid: %s\n", ckaid_print_buf);
	}

	if (keyname) {
		snprintf(sig_buf_name, sizeof(sig_buf_name), "sig-%s.bin",
			 keyname);
	} else {
		snprintf(sig_buf_name, sizeof(sig_buf_name), "sig-%04d.bin",
			 keysize_bits);
	}
	...
	/* now verify the signature using the public key part of this secret */
	{
		...
		e = verify_signed_hash(&pk.u.rsa, s, sizeof(s), &sig,
				       signed_len, sig_val, sig_len);
		if (e)
			puts(e);
		assert(e == NULL);
