...
int count_secrets(struct secret *secret, struct private_key_stuff *pks,
		  void *uservoid)
{
...
void verify_sig_key(const char *keyfile, unsigned int keysize)
{
	...
	sign_hash(pks1, thingtosign, signed_len, signature_buf, keysize);

	hexdump(stdout, signature_buf, 0, sizeof(signature_buf));
	{
		char outname[512];
		FILE *outfile;
		snprintf(outname, sizeof(outname), "OUTPUT/sig-%s.bin",
			 keyfile);
...

	{
		...
		e = verify_signed_hash(k, s, sizeof(s), &sig, signed_len,
				       sig_val, sig_len);
		if (e)
			puts(e);
		assert(e == NULL);
		...
		DBG(DBG_CRYPT, DBG_dump("v2rsa decrypted SIG:", sig, hash_len);
		    DBG_dump("v2rsa computed hash:",
			     thingtosign + der_digestinfo_len, hash_len););

		assert(memcmp(sig, thingtosign + der_digestinfo_len,
			      hash_len) == 0);
	}
}
