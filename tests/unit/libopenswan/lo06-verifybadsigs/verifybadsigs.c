...
static void hack_remove_pad_add_trailing(u_char *sig_val, size_t sig_len)
{
	...
	if (padlen > 8) {
		memmove(s + 1, p - 1,
			rest + 1); // keep 2 bytes of pad, shift rest down
...
struct hack {
	...
	int expected_error;
} hacks[] = { { "zero-first-pad-byte", hack_zero_first_pad_byte, 4 },
			 ...
			 unsigned int keysize)
{
	...
	snprintf(secretsfile, sizeof(secretsfile),
		 "../lo02-verifysigs/key-%s.secrets", keyfile);
	...
	sign_hash_hack(hack, pks1, thingtosign, signed_len, signature_buf,
		       keysize);
...

	{
		...
		err = verify_signed_hash(k, s, sizeof(s), &sig, signed_len,
					 sig_val, sig_len);
		assert(err != NULL);
		...
		printf("<<< %s(\"%s\", \"%s\", %u) = (%ld) \"%s\"\n", __func__,
		       hack->name, keyfile, keysize, num, err);
	}
}
