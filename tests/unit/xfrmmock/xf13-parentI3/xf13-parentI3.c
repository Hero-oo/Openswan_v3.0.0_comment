		      ...
		      const u_char *bytes)
{
	...
	if (st != NULL) {
		...
		st->st_connection->extra_debugging =
			DBG_CONTROL | DBG_CONTROLMORE | DBG_NETKEY |
			DBG_PRIVATE | DBG_CRYPT;
	}
	cur_debugging = cur_debugging | DBG_CONTROL | DBG_CONTROLMORE |
			DBG_NETKEY | DBG_PRIVATE | DBG_CRYPT;
...
*/
void recv_pcap_I3_process(u_char *user, const struct pcap_pkthdr *h,
			  const u_char *bytes)
{
	struct state *st = NULL;
	struct pcr_kenonce *kn = &crypto_req->pcr_d.kn;
