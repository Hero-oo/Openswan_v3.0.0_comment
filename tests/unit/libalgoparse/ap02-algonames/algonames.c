...
struct artab {
	...
	{ IKEv2_TRANS_TYPE_INTEG, "hmac_sha2_256_128",
	  IKEv2_AUTH_HMAC_SHA2_256_128 },
	{ IKEv2_TRANS_TYPE_INTEG, "hmac_sha2_512_256",
...
static void regress(void)
{
	...
	for (r = atodatatab; r->ascii != NULL; r++) {
		...
		case IKEv2_TRANS_TYPE_ENCR:
			item = ealg_getbyname(r->ascii, strlen(r->ascii),
					      &auxinfo);
			break;
		case IKEv2_TRANS_TYPE_DH:
			item = modp_getbyname(r->ascii, strlen(r->ascii),
					      &auxinfo);
			break;
		default:
			item = enum_search_nocase(lookup, r->ascii,
						  strlen(r->ascii));
		}
