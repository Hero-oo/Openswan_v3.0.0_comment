...
int main(int argc, char *argv[])
{
	...
	alg_info_esp = alg_info_esp_create_from_str(argv[1], &err);
...
struct artab {
...
} atodatatab[] = {
	{
		...
		0,
		"3des(3)_000-hmac_md5_96(1)_000; flags=-strict",
		FALSE,
	},
	{
	...
	},
	{
		...
		"aes_cbc(12)_128-hmac_md5_96(1)_000-MODP1024(2); flags=-strict",
		FALSE,
	},
	{
		"aes-sha1",
		...
		0,
		"aes_cbc(12)_128-hmac_sha1_96(2)_000; flags=-strict",
		FALSE,
	},
	{ "aes-sha1-modp2048", IKEv2_ENCR_AES_CBC, 128, IKEv2_AUTH_HMAC_SHA1_96,
	  OAKLEY_GROUP_MODP2048,
	  "aes_cbc(12)_128-hmac_sha1_96(2)_000-MODP2048(14); flags=-strict",
	  FALSE },
	...
	/* a modern definition from draft-ietf-ipsecme-rfc7321bis/ */
	{
		...
		0,
		"aes_cbc(12)_256-hmac_sha2_256_128(12)_000; flags=-strict",
		FALSE,
	},
	{
		NULL,
	...
	regress(pgm) char *pgm;
{
	...
	for (r = atodatatab; r->ascii != NULL; r++) {
		err = NULL;
		alg_info_esp = alg_info_esp_create_from_str(r->ascii, &err);
		if (r->err && err != NULL) {
		...
		if (alg_info_esp->esp[0].enckeylen != r->eklen) {
			fprintf(stderr,
				...
				"  got:              %d\n",
				r->ascii, (int)r->eklen,
				(int)alg_info_esp->esp[0].enckeylen);
		}
