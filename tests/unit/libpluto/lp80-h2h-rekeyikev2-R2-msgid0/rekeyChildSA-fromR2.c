...
void recv_pcap_packet2_and_rekey(u_char *user, const struct pcap_pkthdr *h,
				 const u_char *bytes)
{
	...
	DBG_log("%s() call %d: st_msgid=%d st_msgid_lastack=%d st_msgid_nextuse=%d st_msgid_lastrecv=%d\n",
		__func__, call_counter, st->st_msgid, st->st_msgid_lastack,
		st->st_msgid_nextuse, st->st_msgid_lastrecv);
