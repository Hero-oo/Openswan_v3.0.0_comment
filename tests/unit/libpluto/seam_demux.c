...
bool send_packet_srcnat(struct state *st, const char *where, bool verbose,
			...
			ip_address outsideoffirewall)
{
	...
	fprintf(stderr,
		...
		st->st_interface->ip_dev->id_rname, st->st_interface->port, b1,
		ntohs(outsideoffirewall.u.v4.sin_port), b2, st->st_remoteport,
		st->st_serialno);
	...
	if (packet_save) {
		...
		*dlt = PF_INET;
		...
		ulp = &buf[sizeof(struct iphdr) + sizeof(struct udphdr) +
			   4]; /* +4 because of pcap_pkthdr */
		if ((st->st_interface->ike_float == TRUE) &&
		    (st->st_tpacket.len != 1)) {
...
void complete_state_transition(struct msg_digest **mdp, stf_status result)
{
