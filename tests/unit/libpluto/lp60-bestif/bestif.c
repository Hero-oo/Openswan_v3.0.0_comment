...
int run_permutted_test(unsigned int numbers[6])
{
	...
	best1 = pick_matching_interfacebyfamily(interfaces, 500, AF_INET, &sr1);

	printf("%d%d%d%d%d%d ", numbers[0], numbers[1], numbers[2], numbers[3],
	       numbers[4], numbers[5]);
	if (best1) {
	...
	for (numbers[0] = 0; numbers[0] < IFACES_COUNT; numbers[0]++) {
		for (numbers[1] = 0; numbers[1] < IFACES_COUNT; numbers[1]++) {
			if (numbers[1] == numbers[0])
				continue;
			for (numbers[2] = 0; numbers[2] < IFACES_COUNT;
			     numbers[2]++) {
				if (numbers[2] == numbers[0])
					continue;
				if (numbers[2] == numbers[1])
					continue;
				for (numbers[3] = 0; numbers[3] < IFACES_COUNT;
				     numbers[3]++) {
