...
void do_hash_init_abort(void *ctx)
{
	abort();
}
void do_hash_update_abort(void *ctx, const u_char *b, size_t s)
{
	abort();
}
void do_hash_final_abort(u_int8_t *out, void *ctx)
{
	abort();
}
...
struct ike_encr_desc e1 = { .common = { .name = "encrypt",
					.officname = "funfun" },
			    .keydeflen = 128,
			    .keyminlen = 128,
			    .keymaxlen = 256,
