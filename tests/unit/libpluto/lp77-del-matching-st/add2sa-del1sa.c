...
static void find_cookie_collision(u_int8_t *out_icookie, u_int8_t *out_rcookie,
				  const u_int8_t *in_icookie,
				  const u_int8_t *in_rcookie)
{
	long loops = 0;
	...
	do {
		...
		out_bucket =
			compute_icookie_rcookie_hash(out_icookie, out_rcookie);
...
int main(int argc, char *argv[])
{
	...
	u_int8_t st3_icookie[COOKIE_SIZE] = { 0x81, 0x03, 0x3d, 0x77,
					      0xf9, 0xe9, 0x4d, 0x44 };
	u_int8_t st3_rcookie[COOKIE_SIZE] = { 0xa9, 0xe0, 0xb6, 0x71,
					      0x7b, 0xa8, 0x07, 0x09 };
	...
	if (!regression) {
		find_cookie_collision(st3_icookie, st3_rcookie, st1->st_icookie,
				      st1->st_rcookie);
	}
