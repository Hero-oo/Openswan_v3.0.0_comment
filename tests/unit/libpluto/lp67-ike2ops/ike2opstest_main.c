...
int main(int argc, char *argv[])
{
	...
	setbuf(stdout,
	       NULL); /* make stdout unbuffered so stdout/stderr interleave */
...
#if 1
	{
		...
		db_attr_add_values(ctx, OAKLEY_ENCRYPTION_ALGORITHM,
				   OAKLEY_AES_CBC);
		db_attr_add_values(ctx, OAKLEY_HASH_ALGORITHM, OAKLEY_SHA1);
	...
	sa_v2_print(sadb);
	if (!extrapolate_v1_from_v2(sadb, POLICY_PSK | POLICY_RSASIG,
				    INITIATOR)) {
		DBG_log("failed to create v1");
		exit(11);
	}
