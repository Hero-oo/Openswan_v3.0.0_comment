...
void recv_pcap_packet_gen(u_char *user, const struct pcap_pkthdr *h,
			  const u_char *bytes)
{
	...
	case DLT_NULL:
		...
		if (*dlt != PF_INET) {
			fprintf(stderr,
				"DLT_NULL - can not process packet in DLT=%08x\n",
				*dlt);
			return;
		}
		dlt++;
	...
	fprintf(stderr, "picking: %s:%u %s outside: %u <=> d: %u\n",
		ifp->ip_dev->id_rname, ifp->port, ifp->ike_float ? "float" : "",
		(ifp->ike_float ? outside_port4500 : outside_port500),
		ntohs(udp->dest));
#endif
	md->iface = ifp;
	...
	if (natt_skip_nonesp(ifp, cur_from, cur_from_port, &ike, &packet_len) !=
	    TRUE) {
		exit(11);
	}
	...
	DBG_log("*received %d bytes from %s:%u on %s (port=%d)",
		(int)pbs_room(&md->packet_pbs), ip_str(&md->sender),
		(unsigned)md->sender_port, ifp->ip_dev->id_rname, ifp->port);
