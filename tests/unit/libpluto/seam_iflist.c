...
void add_if_to_list(struct ifmake *ifm)
{
	...
	ifport->socktypename =
		(ifm->family == AF_INET6 ? "AF_INET" : "AF_INET6");

	ttoaddr_num(ifm->ifaddr, 0, ifm->family, &ifport->ip_addr);
	addrtot(&ifport->ip_addr, 0, ifport->addrname,
		sizeof(ifport->addrname));
	ifport->fd = -1;
...
struct ifmake if_02 = { AF_INET6, "2606:2800:220:1:248:1893:25c8:1946", 500,
			FALSE, "eth0" };
