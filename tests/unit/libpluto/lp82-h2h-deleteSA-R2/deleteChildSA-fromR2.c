...
void recv_pcap_packet2_and_delete(u_char *user, const struct pcap_pkthdr *h,
				  const u_char *bytes)
{
	...
	DBG_log("%s() call %d: (parent) st_msgid=%d st_msgid_lastack=%d st_msgid_nextuse=%d st_msgid_lastrecv=%d\n",
		__func__, call_counter, pst->st_msgid, pst->st_msgid_lastack,
		pst->st_msgid_nextuse, pst->st_msgid_lastrecv);
