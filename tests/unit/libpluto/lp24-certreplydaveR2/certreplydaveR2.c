...
void recv_pcap_packet3(u_char *user, const struct pcap_pkthdr *h,
		       const u_char *bytes)
{
	struct state *st;
	...
	if (st) {
		...
		clonetowirechunk(&kn->thespace, kn->space, &kn->n, SS(nr.ptr),
				 SS(nr.len));
		clonetowirechunk(&kn->thespace, kn->space, &kn->gi, SS(gr.ptr),
				 SS(gr.len));
...
void recv_pcap_packet4(u_char *user, const struct pcap_pkthdr *h,
		       const u_char *bytes)
{
	struct state *st;
	//struct pcr_kenonce *kn = &crypto_req->pcr_d.kn;
...
recv_pcap recv_inputs[] = { recv_pcap_packet_with_ke, recv_pcap_packet2_with_ke,
			    recv_pcap_packet3, recv_pcap_packet4 };
