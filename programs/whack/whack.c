...
#define OO OPTION_OFFSET
	...
	{ "deletestate", required_argument, NULL,
	  OPT_DELETESTATE + OO + NUMERIC_ARG },
	...
	{ "clientprotoport", required_argument, NULL,
	  END_CLIENTPROTOPORT + OO },
	...
	{ "disablearrivalcheck", no_argument, NULL,
	  CD_DISABLEARRIVALCHECK + OO },
	{ "initiateontraffic", no_argument, NULL,
	  CD_SHUNT0 + (POLICY_SHUNT_TRAP >> POLICY_SHUNT_SHIFT << AUX_SHIFT) +
		  OO },
	{ "pass", no_argument, NULL,
	  CD_SHUNT0 + (POLICY_SHUNT_PASS >> POLICY_SHUNT_SHIFT << AUX_SHIFT) +
		  OO },
	{ "drop", no_argument, NULL,
	  CD_SHUNT0 + (POLICY_SHUNT_DROP >> POLICY_SHUNT_SHIFT << AUX_SHIFT) +
		  OO },
	{ "reject", no_argument, NULL,
	  CD_SHUNT0 + (POLICY_SHUNT_REJECT >> POLICY_SHUNT_SHIFT << AUX_SHIFT) +
		  OO },
	{ "failnone", no_argument, NULL,
	  CD_FAIL0 + (POLICY_FAIL_NONE >> POLICY_FAIL_SHIFT << AUX_SHIFT) +
		  OO },
	{ "failpass", no_argument, NULL,
	  CD_FAIL0 + (POLICY_FAIL_PASS >> POLICY_FAIL_SHIFT << AUX_SHIFT) +
		  OO },
	{ "faildrop", no_argument, NULL,
	  CD_FAIL0 + (POLICY_FAIL_DROP >> POLICY_FAIL_SHIFT << AUX_SHIFT) +
		  OO },
	{ "failreject", no_argument, NULL,
	  CD_FAIL0 + (POLICY_FAIL_REJECT >> POLICY_FAIL_SHIFT << AUX_SHIFT) +
		  OO },
	...
	{ "dpdtimeout", required_argument, NULL,
	  CD_DPDTIMEOUT + OO + NUMERIC_ARG },
	{ "dpdaction", required_argument, NULL, CD_DPDACTION + OO },
	{ "firstmsgid", required_argument, NULL,
	  CD_FIRSTMSGID + OO + NUMERIC_ARG },
#ifdef XAUTH
	...
	{ "certtype", required_argument, NULL,
	  END_CERTTYPE + OO + NUMERIC_ARG },
	{ "ipv4", no_argument, NULL, CD_CONNIPV4 + OO },
	{ "ipv6", no_argument, NULL, CD_CONNIPV6 + OO },

	{ "ikelifetime", required_argument, NULL,
	  CD_IKELIFETIME + OO + NUMERIC_ARG },
	{ "ipseclifetime", required_argument, NULL,
	  CD_IPSECLIFETIME + OO + NUMERIC_ARG },
	{ "rekeymargin", required_argument, NULL,
	  CD_RKMARGIN + OO + NUMERIC_ARG },
	{ "rekeywindow", required_argument, NULL,
	  CD_RKMARGIN + OO + NUMERIC_ARG }, /* OBSOLETE */
	{ "rekeyfuzz", required_argument, NULL, CD_RKFUZZ + OO + NUMERIC_ARG },
	{ "keyingtries", required_argument, NULL,
	  CD_KTRIES + OO + NUMERIC_ARG },
			    ...
			    const struct whack_message *msg)
{
	...
	if ((msg->policy & POLICY_DONT_REKEY) == LEMPTY && life <= mint) {
		...
		snprintf(
			...
			which, (unsigned long)life,
			(unsigned long)msg->sa_rekey_margin,
			(unsigned long)msg->sa_rekey_fuzz, (unsigned long)mint);
		diag(buf);
	}
}
...
int main(int argc, char **argv)
{
	...
	lset_t opts_seen = LEMPTY, opts2_seen = LEMPTY, lst_seen = LEMPTY,
	       cd_seen = LEMPTY, end_seen = LEMPTY, end_seen_before_to = LEMPTY;
	const char *af_used_by = NULL, *tunnel_af_used_by = NULL;
	 ...
	 */
		volatile int c =
			getopt_long(argc, argv, "", long_opts, &long_index) -
			OPTION_OFFSET;
		int aux = 0;
	 ...
	 */
		...
		case ':' -
			OPTION_OFFSET: /* diagnostic already printed by getopt_long */
		case '?' -
			OPTION_OFFSET: /* diagnostic already printed by getopt_long */
		...
		case OPT_CTLBASE: /* --port <ctlbase> */
			if (snprintf(ctl_addr.sun_path,
				     sizeof(ctl_addr.sun_path), "%s%s", optarg,
				     CTL_SUFFIX) == -1)
				diag("<ctlbase>" CTL_SUFFIX
		...
		case OPT_PUBKEYRSA: /* --pubkeyrsa <key> */
		{
			...
			ugh = ttodatav(optarg, 0, 0, keyspace, sizeof(keyspace),
	 ...
	 */
	     ...
	     */
			if (LDISJOINT(cd_seen, LELEM(CD_TUNNELIPV4 - CD_FIRST) |
						       LELEM(CD_TUNNELIPV6 -
							     CD_FIRST)) &&
	     ...
	     * both actions */
			...
			gotxauthname = TRUE;
			xauthname[0] = '\0';
			strncat(xauthname, optarg,
				sizeof(xauthname) - strlen(xauthname) - 1);
		...
		case OPT_XAUTHPASS:
			gotxauthpass = TRUE;
			xauthpass[0] = '\0';
			strncat(xauthpass, optarg,
				sizeof(xauthpass) - strlen(xauthpass) - 1);
		...
		case DBGOPT_ALL: /* --debug-all */
			msg.debugging |=
				DBG_ALL; /* note: does not include PRIVATE */
			continue;
	...
	case LELEM(OPT_OPPO_HERE) | LELEM(OPT_OPPO_THERE):
		...
		if (LIN(cd_seen, LELEM(CD_TUNNELIPV4 - CD_FIRST) |
	...
	if (!LDISJOINT(opts_seen, LELEM(OPT_ROUTE) | LELEM(OPT_UNROUTE) |
					  LELEM(OPT_INITIATE) |
					  LELEM(OPT_TERMINATE) |
	...
	if (!(msg.whack_connection || msg.whack_key || msg.whack_myid ||
	      msg.whack_delete || msg.whack_deletestate || msg.whack_initiate ||
	      ...
	      msg.whack_reread || msg.whack_crash || msg.whack_status ||
	...
	check_life_time(msg.sa_ike_life_seconds,
			OAKLEY_ISAKMP_SA_LIFETIME_MAXIMUM, "ikelifetime", &msg);
			...
			for (;;) {
				...
				*be = '\0';
				...
				for (;;) {
					...
					if (write(STDOUT_FILENO, ls, le - ls) !=
					    (le - ls)) {
						int e = errno;
						fprintf(stderr,
