...
static int send_whack_msg_to_file(struct starter_config *cfg,
		...
		if (whackall) {
			for (conn = cfg->conns.tqh_first; conn != NULL;
				...
				if (conn->desired_state == STARTUP_ADD ||
					...
					if (starter_whack_add_conn(cfg, conn) !=
					    0) {
						fprintf(stderr,
							"failed to load conn: %s\n",
							conn->name);
					}
				}
			}
