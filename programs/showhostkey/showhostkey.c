...
void showhostkey_log(int mess_no, const char *message, ...)
{
	va_list args;
...
int print_key(struct secret *secret, struct private_key_stuff *pks,
	      void *uservoid, bool disclose)
{
	...
	if (pks->kind == PPK_PSK || pks->kind == PPK_XAUTH) {
		datatot(pks->u.preshared_secret.ptr,
			pks->u.preshared_secret.len, 'x', pskbuf,
			sizeof(pskbuf));
	}
...
int list_key(struct secret *secret, struct private_key_stuff *pks,
	     void *uservoid)
{
	return print_key(secret, pks, uservoid, FALSE);
}

int dump_key(struct secret *secret, struct private_key_stuff *pks,
	     void *uservoid)
{
	return print_key(secret, pks, uservoid, TRUE);
}

int pickbyid(struct secret *secret, struct private_key_stuff *pks,
	     void *uservoid)
{
...
struct secret *pick_key(struct secret *host_secrets, char *idname)
{
	struct id id;
	struct secret *s;
	err_t e;
	...
	s = osw_find_secret_by_id(host_secrets, PPK_RSA, &id, NULL, NULL, NULL,
				  TRUE /* asymmetric */);
...
void show_dnskey(struct secret *s, char *idname, int precedence, char *gateway)
{
	char qname[256];
	char base64[8192];
	int gateway_type = 0;
	...
	if (gateway != NULL) {
		...
		if (ttoaddr(gateway, strlen(gateway), AF_INET, &test) == NULL)
			gateway_type = 1;
		else if (ttoaddr(gateway, strlen(gateway), AF_INET6, &test) ==
			 NULL)
			gateway_type = 2;
		else {
	...
	printf("%s.    IN    IPSECKEY  %d %d 2 %s ", qname, precedence,
	       gateway_type, (gateway == NULL) ? "." : gateway);
	{
...
void show_confkey(struct secret *s, char *idname, char *side)
{
	char base64[8192];
	const struct private_key_stuff *pks = osw_get_pks(s);
	unsigned char *keyblob;
...
usage:
	...
	if ((left_flg + right_flg + dump_flg + list_flg + ipseckey_flg +
	     dhclient_flg) > 1) {
	...
	if (ipseckey_flg) {
		show_dnskey(s, keyid, precedence, gateway);
	}
