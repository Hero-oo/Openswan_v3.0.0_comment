...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt_long(argc, argv, "", longopts, 0)) != EOF) {
		...
		case 'c':
			if (snprintf(ctlbuf, sizeof(ctlbuf), "%s%s", optarg,
				     CTL_SUFFIX) == -1) {
				fprintf(stderr, "<ctlbase>" CTL_SUFFIX
						" must be fit in a 1024 bytes");
				exit(4);
			}
	...
	if (optind == argc && !all && !listroute && !liststart && !search &&
	    !typeexport && !checkconfig) {
		usage();
	}
	...
	if (success != SECSuccess) {
		fprintf(stderr,
			"failed to initialize NSS, unable to proceed\n");
		exit(2);
	}
#endif
	...
	if (!configfile) {
		configfile =
			alloc_bytes(strlen(confdir) + sizeof("/ipsec.conf") + 2,
				    "conf file");
	...
	if (defaultroute) {
		err_t e;
		char b[ADDRTOT_BUF];
		e = ttoaddr(defaultroute, strlen(defaultroute), AF_INET,
			    &cfg->dr);
		if (e) {
	...
	if (defaultnexthop) {
		err_t e;
		char b[ADDRTOT_BUF];
		e = ttoaddr(defaultnexthop, strlen(defaultnexthop), AF_INET,
			    &cfg->dnh);
		if (e) {
	...
	if (all) {
		...
		for (kd = ipsec_conf_keywords_v2; kd->keyname != NULL; kd++) {
			...
			case kt_bool:
				printf("export %s%s='%s'\n", varprefix,
				       kd->keyname,
				       cfg->setup.options[kd->field] ? "yes" :
								       "no");
				break;

			case kt_list:
				printf("export %s%s='", varprefix, kd->keyname);
