...
#ifdef USE_1DES
static void do_des(u_int8_t *buf, size_t buf_len, u_int8_t *key,
		   ...
		   size_t key_size, u_int8_t *iv, bool enc)
{
	...
	oswcrypto.des_ncbc_encrypt((des_cblock *)buf, (des_cblock *)buf,
				   buf_len, ks, (des_cblock *)iv, enc);
}
#endif
		    ...
		    size_t key_size, u_int8_t *iv, bool enc)
{
	...
	oswcrypto.des_ede3_cbc_encrypt((des_cblock *)buf, (des_cblock *)buf,
				       buf_len, ks[0], ks[1], ks[2],
				       (des_cblock *)iv, enc);
}
#endif
			...
			size_t size, struct state *st)
{
	...
	e->do_crypt(buf, size, st->st_enc_key.ptr, st->st_enc_key.len,
		    st->st_new_iv, enc);
