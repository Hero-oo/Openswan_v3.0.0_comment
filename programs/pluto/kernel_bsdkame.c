			       ...
			       struct state *st)
{
	...
	if (-1 == snprintf(cmd, sizeof(cmd),
			   ...
			   "%s" /* actual script */
			   ,
			   verb, verb_suffix, common_shell_out_str,
			   sr->this.updown == NULL ? DEFAULT_UPDOWN :
						     sr->this.updown)) {
		loglog(RC_LOG_SERIOUS, "%s%s command too long!", verb,
		       verb_suffix);
		return FALSE;
	...
	case SADB_SATYPE_AH:
		...
		DBG(DBG_KLIPS,
		    DBG_log("algregister_ah(%p) exttype=%d alg_id=%d, "
			    ...
			    "ret=%d",
			    alg, supp_exttype, alg->sadb_alg_id,
			    alg->sadb_alg_ivlen, alg->sadb_alg_minbits,
			    alg->sadb_alg_maxbits, ret));
		break;

	case SADB_SATYPE_ESP:
		ret = kernel_alg_add(satype, supp_exttype, alg);
		DBG(DBG_KLIPS,
		    DBG_log("algregister(%p) alg_id=%d, "
			    "alg_ivlen=%d, alg_minbits=%d, alg_maxbits=%d, "
			    "ret=%d",
			    alg, alg->sadb_alg_id, alg->sadb_alg_ivlen,
			    alg->sadb_alg_minbits, alg->sadb_alg_maxbits, ret));
		break;

	case SADB_X_SATYPE_IPCOMP:
		DBG(DBG_KLIPS,
		    DBG_log("ipcomp algregister(%p) alg_id=%d, "
			    "alg_ivlen=%d, alg_minbits=%d, alg_maxbits=%d, "
			    "ret=%d",
			    alg, alg->sadb_alg_id, alg->sadb_alg_ivlen,
			    alg->sadb_alg_minbits, alg->sadb_alg_maxbits, ret));
 ...
 */
static bool
bsdkame_raw_eroute(const ip_address *this_host, const ip_subnet *this_client,
		   ...
		   const char *text_said UNUSED, char *policy_label UNUSED)
{
	...
	ret = pfkey_send_spdadd(pfkeyfd, saddr, this_client->maskbits, daddr,
				that_client->maskbits,
				transport_proto ? transport_proto :
						  255 /* proto */,
				(caddr_t)policy_struct, policylen, pfkey_seq);
	...
	if (ret < 0) {
		...
		DBG_log("ret = %d from send_spdadd: %d (%s) addr=%p/%p seq=%u opname=eroute\n",
			ret, __ipsec_errcode, ipsec_strerror(), saddr, daddr,
			pfkey_seq);
		return FALSE;
	}
	return TRUE;
				 ...
				 const char *opname)
{
	...
	case ERO_ADD: {
		...
		policy_struct->sadb_x_policy_type = policy;
		policy_struct->sadb_x_policy_dir = IPSEC_DIR_OUTBOUND;
		policy_struct->sadb_x_policy_id =
			0; /* needs to be set, and recorded */
		...
		pfkey_seq++;
		ret = pfkey_send_spdadd(pfkeyfd, saddr, mine->maskbits, daddr,
					his->maskbits, 255 /* proto */,
		...
		if (ret < 0) {
			...
			DBG_log("ret = %d from send_spdadd: %d (%s) addr=%p/%p seq=%u opname=%s\n",
				ret, __ipsec_errcode, ipsec_strerror(), saddr,
				daddr, pfkey_seq, opname);
			return FALSE;
		}
		return TRUE;
	}
	...
	case ERO_DELETE: {
		...
		pfkey_seq++;
		ret = pfkey_send_spddelete(pfkeyfd, saddr, mine->maskbits,
					   daddr, his->maskbits,
					   255 /* proto */,
					   (caddr_t)policy_struct, policylen,
					   pfkey_seq);
 ...
 *
 */
static bool bsdkame_sag_eroute(struct state *st, const struct spd_route *sr,
			       unsigned op UNUSED, const char *opname UNUSED)
{
	int proto;
	...
	return bsdkame_raw_eroute(
		&sr->this.host_addr, &sr->this.client, &sr->that.host_addr,
		&sr->that.client, SPI_TRAP /* Spi# not used in KAME/BSD */
		,
		proto, sr->this.protocol, 0 /* esatype unused */
		,
...
static bool bsdkame_add_sa(const struct kernel_sa *sa, bool replace)
{
	...
	if ((sa->enckeylen + sa->authkeylen) > sizeof(keymat)) {
		openswan_log(
			...
			(sa->enckeylen + sa->authkeylen), sizeof(keymat));
		return FALSE;
	}
	...
	DBG(DBG_KLIPS,
	    DBG_log("calling pfkey_send_x1 for pfkeyseq=%d encalg=%d/%d authalg=%d/%d spi=%08x, reqid=%u, satype=%d\n",
		    pfkey_seq, sa->encalg, sa->enckeylen, sa->authalg,
		    sa->authkeylen, sa->spi, sa->reqid, satype));

	ret = pfkey_send_x1(pfkeyfd, (replace ? SADB_UPDATE : SADB_ADD), satype,
			    mode, saddr, daddr, sa->spi, sa->reqid, /* reqid */
			    64, /* wsize, replay window size */
			    keymat, sa->encalg, sa->enckeylen, sa->authalg,
			    sa->authkeylen, 0, /*flags */
			    0, /* l_alloc */
			    0, /* l_bytes */
...
static bool bsdkame_except_socket(int socketfd, int family)
{
	...
	if (setsockopt(socketfd, level, optname, &policy, sizeof(policy)) ==
	    -1) {
