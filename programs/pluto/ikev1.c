...
#define SMF_RPKE_AUTH \
	(LELEM(OAKLEY_RSA_ENC_REV) | LELEM(OAKLEY_ELGAMAL_ENC_REV))
/* misc flags */
...
#define PHONY_STATE(X)                                        \
	{                                                     \
		X, X, 0, 0, P(VID) | P(CR), PT(NONE), 0, NULL \
	}
...
#define P(n) LELEM(PT(n))
     ...
     * HDR, SA --> HDR, SA
     */
	{ STATE_MAIN_R0, STATE_MAIN_R1, SMF_ALL_AUTH | SMF_REPLY, P(SA),
	  P(VID) | P(CR), PT(NONE), EVENT_RETRANSMIT, main_inI1_outR1 },
	...
	{ STATE_MAIN_I1, STATE_MAIN_I2,
	  SMF_ALL_AUTH | SMF_INITIATOR | SMF_REPLY, P(SA), P(VID) | P(CR),
	  PT(NONE) /* don't know yet */
	  ,
	...
	{ STATE_MAIN_I2, STATE_UNDEFINED,
	  ...
	  P(KE) | P(ID) | P(NONCE), P(VID) | P(CR), PT(HASH), EVENT_RETRANSMIT,
	  unexpected /* ??? not yet implemented */ },

	{ STATE_MAIN_I2, STATE_UNDEFINED,
	  SMF_ALL_AUTH | SMF_INITIATOR | SMF_OUTPUT_ENCRYPTED | SMF_REPLY,
	  P(NONCE) | P(KE) | P(ID), P(VID) | P(CR), PT(HASH), EVENT_RETRANSMIT,
	  unexpected /* ??? not yet implemented */ },
	...
	{ STATE_MAIN_R2, STATE_MAIN_R3,
	  SMF_PSK_AUTH | SMF_FIRST_ENCRYPTED_INPUT | SMF_ENCRYPTED | SMF_REPLY |
		  SMF_RELEASE_PENDING_P2,
	  P(ID) | P(HASH), P(VID) | P(CR), PT(NONE), EVENT_SA_REPLACE,
	  main_inI3_outR3 },

	{ STATE_MAIN_R2, STATE_MAIN_R3,
	  SMF_DS_AUTH | SMF_FIRST_ENCRYPTED_INPUT | SMF_ENCRYPTED | SMF_REPLY |
		  SMF_RELEASE_PENDING_P2,
	  P(ID) | P(SIG), P(VID) | P(CR) | P(CERT), PT(NONE), EVENT_SA_REPLACE,
	  main_inI3_outR3 },

	{ STATE_MAIN_R2, STATE_UNDEFINED,
	  SMF_PKE_AUTH | SMF_RPKE_AUTH | SMF_FIRST_ENCRYPTED_INPUT |
		  SMF_ENCRYPTED | SMF_REPLY | SMF_RELEASE_PENDING_P2,
	  P(HASH), P(VID) | P(CR), PT(NONE), EVENT_SA_REPLACE,
	  unexpected /* ??? not yet implemented */ },
	...
	{ STATE_MAIN_I3, STATE_MAIN_I4,
	  SMF_PSK_AUTH | SMF_INITIATOR | SMF_FIRST_ENCRYPTED_INPUT |
		  SMF_ENCRYPTED | SMF_RELEASE_PENDING_P2,
	  P(ID) | P(HASH), P(VID) | P(CR), PT(NONE), EVENT_SA_REPLACE,
	  main_inR3 },

	{ STATE_MAIN_I3, STATE_MAIN_I4,
	  SMF_DS_AUTH | SMF_INITIATOR | SMF_FIRST_ENCRYPTED_INPUT |
		  SMF_ENCRYPTED | SMF_RELEASE_PENDING_P2,
	  P(ID) | P(SIG), P(VID) | P(CR) | P(CERT), PT(NONE), EVENT_SA_REPLACE,
	  main_inR3 },
	...
	{ STATE_MAIN_I3, STATE_UNDEFINED,
	  SMF_PKE_AUTH | SMF_RPKE_AUTH | SMF_INITIATOR |
		  SMF_FIRST_ENCRYPTED_INPUT | SMF_ENCRYPTED |
		  SMF_RELEASE_PENDING_P2,
	  P(HASH), P(VID) | P(CR), PT(NONE), EVENT_SA_REPLACE,
	  unexpected /* ??? not yet implemented */ },
	...
	{ STATE_MAIN_R3, STATE_UNDEFINED,
	  SMF_ALL_AUTH | SMF_ENCRYPTED | SMF_RETRANSMIT_ON_DUPLICATE, LEMPTY,
	  LEMPTY, PT(NONE), EVENT_NULL, unexpected },

	/* STATE_MAIN_I4: can only get here due to packet loss */
	{ STATE_MAIN_I4, STATE_UNDEFINED,
	  SMF_ALL_AUTH | SMF_INITIATOR | SMF_ENCRYPTED, LEMPTY, LEMPTY,
...
/* No state for aggr_outI1: -->HDR, SA, KE, Ni, IDii */
	...
	{ STATE_AGGR_R0, STATE_AGGR_R1, SMF_DS_AUTH | SMF_REPLY,
	  P(SA) | P(KE) | P(NONCE) | P(ID), P(VID) | P(NATD_RFC), PT(NONE),
	...
	{ STATE_AGGR_I1, STATE_AGGR_I2,
	  ...
	  PT(NONE), EVENT_SA_REPLACE, aggr_inR1_outI2 },
	...
	{ STATE_AGGR_R1, STATE_AGGR_R2,
	  SMF_PSK_AUTH | SMF_FIRST_ENCRYPTED_INPUT | SMF_ENCRYPTED |
		  SMF_RELEASE_PENDING_P2,
	  P(HASH), P(VID) | P(NATD_RFC), PT(NONE), EVENT_SA_REPLACE,
	  aggr_inI2 },
	...
	{ STATE_AGGR_R1, STATE_AGGR_R2,
	  SMF_DS_AUTH | SMF_FIRST_ENCRYPTED_INPUT | SMF_ENCRYPTED |
		  SMF_RELEASE_PENDING_P2,
	  P(SIG), P(VID) | P(NATD_RFC), PT(NONE), EVENT_SA_REPLACE, aggr_inI2 },

	/* STATE_AGGR_I2: can only get here due to packet loss */
	{ STATE_AGGR_I2, STATE_UNDEFINED,
	  SMF_ALL_AUTH | SMF_INITIATOR | SMF_RETRANSMIT_ON_DUPLICATE, LEMPTY,
	  LEMPTY, PT(NONE), EVENT_NULL, unexpected },

	/* STATE_AGGR_R2: can only get here due to packet loss */
	{ STATE_AGGR_R2, STATE_UNDEFINED, SMF_ALL_AUTH, LEMPTY, LEMPTY,
	  PT(NONE), EVENT_NULL, unexpected },
#else
	/*
	...
	{ STATE_QUICK_R0, STATE_QUICK_R1,
	  SMF_ALL_AUTH | SMF_ENCRYPTED | SMF_REPLY, P(HASH) | P(SA) | P(NONCE),
	  /* P(SA) | */ P(KE) | P(ID) | P(NATOA_RFC), PT(NONE),
	...
	{ STATE_QUICK_I1, STATE_QUICK_I2,
	  ...
	  P(HASH) | P(SA) | P(NONCE),
	  /* P(SA) | */ P(KE) | P(ID) | P(NATOA_RFC), PT(HASH),
	  EVENT_SA_REPLACE, quick_inR1_outI2 },

	/* STATE_QUICK_R1: HDR*, HASH(3) --> done
     * Installs outbound IPsec SAs, routing, etc.
     */
	{ STATE_QUICK_R1, STATE_QUICK_R2, SMF_ALL_AUTH | SMF_ENCRYPTED, P(HASH),
	  LEMPTY, PT(NONE), EVENT_SA_REPLACE, quick_inI2 },
	...
	{ STATE_QUICK_R2, STATE_UNDEFINED, SMF_ALL_AUTH | SMF_ENCRYPTED, LEMPTY,
	  LEMPTY, PT(NONE), EVENT_NULL, unexpected },

	/***** informational messages *****/

	/* STATE_INFO: */
	{ STATE_INFO, STATE_UNDEFINED, SMF_ALL_AUTH, LEMPTY, LEMPTY, PT(NONE),
	  EVENT_NULL, informational },

	/* STATE_INFO_PROTECTED: */
	{ STATE_INFO_PROTECTED, STATE_UNDEFINED, SMF_ALL_AUTH | SMF_ENCRYPTED,
	  P(HASH), LEMPTY, PT(NONE), EVENT_NULL, informational },

#ifdef XAUTH
	{ STATE_XAUTH_R0, STATE_XAUTH_R1, SMF_ALL_AUTH | SMF_ENCRYPTED,
	  P(ATTR) | P(HASH), P(VID), PT(NONE), EVENT_NULL,
	  xauth_inR0 }, /*Re-transmit may be done by previous state*/

	{ STATE_XAUTH_R1, STATE_MAIN_R3, SMF_ALL_AUTH | SMF_ENCRYPTED,
	  P(ATTR) | P(HASH), P(VID), PT(NONE), EVENT_SA_REPLACE, xauth_inR1 },
 ...
 */
	...
	{ STATE_MODE_CFG_R0, STATE_MODE_CFG_R1,
	  SMF_ALL_AUTH | SMF_ENCRYPTED | SMF_REPLY, P(ATTR) | P(HASH), P(VID),
	  PT(HASH), EVENT_SA_REPLACE, modecfg_inR0 },

	{ STATE_MODE_CFG_R1, STATE_MODE_CFG_R2, SMF_ALL_AUTH | SMF_ENCRYPTED,
	  P(ATTR) | P(HASH), P(VID), PT(HASH), EVENT_SA_REPLACE, modecfg_inR1 },

	{ STATE_MODE_CFG_R2, STATE_UNDEFINED, SMF_ALL_AUTH | SMF_ENCRYPTED,
	  LEMPTY, LEMPTY, PT(NONE), EVENT_NULL, unexpected },
	...
	{ STATE_XAUTH_I0, STATE_XAUTH_I1,
	  ...
	  P(ATTR) | P(HASH), P(VID), PT(HASH), EVENT_SA_REPLACE, xauth_inI0 },

	{ STATE_XAUTH_I1, STATE_MAIN_I4,
	  SMF_ALL_AUTH | SMF_ENCRYPTED | SMF_REPLY | SMF_RELEASE_PENDING_P2,
	  P(ATTR) | P(HASH), P(VID), PT(HASH), EVENT_SA_REPLACE, xauth_inI1 },
#endif
...
static stf_status informational(struct msg_digest *md)
{
	 ...
	 */
		DBG(DBG_CONTROL,
		    DBG_log("processing informational %s (%d)",
			    enum_name(&ipsec_notification_names, n->isan_type),
			    n->isan_type));
		...
		case ISAKMP_N_CISCO_LOAD_BALANCE:
			if (st && IS_ISAKMP_SA_ESTABLISHED(st->st_state)) {
				...
				DBG_cond_dump(DBG_PARSING,
					      "redirected remote end info:",
					      n_pbs->cur + pbs_left(n_pbs) - 4,
					      4);
				...
				memcpy(&tmp_c->spd.that.host_addr.u.v4.sin_addr
						.s_addr,
				       (u_int32_t *)(n_pbs->cur +
						     pbs_left(n_pbs) - 4),
				...
				tmp_c->IPhost_pair->him.addr =
					tmp_c->spd.that.host_addr;

				/*Initiating connection to the redirected peer*/
				initiate_connection(tmp_name, tmp_whack_sock, 0,
						    pcim_demand_crypto);
				return STF_IGNORE;
			}
			loglog(RC_LOG_SERIOUS,
		...
		default:
#ifdef DEBUG
			if (st != NULL && st->st_connection->extra_debugging &
						  IMPAIR_DIE_ONINFO) {
				loglog(RC_LOG_SERIOUS,
...
#define SEND_NOTIFICATION(t)                                             \
			...
			if (st == NULL) {
				...
				DBG(DBG_PARSING,
				    DBG_dump(
					    ...
					    md->hdr.isa_icookie, COOKIE_SIZE);
				    DBG_dump(
					    "- unknown SA's md->hdr.isa_rcookie:",
					    md->hdr.isa_rcookie, COOKIE_SIZE));
	...
	case ISAKMP_XCHG_MODE_CFG:
	     ...
	     */
			...
			if (st == NULL) {
				openswan_log(
					"Mode Config message is for a non-existent (expired?)"
					" ISAKMP SA");
				/* XXX Could send notification back */
				return;
			}
		 ...
		 */
				...
				openswan_log(
					"received MODECFG message when in state %s, and we aren't xauth client",
					enum_name(&state_names, st->st_state));
			...
			if (st->st_connection->spd.this.xauth_server &&
			    IS_PHASE1(
				    st->st_state)) /* Switch from Phase1 to Mode Config */
			{
				openswan_log(
     ...
     * they are idempotent!
     */
	if (st != NULL && st->st_rpacket.ptr != NULL &&
     ...
     *
     */
	if ((md->hdr.isa_flags & ISAKMP_FLAG_ENCRYPTION) && st != NULL &&
...
void process_packet_tail(struct msg_digest **mdp)
{
	 ...
	 */
		...
		DBG_cond_dump(DBG_CRYPT, "next IV:", st->st_new_iv,
			      st->st_new_iv_len);
	} else {
	...
	if (md->hdr.isa_xchg == ISAKMP_XCHG_INFO &&
		...
		if (!memeq(hash_pbs->cur, hash_val, hash_len)) {
			...
			DBG(DBG_CRYPT,
			    DBG_log("received 'informational' message HASH(1) data ok"));
		}
	}
	...
	/* XXX Handle Notifications */
	{
		...
		while (p != NULL) {
			...
			DBG_cond_dump(DBG_PARSING, "info:", p->pbs.cur,
				      pbs_left(&p->pbs));
		...
		while (p != NULL) {
			...
			DBG_cond_dump(DBG_PARSING, "del:", p->pbs.cur,
				      pbs_left(&p->pbs));
			p = p->next;
		}
...
static void update_retransmit_history(struct state *st, struct msg_digest *md)
{
	...
	if (md->encrypted) {
		...
		clonetochunk(st->st_rpacket, md->packet_pbs.start,
			     pbs_room(&md->packet_pbs), "raw packet");
	}
}
...
void complete_v1_state_transition(struct msg_digest **mdp, stf_status result)
{
	...
	/* advance the state */
	DBG(DBG_CONTROL, DBG_log("complete state transition with %s",
	...
	passert(result == STF_INLINE || result == STF_IGNORE ||
		result == STF_SUSPEND || st == NULL ||
		(st && st->st_calculating == FALSE));
	...
	case STF_OK:
		...
		if (st->st_reserve_msgid == FALSE &&
		    st->st_clonedfrom != SOS_NOBODY && st->st_msgid != 0) {
			struct state *p1st =
				state_with_serialno(st->st_clonedfrom);
	 ...
	 * from STATE_AGGR_R1 to STATE_AGGR_R2 */
		...
		if (smc->flags & SMF_REPLY) {
			DBG(DBG_CONTROL, char buf[ADDRTOT_BUF];
			    DBG_log("sending reply packet to %s:%u (from port %u)",
				    (addrtot(&st->st_remoteaddr, 0, buf,
					     sizeof(buf)),
				     buf),
				    st->st_remoteport, st->st_interface->port));

			close_output_pbs(
				&reply_stream); /* good form, but actually a no-op */
		...
		if (!md->event_already_set) {
		     ...
		     */
				...
				if (kind != EVENT_SA_EXPIRE) {
					schedule_sa_replace_event(
						smc->flags & SMF_INITIATOR,
						delay, c, st);
				} else {
					event_schedule(kind, delay, st);
				}
		...
		/* tell whack and log of progress */
		{
			...
			loglog(w, "%s: %s%s",
			       enum_name(&state_names, st->st_state), story,
			       sadetails);
		}
		...
		DBG(DBG_CONTROL,
		    DBG_log("modecfg pull: %s policy:%s %s",
			    (st->quirks.modecfg_pull_mode ? "quirk-poll" :
							    "noquirk"),
		...
		if (!(smc->flags & SMF_INITIATOR) &&
			...
			change_state(md->transition_state,
				     STATE_MAIN_R3); /* ISAKMP is up... */
			set_cur_state(st);
			quick_outI1(
				st->st_whack_sock, st, st->st_connection,
	...
	case STF_FATAL:
		...
		whack_log(RC_FATAL, "encountered fatal error in state %s",
			  enum_name(&state_names, st->st_state));
#ifdef HAVE_NM
		if (st->st_connection->remotepeertype == CISCO &&
		    st->st_connection->nmconfigured) {
			if (!do_command(st->st_connection,
					&st->st_connection->spd, "disconnectNM",
					st)) {
				DBG(DBG_CONTROL,
	...
	case STF_FAIL:
	     ...
	     */
		whack_log(RC_NOTIFICATION + md->note, "%s: %s",
			  enum_name(&state_names, from_state),
						 ...
						 lset_t policy)
{
	...
	if ((policy & POLICY_PFS) && c->alg_info_esp &&
	    c->alg_info_esp->esp[0].pfs_group) {
