...
struct adns_continuation {
	...
	struct gw_info
		*gateways_from_dns; /* answer, if looking for our TXT rrs */
#ifdef USE_KEYRR
...
extern err_t start_adns_hostname(sa_family_t addr_family, const char *hostname,
				 cont_fn_t cont_fn,
				 struct adns_continuation *cr);
...
extern void gw_addref(struct gw_info *gw), gw_delref(struct gw_info **gwp);
