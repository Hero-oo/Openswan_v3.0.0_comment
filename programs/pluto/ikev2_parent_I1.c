 ...
 *
 */
stf_status ikev2parent_outI1(int whack_sock, struct connection *c,
			     ...
			     struct xfrm_user_sec_ctx_ike *uctx UNUSED)
{
	...
	return ikev2parent_outI1_withstate(st, whack_sock, c, predecessor,
					   policy, try, importance, uctx);
}
...
stf_status
ikev2parent_outI1_withstate(struct state *st, int whack_sock,
			    struct connection *c, struct state *predecessor,
			    lset_t policy,
			    ...
			    struct xfrm_user_sec_ctx_ike *uctx UNUSED)
{
	...
	if (predecessor == NULL)
		...
		openswan_log("initiating v2 parent SA to replace #%lu",
	...
	if (predecessor != NULL) {
		...
		loglog(RC_NEW_STATE + STATE_PARENT_I1, "%s: initiate",
		       enum_name(&state_names, st->st_state));
	}
	...
	/* now. we need to go calculate the nonce, and the KE */
	{
		struct ke_continuation *ke =
			alloc_thing(struct ke_continuation, "ikev2_outI1 KE");
		stf_status e;
					    ...
					    struct state *st)
{
	...
	/* HDR out */
	{
		...
		/* testing fake minor new IKE version, should success */
			...
			hdr.isa_version = IKEv2_MAJOR_VERSION << ISA_MAJ_SHIFT |
					  IKEv2_MINOR_BUMP;
		else {
	...
	if (st->st_dcookie.ptr) {
		...
		ship_v2N(ISAKMP_NEXT_NONE, ISAKMP_PAYLOAD_NONCRITICAL,
			 PROTO_ISAKMP, &child_spi, v2N_COOKIE, &st->st_dcookie,
			 &md->rbody);
	}
