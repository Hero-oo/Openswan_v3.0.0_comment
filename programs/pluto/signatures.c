...
static bool take_a_crack(struct tac_state *s, struct pubkey *kr,
			 const char *story USED_BY_DEBUG)
{
	...
	if (ugh == NULL) {
		...
		DBG(DBG_CRYPT | DBG_CONTROL,
		    DBG_log("an RSA Sig check passed with *%s [%s]", k->keyid,
			    story));
		return TRUE;
	} else {
		DBG(DBG_CRYPT,
		...
		if (s->best_ugh == NULL || s->best_ugh[0] < ugh[0])
			s->best_ugh = ugh;
		if (ugh[0] > '0' && s->tn - s->tried + KEYID_BUF + 2 <
 ...
 *
 */
static stf_status check_specific_key(struct tac_state *s, struct pubkey *key)
{
	time_t tnow;
...
stf_status check_signature_gen(
