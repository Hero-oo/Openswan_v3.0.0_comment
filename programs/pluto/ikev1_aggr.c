				      ...
				      struct pluto_crypto_req *r, err_t ugh)
{
	...
	/* set up second calculation */
	{
		struct dh_continuation *dh =
			alloc_thing(struct dh_continuation, "aggr outR1 DH");
		dh->md = md;
		set_suspended(st, md);
...
static stf_status aggr_inI1_outR1_common(struct msg_digest *md, int authtype)
{
	...
	struct connection *c = find_host_connection(
		ANY_MATCH, &md->iface->ip_addr, md->iface->port, KH_IPADDR,
	...
	if (c == NULL)
		...
		c = find_host_connection(ANY_MATCH, &md->iface->ip_addr,
					 md->iface->port, KH_ANY, &md->sender,
					 md->sender_port, LEMPTY,
					 POLICY_IKEV1_DISABLE, &policy_hint);
	...
	if (c == NULL) {
		...
		lset_t policy = preparse_isakmp_sa_body(&pre_sa_pbs) |
				POLICY_AGGRESSIVE;
		c = find_host_connection(ANY_MATCH, &md->iface->ip_addr,
					 pluto_port500, KH_ANY,
					 (ip_address *)NULL, md->sender_port,
					 policy, POLICY_IKEV1_DISABLE,
					 &policy_hint);
		if (c == NULL || (c->policy & POLICY_AGGRESSIVE) == 0) {
			loglog(RC_LOG_SERIOUS,
			       "initial Aggressive Mode message from %s"
	...
	/* Set up state */
	cur_state = md->st = st =
		new_state(); /* (caller will reset cur_state) */
	st->st_ikev2_orig_initiator =
		FALSE; /* we are responding to this exchange */
	st->st_connection = c;
	...
	openswan_log(
		"responding to Aggressive Mode, state #%lu, connection \"%s\""
		" from %s",
	...
	/* SA body in */
	{
		...
		RETURN_STF_FAILURE(parse_isakmp_sa_body(
			&sabs, &sa_pd->payload.sa, NULL, FALSE, st));
	}
				       ...
				       struct pluto_crypto_req *r)
{
	...
	/* KE */
	if (!justship_KE(&st->st_gr, &md->rbody, ISAKMP_NEXT_NONCE))
		return STF_INTERNAL_ERROR;
	...
	/* HASH_R or SIG_R out */
	{
		...
		if (auth_payload == ISAKMP_NEXT_HASH) {
			/* HASH_R out */
			if (!out_generic_raw(ISAKMP_NEXT_VID, &isakmp_hash_desc,
					     &md->rbody, hash_val, hash_len,
					     "HASH_R"))
				return STF_INTERNAL_ERROR;
		} else {
			/* SIG_R out */
			...
			if (sig_len == 0) {
				loglog(RC_LOG_SERIOUS,
				       "unable to locate my private key for RSA Signature (IKEv1 aggressive responder)");
				return STF_FAIL + AUTHENTICATION_FAILED;
			}

			if (!out_generic_raw(ISAKMP_NEXT_VID,
					     &isakmp_signature_desc, &md->rbody,
					     sig_val, sig_len, "SIG_R"))
				return STF_INTERNAL_ERROR;
		}
	}
	...
	if (st->hidden_variables.st_nat_traversal) {
		if (st->hidden_variables.st_nat_traversal & NAT_T_WITH_NATD) {
			if (!nat_traversal_add_natd(ISAKMP_NEXT_VID, &md->rbody,
						    md))
				return STF_INTERNAL_ERROR;
		}
		if (!out_vid(ISAKMP_NEXT_NONE, &md->rbody,
			     md->quirks.nat_traversal_vid)) {
			return STF_INTERNAL_ERROR;
		}
	}
...
stf_status aggr_inR1_outI2(struct msg_digest *md)
{
	...
	/* verify echoed SA */
	{
		...
		notification_t r = parse_isakmp_sa_body(
...
#ifdef NAT_TRAVERSAL
	DBG(DBG_CONTROLMORE,
	    DBG_log("inR1: checking NAT-t: %d and %d", nat_traversal_enabled,
		    st->hidden_variables.st_nat_traversal));
	if (st->hidden_variables.st_nat_traversal & NAT_T_WITH_NATD) {
	...
	/* set up second calculation */
	{
		...
		return start_dh_secretiv(&dh->dh_pcrc, st, st->st_import,
					 INITIATOR, st->st_oakley.group->group);
	}
}
				       ...
				       struct key_continuation *kc)
{
	...
	/* HASH_I or SIG_I out */
	{
		...
		if (auth_payload == ISAKMP_NEXT_HASH) {
			...
			if (sig_len == 0) {
				loglog(RC_LOG_SERIOUS,
				       "unable to locate my private key for RSA Signature  (IKEv1 aggressive initiator)");
				return STF_FAIL + AUTHENTICATION_FAILED;
			}
...
stf_status aggr_inI2_tail(struct msg_digest *md, struct key_continuation *kc)
{
	struct state *const st = md->st;
	struct connection *c = st->st_connection;
	u_char buffer[1024];
	...
	/* Reconstruct the peer ID so the peer hash can be authenticated */
	{
		...
		if (!out_struct(&id_hd, &isakmp_ipsec_identification_desc, &pbs,
				&id_pbs) ||
		    !out_chunk(id_b, &id_pbs, "my identity"))
			return STF_INTERNAL_ERROR;
	...
	{
		stf_status r =
			aggr_id_and_auth(md, FALSE, aggr_inI2_continue, kc);
...
stf_status aggr_outI1(int whack_sock, struct connection *c,
		      ...
		      struct xfrm_user_sec_ctx_ike *uctx)
{
	...
	st->st_ikev2_orig_initiator =
		TRUE; /* we are initiating this exchange */
	st->st_connection = c;
	...
	*/
	...
	{
		struct ke_continuation *ke =
			alloc_thing(struct ke_continuation, "aggr_outI1 KE");
		stf_status e;
				  ...
				  struct pluto_crypto_req *r)
{
	...
	/* SA out */
	{
		...
		struct db_sa *oakley_sa =
			ikev1_alg_makedb(st->st_policy, c->alg_info_ike,
					 TRUE /* one proposal for aggr */
					 ,
					 INITIATOR);

		if (oakley_sa == NULL ||
		    !out_sa(&md->rbody, oakley_sa, st, /* oakley mode */ TRUE,
			    INITIATOR, /*aggr */ TRUE, ISAKMP_NEXT_KE)) {
			reset_cur_state();
			if (oakley_sa)
				free_sa(oakley_sa);
			return STF_INTERNAL_ERROR;
	...
	/* ALWAYS Announce our ability to do Dead Peer Detection to the peer */
	{
		...
		if (nat_traversal_enabled || c->spd.this.xauth_client ||
	...
	clonetochunk(st->st_tpacket, reply_stream.start,
		     pbs_offset(&reply_stream), "reply packet from aggr_outI1");
...
tpm_ignore:
	...
	whack_log(RC_NEW_STATE + STATE_AGGR_I1, "%s: initiate",
