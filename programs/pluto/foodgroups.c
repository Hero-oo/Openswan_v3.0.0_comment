...
static int ipcmp(ip_address *a, ip_address *b)
{
	if (addrtypeof(a) != addrtypeof(b)) {
		...
		const struct sockaddr *sa = sockaddrof(a), *sb = sockaddrof(b);

		passert(addrtypeof(a) ==
			AF_INET); /* not yet implemented IPv6 version :-( */
		return (ntohl(((const struct sockaddr_in *)sa)->sin_addr.s_addr) <
			ntohl(((const struct sockaddr_in *)sb)
				      ->sin_addr.s_addr)) ?
			       -1 :
			       1;
	}
...
static void read_foodgroup(struct fg_groups *g)
{
	...
	if (!lexopen(&flp_space, fg_path, TRUE)) {
		DBG(DBG_CONTROL, char cwd[PATH_MAX];
		    DBG_log("no group file \"%s\" (pwd:%s)", fg_path,
...
void load_groups(void)
{
	...
	DBG(DBG_CONTROL, {
		...
		for (t = new_targets; t != NULL; t = t->next) {
			...
			subnettot(&t->group->connection->spd.this.client, 0,
				  asource, sizeof(asource));
			subnettot(&t->subnet, 0, atarget, sizeof(atarget));
			DBG_log("%s->%s %s", asource, atarget,
     ...
     */
	{
		struct fg_targets *op = targets, *np = new_targets;
...
void route_group(struct connection *c)
{
	...
	if (!NEVER_NEGOTIATE(c->policy) && !HAS_IPSEC_POLICY(c->policy)) {
		...
		for (t = targets; t != NULL; t = t->next) {
			if (t->group == g) {
				struct connection *ci =
					con_by_name(t->name, FALSE);
