...
enum smf2_flags {
	...
	SMF2_MATCH_REQUEST =
		LELEM(4), // microcode will only match incoming request messages
	SMF2_MATCH_RESPONSE =
		LELEM(5), // microcode will only match incoming request messages
};
...
static const lset_t everywhere_payloads = P(N) |
					  P(V); /* can appear in any packet */
static const lset_t repeatable_payloads =
	P(N) | P(D) | P(CP) | P(V); /* if one can appear, many can appear */
...
struct state_v2_microcode v2_state_microcode_table[] = {
	/* state 0 */
	{
		...
		.flags = SMF2_MATCH_RESPONSE | SMF2_INITIATOR |
			 SMF2_STATENEEDED | SMF2_REPLY,
	...
	/* state 3 */
	{
		...
		.flags =
			SMF2_MATCH_REQUEST |
			/* not SMF2_INITIATOR, not SMF2_STATENEEDED */ SMF2_REPLY,
	...
	/* state 4 */
	{
		...
		.flags = SMF2_MATCH_REQUEST |
			 /* not SMF2_INITIATOR */ SMF2_STATENEEDED | SMF2_REPLY,
		.req_clear_payloads = P(E),
	...
	/* state 14 */
	{
		...
		.state = STATE_PARENT_I3,
		.next_state = STATE_CHILD_C1_KEYED,
		.flags = SMF2_MATCH_REQUEST | SMF2_INITIATOR |
			 SMF2_STATENEEDED | SMF2_REPLY,
		.req_clear_payloads = P(E),
		.req_enc_payloads = P(SA) | P(TSi) | P(TSr) | P(Ni),
		.opt_enc_payloads = P(KE),
	...
	/* state 19 */
	{
		...
		.state = STATE_PARENT_R2,
		.next_state = STATE_CHILD_C1_KEYED,
		.flags = SMF2_MATCH_RESPONSE |
			 /* not SMF2_INITIATOR */ SMF2_STATENEEDED,
		.req_clear_payloads = P(E),
		.req_enc_payloads = P(SA) | P(TSi) | P(TSr) | P(Nr),
		.opt_enc_payloads = 0,
		.processor = ikev2child_inCR1,
	...
	/* last entry */
	{ .svm_name = "invalid-transition", .state = STATE_IKEv2_ROOF }
};
 ...
 */
static stf_status ikev2_collect_payloads(struct msg_digest *md,
					 pb_stream *in_pbs,
					 ...
					 unsigned int np)
{
	...
	while (np != ISAKMP_NEXT_NONE) {
		...
		if (sd == NULL || np < ISAKMP_v2PAYLOAD_TYPE_BASE) {
			...
			if (!in_struct(&pd->payload, &ikev2_generic_desc,
				       in_pbs, &pd->pbs)) {
				loglog(RC_LOG_SERIOUS,
		...
		DBG(DBG_PARSING, DBG_log("processing payload: %s (len=%u)\n",
 ...
 */
static stf_status ikev2_process_payloads(struct msg_digest *md UNUSED,
					 lset_t seen, lset_t req_payloads,
					 lset_t opt_payloads)
{
	...
	extra_payloads =
		(seen & ~(req_payloads | opt_payloads | everywhere_payloads));
...
stf_status ikev2_process_encrypted_payloads(struct msg_digest *md,
					    pb_stream *in_pbs, unsigned int np)
{
	stf_status stf;
	lset_t seen = LEMPTY;
	...
	if (svm->req_enc_payloads & ~seen) {
		...
		if (md->chain[ISAKMP_NEXT_v2N] && svm->ntf_processor) {
			...
			DBG(DBG_CONTROL,
			    DBG_log("missing payloads (within encryption) for v2_state: %s. "
				    "Handling encrypted notification.",
				    svm->svm_name));
			...
			if (stf == STF_OK) {
				DBG(DBG_CONTROL,
				    DBG_log("notification handler returned OK; "
...
void process_v2_packet(struct msg_digest **mdp)
{
	...
	if (IKEv2_MSG_FROM_INITIATOR(md->hdr.isa_flags)) {
		...
		if (pst) {
			...
			if (pst->st_msgid_lastrecv != INVALID_MSGID &&
				...
				send_packet(st, "ikev2-responder-retransmit",
					    FALSE);
				return;
			}
		...
		if (!st) {
			...
			if (st) {
				...
				unhash_state(st);
				memcpy(st->st_rcookie, md->hdr.isa_rcookie,
				       COOKIE_SIZE);
				insert_state(st);
			}
		}
		if (!st) {
             ...
             */
			openswan_log(
				"ignored received packet with unknown cookies");
	     ...
	     */
			if (pst->st_msgid_lastack != INVALID_MSGID &&
				...
				if ((st->st_msg_retransmitted % 512) == 1 ||
				    DBGP(DBG_CONTROL)) {
			...
			if (md->msgid_received != MAINMODE_MSGID &&
				...
				if ((pst->st_msg_badmsgid_recv % 512) == 1 ||
				    DBGP(DBG_CONTROL)) {
					loglog(RC_LOG_SERIOUS,
					       "dropping reply: expecting [%u,%u> received: %u",
	...
	if (st) {
		...
		DBG(DBG_CONTROL,
		    DBG_log("state found and its state is:%s msgid: %05u",
			    enum_show(&state_names, from_state),
			    md->msgid_received));
	}

	stf_status stf = ikev2_collect_payloads(md, &md->message_pbs, &seen,
						md->hdr.isa_np);
	if (stf != STF_OK) {
	...
	for (svm = v2_state_microcode_table; svm->state != STATE_IKEv2_ROOF;
	     svm_num++, svm++) {
		DBG(DBG_CONTROLMORE,
		    DBG_log("considering state entry: %u", svm_num));
		if (svm->processor == NULL)
			continue; /* let there be empty states for historical reasons */
		...
		if (svm->flags & SMF2_MATCH_REQUEST) {
			...
			if (md->role == INITIATOR) {
				...
				DBG(DBG_CONTROLMORE,
				    DBG_log("  reject: received response, needs request message"));
				continue;
			}
		}
		if (svm->flags & SMF2_MATCH_RESPONSE) {
			...
			if (md->role == RESPONDER) {
				...
				DBG(DBG_CONTROLMORE,
				    DBG_log("  reject: received request, needs response message"));
				continue;
			}
		}
		...
		if (svm->flags & SMF2_STATENEEDED) {
			if (st == NULL) {
				DBG(DBG_CONTROLMORE,
				    DBG_log("  reject:state needed and state unavailable"));
				continue;
			}
		}
		if ((svm->flags & SMF2_STATENEEDED) == 0) {
			if (st != NULL) {
				DBG(DBG_CONTROLMORE,
				    DBG_log("  reject:state unneeded and state available"));
				continue;
			}
		}
		if (svm->state != from_state) {
			DBG(DBG_CONTROLMORE,
			    DBG_log("  reject: in state: %s, needs %s",
		...
		if (svm->recv_type != ike_xchg_type) {
			DBG(DBG_CONTROLMORE,
			    DBG_log("  reject: recv_type: %s, needs %s",
				    enum_name(&exchange_names, ike_xchg_type),
				    enum_name(&exchange_names,
					      svm->recv_type)));
			continue;
		}
	...
	if (svm->state == STATE_IKEv2_ROOF) {
		DBG(DBG_CONTROL,
		    DBG_log("did not find valid state; giving up"));
		...
		if (IKEv2_MSG_FROM_INITIATOR(md->hdr.isa_flags)) {
			...
			if (was_encrypted) {
				...
				send_v2_notification_enc(md, ike_xchg_type,
							 INVALID_MESSAGE_ID,
							 NULL);
			} else {
				/* our notification will be in the clear */
				SEND_V2_NOTIFICATION_XCHG_DATA(
					md, st, ike_xchg_type,
	...
	{
		stf_status stf;
		stf = ikev2_process_payloads(md, seen, svm->req_clear_payloads,
					     svm->opt_clear_payloads);
	...
	DBG(DBG_CONTROL,
	    DBG_log("now proceed with state specific processing using state #%u %s",
		    svm_num, svm->svm_name));
	DBG(DBG_PARSING, if (pbs_left(&md->message_pbs) != 0)
...
bool ikev2_decode_peer_id(struct msg_digest *md, enum phase1_role init)
{
	...
	unsigned int hisID = (init == INITIATOR) ? ISAKMP_NEXT_v2IDr :
						   ISAKMP_NEXT_v2IDi;
	...
	idtoa(&st->ikev2.st_peer_id, st->ikev2.st_peer_buf,
	      sizeof(st->ikev2.st_peer_buf));
	openswan_log("IKEv2 mode peer ID is %s: '%s'",
		     enum_show(&ident_names, id->isai_type),
 ...
 */
bool ikev2_decode_local_id(struct msg_digest *md, enum phase1_role init)
{
	struct state *const st = md->st;
	unsigned int localID = (init == INITIATOR) ? ISAKMP_NEXT_v2IDi :
						     ISAKMP_NEXT_v2IDr;
	...
	idtoa(&st->ikev2.st_local_id, st->ikev2.st_local_buf,
	      sizeof(st->ikev2.st_local_buf));
...
void ikev2_log_parentSA(struct state *st)
{
	...
	if (DBGP(DBG_CRYPT)) {
		...
		datatot(st->st_skey_ei.ptr, st->st_skey_ei.len, 'x', enckeybuf,
			256);
		datatot(st->st_skey_ai.ptr, st->st_skey_ai.len, 'x', authkeybuf,
			256);
		DBG_log("ikev2 I 0x%02x%02x%02x%02x%02x%02x%02x%02x 0x%02x%02x%02x%02x%02x%02x%02x%02x %s:%s %s:%s",
			st->st_icookie[0], st->st_icookie[1], st->st_icookie[2],
			st->st_icookie[3], st->st_icookie[4], st->st_icookie[5],
			st->st_icookie[6], st->st_icookie[7], st->st_rcookie[0],
			st->st_rcookie[1], st->st_rcookie[2], st->st_rcookie[3],
			st->st_rcookie[4], st->st_rcookie[5], st->st_rcookie[6],
			st->st_rcookie[7], authalgo, authkeybuf, encalgo,
			enckeybuf);

		datatot(st->st_skey_er.ptr, st->st_skey_er.len, 'x', enckeybuf,
			256);
		datatot(st->st_skey_ar.ptr, st->st_skey_ar.len, 'x', authkeybuf,
			256);
				     ...
				     u_int16_t ntf_type, chunk_t *data)
{
	...
	send_v2_notification(st, xchg, ntf_type, st->st_icookie, st->st_rcookie,
			     data);
}

void send_v2_notification_from_md(struct msg_digest *md UNUSED,
				  enum isakmp_xchg_types xchg,
				  u_int16_t ntf_type, chunk_t *data)
{
	struct state st;
	...
	send_v2_notification(&st, xchg, ntf_type, md->hdr.isa_icookie,
			     md->hdr.isa_rcookie, data);
}
...
static void success_v2_state_transition(struct msg_digest **mdp)
{
	...
	DBG(DBG_CONTROL,
	    DBG_log("v2_state_transition: st is #%lu; pst is #%lu; transition_st is #%lu",
		    st ? st->st_serialno : 0, pst ? pst->st_serialno : 0,
		    md->transition_state ? md->transition_state->st_serialno :
					   0));
	...
	/* tell whack and log of progress */
	{
		...
		if (IS_PARENT_SA_ESTABLISHED(pst->st_state) &&
			...
			fmt_isakmp_sa_established(pst, sadetails,
						  sizeof(sadetails));
		}

		/* tell whack and logs our progress */
		loglog(w, "%s: %s%s (msgid: %08u/%08u)",
		       enum_name(&state_names, to_state), story, sadetails,
		       md->msgid_received, pst->st_msgid_lastrecv);

		if (st != pst && IS_CHILD_SA(st) &&
			...
			openswan_log(
				"negotiated tunnel [%s,%s proto:%u port:%u-%u] -> [%s,%s proto:%u port:%u-%u]",
				usubl, usubh, st->st_ts_this.ipprotoid,
				st->st_ts_this.startport,
				st->st_ts_this.endport, tsubl, tsubh,
	...
	if (svm->flags & SMF2_REPLY) {
		...
		DBG(DBG_CONTROL, char buf[ADDRTOT_BUF];
		    DBG_log("sending reply packet to %s:%u (from port %u)",
			    (addrtot(&st->st_remoteaddr, 0, buf, sizeof(buf)),
			     buf),
	...
	if (w == RC_SUCCESS) {
		...
		DBG_log("releasing whack for #%lu (sock=%d)", st->st_serialno,
			st->st_whack_sock);
		release_whack(st);
		...
		if (st->st_clonedfrom != 0) {
			pst = state_with_serialno(
				st->st_clonedfrom); /* with failed child sa, we end up here with an orphan?? */
	     ...
	     */
			if (kind != EVENT_SA_EXPIRE) {
				schedule_sa_replace_event(
					svm->flags & SMF2_INITIATOR, delay, c,
					st);
			} else {
				delete_event(st);
...
void complete_v2_state_transition(struct msg_digest **mdp, stf_status result)
{
	struct msg_digest *md = *mdp;
	/* const struct state_v2_microcode *svm=md->svm; */
	struct state *st;
	...
	DBG(DBG_CONTROL,
	    DBG_log("#%lu complete v2 state transition with %s",
		    st ? st->st_serialno : 0, stf_status_name(result)));
	...
	case STF_FATAL:
		...
		passert(st);
		loglog(RC_FATAL, "encountered fatal error in state %s",
		       from_state_name);
	...
	case STF_FAIL:
		loglog(RC_NOTIFICATION + md->note, "%s: %s", from_state_name,
		...
		if (st != NULL && IS_PARENT_SA(st) &&
		    IS_PARENT_SA_HALFOPEN(st->st_state)) {
			delete_state(st);
		}
	}
}

stf_status accept_v2_KE(struct msg_digest *md, struct state *st, chunk_t *ke,
			const char *name)
{
				  ...
				  const char *name)
{
	...
	if (len < MINIMUM_NONCE_SIZE || MAXIMUM_NONCE_SIZE < len) {
		loglog(RC_LOG_SERIOUS, "%s length not between %d and %d", name,
