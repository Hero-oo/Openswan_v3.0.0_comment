	...
	if (!oriented(*c)) {
		...
		if (isanyaddr(&c->spd.that.host_addr)) {
			if (c->spd.that.host_type == KH_IPHOSTNAME &&
			    c->spd.that.host_address_list.address_list ==
				    NULL) {
				loglog(RC_NOPEERIP,
				       "dns resolution for %s not yet complete, still trying",
		...
		if (c->policy & (POLICY_ENCRYPT | POLICY_AUTHENTICATE)) {
			...
			struct db_sa *phase2_sa =
				kernel_alg_makedb(c->policy, alg, TRUE);
			 ...
			 enum crypto_importance importance)
{
	...
	if (count == 0) {
		whack_log(RC_UNKNOWN_NAME, "no connection named \"%s\"", name);
	}
...
bool kick_adns_connection(struct connection *c, err_t ugh)
{
	...
	if (st != NULL && (IS_ISAKMP_SA_ESTABLISHED(st->st_state) ||
			   IS_PARENT_SA_ESTABLISHED(st->st_state))) {
		/* the connection is up, no point in doing it again */
		DBG(DBG_DNS, DBG_log("  DNS already connected, do not reinit"));
		return FALSE;
	}
     ...
     */
	if (c->spd.that.host_type == KH_IPHOSTNAME) {
		kicknow = kicknow || kick_adns_connection_lookup(
					     c, &c->spd.that, ugh == NULL);
	}
	if (!kicknow && c->spd.this.host_type == KH_IPHOSTNAME) {
		kicknow = kicknow || kick_adns_connection_lookup(
					     c, &c->spd.this, ugh == NULL);
	}
	...
	if (kicknow && c->policy & POLICY_UP) {
		...
		DBG(DBG_DNS, DBG_log("  attempting to reinit"));

		set_cur_connection(c);
		addrtot(&c->spd.that.host_addr, 0, targetaddr,
			sizeof(targetaddr));
		loglog(RC_NOPEERIP,
		       "trying to initiate to address %s (name=%s)", targetaddr,
		       c->spd.that.host_addr_name);
...
static const char *const oppo_step_name[] = { "fos_start",
					      "fos_myid_ip_txt",
					      "fos_myid_hostname_txt",
					      "fos_myid_ip_key",
					      "fos_myid_hostname_key",
			...
			err_t ughmsg)
{
	...
	whack_log(RC_OPPOFAILURE,
		  "Can not opportunistically initiate for %s to %s: %s", ocb,
		  pcb, ughmsg);
	 ...
	 */
		...
		(void)replace_bare_shunt(&b->our_client, &b->peer_client,
					 b->policy_prio, failure_shunt,
					 failure_shunt != 0, b->transport_proto,
					 ughmsg);
	}
}

static int
initiate_ondemand_body(struct find_oppo_bundle *b, struct adns_continuation *ac,
		       err_t ac_ugh,
		       struct xfrm_user_sec_ctx_ike *uctx); /* forward */

int initiate_ondemand(const ip_address *our_client,
		      const ip_address *peer_client, int transport_proto,
		      bool held, int whackfd,
		      struct xfrm_user_sec_ctx_ike *uctx UNUSED, err_t why)
{
	struct find_oppo_bundle b;
	...
	b.whackfd = whackfd;
	b.step = fos_start;
	return initiate_ondemand_body(&b, NULL, NULL, uctx);
}

static void continue_oppo(struct adns_continuation *acr, err_t ugh)
{
	struct find_oppo_continuation *cr =
		(void *)acr; /* inherit, damn you! */
	struct connection *c;
	 ...
	 */
		...
		(void)initiate_ondemand_body(&cr->b, &cr->ac, ugh, NULL_POLICY);
				  ...
				  struct xfrm_user_sec_ctx_ike *uctx)
{
	...
	if (isanyaddr(&b->our_client) || isanyaddr(&b->peer_client)) {
		...
		work = 0;
	} else if (!(c = find_connection_for_clients(&sr, &b->our_client,
						     &b->peer_client,
						     b->transport_proto))) {
	 ...
	 * this connection, so we initiate it */
		...
		ipsecdoi_initiate(b->whackfd, NULL, NULL, c, c->policy, 1,
				  SOS_NOBODY, pcim_local_crypto, uctx);
	 ...
	 */
		...
		passert(c->policy &
			POLICY_OPPO); /* can't initiate Road Warrior connections */
		 ...
		 */
			...
			next_step =
				fos_his_client; /* unless we decide to look for KEY RR */
		 ...
		 */
		     ...
		     */
				...
				for (kr = ac->keys_from_dns; kr != NULL;
				     kr = kr->next) {
		 ...
		 */
			...
			c = build_outgoing_opportunistic_connection(
				ac->gateways_from_dns, &b->our_client,
				&b->peer_client);
		     ...
		     */
				...
				addrtot(&b->our_client, 0, ocb, sizeof(ocb));
				addrtot(&b->peer_client, 0, pcb, sizeof(pcb));
				passert(id_is_ipaddr(
					&ac->gateways_from_dns->gw_id));
			 ...
			 */
				...
				ipsecdoi_initiate(
					b->whackfd, NULL, NULL, c, c->policy, 1,
	     ...
	     * record this fact.
	     */
			if (b->held && sr->routing == RT_ROUTED_PROSPECTIVE &&
			    eclipsable(sr)) {
	     ...
	     */
			...
			case fos_myid_ip_key:
				if (c->spd.this.id.kind == ID_MYID &&
					...
					ugh = start_adns_query(
						&myids[MYID_IP],
						(const struct id *)
							NULL /* security gateway meaningless */
						,
						ns_t_key, continue_oppo,
						&cr->ac);
					break;
				}
		     ...
		     */
					...
					ugh = start_adns_query(
						&id,
						&c->spd.this
							 .id /* we are the security gateway */
						,
						ns_t_txt, continue_oppo,
						&cr->ac);
					break;
				}
			...
			case fos_his_client: /* TXT for his client */
				...
				ugh = start_adns_query(
					&id,
					(const struct id *)
						NULL /* security gateway unconstrained */
					,
					ns_t_txt, continue_oppo, &cr->ac);
				break;
	 ...
	 */
		...
		for (d = connections; d != NULL;) {
			struct connection *next =
				d->ac_next; /* might move underneath us */
			...
			if (c != d &&
			    ...
			    !(c->spd.that.host_type == KH_IPHOSTNAME &&
			      ...
			      (strcasecmp(c->spd.that.host_addr_name,
					  d->spd.that.host_addr_name) == 0))) {
...
#define PENDING_DDNS_INTERVAL \
	(60) /* time before retrying DDNS host lookup
                                     for phase 1*/
...
#define PENDING_PHASE2_INTERVAL \
	(60 * 2) /*time between scans of pending phase2*/
...
void connection_check_phase2(void)
{
	...
	for (c = connections; c != NULL; c = cnext) {
		...
		DBG(DBG_CONTROL,
		    DBG_log("pending review: connection \"%s\" being checked",
			    c->name));
		...
		if (c->newest_isakmp_sa == SOS_NOBODY ||
			...
			if (kicknow) {
                 ...
                 */
				p1st = find_phase1_state(
					c, ISAKMP_SA_ESTABLISHED_STATES |
						   PHASE1_INITIATOR_STATES);
				if (p1st) {
					delete_event(p1st);
					event_schedule(EVENT_SA_REPLACE, 0,
						       p1st);
				} else {
			...
			DBG(DBG_CONTROL,
			    DBG_log("pending review: connection \"%s\" not time for review",
				    c->name));
		}
	}
}
