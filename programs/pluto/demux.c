	 ...
	 */
		if (md->packet_pbs.roof - md->packet_pbs.cur >=
			...
			if (md->maj != ISAKMP_MAJOR_VERSION &&
				...
				return;
			} else if (md->maj == ISAKMP_MAJOR_VERSION &&
				   md->min != ISAKMP_MINOR_VERSION) {
...
bool natt_skip_nonesp(const struct iface_port *ifp, const ip_address *cur_from,
		      unsigned short cur_from_port, u_int8_t **p_buffer,
		      int *p_packet_len)
{
...
static bool read_packet(struct msg_digest *md)
{
...
#if defined(HAVE_UDPFROMTO)
	packet_len = recvfromto(ifp->fd, bigbuffer, sizeof(bigbuffer),
				/*flags*/ 0, &from.sa, &from_len, &to.sa,
				&to_len);
#else
	packet_len = recvfrom(ifp->fd, bigbuffer, sizeof(bigbuffer),
			      /*flags*/ 0, &from.sa, &from_len);
#endif
     ...
     */
	if (packet_len == -1 && from_len == sizeof(from) &&
	    all_zero((const void *)&from.sa, sizeof(from))) {
		/* "from" is untouched -- not set by recvfrom */
		from_ugh = undisclosed;
	} else if (from_len < (int)(offsetof(struct sockaddr, sa_family) +
				    sizeof(from.sa.sa_family))) {
		from_ugh = "truncated";
	} else {
		...
		if (afi == NULL) {
			...
			case AF_INET:
				from_ugh =
					initaddr((void *)&from.sa_in4.sin_addr,
			...
			case AF_INET6:
				from_ugh =
					initaddr((void *)&from.sa_in6.sin6_addr,
	...
	if (packet_len == -1) {
		if (from_ugh == undisclosed && errno == ECONNREFUSED) {
	     ...
	     */
			...
			log_errno((e, "recvfrom on %s from %s:%u failed",
				   ifp->ip_dev->id_rname, ip_str(&md->sender),
...
#ifdef NAT_TRAVERSAL
	if (natt_skip_nonesp(ifp, cur_from, cur_from_port, &_buffer,
			     &packet_len) != TRUE) {
		return FALSE;
	}
#endif
	...
	DBG(DBG_RAW | DBG_CRYPT | DBG_PARSING | DBG_CONTROL, {
		DBG_log("*received %d bytes from %s:%u on %s (port=%d) at %s",
			(int)pbs_room(&md->packet_pbs), ip_str(cur_from),
			(unsigned)cur_from_port, ifp->ip_dev->id_rname,
			ifp->port, oswtimestr());
	});
