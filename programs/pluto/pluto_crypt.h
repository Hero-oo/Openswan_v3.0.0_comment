...
typedef void (*crypto_req_func)(struct pluto_crypto_req_cont *,
				struct pluto_crypto_req *, err_t ugh);

struct pluto_crypto_req_cont {
	TAILQ_ENTRY(pluto_crypto_req_cont)
	pcrc_list;
...
extern void pluto_crypto_allocchunk(wire_chunk_t *space, wire_chunk_t *new,
				    size_t howbig);
extern void pluto_crypto_copychunk(wire_chunk_t *spacetrack,
				   unsigned char *space, wire_chunk_t *new,
				   chunk_t data);

/* actual helper functions */
extern stf_status build_ke(struct pluto_crypto_req_cont *cn, struct state *st,
...
extern stf_status
start_dh_secretiv(struct pluto_crypto_req_cont *cn, struct state *st,
		  enum crypto_importance importance,
		  enum phase1_role init /* TRUE=g_init,FALSE=g_r */
		  ,
		  u_int16_t oakley_group_p);

extern void finish_dh_secretiv(struct state *st, struct pluto_crypto_req *r);
...
extern void finish_dh_secret(struct state *st, struct pluto_crypto_req *r);
...
extern void finish_dh_v2(struct state *st, struct pluto_crypto_req *r);
...
extern int unpack_v2KE(struct state *st, struct pluto_crypto_req *r,
		       chunk_t *g);
extern void unpack_KE(struct state *st, struct pluto_crypto_req *r, chunk_t *g);
extern void unpack_nonce(chunk_t *n, struct pluto_crypto_req *r);

extern void __validate_key_lengths(struct state *st, const char *vers,
				   const char *fn, int ln);
#define ikev2_validate_key_lengths(st) \
	__validate_key_lengths(st, "v2", __func__, __LINE__)
#define ikev1_validate_key_lengths(st) \
	__validate_key_lengths(st, "v1", __func__, __LINE__)

static inline void clonetowirechunk(wire_chunk_t *thespace,
				    unsigned char *space,
				    wire_chunk_t *wiretarget,
				    const void *origdat, const size_t origlen)
{
	char *gip;
