...
#define NE(x)         \
	{             \
		x, #x \
	} /* Name Entry -- shorthand for sparse_names */

static sparse_names pfkey_type_names = {
	NE(K_SADB_RESERVED),  NE(K_SADB_GETSPI),
	NE(K_SADB_UPDATE),    NE(K_SADB_ADD),
	NE(K_SADB_DELETE),    NE(K_SADB_GET),
	NE(K_SADB_ACQUIRE),   NE(K_SADB_REGISTER),
	NE(K_SADB_EXPIRE),    NE(K_SADB_FLUSH),
	NE(K_SADB_DUMP),      NE(K_SADB_X_PROMISC),
	NE(K_SADB_X_PCHANGE), NE(K_SADB_X_GRPSA),
	NE(K_SADB_X_ADDFLOW), NE(K_SADB_X_DELFLOW),
	NE(K_SADB_X_DEBUG),   NE(K_SADB_X_NAT_T_NEW_MAPPING),
	NE(K_SADB_X_PLUMBIF), NE(K_SADB_X_UNPLUMBIF),
	{ 0, sparse_end }
};

#ifdef NEVER /* not needed yet */
static sparse_names pfkey_ext_names = { NE(K_SADB_EXT_RESERVED),
					NE(K_SADB_EXT_SA),
...
static bool pfkey_get_response(pfkey_buf *buf, pfkey_seq_t seq)
{
	while (pfkey_get(buf)) {
		if (buf->msg.sadb_msg_pid == (unsigned)pid &&
			...
			pfkey_item *it = alloc_bytes(
				offsetof(pfkey_item, buf) + bl, "pfkey_item");
 ...
 */
static void
process_pfkey_acquire(pfkey_buf *buf,
		      struct sadb_ext *extensions[K_SADB_EXT_MAX + 1])
{
	...
	if (buf->msg.sadb_msg_pid == 0 /* we only wish to hear from kernel */
	    &&
	    !(ugh = src_proto == dst_proto ? NULL :
					     "src and dst protocols differ") &&
	    !(ugh = addrtypeof(src) == addrtypeof(dst) ?
			    NULL :
			    "conflicting address types") &&
	    !(ugh = addrtosubnet(src, &ours)) &&
	    !(ugh = addrtosubnet(dst, &his)))
		record_and_initiate_opportunistic(&ours, &his, 0, NULL_POLICY,
						  "%acquire-pfkey");
...
static void pfkey_async(pfkey_buf *buf)
{
	...
	if (pfkey_msg_parse(&buf->msg, NULL, extensions, EXT_BITS_OUT)) {
		plog("pfkey_async:"
		     ...
		     buf->msg.sadb_msg_len, buf->msg.sadb_msg_errno,
		...
		DBG(DBG_CONTROL | DBG_KLIPS,
		    DBG_log("pfkey_async:"
			    ...
			    sparse_val_show(pfkey_type_names,
					    buf->msg.sadb_msg_type),
			    buf->msg.sadb_msg_len, buf->msg.sadb_msg_errno,
			    buf->msg.sadb_msg_satype, buf->msg.sadb_msg_seq,
			    buf->msg.sadb_msg_pid));
...
#define ORPHAN_HOLD_PROCESSING_LIMIT 200
	...
	while (orphaned_holds != NULL && !pfkey_input_ready() && limit-- > 0)
		record_and_initiate_opportunistic(
			&orphaned_holds->ours, &orphaned_holds->his,
			orphaned_holds->transport_proto, NULL_POLICY,
			"%hold found-pfkey");
...
static bool pfkey_build(int error, const char *description,
			    ...
			    struct sadb_ext *extensions[K_SADB_EXT_MAX + 1])
{
	...
	return pfkey_build(pfkey_msg_hdr_build(&extensions[0], msg_type, satype,
					       0, ++pfkey_seq, pid),
			   description, text_said, extensions);
}

/* pfkey_build + pfkey_address_build */
static bool pfkeyext_address(u_int16_t exttype, const ip_address *address,
			     const char *description, const char *text_said,
			     struct sadb_ext *extensions[K_SADB_EXT_MAX + 1])
{
	...
	return pfkey_build(pfkey_address_build(extensions + exttype, exttype, 0,
					       0, sockaddrof(&t)),
			   description, text_said, extensions);
}

/* pfkey_build + pfkey_x_protocol_build */
static bool pfkeyext_protocol(int transport_proto, const char *description,
			      const char *text_said,
			      struct sadb_ext *extensions[K_SADB_EXT_MAX + 1])
{
	return (transport_proto == 0) ?
		       TRUE :
		       pfkey_build(pfkey_x_protocol_build(
					   extensions + K_SADB_X_EXT_PROTOCOL,
					   transport_proto),
				   description, text_said, extensions);
}
			     ...
			     pfkey_buf *response)
{
	...
	if (error != 0) {
		loglog(RC_LOG_SERIOUS,
		       "pfkey_msg_build of %s %s failed, code %d", description,
		       text_said, error);
		success = FALSE;
	} else {
...
logerr:
		 ...
		 */
				pfkey_buf b;
				pfkey_buf *bp = response != NULL ? response :
								   &b;
...
static void pfkey_register_proto(unsigned int sadb_register, unsigned satype,
				 const char *satypename)
{
	struct sadb_ext *extensions[K_SADB_EXT_MAX + 1];
	pfkey_buf pfb;

	if (!(pfkey_msg_start(sadb_register, satype, satypename, NULL,
			      extensions) &&
...
bool pfkey_raw_eroute(const ip_address *this_host, const ip_subnet *this_client,
		      const ip_address *that_host, const ip_subnet *that_client,
		      ipsec_spi_t spi, unsigned int proto UNUSED,
		      unsigned int transport_proto, enum eroute_type esatype,
		      const struct pfkey_proto_info *proto_info UNUSED,
		      time_t use_lifetime UNUSED, enum pluto_sadb_operations op,
		      const char *text_said, char *policy_label UNUSED)
{
	struct sadb_ext *extensions[K_SADB_EXT_MAX + 1];
	ip_address sflow_ska, dflow_ska, smask_ska, dmask_ska;
	...
	if (!pfkeyext_protocol(transport_proto, "pfkey_x_protocol", text_said,
			       extensions)) {
		return FALSE;
	}
...
bool pfkey_add_sa(struct kernel_sa *sa, bool replace)
{
	...
	success = pfkey_msg_start(replace ? K_SADB_UPDATE : K_SADB_ADD,
				  klips_satype, "pfkey_msg_hdr Add SA",
	...
	success = pfkey_build(
		pfkey_sa_build(&extensions[K_SADB_EXT_SA], K_SADB_EXT_SA,
	...
	success = pfkeyext_address(K_SADB_EXT_ADDRESS_SRC, sa->src,
				   "pfkey_addr_s Add SA", sa->text_said,
				   extensions);
	if (!success)
		return FALSE;
	...
	if (sa->esp_info.authkeylen != 0) {
		success = pfkey_build(
			pfkey_key_build(&extensions[K_SADB_EXT_KEY_AUTH],
					K_SADB_EXT_KEY_AUTH,
	...
	if (sa->ref != IPSEC_SAREF_NULL || sa->refhim != IPSEC_SAREF_NULL) {
		success = pfkey_build(
			pfkey_saref_build(&extensions[K_SADB_X_EXT_SAREF],
					  sa->ref, sa->refhim),
			"pfkey_key_sare Add SA", sa->text_said, extensions);
		if (!success)
			return FALSE;
	}
#endif
	...
	if (sa->esp_info.enckeylen != 0) {
		success = pfkey_build(
			pfkey_key_build(&extensions[K_SADB_EXT_KEY_ENCRYPT],
	...
	if (sa->natt_type != 0) {
		success = pfkey_build(
			pfkey_x_nat_t_type_build(
				...
				sa->natt_type),
			"pfkey_nat_t_type Add ESP SA", sa->text_said,
			extensions);
		DBG(DBG_KLIPS,
		...
		if (sa->natt_sport != 0) {
			success = pfkey_build(
				pfkey_x_nat_t_port_build(
					...
					sa->natt_sport),
				"pfkey_nat_t_sport Add ESP SA", sa->text_said,
				extensions);
			DBG(DBG_KLIPS, DBG_log("setting natt_sport to %d\n",
					       sa->natt_sport));
			if (!success)
				return FALSE;
		}
		...
		if (sa->natt_dport != 0) {
			success = pfkey_build(
				pfkey_x_nat_t_port_build(
					...
					sa->natt_dport),
				"pfkey_nat_t_dport Add ESP SA", sa->text_said,
				extensions);
			DBG(DBG_KLIPS, DBG_log("setting natt_dport to %d\n",
					       sa->natt_dport));
			if (!success)
				return FALSE;
		}
		...
		if (sa->natt_type != 0 && !isanyaddr(sa->natt_oa)) {
			success = pfkeyext_address(K_SADB_X_EXT_NAT_T_OA,
						   ...
						   sa->text_said, extensions);
			DBG(DBG_KLIPS, DBG_log("setting nat_oa to %s\n",
					       ip_str(sa->natt_oa)));
			if (!success)
				return FALSE;
		}
...
bool pfkey_grp_sa(const struct kernel_sa *sa0, const struct kernel_sa *sa1)
{
	...
	return pfkey_msg_start(K_SADB_X_GRPSA, klips_satype1,
	       ...
	       && pfkey_build(pfkey_x_satype_build(
 ...
 */
bool pfkey_shunt_eroute(struct connection *c, const struct spd_route *sr,
			...
			const char *opname)
{
	...
	if (sr->routing == RT_ROUTED_ECLIPSED && c->kind == CK_TEMPLATE) {
		...
		if (ue != NULL) {
			...
			return pfkey_shunt_eroute(
				ue, esr, RT_ROUTED_PROSPECTIVE,
				(K_SADB_X_ADDFLOW | (SADB_X_SAFLAGS_REPLACEFLOW
						     << KLIPS_OP_FLAG_SHIFT)),
				"restoring eclipsed");
		}
	}
...

	{
		...
		return pfkey_raw_eroute(&sr->this.host_addr, &sr->this.client,
					fam->any, &sr->that.client, htonl(spi),
					SA_INT, sr->this.protocol, ET_INT,
...
bool pfkey_sag_eroute(struct state *st, const struct spd_route *sr, unsigned op,
		      const char *opname)
{
...
#define PROTO_INFO_COUNT 4
	...
	if (tunnel) {
		...
		for (j = i + 1; j < PROTO_INFO_COUNT && proto_info[j].proto;
		     j++) {
	...
	return eroute_connection(NULL_STATE, sr, inner_spi, inner_proto,
				 inner_esatype, proto_info + i, op, opname,
				 NULL_POLICY);
}
...
void pfkey_scan_proc_shunts(void)
{
	...
	DBG(DBG_CONTROL, DBG_log("scanning for shunt eroutes"))
...
bool pfkey_was_eroute_idle(struct state *st, time_t idle_max)
{
	...
	if (f == NULL) { /** Can't open the file, perhaps were are on 26sec? */
		...
		while (f != NULL) {
			...
			dst = st->st_connection->spd.this
				      .host_addr; /* inbound SA */
			if (st->st_ah.present) {
				proto = SA_AH;
			...
			if (strncmp(line, text_said, strlen(text_said)) == 0) {
				...
				if (p ==
				    NULL) { /* SAs which haven't been used yet
                                         don't have it */
...
void pfkey_set_debug(int cur_debug, openswan_keying_debug_func_t debug_func,
		     openswan_keying_debug_func_t error_func)
{
#ifdef DEBUG
	pfkey_lib_debug = (cur_debug & DBG_PFKEY ? PF_KEY_DEBUG_PARSE_MAX :
						   PF_KEY_DEBUG_PARSE_NONE);
...
void pfkey_remove_orphaned_holds(int transport_proto, const ip_subnet *ours,
				 const ip_subnet *his)
{
	/*
...
bool pfkey_plumb_mast_device(int mast_dev)
{
	...
	if ((error = pfkey_msg_hdr_build(&extensions[0], K_SADB_X_PLUMBIF, 0, 0,
					 ++pfkey_seq, pid))) {
		return FALSE;
	}
