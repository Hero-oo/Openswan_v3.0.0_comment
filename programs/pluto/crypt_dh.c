...
static PK11SymKey *pk11_extract_derive_wrapper_osw(PK11SymKey *base,
						   ...
						   int keySize)
{
	...
	return PK11_Derive_osw(base, CKM_EXTRACT_KEY_FROM_KEY, &param, target,
			       operation, keySize);
}
/*
...
nss_encryption_mech(const struct ike_encr_desc *encrypter)
{
	...
	case OAKLEY_AES_CBC:
		...
		break;
	default:
		loglog(RC_LOG_SERIOUS, "NSS: Unsupported encryption mechanism");
 ...
 */
#ifdef HAVE_LIBNSS
static bool calc_dh_shared(chunk_t *shared, const chunk_t g, chunk_t secret,
			   const struct oakley_group_desc *group, chunk_t pubk)
{
	...
	remote_pubk = (SECKEYPublicKey *)PORT_ArenaZAlloc(
	...
	dhshared = PK11_PubDerive(privk, remote_pubk, PR_FALSE, NULL, NULL,
				  CKM_DH_PKCS_DERIVE,
				  CKM_CONCATENATE_DATA_AND_BASE, CKA_DERIVE,
				  group->bytes,
	...
	if (group->bytes > dhshared_len) {
		DBG(DBG_CRYPT, DBG_log("Dropped %lu leading zeros",
				       (long)group->bytes - dhshared_len));
		chunk_t zeros;
		...
		newdhshared = PK11_Derive(
			dhshared, CKM_CONCATENATE_DATA_AND_BASE, &params,
			CKM_CONCATENATE_DATA_AND_BASE, CKA_DERIVE, 0);
	...
	DBG(DBG_CRYPT,
	    DBG_log("calc_dh_shared(): time elapsed (%s): %ld usec",
		    enum_show(&oakley_group_names, group->group), tv_diff););
...
#else
static bool calc_dh_shared(chunk_t *shared, const chunk_t g, const MP_INT *sec,
...
#ifdef HAVE_LIBNSS
static void skeyid_preshared(const chunk_t pss, const chunk_t ni,
			     const chunk_t nr, const chunk_t shared_chunk,
			     const struct ike_prf_desc *hasher,
			     chunk_t *skeyid_chunk)
#else
static void skeyid_preshared(const chunk_t pss, const chunk_t ni,
			     const chunk_t nr,
			     const struct ike_prf_desc *hasher, chunk_t *skeyid)
#endif
{
	struct hmac_ctx ctx;
	...
	DBG(DBG_CRYPT,
	    ...
	    DBG_dump_chunk("ni: ", ni); DBG_dump_chunk("nr: ", nr));
	...
	PK11SymKey *tkey4 = pk11_derive_wrapper_osw(
		shared, CKM_CONCATENATE_DATA_AND_BASE, buf1_chunk,
		CKM_EXTRACT_KEY_FROM_KEY, CKA_DERIVE, 0);
	/* nss_symkey_log(tkey4, "pss+ipad+shared"); */

	CK_EXTRACT_PARAMS bs = 0;
	PK11SymKey *tkey5 = pk11_extract_derive_wrapper_osw(
		tkey4, bs, CKM_CONCATENATE_BASE_AND_DATA, CKA_DERIVE,
		HMAC_BUFSIZE);
	/* nss_symkey_log(tkey5, "pss+ipad"); */

	PK11SymKey *tkey6 =
		pk11_derive_wrapper_osw(tkey5, CKM_CONCATENATE_BASE_AND_DATA,
					nir, mechanism, CKA_DERIVE, 0);
	pfree(nir.ptr);
	...
	PK11SymKey *tkey2 = PK11_Derive_osw(tkey6, mechanism, NULL,
					    CKM_CONCATENATE_DATA_AND_BASE,
					    CKA_DERIVE, 0);
	/* nss_symkey_log(tkey2, "pss : tkey2"); */

	PK11SymKey *tkey3 =
		pk11_derive_wrapper_osw(tkey2, CKM_CONCATENATE_DATA_AND_BASE,
					buf2_chunk, mechanism, CKA_DERIVE, 0);
	skeyid = PK11_Derive_osw(tkey3, mechanism, NULL,
				 CKM_CONCATENATE_BASE_AND_DATA, CKA_DERIVE, 0);
	/* nss_symkey_log(tkey2, "pss : tkey3"); */

	skeyid_chunk->len = sizeof(PK11SymKey *);
	skeyid_chunk->ptr =
		alloc_bytes(skeyid_chunk->len, "calculated skeyid(pss)");
	...
	DBG(DBG_CRYPT, DBG_dump_chunk("NSS: st_skeyid in skeyid_preshared(): ",
				      *skeyid_chunk));
#else
	DBG(DBG_CRYPT, DBG_log("Skey inputs (PSK+NI+NR)");
	    DBG_dump_chunk("ni: ", ni); DBG_dump_chunk("nr: ", nr));
	...
	DBG(DBG_CRYPT, DBG_dump_chunk("keyid: ", *skeyid));
#endif
}

static void skeyid_digisig(const chunk_t ni, const chunk_t nr,
			   ...
			   chunk_t *skeyid_chunk)
{
	...
	DBG(DBG_CRYPT, DBG_log("skeyid inputs (digi+NI+NR+shared) hasher: %s",
			       hasher->common.name);
	    DBG_dump_chunk("shared-secret: ", shared_chunk);
	    DBG_dump_chunk("ni: ", ni); DBG_dump_chunk("nr: ", nr));
	...
	PK11SymKey *tkey1 =
		pk11_derive_wrapper_osw(shared, CKM_CONCATENATE_DATA_AND_BASE,
					buf1_chunk, mechanism, CKA_DERIVE, 0);
	PK11SymKey *tkey2 = PK11_Derive_osw(tkey1, mechanism, NULL,
					    CKM_CONCATENATE_DATA_AND_BASE,
					    CKA_DERIVE, 0);
	PK11SymKey *tkey3 =
		pk11_derive_wrapper_osw(tkey2, CKM_CONCATENATE_DATA_AND_BASE,
 ...
 * See draft-ietf-ipsec-ike-01.txt 4.1
 */
static void calc_skeyids_iv(
	struct pcr_skeyid_q *skq, chunk_t shared_chunk,
	...
	,
	chunk_t *new_iv, chunk_t *enc_key_chunk)
{
	oakley_auth_t auth = skq->auth;
	const struct ike_prf_desc *hasher =
		crypto_get_hasher(v2prf_to_integ(skq->v2_prf));
	chunk_t pss;
	chunk_t ni;
	chunk_t nr;
	chunk_t gi;
	...
	/* generate SKEYID_* from SKEYID */
	{
		...
		hmac_pad =
			hmac_pads(0x00, HMAC_BUFSIZE - hasher->hash_digest_len);
		...
		PK11SymKey *tkey1 = pk11_derive_wrapper_osw(
			skeyid, CKM_CONCATENATE_BASE_AND_DATA, hmac_pad,
			CKM_XOR_BASE_AND_DATA, CKA_DERIVE, HMAC_BUFSIZE);
     ...
     */
		...
		PK11SymKey *tkey2 = pk11_derive_wrapper_osw(
			tkey1, CKM_XOR_BASE_AND_DATA, hmac_ipad,
			CKM_CONCATENATE_BASE_AND_KEY, CKA_DERIVE, 0);
		...
		PK11SymKey *tkey3 = PK11_Derive_osw(
			tkey2, CKM_CONCATENATE_BASE_AND_KEY, &param,
			CKM_CONCATENATE_BASE_AND_DATA, CKA_DERIVE, 0);
		PR_ASSERT(tkey3 != NULL);

		PK11SymKey *tkey4 = pk11_derive_wrapper_osw(
			tkey3, CKM_CONCATENATE_BASE_AND_DATA, icookie,
			CKM_CONCATENATE_BASE_AND_DATA, CKA_DERIVE, 0);
		PR_ASSERT(tkey4 != NULL);

		PK11SymKey *tkey5 = pk11_derive_wrapper_osw(
			tkey4, CKM_CONCATENATE_BASE_AND_DATA, rcookie,
			CKM_CONCATENATE_BASE_AND_DATA, CKA_DERIVE, 0);

		PR_ASSERT(tkey5 != NULL);

		PK11SymKey *tkey6 = pk11_derive_wrapper_osw(
			tkey5, CKM_CONCATENATE_BASE_AND_DATA, hmac_zerobyte,
			nss_key_derivation_mech(hasher), CKA_DERIVE, 0);

		PR_ASSERT(tkey6 != NULL);

		PK11SymKey *tkey7 = PK11_Derive_osw(
			tkey6, nss_key_derivation_mech(hasher), NULL,
			CKM_CONCATENATE_BASE_AND_DATA, CKA_DERIVE, 0);
		PR_ASSERT(tkey7 != NULL);

		PK11SymKey *tkey8 = pk11_derive_wrapper_osw(
			tkey1, CKM_XOR_BASE_AND_DATA, hmac_opad,
			CKM_CONCATENATE_BASE_AND_KEY, CKA_DERIVE, 0);
		...
		PK11SymKey *tkey9 = PK11_Derive_osw(
			tkey8, CKM_CONCATENATE_BASE_AND_KEY, &param,
			nss_key_derivation_mech(hasher), CKA_DERIVE, 0);
		PR_ASSERT(tkey9 != NULL);

		skeyid_d = PK11_Derive_osw(tkey9,
		...
		PK11SymKey *tkey10 = PK11_Derive_osw(
			tkey2, CKM_CONCATENATE_BASE_AND_KEY, &param,
			CKM_CONCATENATE_BASE_AND_KEY, CKA_DERIVE, 0);
		...
		PK11SymKey *tkey11 = PK11_Derive_osw(
			tkey10, CKM_CONCATENATE_BASE_AND_KEY, &param,
			CKM_CONCATENATE_BASE_AND_DATA, CKA_DERIVE, 0);
		PR_ASSERT(tkey11 != NULL);

		PK11SymKey *tkey12 = pk11_derive_wrapper_osw(
			tkey11, CKM_CONCATENATE_BASE_AND_DATA, icookie,
			CKM_CONCATENATE_BASE_AND_DATA, CKA_DERIVE, 0);
		PR_ASSERT(tkey12 != NULL);

		PK11SymKey *tkey13 = pk11_derive_wrapper_osw(
			tkey12, CKM_CONCATENATE_BASE_AND_DATA, rcookie,
			CKM_CONCATENATE_BASE_AND_DATA, CKA_DERIVE, 0);
		PR_ASSERT(tkey13 != NULL);

		PK11SymKey *tkey14 = pk11_derive_wrapper_osw(
			tkey13, CKM_CONCATENATE_BASE_AND_DATA, hmac_val1,
			nss_key_derivation_mech(hasher), CKA_DERIVE, 0);
		PR_ASSERT(tkey14 != NULL);

		PK11SymKey *tkey15 = PK11_Derive_osw(
			tkey14, nss_key_derivation_mech(hasher), NULL,
			CKM_CONCATENATE_BASE_AND_DATA, CKA_DERIVE, 0);
		...
		PK11SymKey *tkey16 = PK11_Derive_osw(
			tkey8, CKM_CONCATENATE_BASE_AND_KEY, &param,
			nss_key_derivation_mech(hasher), CKA_DERIVE, 0);
		PR_ASSERT(tkey16 != NULL);

		skeyid_a = PK11_Derive_osw(tkey16,
					   nss_key_derivation_mech(hasher),
					   NULL, CKM_CONCATENATE_BASE_AND_DATA,
					   CKA_DERIVE, 0);
		...
		PK11SymKey *tkey17 = PK11_Derive_osw(
			tkey2, CKM_CONCATENATE_BASE_AND_KEY, &param,
			CKM_CONCATENATE_BASE_AND_KEY, CKA_DERIVE, 0);
		...
		PK11SymKey *tkey18 = PK11_Derive_osw(
			tkey17, CKM_CONCATENATE_BASE_AND_KEY, &param,
			CKM_CONCATENATE_BASE_AND_DATA, CKA_DERIVE, 0);
		PR_ASSERT(tkey18 != NULL);

		PK11SymKey *tkey19 = pk11_derive_wrapper_osw(
			tkey18, CKM_CONCATENATE_BASE_AND_DATA, icookie,
			CKM_CONCATENATE_BASE_AND_DATA, CKA_DERIVE, 0);
		PR_ASSERT(tkey19 != NULL);

		PK11SymKey *tkey20 = pk11_derive_wrapper_osw(
			tkey19, CKM_CONCATENATE_BASE_AND_DATA, rcookie,
			CKM_CONCATENATE_BASE_AND_DATA, CKA_DERIVE, 0);
		PR_ASSERT(tkey20 != NULL);

		PK11SymKey *tkey21 = pk11_derive_wrapper_osw(
			tkey20, CKM_CONCATENATE_BASE_AND_DATA, hmac_val2,
			nss_key_derivation_mech(hasher), CKA_DERIVE, 0);
		PR_ASSERT(tkey21 != NULL);

		PK11SymKey *tkey22 = PK11_Derive_osw(
			tkey21, nss_key_derivation_mech(hasher), NULL,
			CKM_CONCATENATE_BASE_AND_DATA, CKA_DERIVE, 0);
		...
		PK11SymKey *tkey23 = PK11_Derive_osw(
			tkey8, CKM_CONCATENATE_BASE_AND_KEY, &param,
			nss_key_derivation_mech(hasher), CKA_DERIVE, 0);
     ...
     * See RFC 2409 "IKE" Appendix B*/
		...
		if (keysize <= hasher->hash_digest_len) {
			skeyid_e = PK11_Derive_osw(
				tkey23, nss_key_derivation_mech(hasher), NULL,
				CKM_EXTRACT_KEY_FROM_KEY, CKA_DERIVE, 0);
			PR_ASSERT(skeyid_e != NULL);
			/* nss_symkey_log(skeyid_e, "skeyid_e"); */

			enc_key = PK11_DeriveWithFlags(
				skeyid_e, CKM_EXTRACT_KEY_FROM_KEY, &param1,
				nss_encryption_mech(encrypter), CKA_FLAGS_ONLY,
				keysize, CKF_ENCRYPT | CKF_DECRYPT);
			...
			PK11SymKey *tkey25 = pk11_derive_wrapper_osw(
				skeyid_e, CKM_CONCATENATE_BASE_AND_DATA,
				hmac_pad, CKM_XOR_BASE_AND_DATA, CKA_DERIVE,
				HMAC_BUFSIZE);
			PR_ASSERT(tkey25 != NULL);

			PK11SymKey *tkey26 = pk11_derive_wrapper_osw(
				tkey25, CKM_XOR_BASE_AND_DATA, hmac_ipad,
				CKM_CONCATENATE_BASE_AND_DATA, CKA_DERIVE, 0);
			PR_ASSERT(tkey26 != NULL);

			PK11SymKey *tkey27 = pk11_derive_wrapper_osw(
				tkey26, CKM_CONCATENATE_BASE_AND_DATA,
				hmac_zerobyte, nss_key_derivation_mech(hasher),
				CKA_DERIVE, 0);
			PR_ASSERT(tkey27 != NULL);

			PK11SymKey *tkey28 = PK11_Derive_osw(
				tkey27, nss_key_derivation_mech(hasher), NULL,
				CKM_CONCATENATE_BASE_AND_DATA, CKA_DERIVE, 0);
			PR_ASSERT(tkey28 != NULL);

			PK11SymKey *tkey29 = pk11_derive_wrapper_osw(
				tkey25, CKM_XOR_BASE_AND_DATA, hmac_opad,
				CKM_CONCATENATE_BASE_AND_KEY, CKA_DERIVE, 0);
			...
			PK11SymKey *tkey30 = PK11_Derive_osw(
				tkey29, CKM_CONCATENATE_BASE_AND_KEY, &param,
				nss_key_derivation_mech(hasher), CKA_DERIVE, 0);
			PR_ASSERT(tkey30 != NULL);

			PK11SymKey *tkey31 = PK11_Derive_osw(
				tkey30, nss_key_derivation_mech(hasher), NULL,
				CKM_CONCATENATE_BASE_AND_KEY, CKA_DERIVE, 0);
			...
			PK11SymKey *tkey32 = pk11_derive_wrapper_osw(
				skeyid_e, CKM_CONCATENATE_BASE_AND_DATA,
				hmac_pad, CKM_XOR_BASE_AND_DATA, CKA_DERIVE,
				HMAC_BUFSIZE);
			PR_ASSERT(tkey32 != NULL);

			PK11SymKey *tkey33 = pk11_derive_wrapper_osw(
				tkey32, CKM_XOR_BASE_AND_DATA, hmac_ipad,
				CKM_CONCATENATE_BASE_AND_KEY, CKA_DERIVE, 0);
			PR_ASSERT(tkey33 != NULL);

			PK11SymKey *tkey36 = pk11_derive_wrapper_osw(
				tkey32, CKM_XOR_BASE_AND_DATA, hmac_opad,
				CKM_CONCATENATE_BASE_AND_KEY, CKA_DERIVE, 0);
			...
			for (;;) {
				...
				PK11SymKey *tkey34 = PK11_Derive_osw(
					tkey33, CKM_CONCATENATE_BASE_AND_KEY,
					&param, nss_key_derivation_mech(hasher),
					CKA_DERIVE, 0);
				PR_ASSERT(tkey34 != NULL);

				PK11SymKey *tkey35 = PK11_Derive_osw(
					tkey34, nss_key_derivation_mech(hasher),
					NULL, CKM_CONCATENATE_BASE_AND_DATA,
					CKA_DERIVE, 0);
				...
				PK11SymKey *tkey37 = PK11_Derive_osw(
					tkey36, CKM_CONCATENATE_BASE_AND_KEY,
					&param, nss_key_derivation_mech(hasher),
					CKA_DERIVE, 0);
				PR_ASSERT(tkey37 != NULL);

				PK11SymKey *tkey38 = PK11_Derive_osw(
					tkey37, nss_key_derivation_mech(hasher),
					NULL, CKM_CONCATENATE_BASE_AND_KEY,
					CKA_DERIVE, 0);
				...
				if (i >= keysize) {
					...
					PK11SymKey *tkey39 = PK11_Derive_osw(
						keymat,
					...
					DBG(DBG_CRYPT,
					    DBG_log("NSS: Freed 25-39 symkeys\n"));
					break;
				} else {
					keyhandle =
						PK11_GetSymKeyHandle(tkey38);
					...
					DBG(DBG_CRYPT,
					    DBG_log("NSS: Freed symkeys 31 34 35 37\n"));
				}
			} /*end for*/
		} /*end else skeyid_e */
		...
		skeyid_d_chunk->ptr =
			alloc_bytes(skeyid_d_chunk->len, "calculated skeyid_d");
		...
		skeyid_a_chunk->ptr =
			alloc_bytes(skeyid_a_chunk->len, "calculated skeyid_a");
		...
		skeyid_e_chunk->ptr =
			alloc_bytes(skeyid_e_chunk->len, "calculated skeyid_e");
		...
		enc_key_chunk->ptr =
			alloc_bytes(enc_key_chunk->len, "calculated enc_key");
	...
	/* generate SKEYID_* from SKEYID */
	{
		...
		hmac_final_chunk(*skeyid_d_chunk,
				 "st_skeyid_d_chunk in generate_skeyids_iv()",
				 &ctx);
		...
		hmac_final_chunk(*skeyid_a_chunk,
				 "st_skeyid_a_chunk in generate_skeyids_iv()",
				 &ctx);
		...
		hmac_final_chunk(*skeyid_e_chunk,
				 "st_skeyid_e_chunk in generate_skeyids_iv()",
				 &ctx);
	}
	...
	/* generate IV */
	{
		...
		DBG(DBG_CRYPT, DBG_dump_chunk("DH_i:", gi);
	...
	DBG(DBG_CRYPT, DBG_dump_chunk("Skeyid:  ", *skeyid_chunk);
...
void calc_dh_iv(struct pluto_crypto_req *r)
{
...
#ifndef HAVE_LIBNSS
	pluto_crypto_allocchunk(&skr->thespace, &skr->shared, group->bytes);
...
#ifndef HAVE_LIBNSS
	DBG(DBG_CRYPT, DBG_dump_chunk("long term secret: ", ltsecret));
	...
	calc_skeyids_iv(&dhq, shared, dhq.keysize, &skeyid, &skeyid_d,
			&skeyid_a, &skeyid_e, &new_iv, &enc_key);
...
static void calc_skeyseed_v2(struct pcr_skeyid_q *skq, chunk_t shared,
			     const size_t keysize, chunk_t *skeyseed,
			     chunk_t *SK_d, chunk_t *SK_ai, chunk_t *SK_ar,
			     chunk_t *SK_ei, chunk_t *SK_er, chunk_t *SK_pi,
			     chunk_t *SK_pr)
{
	...
	chunk_t hmac_opad, hmac_ipad, hmac_pad_prf,
		counter; /*hmac_pad_integ, hmac_zerobyte, hmac_val1, hmac_val2;*/
	CK_OBJECT_HANDLE keyhandle;
	SECItem param, param1;
	DBG(DBG_CRYPT, DBG_log("NSS: Started key computation\n"));

	PK11SymKey *skeyseed_k, *SK_d_k, *SK_ai_k, *SK_ar_k, *SK_ei_k, *SK_er_k,
		*SK_pi_k, *SK_pr_k;
#endif
	...
	if(vpss.prf_hasher->hash_key_size == 0) {
		...
		/* SKEYSEED */
		DBG(DBG_CRYPT, DBG_dump("Input to SKEYSEED: ", kb, keybytes));
	...
	/* now we have to generate the keys for everything */
	{
		...
		const struct ike_integ_desc *integ_hasher =
			ike_alg_get_integ(skq->v2_integ);
#ifdef HAVE_LIBNSS
		...
		vpss.t.len = 0;
		total_keysize =
			skd_bytes + (2 * (ska_bytes + ske_bytes + skp_bytes));

		DBG(DBG_CRYPT, DBG_log("PRF+ input");
		...
		PK11SymKey *tkey1 = pk11_derive_wrapper_osw(
			skeyseed_k, CKM_CONCATENATE_BASE_AND_DATA, hmac_pad_prf,
			CKM_XOR_BASE_AND_DATA, CKA_DERIVE, HMAC_BUFSIZE);
		PR_ASSERT(tkey1 != NULL);
		...
		for (;;) {
			...
			if (vpss.counter[0] == 0x01) {
				...
				tkey3 = pk11_derive_wrapper_osw(
					tkey2, CKM_CONCATENATE_BASE_AND_DATA,
					vpss.ni, CKM_CONCATENATE_BASE_AND_DATA,
					CKA_DERIVE, 0);
				PK11_FreeSymKey(tkey2);
			} else {
				PK11SymKey *tkey2 = pk11_derive_wrapper_osw(
				...
				PK11SymKey *tkey12 = PK11_Derive_osw(
					tkey2, CKM_CONCATENATE_BASE_AND_KEY,
					&param, CKM_CONCATENATE_BASE_AND_DATA,
					CKA_DERIVE, 0);
				PR_ASSERT(tkey12 != NULL);

				tkey3 = pk11_derive_wrapper_osw(
					tkey12, CKM_CONCATENATE_BASE_AND_DATA,
					vpss.ni, CKM_CONCATENATE_BASE_AND_DATA,
					CKA_DERIVE, 0);
				PK11_FreeSymKey(tkey2);
				PK11_FreeSymKey(tkey12);
			...
			PK11SymKey *tkey4 = pk11_derive_wrapper_osw(
				tkey3, CKM_CONCATENATE_BASE_AND_DATA, vpss.nr,
				CKM_CONCATENATE_BASE_AND_DATA, CKA_DERIVE, 0);
			PR_ASSERT(tkey4 != NULL);

			PK11SymKey *tkey5 = pk11_derive_wrapper_osw(
				tkey4, CKM_CONCATENATE_BASE_AND_DATA, vpss.spii,
				CKM_CONCATENATE_BASE_AND_DATA, CKA_DERIVE, 0);
			PR_ASSERT(tkey5 != NULL);

			PK11SymKey *tkey6 = pk11_derive_wrapper_osw(
				tkey5, CKM_CONCATENATE_BASE_AND_DATA, vpss.spir,
				CKM_CONCATENATE_BASE_AND_DATA, CKA_DERIVE, 0);
			PR_ASSERT(tkey6 != NULL);

			PK11SymKey *tkey7 = pk11_derive_wrapper_osw(
				tkey6, CKM_CONCATENATE_BASE_AND_DATA, counter,
				nss_key_derivation_mech(hasher), CKA_DERIVE, 0);
			PR_ASSERT(tkey7 != NULL);

			PK11SymKey *tkey8 = PK11_Derive_osw(
				tkey7, nss_key_derivation_mech(hasher), NULL,
				CKM_CONCATENATE_BASE_AND_DATA, CKA_DERIVE, 0);
			PR_ASSERT(tkey8 != NULL);

			PK11SymKey *tkey9 = pk11_derive_wrapper_osw(
				tkey1, CKM_XOR_BASE_AND_DATA, hmac_opad,
				CKM_CONCATENATE_BASE_AND_KEY, CKA_DERIVE, 0);
			PR_ASSERT(tkey9 != NULL);
			...
			PK11SymKey *tkey10 = PK11_Derive_osw(
				tkey9, CKM_CONCATENATE_BASE_AND_KEY, &param,
				nss_key_derivation_mech(hasher), CKA_DERIVE, 0);
			PR_ASSERT(tkey10 != NULL);

			if (vpss.counter[0] == 0x01) {
				finalkey = PK11_Derive_osw(
					tkey10, nss_key_derivation_mech(hasher),
					NULL, CKM_CONCATENATE_BASE_AND_KEY,
					CKA_DERIVE, 0);
				PR_ASSERT(finalkey != NULL);

				tkey11 = PK11_Derive_osw(
					tkey10, nss_key_derivation_mech(hasher),
					NULL, CKM_CONCATENATE_BASE_AND_KEY,
					CKA_DERIVE, 0);
				PR_ASSERT(tkey11 != NULL);
		...
		CK_EXTRACT_PARAMS bs = 0;
		SK_d_k = pk11_extract_derive_wrapper_osw(
			finalkey, bs, CKM_CONCATENATE_BASE_AND_DATA, CKA_DERIVE,
			skd_bytes);

		bs = skd_bytes * BITS_PER_BYTE;
		SK_ai_k = pk11_extract_derive_wrapper_osw(
			finalkey, bs, CKM_CONCATENATE_BASE_AND_DATA, CKA_DERIVE,
			ska_bytes);

		bs = (skd_bytes + ska_bytes) * BITS_PER_BYTE;
		SK_ar_k = pk11_extract_derive_wrapper_osw(
			finalkey, bs, CKM_CONCATENATE_BASE_AND_DATA, CKA_DERIVE,
			ska_bytes);
		...
		SK_ei_k = PK11_DeriveWithFlags(
			finalkey, CKM_EXTRACT_KEY_FROM_KEY, &param1,
			nss_encryption_mech(encrypter), CKA_FLAGS_ONLY,
			ske_bytes, CKF_ENCRYPT | CKF_DECRYPT);

		bs = (skd_bytes + (2 * ska_bytes) + ske_bytes) * BITS_PER_BYTE;
		param1.data = (unsigned char *)&bs;
		param1.len = sizeof(bs);
		SK_er_k = PK11_DeriveWithFlags(
			finalkey, CKM_EXTRACT_KEY_FROM_KEY, &param1,
			nss_encryption_mech(encrypter), CKA_FLAGS_ONLY,
			ske_bytes, CKF_ENCRYPT | CKF_DECRYPT);
		...
		bs = (skd_bytes + (2 * ska_bytes) + (2 * ske_bytes) +
		      skp_bytes) *
		     BITS_PER_BYTE;
		SK_pr_k = pk11_extract_derive_wrapper_osw(
			finalkey, bs, CKM_CONCATENATE_BASE_AND_DATA, CKA_DERIVE,
			skp_bytes);
	...
	DBG(DBG_CRYPT, DBG_dump_chunk("shared:  ", shared);
	    DBG_dump_chunk("skeyseed:", *skeyseed);
	    DBG_dump_chunk("SK_d:", *SK_d); DBG_dump_chunk("SK_ai:", *SK_ai);
	    DBG_dump_chunk("SK_ar:", *SK_ar); DBG_dump_chunk("SK_ei:", *SK_ei);
	    DBG_dump_chunk("SK_er:", *SK_er); DBG_dump_chunk("SK_pi:", *SK_pi);
...
void calc_dh_v2(struct pluto_crypto_req *r)
{
	...
	/* okay, so now calculate IV */
	calc_skeyseed_v2(&dhq, shared, dhq.keysize, &skeyseed, &SK_d, &SK_ai,
