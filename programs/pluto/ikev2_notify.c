...
{
	SHA1_CTX srchash;
	unsigned char thingstohash[COOKIE_SIZE /*spiI*/ + COOKIE_SIZE /*spiR*/ +
				   16 /*IPaddr*/ + 2 /*port*/];
	unsigned char *next = thingstohash;
	unsigned char *addrptr;
	unsigned int addrlen;

	memcpy(next, cookie_i, COOKIE_SIZE);
	next += COOKIE_SIZE;
	memcpy(next, cookie_r, COOKIE_SIZE);
	next += COOKIE_SIZE;

	addrlen = addrbytesptr(&addr, &addrptr);
	memcpy(next, addrptr, addrlen);
	next += addrlen;
	...
	DBG(DBG_EMITTING | DBG_PARSING | DBG_CONTROLMORE,
	    DBG_dump("nat chunk", thingstohash, (next - thingstohash)));
...
stf_status process_nat_payload(struct state *st, struct msg_digest *md,
			       ...
			       chunk_t *data)
{
	...
	calculate_nat_hash(md->hdr.isa_icookie, md->hdr.isa_rcookie, *addr,
			   port, digest);
	...
	DBG(DBG_PARSING,
	    DBG_dump_chunk("calculated nat-t  h", calculated_hash));

	if (same_chunk(*data, calculated_hash)) {
		DBG(DBG_PARSING | DBG_CONTROLMORE,
		    DBG_log("nat-t payloads for %s match: no NAT",
			    payload_name));
	} else {
		...
		case v2N_NAT_DETECTION_SOURCE_IP:
			...
			loglog(RC_COMMENT,
			       "detected that they are NATed at: %s:%u",
			       addrbuf, port);
			st->hidden_variables.st_nat_traversal |=
...
stf_status ikev2_process_notifies(struct state *st, struct msg_digest *md)
{
	...
	for (p = md->chain[ISAKMP_NEXT_v2N]; p != NULL; p = p->next) {
		payload_name = enum_name(&ikev2_notify_names,
		...
		if (payload_name == NULL) {
			if (p->payload.v2n.isan_type < v2N_INITIAL_CONTACT) {
				...
				payload_name =
					enum_show(&ikev2_notify_names,
						  p->payload.v2n.isan_type);
				payload_name_buf[0] = '\0';
				strncat(payload_name_buf, payload_name,
					sizeof(payload_name_buf) - 1);
		...
		case v2N_ESP:
			...
			if (pbs_left(data_pbs) < p->payload.v2n.isan_spisize) {
				loglog(RC_LOG_SERIOUS,
				       "notify payload %s received with too small spisize: %lu < %u (dropped)",
				       payload_name,
				       (long unsigned)pbs_left(data_pbs),
				       p->payload.v2n.isan_spisize);
			}
			setchunk(spi, data_pbs->cur,
				 p->payload.v2n.isan_spisize);
		...
		case v2N_NAT_DETECTION_DESTINATION_IP:
		case v2N_NAT_DETECTION_SOURCE_IP:
			process_nat_payload(st, md, p, payload_name,
					    p->payload.v2n.isan_type, &data);
			break;

		default:
			loglog(RC_LOG,
			       "received (ignored) notify: %s (spisize=%u, data=%u)",
			       payload_name, (unsigned int)spi.len,
...
bool ship_v2N(unsigned int np, u_int8_t critical, u_int8_t protoid,
	      chunk_t *spi, u_int16_t type, chunk_t *n_data, pb_stream *rbody)
{
