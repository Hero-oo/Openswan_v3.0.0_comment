...
#endif
		,
		pluto_shared_secrets_file, &pass, NULL);
}
...
{
	whack_log(RC_COMMENT, "using secrets file: %s",
			 ...
			 void *uservoid UNUSED)
{
	...
	whack_log(RC_COMMENT, "    %d: %s %s %s%s",
		  osw_get_secretlineno(secret), kind, idb1, idb2, more);
 ...
 */
static struct secret *osw_get_secret(const struct connection *c,
				     const struct id *my_id,
				     const struct id *his_id,
				     enum PrivateKeyKind kind, bool asym)
{
	char idme[IDTOA_BUF], idhim[IDTOA_BUF], idhim2[IDTOA_BUF];
	...
	DBG(DBG_CONTROL,
	    DBG_log("started looking for secret for %s->%s of kind %s", idme,
		    idhim, enum_name(&ppk_names, kind)));

	/* is there a certificate assigned to this connection? */
	if (kind == PPK_RSA && c->spd.this.sendcert != cert_forcedtype &&
		...
		osw_public_key *my_public_key =
			allocate_RSA_public_key(c->spd.this.cert);
	...
	if (his_id_was_instantiated(c) && (!(c->policy & POLICY_AGGRESSIVE)) &&
		...
		DBG(DBG_CONTROL, DBG_log("instantiating him to 0.0.0.0"));
...
#endif
#ifdef NAT_TRAVERSAL
	else if ((c->policy & POLICY_PSK) && (kind == PPK_PSK) &&
		 (((c->kind == CK_TEMPLATE) &&
		  ...
		  ((c->kind == CK_INSTANCE) &&
		   ...
		   && (isanyaddr(&c->spd.that.host_addr))))) {
		DBG(DBG_CONTROL, DBG_log("replace him to 0.0.0.0"));
	...
	DBG(DBG_CONTROL,
	    DBG_log("actually looking for secret for %s->%s of kind %s", idme,
		    idhim2, enum_name(&ppk_names, kind)));

	best = osw_find_secret_by_id(pluto_secrets, kind, my_id,
				     c->spd.this.key1, c->spd.this.key2, his_id,
				     asym);
 ...
 */
struct secret *osw_get_xauthsecret(const struct connection *c UNUSED,
				   char *xauthname)
{
	struct secret *best = NULL;
	struct id xa_id;
	...
	best = osw_find_secret_by_id(pluto_secrets, PPK_XAUTH, &xa_id, NULL,
				     NULL, NULL, TRUE);
...
{
	struct secret *s = osw_get_secret(c, &c->spd.this.id, &c->spd.that.id,
					  PPK_PSK, FALSE);
...
#ifdef DEBUG
	DBG(DBG_PRIVATE, if (s == NULL) DBG_log("no Preshared Key Found");
	    else DBG_dump_chunk("Preshared Key", pks->u.preshared_secret););
#endif
...
#ifdef DEBUG
	DBG(DBG_PRIVATE, if (s == NULL) DBG_log("no RSA key Found");
	    else DBG_log("rsa key %s found", pks->pub->u.rsa.keyid););
#endif
...
struct pubkey *osw_get_public_key_by_end(struct end *him)
{
	...
	for (p = pluto_pubkeys; p != NULL; p = *pp) {
		...
		if (key->alg == PUBKEY_ALG_RSA && same_id(&him->id, &key->id) &&
...
err_t add_public_key(const struct id *id, enum dns_auth_level dns_auth_level,
		     enum pubkey_alg alg, const chunk_t *key,
...
void list_public_keys(bool utc, bool check_pub_keys)
{
	...
	while (p != NULL) {
		...
		if (key->alg == PUBKEY_ALG_RSA) {
			...
			check_expiry_msg = check_expiry(
				key->until_time, PUBKEY_WARNING_INTERVAL, TRUE);
			...
			if (!check_pub_keys ||
				...
				datatot(key->key_ckaid, sizeof(key->key_ckaid),
					'G', ckaid_print_buf,
					sizeof(ckaid_print_buf));

				whack_log(
					RC_COMMENT,
					"%s, %4d RSA %s key %s/%s (%s private key), until %s %s",
					timetoa(&key->installed_time, utc,
						installed_buf,
						sizeof(installed_buf)),
					8 * key->u.rsa.k,
					enum_name(&dns_auth_level_names,
						  key->dns_auth_level),
					key->u.rsa.keyid, ckaid_print_buf,
				...
				whack_log(RC_COMMENT, "       %s '%s'",
					  enum_show(&ident_names, key->id.kind),
					  id_buf);
