			      ...
			      unsigned char *idhash, pb_stream *a_pbs)
{
	...
	ikev2_calculate_sighash(st, role, idhash, st->st_firstpacket_me,
	...
	DBG(DBG_CRYPT, DBG_dump("v2rsa octets", signed_octets, signed_len));

	{
		u_char sig_val[RSA_MAX_OCTETS];

		/* now generate signature blob */
		sign_hash(pks, signed_octets, signed_len, sig_val, sz);
			   ...
			   struct pubkey *kr, struct state *st)
{
	...
	if (sig_len != k->k) {
		DBG_log("sig_len: %u != k->k: %u", (unsigned int)sig_len,
			(unsigned int)k->k);
		return "1"
		       "SIG length does not match public key length";
	}

	if ((e = verify_signed_hash(k, s, sizeof(s), &sig,
				    hash_len + der_digestinfo_len, sig_val,
				    sig_len)) != NULL) {
		return e;
	}
	...
	DBG(DBG_CRYPT, DBG_dump("v2rsa decrypted SIG:", sig, hash_len);
	    DBG_dump("v2rsa computed hash:", hash_val, hash_len););

	if (memcmp(sig, hash_val, hash_len) != 0) {
		return "9"
		       "authentication failure: received SIG does not match computed HASH, but message is well-formed";
	}
