 ...
 *
 */
extern stf_status ikev2parent_outI1(int whack_sock, struct connection *c,
				    struct state *predecessor,
				    so_serial_t *newstateno, lset_t policy,
				    unsigned long try,
				    enum crypto_importance importance,
				    struct xfrm_user_sec_ctx_ike *uctx);

extern stf_status ipsec_outI1(int whack_sock, struct state *isakmp_sa,
			      struct connection *c, lset_t policy,
			      unsigned long try, so_serial_t replacing,
			      struct xfrm_user_sec_ctx_ike *uctx);

extern stf_status
ikev2child_outC1(int whack_sock, struct state *parentst, struct connection *c,
		 lset_t policy, unsigned long try /* how many attempts so far */
		 ,
		 so_serial_t replacing,
		 struct xfrm_user_sec_ctx_ike *uctx UNUSED);

extern stf_status ikev2parent_outI1_withstate(
	struct state *st, int whack_sock, struct connection *c,
	struct state *predecessor, lset_t policy,
	unsigned long try /* how many attempts so far */
	,
	enum crypto_importance importance, struct xfrm_user_sec_ctx_ike *uctx);

extern void ikev2_delete_out(struct state *st);

extern bool ikev2_out_attr(unsigned int type, unsigned long val,
			   struct_desc *attr_desc,
			   enum_names **attr_val_descs USED_BY_DEBUG,
			   pb_stream *pbs);

extern bool ikev2_out_sa(pb_stream *outs, unsigned int protoid,
			 struct db_sa *sadb, struct state *st, bool parentSA,
			 u_int8_t np);
...
#define SEND_V2_NOTIFICATION_XCHG_DATA(md, st, xchg, type, data)               \
	{                                                                      \
		if (st)                                                        \
			send_v2_notification_from_state(st, xchg, type, data); \
		else                                                           \
			send_v2_notification_from_md(md, xchg, type, data);    \
	}
...
extern stf_status accept_v2_KE(struct msg_digest *md, struct state *st,
			       chunk_t *ke, const char *name);
...
extern bool ikev2_calculate_rsa_sha1(struct state *st, enum phase1_role role,
				     unsigned char *idhash, pb_stream *a_pbs);

extern bool ikev2_calculate_psk_auth(struct state *st, enum phase1_role role,
				     unsigned char *idhash, pb_stream *a_pbs);

extern stf_status ikev2_verify_rsa_sha1(struct state *st, enum phase1_role role,
...
extern stf_status ikev2_verify_psk_auth(struct state *st, enum phase1_role role,
					struct IDhost_pair *hp,
					unsigned char *idhash,
					pb_stream *sig_pbs);

extern stf_status ikev2_emit_ipsec_sa(struct msg_digest *md, pb_stream *outpbs,
				      unsigned int np, struct connection *c,
				      lset_t policy);

extern stf_status ikev2_derive_child_keys(struct state *st,
					  enum phase1_role role);

extern int ikev2_evaluate_connection_fit(
	struct connection *d, struct state *st, struct spd_route *sr,
	enum phase1_role role, struct traffic_selector *tsi,
	struct traffic_selector *tsr, unsigned int tsi_n, unsigned int tsr_n);

extern int ikev2_evaluate_connection_port_fit(
	const struct connection *d, const struct spd_route *sr,
	enum phase1_role role, const struct traffic_selector *tsi,
	const struct traffic_selector *tsr, int tsi_n, int tsr_n,
	int *best_tsi_i, int *best_tsr_i);

extern int ikev2_evaluate_connection_protocol_fit(
	const struct connection *d, const struct spd_route *sr,
	enum phase1_role role, const struct traffic_selector *tsi,
	const struct traffic_selector *tsr, int tsi_n, int tsr_n,
	int *best_tsi_i, int *best_tsr_i);

extern stf_status ikev2_emit_ts(struct msg_digest *md, pb_stream *outpbs,
				unsigned int np, struct traffic_selector *ts);

extern stf_status ikev2_calc_emit_ts(struct msg_digest *md, pb_stream *outpbs,
				     enum phase1_role role,
				     struct connection *c0, lset_t policy);
...
extern stf_status
ikev2_child_ts_evaluate(struct traffic_selector tsi[16], unsigned int tsi_n,
			struct traffic_selector tsr[16], unsigned int tsr_n,
			enum phase1_role role, struct state *pst,
			struct connection *c, struct connection **b_result,
...
extern void calculate_nat_hash(const unsigned char cookie_i[COOKIE_SIZE],
			       ...
			       const ip_address addr, const unsigned short port,
			       unsigned char digest[SHA1_DIGEST_SIZE]);

extern stf_status process_nat_payload(struct state *st, struct msg_digest *md,
				      struct payload_digest *p,
				      const char *payload_name,
				      v2_notification_t notify_type,
				      chunk_t *data);

extern stf_status ikev2_process_notifies(struct state *st,
					 struct msg_digest *md);

extern void ikev2_enable_nat_keepalives(struct state *st);

extern bool doi_send_ikev2_certreq_thinking(struct state *st,
					    enum phase1_role role);
...
extern stf_status ikev2_send_certreq(struct state *st, struct msg_digest *md,
				     enum phase1_role role, pb_stream *outpbs);
extern stf_status ikev2_send_cert(struct state *st, struct msg_digest *md,
				  enum phase1_role role, pb_stream *outpbs);
extern bool ship_v2N(unsigned int np, u_int8_t critical, u_int8_t protoid,
		     chunk_t *spi, u_int16_t type, chunk_t *n_data,
		     pb_stream *rbody);

extern bool justship_v2KE(struct state *st UNUSED, chunk_t *g,
			  unsigned int oakley_group, pb_stream *outs,
			  u_int8_t np);
extern bool justship_v2Nonce(struct state *st, pb_stream *outpbs,
...
extern unsigned char *ikev2_authloc(struct msg_digest *md, pb_stream *e_pbs);

extern stf_status
ikev2_encrypt_msg(struct msg_digest *md, enum phase1_role init,
...
extern stf_status allocate_msgid_from_parent(struct state *pst,
					     msgid_t *newid_p);
...
extern void ikev2_calculate_sighash(struct state *st, enum phase1_role role,
				    unsigned char *idhash, chunk_t firstpacket,
				    unsigned char *sig_octets);

extern bool spdb_v2_match_parent(struct db_sa *sadb, unsigned propnum,
				 unsigned encr_transform, int encr_keylen,
				 unsigned integ_transform,
				 unsigned prf_transform, unsigned dh_transform);

extern bool spdb_v2_match_child(struct db_sa *sadb, unsigned propnum,
				unsigned encr_transform, int encr_keylen,
				unsigned integ_transform,
