...
void lock_certs_and_keys(const char *who)
{
	...
	DBG(DBG_CONTROLMORE, DBG_log("certs and keys locked by '%s'", who))
}
...
{
	DBG(DBG_CONTROLMORE, DBG_log("certs and keys unlocked by '%s'", who))
...
void lock_authcert_list(const char *who)
{
	...
	DBG(DBG_CONTROLMORE, DBG_log("authcert list locked by '%s'", who))
}
...
{
	DBG(DBG_CONTROLMORE, DBG_log("authcert list unlocked by '%s'", who))
...
void lock_crl_list(const char *who)
{
	...
	DBG(DBG_CONTROLMORE, DBG_log("crl list locked by '%s'", who))
}
...
{
	DBG(DBG_CONTROLMORE, DBG_log("crl list unlocked by '%s'", who))
...
extern void lock_ocsp_cache(const char *who)
{
	...
	DBG(DBG_CONTROLMORE, DBG_log("ocsp cache locked by '%s'", who))
}
...
{
	DBG(DBG_CONTROLMORE, DBG_log("ocsp cache unlocked by '%s'", who))
			...
			chunk_t *blob LIBCURL_UNUSED)
{
	...
	if (curl != NULL) {
		...
		*(uri + url.len) = '\0';
		...
		DBG(DBG_CONTROL, DBG_log("Trying cURL '%s'", uri))
	...
	if (entry != NULL) {
		...
		if (attr != NULL) {
			struct berval **values =
				ldap_get_values_len(ldap, entry, attr);
...
static err_t fetch_ldap_url(chunk_t url, chunk_t *blob)
{
	...
	DBG(DBG_CONTROL, DBG_log("Trying LDAP URL '%s'", ldap_url))
	...
	if (rc == LDAP_SUCCESS) {
		...
		if (ldap != NULL) {
			int ldap_version = (LDAP_VER == 2) ? LDAP_VERSION2 :
							     LDAP_VERSION3;
...
#else
static err_t fetch_ldap_url(chunk_t url UNUSED, chunk_t *blob UNUSED)
{
...
static err_t fetch_asn1_blob(chunk_t url, chunk_t *blob)
{
	...
	if (is_asn1(*blob)) {
		DBG(DBG_PARSING, DBG_log("  fetched blob coded in DER format"))
	} else {
		bool pgp = FALSE;
...
static void fetch_ocsp_status(ocsp_location_t *location LIBCURL_UNUSED)
{
#ifdef LIBCURL
	chunk_t request;
	chunk_t response = empty_chunk;
	...
	DBG(DBG_CONTROL, DBG_log("sending ocsp request to location '%.*s'",
				 (int)location->uri.len, location->uri.ptr))
	DBG(DBG_RAW, DBG_dump_chunk("OCSP request", request))
	...
	if (curl != NULL) {
		...
		*(uri + location->uri.len) = '\0';
		...
		if (res == CURLE_OK) {
			DBG(DBG_CONTROL, DBG_log("received ocsp response"))
			DBG(DBG_RAW, DBG_dump_chunk("OCSP response", response))
			parse_ocsp(location, response);
		} else {
			plog("failed to fetch ocsp status (%s): %s", uri,
			     errorbuffer);
		}
...
static void fetch_thread(void *arg UNUSED)
{
	...
	DBG(DBG_CONTROL, DBG_log("fetch thread started"))
	...
	while (1) {
		...
		status = pthread_cond_timedwait(
			&fetch_wake_cond, &fetch_wake_mutex, &wait_interval);
		...
		if (status == ETIMEDOUT) {
			...
			DBG(DBG_CONTROL, DBG_log("fetch thread was woken up"))
		}
		fetch_ocsp();
		fetch_crls();
	}
...
void init_fetch(void)
{
	...
	if (crl_check_interval > 0) {
		...
		if (status != 0) {
			plog("fetching thread could not be started, status = %d",
			     status);
		}
	}
}
...
void add_crl_fetch_request(chunk_t issuer, const generalName_t *gn)
{
	...
	*req = empty_fetch_req;
	...
	DBG(DBG_CONTROL, DBG_log("crl fetch request added"))
	unlock_crl_fetch_list("add_crl_fetch_request");
}
