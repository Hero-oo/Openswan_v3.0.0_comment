...
extern bool justship_nonce(chunk_t *n, pb_stream *outs, u_int8_t np,
			   const char *name);

/* calls previous two routines */
extern bool ship_nonce(chunk_t *n, struct pluto_crypto_req *r, pb_stream *outs,
		       u_int8_t np, const char *name);

extern notification_t accept_v1_nonce(struct msg_digest *md, chunk_t *dest,
				      const char *name);

extern bool justship_KE(chunk_t *g, pb_stream *outs, u_int8_t np);

/* just calls previous two routines now */
extern bool ship_KE(struct state *st, struct pluto_crypto_req *r, chunk_t *g,
		    pb_stream *outs, u_int8_t np);

/* **MAIN MODE FUNCTIONS** in ikev1_main.c */
extern stf_status main_outI1(int whack_sock, struct connection *c,
			     struct state *predecessor, so_serial_t *newstateno,
			     lset_t policy, unsigned long try,
			     enum crypto_importance importance,
			     struct xfrm_user_sec_ctx_ike *uctx);

extern stf_status aggr_outI1(int whack_sock, struct connection *c,
			     struct state *predecessor, so_serial_t *newstateno,
			     lset_t policy, unsigned long try,
			     enum crypto_importance importance,
			     struct xfrm_user_sec_ctx_ike *uctx);

extern stf_status aggr_not_present(int whack_sock, struct connection *c,
...
#ifdef HAVE_LIBNSS
extern void main_mode_hash_body(struct state *st, bool hashi /* Initiator? */
				,
...
extern err_t try_RSA_signature_v1(const u_char hash_val[MAX_DIGEST_LEN],
...
extern void key_continue(struct adns_continuation *cr, err_t ugh,
			 key_tail_fn *tail);

extern stf_status oakley_id_and_auth(
...
extern bool do_command(struct connection *c, const struct spd_route *sr,
		       const char *verb, struct state *st);
#endif

extern bool out_sa(pb_stream *outs, struct db_sa *sadb, struct state *st,
		   bool oakley_mode, enum phase1_role role,
		   bool aggressive_mode, u_int8_t np);
...
extern notification_t parse_ipsec_sa_body(
	...
	struct state *st); /* current state object */
/* spdb_v1_struct.c */
extern struct db_sa *ikev1_alg_makedb(lset_t policy UNUSED,
				      struct alg_info_ike *ei, bool oneproposal,
