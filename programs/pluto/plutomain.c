...
static void usage(const char *mess)
{
	...
	exit(mess == NULL ?
		     0 :
		     1); /* not exit_pluto because we are not initialized yet */
}
...
#define DBG_OFFSET 256
		static const struct option long_opts[] = {
			...
			{ "debug-private", no_argument, NULL,
			  DBG_PRIVATE + DBG_OFFSET },
			{ "debug-pfkey", no_argument, NULL,
			  DBG_PFKEY + DBG_OFFSET },
...
#else
				openswan_log(
					"Labelled IPsec not enabled; value %ld ignored.",
					value);
#endif
			}
			continue;
	 ...
	 */
		...
		case 'b': /* --ctlbase <path> */
			...
			if (snprintf(ctl_addr.sun_path,
				     sizeof(ctl_addr.sun_path), "%s%s", ctlbase,
				     CTL_SUFFIX) == -1)
				usage("<path>" CTL_SUFFIX
				      " too long for sun_path");
			if (snprintf(info_addr.sun_path,
				     sizeof(info_addr.sun_path), "%s%s",
				     ctlbase, INFO_SUFFIX) == -1)
				usage("<path>" INFO_SUFFIX
				      " too long for sun_path");
			if (snprintf(pluto_lock, sizeof(pluto_lock), "%s%s",
				     ctlbase, LOCK_SUFFIX) == -1)
...
#undef DBG_OFFSET
	...
	if (coredir)
		if (chdir(coredir) == -1) {
			int e = errno;
			openswan_log(
				"pluto: chdir() do dumpdir failed (%d %s)\n", e,
				strerror(e));
		}
     ...
     */
	{
		...
		openswan_log(
			"Starting Pluto (Openswan Version %s%s; Vendor ID %s) pid:%u",
	     ...
	     * stuff too.
	     */
			openswan_log("@(#) built on " __DATE__ ":" __TIME__
				     " by " BUILDER);
		}
	}
...
#ifdef HAVE_STATSD
	openswan_log(
		"HAVE_STATSD notification via /bin/openswan-statsd enabled");
#else
...
#ifdef NAT_TRAVERSAL
	init_nat_traversal(nat_traversal, keep_alive, force_keepalive,
			   nat_t_spf);
#endif
