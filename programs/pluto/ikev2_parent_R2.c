			    ...
			    struct pluto_crypto_req *r)
{
	...
	hp = find_ID_host_pair(st->ikev2.st_local_id, st->ikev2.st_peer_id);
	...
	if (hp == NULL) {
		loglog(RC_LOG_SERIOUS,
		       "No policy for initiator with id=%s (me:%s)",
...

	{
		...
		DBG(DBG_CRYPT,
		    DBG_dump_chunk("parent SA IDi pi verify", st->st_skey_pi));
		DBG(DBG_CRYPT,
		    DBG_dump("parent SA I2 idhash verify", idstart, idlen));
	...
	/* process CERT payload */
	{
		if (md->chain[ISAKMP_NEXT_v2CERT]) {
			/* should we check if we should accept a cert payload ?
                 ...
                 */
			openswan_log(
				"v2_CERT received on reponder, attempting to validate");
	...
	if (md->chain[ISAKMP_NEXT_v2CERTREQ]) {
		...
		ikev2_decode_cr(md,
				&st->st_connection->ikev2_requested_ca_hashes);
	...
	case v2_AUTH_RSA: {
		stf_status authstat = ikev2_verify_rsa_sha1(
			st, RESPONDER, hp, idhash_in, NULL /* keys from DNS */
			,
	...
	default:
		openswan_log("authentication method: %s not supported",
			     enum_name(&ikev2_auth_names,
				       md->chain[ISAKMP_NEXT_v2AUTH]
					       ->payload.v2a.isaa_type));
		return STF_FATAL;
	}
	...
	DBG(DBG_CONTROL,
	    DBG_log("PARENT SA now authenticated, building child and reply"));
	...
	/* send response */
	{
		...
		if (send_cert) {
			stf_status certstat = ikev2_send_cert(st, md, RESPONDER,
							      &e_pbs_cipher);
			if (certstat != STF_OK)
				return certstat;

			/* CERTREQ was fulfiled, don't send again */
			if (st->st_connection->spd.this.sendcert ==
			    cert_sendifasked)
		...
		DBG(DBG_CONTROLMORE, DBG_log("going to assemble AUTH payload"));

		/* now send AUTH payload */
		{
			stf_status authstat = ikev2_send_auth(
				c, st, RESPONDER, idhash_out, &e_pbs_cipher);
			if (authstat != STF_OK)
				return authstat;
		}
		...
		if (child_SA_present) {
			...
			if (ret > STF_FAIL) {
				...
				ship_v2N(ISAKMP_NEXT_NONE,
					 ISAKMP_PAYLOAD_CRITICAL, PROTO_ISAKMP,
					 NULL, ret - STF_FAIL, NULL,
					 &e_pbs_cipher);
			} else if (ret != STF_OK) {
...

		{
			...
			ret = ikev2_encrypt_msg(md, RESPONDER, authstart, iv,
