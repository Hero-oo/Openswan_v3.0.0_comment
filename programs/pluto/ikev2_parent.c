...
static bool ikev2_get_dcookie(u_char *dcookie, chunk_t st_ni, ip_address *addr,
			      u_int8_t *spiI);
 ...
 */
int unpack_v2KE(struct state *st, struct pluto_crypto_req *r, chunk_t *g)
{
 ...
 * used by IKEv2: parent, child (PFS)
 */
bool justship_v2KE(struct state *st UNUSED, chunk_t *g,
		   unsigned int oakley_group, pb_stream *outs, u_int8_t np)
{
	...
	if (!g->len) {
		loglog(RC_FATAL,
		       "ikev2 g^x is not initialized (zero length), aborting");
		return FALSE;
	}
...
static bool ship_v2KE(struct state *st, struct pluto_crypto_req *r, chunk_t *g,
...
bool justship_v2Nonce(struct state *st UNUSED, pb_stream *outpbs,
		      chunk_t *nonce, unsigned int np)
{
	...
	if (DBGP(IMPAIR_SEND_BOGUS_ISAKMP_FLAG)) {
		openswan_log(
			" setting bogus ISAKMP_PAYLOAD_OPENSWAN_BOGUS flag in ISAKMP payload");
...
bool justship_v2nat(struct state *st, pb_stream *outpbs)
{
	...
	calculate_nat_hash(st->st_icookie, st->st_rcookie, st->st_localaddr,
			   st->st_localport, digest);
	...
	calculate_nat_hash(st->st_icookie, st->st_rcookie, st->st_remoteaddr,
			   st->st_remoteport, digest);
	setchunk(hash_chunk, digest, SHA1_DIGEST_SIZE);
	success = ship_v2N(ISAKMP_NEXT_NONE, ISAKMP_PAYLOAD_NONCRITICAL,
...
void ikev2_padup_pre_encrypt(struct msg_digest *md, pb_stream *e_pbs_cipher)
{
...
unsigned char *ikev2_authloc(struct msg_digest *md, pb_stream *e_pbs)
{
	unsigned char *b12;
	struct state *st = md->st;
	struct state *pst = st;
...
stf_status ikev2_encrypt_msg(struct msg_digest *md, enum phase1_role init,
			     unsigned char *authstart, unsigned char *iv,
			     unsigned char *encstart, unsigned char *authloc,
			     pb_stream *e_pbs UNUSED, pb_stream *e_pbs_cipher)
{
	struct state *st = md->st;
	struct state *pst = st;
	...
	if (authloc < authstart) {
		loglog(RC_CRYPTOFAILED,
		       "ikev2 encrypt internal error: authloc<authstart. Please report.");
		return STF_FAIL;
	}

	if (init == INITIATOR) {
		DBG(DBG_CONTROLMORE,
		    DBG_log("encrypting as INITIATOR, parent SA #%lu",
			    pst->st_serialno));
		cipherkey = &pst->st_skey_ei;
		authkey = &pst->st_skey_ai;
	} else {
		DBG(DBG_CONTROLMORE,
		    DBG_log("encrypting as RESPONDER, parent SA #%lu",
			    pst->st_serialno));
	...
	/* encrypt the block */
	{
		...
		/* now, encrypt */
		(pst->st_oakley.encrypter->do_crypt)(encstart, cipherlen,
						     cipherkey->ptr,
						     cipherkey->len, savediv,
						     TRUE);
	...
	/* okay, authenticate from beginning of IV */
	{
		...
		DBG(DBG_PARSING, DBG_dump("data being hmac:", authstart,
					  authloc - authstart);
...
stf_status ikev2_decrypt_msg(struct msg_digest *md, enum phase1_role init)
{
	struct state *st = md->st;
	...
	if (init == INITIATOR) {
		DBG(DBG_CONTROLMORE,
		    DBG_log("decrypting as INITIATOR, using RESPONDER keys"));
		cipherkey = &pst->st_skey_er;
		authkey = &pst->st_skey_ar;
	} else {
		DBG(DBG_CONTROLMORE,
		    DBG_log("decrypting as RESPONDER, using INITIATOR keys"));
		cipherkey = &pst->st_skey_ei;
		authkey = &pst->st_skey_ai;
	}
	...
	{
		unsigned char *b12 =
			alloca(pst->st_oakley.integ_hasher->hash_digest_len);
		struct hmac_ctx ctx;
		...
		if (st->st_interface != md->iface) {
			DBG(DBG_CONTROL,
			    DBG_log("changing iface from %s:%u to %s:%u",
				    st->st_interface->addrname,
				    st->st_interface->port, md->iface->addrname,
				    md->iface->port));
	...
	/* decrypt */
	{
		...
		/* now, decrypt */
		(pst->st_oakley.encrypter->do_crypt)(encstart, enclen,
						     cipherkey->ptr,
						     cipherkey->len, iv, FALSE);
		...
		DBG(DBG_CRYPT, DBG_dump("decrypted payload:", encstart, enclen);
...
static stf_status ikev2_send_auth(struct connection *c, struct state *st,
				  enum phase1_role role,
				  unsigned char *idhash_out, pb_stream *outpbs)
{
	struct ikev2_a a;
	pb_stream a_pbs;
	struct state *pst = st;
...
static void ikev2_update_nat_ports(struct state *st)
{
	if (st->hidden_variables.st_nat_traversal & NAT_T_DETECTED) {
		if (st->st_remoteport == pluto_port500) {
			loglog(RC_COMMENT,
			       "NAT-T detected, moving to port 4500");
			st->st_remoteport = pluto_port4500;
		}

		/* now pick a new local interface definition for sending traffic out of */
		st->st_interface = pick_matching_interfacebyfamily(
			interfaces, pluto_port4500,
		...
		if (st->st_interface) {
			...
			loglog(RC_COMMENT,
			       "failed to find port 4500 interface");
		}
	}
}
			      ...
			      u_int8_t *spiI)
{
	size_t addr_length;
	SHA1_CTX ctx_sha1;
	unsigned char addr_buff[sizeof(union {
		struct in_addr A;
		struct in6_addr B;
	})];
	...
	SHA1Update(&ctx_sha1, ikev2_secret_of_the_day, SHA1_DIGEST_SIZE);
	SHA1Final(dcookie, &ctx_sha1);
	DBG(DBG_PRIVATE, DBG_log("ikev2 secret_of_the_day used %s, length %d",
				 ikev2_secret_of_the_day, SHA1_DIGEST_SIZE););
...
void send_v2_notification(struct state *p1st, enum isakmp_xchg_types xchg_type,
			  notification_t ntf_type, u_char *icookie,
			  u_char *rcookie, chunk_t *notify_data)
{
	pb_stream reply;
	pb_stream rbody;
	...
	zero(reply_buffer);
	init_pbs(&reply, reply_buffer, sizeof(reply_buffer),
		 "notification msg");
	...
	/* HDR out */
	{
		...
		zero(&n_hdr);
			/* default to 0 */ /* AAA should we copy from MD? */
		...
		n_hdr.isa_flags = ISAKMP_FLAGS_R |
				  IKEv2_ORIG_INITIATOR_FLAG(p1st);
...
int send_v2_notification_enc(struct msg_digest *md,
			     enum isakmp_xchg_types xchg_type,
			     notification_t ntf_type, chunk_t *notify_data)

{
	...
	/* HDR out */
	{
         ...
         * the parent SA.
         */
		r_hdr.isa_flags = ISAKMP_FLAGS_R | ISAKMP_FLAGS_E |
	...
	/* send Notification */
	ret = ship_v2N(ISAKMP_NEXT_NONE, ISAKMP_PAYLOAD_NONCRITICAL,
		       PROTO_ISAKMP, NULL, ntf_type, notify_data,
		       &e_pbs_cipher);
	if (!ret)
		return STF_INTERNAL_ERROR;
...

	{
		...
		ret = ikev2_encrypt_msg(md, i_am, authstart, iv, encstart,
...
stf_status process_informational_ikev2(struct msg_digest *md)
{
...

	{
		...
		if (IKEv2_MSG_FROM_INITIATOR(md->hdr.isa_flags)) {
			...
			if (!out_struct(&e, &ikev2_e_desc, &md->rbody,
					&e_pbs)) {
				return STF_INTERNAL_ERROR;
			}
			...
			if (md->chain[ISAKMP_NEXT_v2D]) {
				for (p = md->chain[ISAKMP_NEXT_v2D]; p != NULL;
					...
					case PROTO_IPSEC_ESP: {
						...
						for (i = 0;
							...
							spi = p->pbs.cur +
							      (i *
							       v2del->isad_spisize);
							DBG(DBG_CONTROLMORE,
							    DBG_log("received delete request for %s SA(0x%08lx)",
								    enum_show(
									    &protocol_names,
									    v2del->isad_protoid),
								    (unsigned long)ntohl(
							...
							struct state *dst = find_state_ikev2_child_to_delete(
								st->st_icookie,
								st->st_rcookie,
								v2del->isad_protoid,
								*(ipsec_spi_t *)
						...
						if (!j) {
							DBG(DBG_CONTROLMORE,
							    DBG_log("This delete payload does not contain a single spi that has any local state, ignoring"));
							return STF_IGNORE;
						} else {
							DBG(DBG_CONTROLMORE,
							    DBG_log("Number of SPIs to be sent %d",
								    j);
							    DBG_dump(
								    " Emit SPIs",
								    spi_buf,
		...
		if (md->chain[ISAKMP_NEXT_v2D]) {
			for (p = md->chain[ISAKMP_NEXT_v2D]; p != NULL;
				...
				if (!st) {
					DBG(DBG_CONTROLMORE,
					    DBG_log("received delete request for %s but parent ISAKMP SA already deleted; ignoring",
						    enum_show(
							    &protocol_names,
							    v2del->isad_protoid)));
					continue;
				...
				case PROTO_ISAKMP:
					...
					DBG(DBG_CONTROLMORE,
					    DBG_log("received delete request for %s via #%ld; deleting #%ld",
						    enum_show(
							    &protocol_names,
							    v2del->isad_protoid),
						    md->st->st_serialno,
				...
				case PROTO_IPSEC_ESP: {
					...
					for (i = 0; i < v2del->isad_nrspi;
					     i++) {
						spi = p->pbs.cur +
						      (i * v2del->isad_spisize);
						DBG(DBG_CONTROLMORE,
						    DBG_log("Now doing actual deletion for request: %s SA(0x%08lx)",
							    enum_show(
								    &protocol_names,
								    v2del->isad_protoid),
							    (unsigned long)ntohl(
						...
						if (dst != NULL) {
							struct ipsec_proto_info *pr =
								v2del->isad_protoid ==
									...
									&dst->st_esp;
							DBG(DBG_CONTROLMORE,
							    DBG_log("received delete request for #%lu via #%lu, our %s SA spi: 0x%08lx",
								    dst->st_serialno,
								    st->st_serialno,
								    enum_show(
									    &protocol_names,
...

	{
		...
		if (ret != STF_OK) {
			loglog(RC_LOG_SERIOUS,
			       "can not allocate new msgid, delete not sent");
			goto end;
		}
...

		{
			...
			ret = ikev2_encrypt_msg(&md, role, authstart, iv,
						encstart, authloc, &e_pbs,
						&e_pbs_cipher);
			if (ret != STF_OK)
				goto end;
		}
