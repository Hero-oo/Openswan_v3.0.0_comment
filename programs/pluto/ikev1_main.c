		      ...
		      struct xfrm_user_sec_ctx_ike *uctx)
{
	...
	if (predecessor == NULL)
		...
		openswan_log("initiating Main Mode to replace #%lu",
			     predecessor->st_serialno);
	 ...
	 */
		...
		if (!out_sa(&md.rbody, st->st_sadb, st, TRUE, INITIATOR, FALSE,
			    np)) {
		...
		passert(st->st_p1isa.ptr ==
			NULL); /* no leak!  (MUST be first time) */
...
#ifdef NAT_TRAVERSAL
	DBG(DBG_NATT,
	    DBG_log("nat traversal enabled: %d", nat_traversal_enabled));
	if (nat_traversal_enabled) {
		int np = --numvidtosend > 0 ? ISAKMP_NEXT_VID :
					      ISAKMP_NEXT_NONE;
	...
	clonetochunk(st->st_tpacket, reply_stream.start,
		     pbs_offset(&reply_stream), "reply packet for main_outI1");
...
tpm_ignore:
	...
	if (predecessor != NULL) {
		...
		whack_log(RC_NEW_STATE + STATE_MAIN_I1, "%s: initiate",
			  enum_name(&state_names, st->st_state));
	}
	reset_cur_state();
	return STF_OK;
...
#ifdef HAVE_LIBNSS
void main_mode_hash_body(struct state *st, bool hashi /* Initiator? */
			 ,
...
void main_mode_hash_body(struct state *st, bool hashi /* Initiator? */
			 ...
			 const pb_stream *idpl /* ID payload, as PBS */
			 ,
			 union hash_ctx *ctx, hash_update_t hash_update_void)
#endif
{
#ifndef HAVE_LIBNSS
...
#define hash_update_chunk(ctx, ch) hash_update((ctx), (ch).ptr, (ch).len)
     ...
     */
#ifdef HAVE_LIBNSS
	hmac_update(ctx, idpl->start + sizeof(struct isakmp_generic),
		    pbs_offset(idpl) - sizeof(struct isakmp_generic));
#else
	hash_update(ctx, idpl->start + sizeof(struct isakmp_generic),
		    pbs_offset(idpl) - sizeof(struct isakmp_generic));
#endif
...
main_mode_hash(
	struct state *st, u_char *hash_val /* resulting bytes */
	,
 ...
 */
size_t RSA_sign_hash(struct connection *c, u_char sig_val[RSA_MAX_OCTETS],
...
#endif
				   ,
				   gateways_from_dns, try_RSA_signature_v1);
}
...
bool encrypt_message(pb_stream *pbs, struct state *st)
{
	...
	DBG(DBG_CRYPT,
	    DBG_log("encrypting %d using %s", (unsigned int)enc_len,
		    enum_show(&oakley_enc_names,
			      st->st_oakley.encrypter->common.ikev1_algo_id)));
...
stf_status main_inI1_outR1(struct msg_digest *md)
{
...
#if defined(openpgp_peer)
	{
		...
		for (p = md->chain[ISAKMP_NEXT_VID]; p != NULL; p = p->next) {
			int vid_len = sizeof(pgp_vendorid) - 1 <
						      pbs_left(&p->pbs) ?
					      sizeof(pgp_vendorid) - 1 :
					      pbs_left(&p->pbs);
	...
	c = find_host_connection(ANY_MATCH, &md->iface->ip_addr, pluto_port500,
				 KH_IPADDR, &md->sender, md->sender_port,
				 LEMPTY, POLICY_IKEV1_DISABLE, &policy_hint);
	 ...
	 */
		{
			struct connection *d;
			d = find_host_connection(
				ANY_MATCH, &md->iface->ip_addr, pluto_port500,
				KH_ANY, (ip_address *)NULL, md->sender_port,
				policy, POLICY_IKEV1_DISABLE, &policy_hint);
		...
		if (c == NULL) {
			...
			loglog(RC_LOG_SERIOUS,
			       ...
			       ip_str(&md->iface->ip_addr), pluto_port500,
			       c->name);
			/* XXX notification is in order! */
			return STF_IGNORE;
		} else {
	     ...
	     */
			DBG(DBG_CONTROL,
			    DBG_log("instantiating \"%s\" for initial Main Mode message received on %s:%u",
				    c->name, ip_str(&md->iface->ip_addr),
				    pluto_port500));
	...
	st->st_policy = c->policy &
			~POLICY_IPSEC_MASK; /* only as accurate as connection */
		 ...
		 "reply packet");
	{
		...
		r_hdr.isa_flags &=
			~ISAKMP_FLAG_COMMIT; /* we won't ever turn on this bit */
	...
	/* start of SA out */
	{
		...
		r_sa.isasa_np = numvidtosend ? ISAKMP_NEXT_VID :
					       ISAKMP_NEXT_NONE;
	...
	if (SEND_PLUTO_VID || openpgp_peer) {
		char *vendorid = (openpgp_peer) ? pgp_vendorid : pluto_vendorid;
...
#ifdef NAT_TRAVERSAL
	DBG(DBG_NATT,
	    DBG_log("sender checking NAT-T: %d and %d", nat_traversal_enabled,
...
stf_status main_inR1_outI2(struct msg_digest *md)
{
	...
	/* verify echoed SA */
	{
		...
		RETURN_STF_FAILURE(parse_isakmp_sa_body(
			&sapd->pbs, &sapd->payload.sa, NULL, TRUE, st));
	}
...

	{
		struct ke_continuation *ke =
			alloc_thing(struct ke_continuation, "outI2 KE");
		ke->md = md;
 ...
 */
bool justship_KE(chunk_t *g, pb_stream *outs, u_int8_t np)
{
	return out_generic_chunk(np, &isakmp_keyex_desc, outs, *g,
				 "keyex value");
}

bool ship_KE(struct state *st, struct pluto_crypto_req *r, chunk_t *g,
				       ...
				       struct pluto_crypto_req *r)
{
	...
	/* KE out */
	if (!ship_KE(st, r, &st->st_gi, &md->rbody, ISAKMP_NEXT_NONCE))
		return STF_INTERNAL_ERROR;

#ifdef DEBUG
	/* Ni out */
	if (!ship_nonce(&st->st_ni, r, &md->rbody,
			(cur_debugging & IMPAIR_BUST_MI2) ? ISAKMP_NEXT_VID :
							    ISAKMP_NEXT_NONE,
			"Ni"))
		return STF_INTERNAL_ERROR;
...
#ifdef NAT_TRAVERSAL
	DBG(DBG_NATT,
	    DBG_log("NAT-T checking st_nat_traversal for NAT_T_WITH_NATD"));
...
stf_status main_inI2_outR2(struct msg_digest *md)
{
...
#ifdef NAT_TRAVERSAL
	DBG(DBG_NATT,
	    DBG_log("inI2: checking NAT-T: %d and %d", nat_traversal_enabled,
		    st->hidden_variables.st_nat_traversal));
	...
	{
		struct ke_continuation *ke =
			alloc_thing(struct ke_continuation, "inI2_outR2 KE");
				...
				struct pluto_crypto_req *r)
{
	...
	/* KE out */
	if (!ship_KE(st, r, &st->st_gr, &md->rbody, ISAKMP_NEXT_NONCE)) {
		osw_abort();
		return STF_INTERNAL_ERROR;
	}
...
#ifdef DEBUG
	{
		...
		if (!ship_nonce(&st->st_nr, r, &md->rbody, next_payload, "Nr"))
			return STF_INTERNAL_ERROR;
     ...
     */
		struct dh_continuation *dh = alloc_thing(
			struct dh_continuation, "main_inI2_outR2_tail");
		stf_status e;
		...
		e = start_dh_secretiv(&dh->dh_pcrc, st, st->st_import,
				      RESPONDER, st->st_oakley.group->group);
...
static void doi_log_cert_thinking(struct msg_digest *md UNUSED, u_int16_t auth,
				  enum ipsec_cert_type certtype,
				  enum certpolicy policy, bool gotcertrequest,
				  bool send_cert)
{
	DBG(DBG_CONTROL,
	    DBG_log("thinking about whether to send my certificate:"));

	DBG(DBG_CONTROL, DBG_log("  I have RSA key: %s cert.type: %s ",
					   ...
					   struct pluto_crypto_req *r)
{
	...
	doi_log_cert_thinking(md, st->st_oakley.auth, mycert.type,
	...
	DBG(DBG_CONTROL, DBG_log(" I am %ssending a certificate request",
				 send_cr ? "" : "not "));
	...
	if (send_cert) {
		...
		cert_hd.isacert_np = (send_cr) ? ISAKMP_NEXT_CR :
						 ISAKMP_NEXT_SIG;
	...
	/* HASH_I or SIG_I out */
	{
		...
		if (auth_payload == ISAKMP_NEXT_HASH) {
			...
			size_t sig_len = RSA_sign_hash(
				st->st_connection, sig_val, hash_val, hash_len);

			if (sig_len == 0) {
				loglog(RC_LOG_SERIOUS,
				       "unable to locate my private key for RSA Signature  (IKEv1 mainmode initiator)");
				return STF_FAIL + AUTHENTICATION_FAILED;
			}

			if (!out_generic_raw(ISAKMP_NEXT_NONE,
					     &isakmp_signature_desc, &md->rbody,
					     sig_val, sig_len, "SIG_I"))
				return STF_INTERNAL_ERROR;
		}
	}
...
stf_status main_inR2_outI3(struct msg_digest *md)
{
	...
	return start_dh_secretiv(&dh->dh_pcrc, st, st->st_import, INITIATOR,
				 st->st_oakley.group->group);
}
 ...
 */
stf_status oakley_id_and_auth(
	struct msg_digest *md, bool initiator /* are we the Initiator? */
	,
...
)
{
	...
	case OAKLEY_RSA_SIG:
		r = RSA_check_signature(
			st, hash_val, hash_len, &md->chain[ISAKMP_NEXT_SIG]->pbs
#ifdef USE_KEYRR
			,
		...
		if (r == STF_SUSPEND) {
			...
			struct key_continuation *nkc = alloc_thing(
				struct key_continuation, "key continuation");
			enum key_oppo_step step_done = kc == NULL ? kos_null :
								    kc->step;
			err_t ugh;
 ...
 * to try.
 */
void key_continue(struct adns_continuation *cr, err_t ugh, key_tail_fn *tail)
{
...
#endif
			kc->last_ugh =
				ugh; /* record previous error in case we need it */
			r = (*tail)(kc->md, kc);
		}
				       ...
				       struct key_continuation *kc)
{
	...
	/* HASH_R or SIG_R out */
	{
		...
		if (auth_payload == ISAKMP_NEXT_HASH) {
			...
			if (sig_len == 0) {
				loglog(RC_LOG_SERIOUS,
				       "unable to locate my private key for RSA Signature  (IKEv1 mainmode responder)");
				return STF_FAIL + AUTHENTICATION_FAILED;
			}
	...
	DBG_cond_dump(DBG_CRYPT,
		      "last encrypted block of Phase 1:", st->st_new_iv,
		      st->st_new_iv_len);
				 ...
				 struct key_continuation *kc)
{
	...
	{
		stf_status r =
			main_id_and_auth(md, TRUE, main_inR3_continue, kc);
...
stf_status send_isakmp_notification(struct state *st, u_int16_t type,
				    const void *data, size_t len)
{
...

	{
		...
		DBG(DBG_CRYPT, DBG_log("HASH computed:");
...
static void send_notification(struct state *sndst, u_int16_t type,
			      ...
			      u_char *icookie, u_char *rcookie, u_char *spi,
			      size_t spisize, u_char protoid)
{
	u_char buffer[1024];
	...
	case PAYLOAD_MALFORMED:
		...
		openswan_DBG_dump("payload malformed after IV", sndst->st_iv,
				  sndst->st_iv_len);
	...
	openswan_log("sending %snotification %s to %s:%u",
		     ...
		     ip_str(&sndst->st_remoteaddr), sndst->st_remoteport);
	...
	/* Notification Payload */
	{
		...
		if (!out_struct(&isan, &isakmp_notification_desc, &r_hdr_pbs,
				&not_pbs)) {
			openswan_log(
	...
	if (encst) {
		...
		DBG(DBG_CRYPT, DBG_log("HASH(1) computed:");
		    DBG_dump("", r_hashval, ctx.hmac_digest_len);)
	}
				  ...
				  u_int16_t type)
{
	...
	if (IS_QUICK(state)) {
		...
		send_notification(st, type, NULL, 0, st->st_icookie,
				  st->st_rcookie, NULL, 0, PROTO_ISAKMP);
	}
}
...
void send_notification_from_md(struct msg_digest *md, u_int16_t type)
{
	...
	send_notification(&st, type, NULL, 0, md->hdr.isa_icookie,
			  md->hdr.isa_rcookie, NULL, 0, PROTO_ISAKMP);
}
...
void ikev1_delete_out(struct state *st)
{
	...
	if (isakmp_sa) {
		...
		while (ns != said) {
			...
			ns--;
			isad.isad_doi = ISAKMP_DOI_IPSEC;
			isad.isad_np = ns == said ? ISAKMP_NEXT_NONE :
						    ISAKMP_NEXT_D;
		   ...
		   struct payload_digest *p)
{
	...
	for (i = 0; i < d->isad_nospi; i++) {
	     ...
	     * ISAKMP
	     */
			struct state *dst = find_state_ikev1(
				spi /*iCookie*/
				,
	     ...
	     */
			bool bogus;
			struct state *dst = find_phase2_state_to_delete(
				st, d->isad_protoid,
			...
			if (dst == NULL) {
				loglog(RC_LOG_SERIOUS,
				       ...
				       (unsigned long)ntohl((unsigned long)*(
					       ipsec_spi_t *)spi),
				       bogus ? "our SPI - bogus implementation" :
					       "maybe expired");
			} else {
