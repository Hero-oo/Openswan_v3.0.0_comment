					 ...
					 size_t minlen, size_t maxlen)
{
	...
	do {
		...
		if (m == -1) {
			if (errno != EINTR) {
				openswan_log("Input error on pipe: %s",
					     strerror(errno));
...
static enum helper_exit_status write_pipe(int fd, const unsigned char *stuff)
{
	...
	do {
		...
		if (m == -1) {
			...
			if (errno != EINTR) {
				openswan_log("Output error from master: %s",
					     strerror(errno));
...
#define SIZEOF_PREAMBLE \
	sizeof(ai->ai_addrlen) + sizeof(ai->ai_protocol) + sizeof(ai->ai_family)
int serialize_addr_info(struct addrinfo *result, u_char *ansbuf, int ansbuf_len)
{
	volatile unsigned int size_left = ansbuf_len;
	struct addrinfo *ai;

#define SERIALIZE_THING_LEN(thing, thing_size)     \
	do {                                       \
		memcpy(ansbuf, thing, thing_size); \
		ansbuf += thing_size;              \
		size_left -= thing_size;           \
	} while (0)
 ...
 */
struct addrinfo *deserialize_addr_info(u_char *ansbuf, int ansbuf_len)
{
	unsigned int size_left = ansbuf_len;
	struct addrinfo *ai1, *ai, **ainext;

#define DESERIALIZE_THING_LEN(thing, thing_size)   \
	do {                                       \
	...
	while (size_left >= SIZEOF_PREAMBLE) {
		...
		if (size_left >= t1.ai_addrlen &&
			...
			*ai1 = t1;
			...
			ai1->ai_addr = alloc_bytes(ai1->ai_addrlen,
						   "addrinfo sockaddr");
			if (ai->ai_addr) {
				DESERIALIZE_THING_LEN(ai1->ai_addr,
						      ai1->ai_addrlen);
			}
		}
		if (ai1->ai_addr == NULL) {
			openswan_log(
				"failed to allocated %d bytes in deserialize_addr_info",
				ai->ai_addrlen);
			break;
		}
		ainext = &ai1->ai_next;
...
static int worker(int qfd, int afd)
{
	...
	for (;;) {
		...
		case ns_t_txt:
		case ns_t_key:
			a.result = res_nquery(statp, q.name_buf, ns_c_in,
					      q.type, a.ans, sizeof(a.ans));
			switch (h_errno) {
			case NO_DATA:
		...
		case ns_t_a:
			...
			hints.ai_family =
				q.addr_family; /* Allow IPv4 or IPv6 */
			...
			switch (s) {
			case 0: /* success! */
				a.result = serialize_addr_info(result, a.ans,
							       ADNS_ANS_SIZE);
				break;
			...
			default:
				openswan_log(
					"adns lookup: %s a/aaaa lookup error: %s",
...
static bool spawn_worker(void)
{
	...
	if (p == -1) {
		...
		if (wi_roof == wi) {
			openswan_log("fork(2) error creating first worker: %s",
				     strerror(errno));
			exit(HES_FORK);
		}
		close(qfds[0]);
...
static void query(void)
{
	...
	r = read_pipe(PLUTO_QFD, (unsigned char *)&q->aq, sizeof(q->aq),
		      sizeof(q->aq));
...
static void answer(struct worker_info *w)
{
	...
	enum helper_exit_status r = read_pipe(w->afd, (unsigned char *)&a,
					      offsetof(struct adns_answer, ans),
					      sizeof(a));
...
static int master(void)
{
	for (;;) {
		...
		do {
			setproctitle(progname, "<idle>");
			ndes = osw_select(maxfd + 1, &readfds, NULL, NULL,
					  NULL);
