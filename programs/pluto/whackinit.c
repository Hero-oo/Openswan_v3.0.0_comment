...
int main(int argc, char **argv)
{
	...
	lset_t opts_seen = LEMPTY, cd_seen = LEMPTY;
	...
	if (!(msg.whack_initiate || msg.whack_terminate || msg.whack_status)) {
	...
	if (!pack_str(&msg.name) /* string  1 */
	    || str_roof - next_str <
		       (ptrdiff_t)msg.keyval.len) /* chunk (sort of string 5) */
