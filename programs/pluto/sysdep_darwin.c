...
{
	DBG(DBG_CONTROL, DBG_log("executing %s%s: %s", verb, verb_suffix, cmd));

	{
		...
		for (;;) {
			...
			if (fgets(resp, sizeof(resp), f) == NULL) {
				...
				if (e > resp && e[-1] == '\n')
					e[-1] = '\0'; /* trim trailing '\n' */
				openswan_log("%s%s output: %s", verb,
					     verb_suffix, resp);
			}
		}
...
{
	static const int on =
		TRUE; /* by-reference parameter; constant, we hope */
	...
	int master_sock = safe_socket(PF_INET, SOCK_DGRAM,
				      IPPROTO_UDP); /* Get a UDP socket */
	...
	if (setsockopt(master_sock, SOL_SOCKET, SO_REUSEADDR, (const void *)&on,
		       sizeof(on)) < 0)
	...
	/* bind the socket */
	{
		...
		if (bind(master_sock, sockaddrof(&any), sockaddrlenof(&any)) <
		    0)
			exit_log_errno(
				(e, "bind() failed in find_raw_ifaces4()"));
	}
	...
	while (num < (1024 * 1024)) {
		...
		if (ioctl(master_sock, SIOCGIFCONF, &ifconf) == -1)
			exit_log_errno((
				e, "ioctl(SIOCGIFCONF) in find_raw_ifaces4()"));
	...
	for (bp = buf, j = 0;
	     ...
	     bp = (struct ifreq *)((unsigned char *)bp +
		...
		happy(initaddr((const void *)&rs->sin_addr,
			       sizeof(struct in_addr), AF_INET, &ri.addr));

		DBG(DBG_CONTROL, DBG_log("found %s with address %s", ri.name,
					 ip_str(&ri.addr)));
		ri.next = rifaces;
		       ...
		       const char *verb, struct state *st)
{
	...
	/* figure out which verb suffix applies */
	{
		...
		default:
			loglog(RC_LOG_SERIOUS,
			       "(darwin_do_command)unknown address family");
			return FALSE;
		}
		verb_suffix =
			subnetisaddr(&sr->this.client, &sr->this.host_addr) ?
				hs :
				cs;
	}
	...
	/* form the command string */
	{
		char nexthop_str[sizeof("PLUTO_NEXT_HOP='' ") + ADDRTOT_BUF],
			me_str[ADDRTOT_BUF], myid_str[IDTOA_BUF],
			...
			peerid_str[IDTOA_BUF], peerclient_str[SUBNETTOT_BUF],
		...
		if (addrbytesptr(&sr->this.host_srcip, NULL) != 0 &&
			...
			int l;
			strncat(srcip_str,
				"PLUTO_MY_SOURCEIP=", sizeof(srcip_str));
		...
		if (-1 ==
		    snprintf(cmd, sizeof(cmd),
			     ...
			     "%s" /* actual script */
			     ,
			     verb, verb_suffix, c->name, nexthop_str,
			     c->interface->ip_dev->id_vname, me_str,
			     secure_myid_str, myclient_str, myclientnet_str,
			     myclientmask_str, sr->this.port, sr->this.protocol,
			     peer_str, secure_peerid_str, peerclient_str,
			     peerclientnet_str, peerclientmask_str,
			     sr->that.port, sr->that.protocol,
			     secure_peerca_str, prettypolicy(c->policy),
			     secure_xauth_username_str, srcip_str,
