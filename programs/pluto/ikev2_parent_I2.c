...
stf_status ikev2parent_inR1outI2(struct msg_digest *md)
{
	...
	/* process and confirm the SA selected */
	{
		...
		/* SA body in and out */
		rn = ikev2_parse_parent_sa_body(
			&sa_pd->pbs, &sa_pd->payload.v2sa, NULL, st, FALSE);
			    ...
			    struct pluto_crypto_req *r)
{
	...
	/* send out the IDi payload */
	{
		...
		hmac_init_chunk(&id_ctx, pst->st_oakley.prf_hasher,
				pst->st_skey_pi);
		DBG(DBG_CRYPT, DBG_dump_chunk("parent SA IDi idhash pi calc",
					      pst->st_skey_pi));
		DBG(DBG_CRYPT,
		    DBG_dump("parent SA I2 idhash calc", id_start, id_len));
	...
	if (doi_send_ikev2_cert_thinking(st)) {
		stf_status certstat =
			ikev2_send_cert(st, md, INITIATOR, &e_pbs_cipher);
		if (certstat != STF_OK)
			return certstat;
	...
	/* send out the AUTH payload */
	{
		...
		struct connection *c0 =
			first_pending(pst, &policy, &st->st_whack_sock);
		DBG(DBG_CONTROL, DBG_log(" payload after AUTH will be %s",
					 (c0) ? "ISAKMP_NEXT_v2SA" :
						"ISAKMP_NEXT_NONE/NOTIFY"));

		stf_status authstat = ikev2_send_auth(c, st, INITIATOR, idhash,
						      &e_pbs_cipher);
		if (authstat != STF_OK)
			return authstat;
		...
		if (c0) {
			...
			if (!(st->st_connection->policy & POLICY_TUNNEL)) {
				...
				ship_v2N(ISAKMP_NEXT_NONE,
					 ...
					 &child_spi, v2N_USE_TRANSPORT_MODE,
...

	{
		...
		ret = ikev2_encrypt_msg(md, INITIATOR, authstart, iv, encstart,
...
static stf_status ikev2parent_retry_next_proposal(struct msg_digest *md)
{
	...
	stf = ikev2parent_outI1(whack_sock, c, NULL, &created, c->policy, 1,
				pcim_demand_crypto, NULL_POLICY);
...
stf_status ikev2parent_ntf_inR1(struct msg_digest *md)
{
	...
	if (md->chain[ISAKMP_NEXT_v2N]) {
		...
		for (notify = md->chain[ISAKMP_NEXT_v2N]; notify != NULL;
		     notify = notify->next) {
