 ...
 *
 */
void linux_pfkey_register(void)
{
 ...
 */
bool send_netlink_msg(struct nlmsghdr *hdr, struct nlmsghdr *rbuf,
		      ...
		      const char *text_said)
{
	...
	if (r < 0) {
		...
		loglog(RC_LOG_SERIOUS,
		       ...
		       sparse_val_show(xfrm_type_names, hdr->nlmsg_type),
		       description, text_said, (long)r, (unsigned long)len);
		return FALSE;
	}
	...
	if (rsp.n.nlmsg_len > (size_t)r) {
		loglog(RC_LOG_SERIOUS,
		       ...
		       sparse_val_show(xfrm_type_names, hdr->nlmsg_type),
		       description, text_said, (long)len,
...
bool netlink_policy(struct nlmsghdr *hdr, bool enoent_ok, const char *text_said)
{
	...
	loglog(RC_LOG_SERIOUS,
	       ...
	       sparse_val_show(xfrm_type_names, hdr->nlmsg_type), text_said,
	       error, strerror(error));
	return FALSE;
}

bool netlink_get(void)
{
	struct {
		struct nlmsghdr n;
		char data[1024];
	...
	if (r < 0) {
		...
		openswan_log(
			...
			(long)r, (unsigned long)rsp.n.nlmsg_len);
		return TRUE;
	}
...
bool netkey_do_command(struct connection *c, const struct spd_route *sr,
...
void init_netlink(void)
{
	...
	if (fcntl(netlink_bcast_fd, F_SETFD, FD_CLOEXEC) != 0)
		exit_log_errno(
			(e, "fcntl(FD_CLOEXEC) for bcast in init_netlink()"));

	if (fcntl(netlink_bcast_fd, F_SETFL, O_NONBLOCK) != 0)
		exit_log_errno(
			(e, "fcntl(O_NONBLOCK) for bcast in init_netlink()"));
...
void netlink_scan_bare_shunts(void)
{
	...
	for (bspp = &bare_shunts;;) {
		...
		success = delete_bare_shunt_ptr(bspp,
