...
void init_nat_traversal(bool activate, unsigned int keep_alive_period, bool fka,
			bool spf)
{
	...
	_force_ka = fka;
	_kap = keep_alive_period ? keep_alive_period :
		       ...
		       const ip_address *ip, u_int16_t port)
{
	...
	DBG(
		DBG_NATT, DBG_log("_natd_hash: digest_len:%d",
...
{
	bool r = TRUE;
	DBG(DBG_NATT, DBG_log("nat add vid. port: %d nonike: %d",
			      nat_traversal_support_port_floating,
			      nat_traversal_support_non_ike));
		...
		if (st->st_connection->remotepeertype == CISCO) {
			...
			if (r)
				r = out_vid(ISAKMP_NEXT_VID, outs,
					    VID_NATT_IETF_02_N);
			if (r)
				r = out_vid(nat_traversal_support_non_ike ?
						    ISAKMP_NEXT_VID :
						    np,
...
void nat_traversal_natd_lookup(struct msg_digest *md)
{
	...
	/** Count NAT-D **/
	for (p = md->chain[ISAKMP_NEXT_NATD_RFC], i = 0; p != NULL;
	     p = p->next, i++)
		;
	 ...
	 */
	_natd_hash(st->st_oakley.prf_hasher, hash_me, st->st_icookie,
		   st->st_rcookie, &(md->iface->ip_addr),
		   ntohs(md->iface->port));

	/**
	 * The others with sender IP & port
	 */
	_natd_hash(st->st_oakley.prf_hasher, hash_him, st->st_icookie,
		   st->st_rcookie, &(md->sender), ntohs(md->sender_port));

	for (p = md->chain[ISAKMP_NEXT_NATD_RFC], i = 0;
	     p != NULL && (!found_me || !found_him); p = p->next) {
		DBG(DBG_NATT, DBG_log("NAT_TRAVERSAL hash=%d (me:%d) (him:%d)",
	...
	DBG(DBG_NATT, DBG_log("NAT_TRAVERSAL hash=%d (me:%d) (him:%d)", i,
			      found_me, found_him));
...
bool nat_traversal_add_natd(u_int8_t np, pb_stream *outs, struct msg_digest *md)
{
	...
	_natd_hash(st->st_oakley.prf_hasher, hash, st->st_icookie,
		   is_zero_cookie(st->st_rcookie) ? md->hdr.isa_rcookie :
						    st->st_rcookie,
		   second, secondport);
	return (out_generic_raw(np, &isakmp_nat_d, outs, hash,
				st->st_oakley.prf_hasher->hash_digest_len,
				"NAT-D"));
}
				...
				struct hidden_variables *hv)
{
	...
	/** Count NAT-OA **/
	for (p = md->chain[ISAKMP_NEXT_NATOA_RFC], i = 0; p != NULL;
	     p = p->next, i++)
		;

	DBG(DBG_NATT, DBG_log("NAT-Traversal: received %d NAT-OA.", i););
	...
	if (i == 0) {
		return;
		     ...
		     LELEM(NAT_TRAVERSAL_NAT_BHND_PEER))) {
		loglog(RC_LOG_SERIOUS,
		       "NAT-Traversal: received %d NAT-OA. "
	...
	DBG(DBG_PARSING, DBG_dump("NAT-OA:", p->pbs.start, pbs_room(&p->pbs)););
...
bool nat_traversal_add_natoa(u_int8_t np, pb_stream *outs, struct state *st,
			     bool initiator)
{
...

	{
		...
		DBG(DBG_NATT, DBG_dump("NAT-OAi (S):", ip_val, ip_len););
...

	{
		...
		DBG(DBG_NATT, DBG_dump("NAT-OAr (S):", ip_val, ip_len););
...
void nat_traversal_show_result(u_int32_t nt, u_int16_t sport)
{
	...
	loglog(RC_LOG_SERIOUS, "NAT-Traversal: Result using %s: %s",
	       mth ? mth : "unknown method", rslt ? rslt : "unknown result");
	if ((nt & LELEM(NAT_TRAVERSAL_NAT_BHND_PEER)) &&
	    (sport == IKE_UDP_PORT) && ((nt & NAT_T_WITH_PORT_FLOATING) == 0)) {
		loglog(RC_LOG_SERIOUS,
...
static void nat_traversal_find_new_mapp_state(struct state *st, void *data)
{
     ...
     */
		...
		if (!isanyaddr(&st->st_remoteaddr) ||
		    (nfo->port != pluto_port500 &&
		     nfo->port != pluto_port4500)) {
			addrtot(&st->st_remoteaddr, 0, b1, ADDRTOT_BUF);
			addrtot(&nfo->addr, 0, b2, ADDRTOT_BUF);

			openswan_log(
				"new NAT mapping for #%u, was %s:%u, now %s:%u",
				(unsigned int)st->st_serialno, b1,
				st->st_remoteport, b2, nfo->port);
		}
...
static int nat_traversal_new_mapping(struct state *st, const ip_address *nsrc,
				     u_int16_t nsrcport)
{
...
void nat_traversal_change_port_lookup(struct msg_digest *md, struct state *st)
{
	...
	if (((st->st_state == STATE_MAIN_I3) ||
		...
		DBG(DBG_NATT, DBG_log("NAT-T: floating to port %d",
	...
	{
		DBG(DBG_NATT, char b1[ADDRTOT_BUF]; char b2[ADDRTOT_BUF];
	...
	struct sadb_ext *extensions[K_SADB_EXT_MAX + 1])
{
	...
	if ((!nfo.sa) || (!srcx) || (!dstx)) {
		openswan_log(
			"K_SADB_X_NAT_T_NEW_MAPPING message from KLIPS malformed: "
			"got NULL params");
		return;
	}
	...
	if ((srca->sa_family != AF_INET) || (dsta->sa_family != AF_INET)) {
		...
		initaddr((const void *)&((const struct sockaddr_in *)srca)
				 ->sin_addr,
		...
		initaddr((const void *)&((const struct sockaddr_in *)dsta)
				 ->sin_addr,
		...
		DBG(DBG_NATT, char text_said[SATOT_BUF];
		    char _srca[ADDRTOT_BUF]; char _dsta[ADDRTOT_BUF];
		    ip_said said;
		    ...
		    DBG_log("new klips mapping %s %s:%d %s:%d", text_said,
			    _srca, nfo.sport, _dsta, nfo.dport););
