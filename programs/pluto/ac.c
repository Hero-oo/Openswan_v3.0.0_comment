...
static const asn1Object_t ietfAttrSyntaxObjects[] = {
	...
	{ 2, "octets", ASN1_OCTET_STRING, ASN1_OPT | ASN1_BODY }, /*  4 */
...
static const asn1Object_t roleSyntaxObjects[] = {
	...
	{ 1, "roleAuthority", ASN1_CONTEXT_C_0, ASN1_OPT | ASN1_OBJ }, /*  1 */
	{ 1, "end opt", ASN1_EOC, ASN1_END }, /*  2 */
...
static const asn1Object_t acObjects[] = {
	...
	{ 4, "issuerUID", ASN1_BIT_STRING, ASN1_OPT | ASN1_BODY }, /*  7 */
	{ 4, "end opt", ASN1_EOC, ASN1_END }, /*  8 */
	{ 3, "end opt", ASN1_EOC, ASN1_END }, /*  9 */
	{ 3, "entityName", ASN1_CONTEXT_C_1, ASN1_OPT | ASN1_OBJ }, /* 10 */
	...
	{ 4, "otherObjectTypeID", ASN1_OID, ASN1_OPT | ASN1_BODY }, /* 14 */
	...
	{ 3, "issuerName", ASN1_SEQUENCE, ASN1_OPT | ASN1_OBJ }, /* 19 */
	...
	{ 5, "issuerUID", ASN1_BIT_STRING, ASN1_OPT | ASN1_BODY }, /* 25 */
	...
	{ 5, "otherObjectTypeID", ASN1_OID, ASN1_OPT | ASN1_BODY }, /* 31 */
 ...
 */
static bool parse_ac(chunk_t blob, x509acert_t *ac)
{
	asn1_ctx_t ctx;
	bool critical;
	chunk_t extnID;
	...
	while (objectID < AC_OBJ_ROOF) {
		...
		case AC_OBJ_CERTIFICATE_INFO:
			...
			break;
		case AC_OBJ_VERSION:
			ac->version = (object.len) ? (1 + (u_int)*object.ptr) :
						     1;
			DBG(DBG_PARSING, DBG_log("  v%d", ac->version);)
			if (ac->version != 2) {
				openswan_log(
		...
		case AC_OBJ_HOLDER_ISSUER:
			ac->holderIssuer =
				get_directoryName(object, level, FALSE);
			break;
...
static bool same_x509acert(x509acert_t *a, x509acert_t *b)
{
	return a->signature.len == b->signature.len &&
	       memcmp(a->signature.ptr, b->signature.ptr, b->signature.len) ==
		       0;
}
 ...
 */
static void free_acert(x509acert_t *ac)
{
	if (ac != NULL) {
 ...
 */
static void add_acert(x509acert_t *acert)
{
...
static err_t check_ac_validity(const x509acert_t *ac)
{
	...
	time(&current_time);
	DBG(DBG_CONTROL | DBG_PARSING, char tbuf[TIMETOA_BUF];
 ...
 */
static bool verify_x509acert(x509acert_t *ac, bool strict)
{
	x509cert_t *aacert;
	err_t ugh = NULL;
	time_t valid_until = ac->notAfter;

	DBG(DBG_CONTROL, u_char buf[BUF_LEN];
	...
	DBG(DBG_CONTROL, DBG_log("attribute certificate is valid"))
	...
	DBG(DBG_CONTROL, DBG_log("issuer aacert found"))

	if (!check_signature(ac->certificateInfo, ac->signature, ac->algorithm,
			     aacert)) {
		plog("attribute certificate signature is invalid");
		return FALSE;
	}
	DBG(DBG_CONTROL, DBG_log("attribute certificate signature is valid");)
...
void list_acerts(bool utc)
{
	...
	while (ac != NULL) {
		...
		datatot(ac->serialNumber.ptr, ac->serialNumber.len, ':', buf,
			BUF_LEN);
