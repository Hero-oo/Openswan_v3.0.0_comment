...
bool out_attr(int type, unsigned long val, struct_desc *attr_desc,
	      enum_names **attr_val_descs USED_BY_DEBUG, pb_stream *pbs)
{
	...
	DBG(DBG_EMITTING, enum_names *d = attr_val_descs[type];

	    if (d != NULL) DBG_log("    [%lu is %s]", val, enum_show(d, val)));
	return TRUE;
}
...
struct db_sa *ikev1_alg_makedb(lset_t policy, struct alg_info_ike *ei,
			       bool oneproposal UNUSED, enum phase1_role role)
{
	struct db_sa *sadb;
	char info_buf[1024];
	...
	if (!extrapolate_v1_from_v2(sadb, policy, role)) {
		openswan_log(
			"failed to create v1 PARENTSA policy from v2 settings");
		return NULL;
	}

	DBG(DBG_EMITTING, DBG_log("Translated IKEv2 policy to: ");
	    sa_print(sadb));
				...
				enum phase1_role role)
{
	...
	if (!(policy & POLICY_IKEV1_DISABLE) &&
	    !extrapolate_v1_from_v2(sadb, policy, role)) {
		openswan_log(
			"failed to create v1 IPsec policy from v2 settings");
		return NULL;
	}
...
bool extrapolate_v1_from_v2(struct db_sa *sadb, lset_t policy,
			    enum phase1_role role)
{
	...
	if (sadb->prop_conjs != NULL)
		return TRUE;

	/* the v2 info might be empty */
	if (sadb->prop_disj_cnt < 1)
		return TRUE;

	if (!sadb->prop_v1_ctx) {
		sadb->prop_v1_ctx =
			db_prop_new(sadb->prop_disj->props[0].protoid,
	...
	for (pass = 0; pass < 2; pass++) {
		...
		DBG_log("%u disj_cnt: %d/%d", cur_combo, prop_disj,
			sadb->prop_disj_cnt);
#endif
		for (prop_disj = 0; prop_disj < sadb->prop_disj_cnt;
		     prop_disj++) {
			unsigned int prop_conj;
			struct db_v2_prop *pd = &sadb->prop_disj[prop_disj];
#if EXTRAPOLATE_DEBUG
			/* enable this when debugging problems with cur_combo */
			DBG_log("%u: %u disj_cnt: %d/%d conj: 0/%d", pass,
				cur_combo, prop_disj, sadb->prop_disj_cnt,
				pd->prop_cnt);
#endif

			for (prop_conj = 0; prop_conj < pd->prop_cnt;
			     prop_conj++) {
				...
				DBG_log("%u: %u disj_cnt: %d/%d conj: %d/%d trans: 0/%d",
					pass, cur_combo, prop_disj,
					sadb->prop_disj_cnt, prop_conj,
					pd->prop_cnt, pc->trans_cnt);
#endif

				/* reset the transform values */
				for (i = 0; i < IKEv2_TRANS_TYPE_COUNT; i++) {
					old_transform_values[i] =
						transform_values[i];
					transform_values[i] = -1;
				}

				for (trans_i = 0; trans_i < pc->trans_cnt;
				     trans_i++) {
					unsigned int attr_i;
					...
					if (tr->transform_type >=
					    IKEv2_TRANS_TYPE_COUNT)
						continue;

					/* IKEv1 does not negotiate PRF, so ignore options like that */
					if (tr->transform_type ==
					    IKEv2_TRANS_TYPE_PRF)
						continue;
					...
					if (transform_values[tr->transform_type] ==
					    -1) {
						transform_values
							[tr->transform_type] =
								tr->value;
				...
				{
					bool matched = TRUE;
					for (i = 0; i < IKEv2_TRANS_TYPE_COUNT;
					     i++) {
						if (old_transform_values[i] !=
						    transform_values[i]) {
							matched = FALSE;
						}
					}
					...
					if ((pass == 0 && cur_combo == 0) ||
						...
						DBG(DBG_EMITTING,
						    DBG_log("pass: %u noticed combo %d/%d: dh:%d(%s) encr:%d(%s) integ:%d(%s)",
							    ...
							    enum_name(
								    &trans_type_encr_names,
								    transform_values
									    [IKEv2_TRANS_TYPE_ENCR]),
							    transform_values
								    [IKEv2_TRANS_TYPE_INTEG],
							    enum_name(
								    &trans_type_integ_names,
								    transform_values
									    [IKEv2_TRANS_TYPE_INTEG])));
		...
		if (pass == 0) {
			...
			memset(dtf, 0,
			       sizeof(struct db_trans_flat) * tot_combos);
			cur_dtf = dtf;
			...
			if (tot_combos == 0) {
				openswan_log(
					"can not extrapolate IKEv1 policy from empty IKEv2 policy");
				return FALSE;
			}
			...
			if (transform_values[IKEv2_TRANS_TYPE_ENCR] == -1 ||
			    transform_values[IKEv2_TRANS_TYPE_INTEG] == -1) {
				openswan_log(
					"can not extrapolate IKEv1 policy from v2 policy missing encryption or integrity settings");
				return FALSE;
			}

			if (sadb->parentSA == TRUE) {
				if (transform_values[IKEv2_TRANS_TYPE_DH] ==
				    -1) {
					openswan_log(
						"can not extrapolate IKEv1 parent policy from v2 policy missing group settings");
					return FALSE;
				}
			}
		}
	...
	for (i = 0; i < cur_combo; i++, cur_dtf++) {
		if (sadb->parentSA) {
			...
			for (pol_j = 0; pol_j < elemsof(policies); pol_j++) {
				...
				if ((policy & possible) ==
				    possible) { /* must match exactly */

					unsigned int oakley_auth_alg =
						policies[pol_j][1];
			...
			if (cur_dtf->encr_keylen) {
				db_attr_add_ipsec_values(sadb->prop_v1_ctx,
							 KEY_LENGTH,
							 cur_dtf->encr_keylen);
			}
			db_attr_add_ipsec_values(
				sadb->prop_v1_ctx, AUTH_ALGORITHM,
				v2tov1_integ_child(cur_dtf->integ_transid));
			if (cur_dtf->group_transid) {
				db_attr_add_ipsec_values(
					sadb->prop_v1_ctx, GROUP_DESCRIPTION,
					cur_dtf->group_transid);
			}
	...
	sadb->prop_conjs =
		alloc_thing(struct db_prop_conj, "v1 policy proposal conj");
 ...
 *
 */
bool out_sa(pb_stream *outs, struct db_sa *sadb, struct state *st,
	    bool phase_one_mode, enum phase1_role role,
	    bool aggressive_mode UNUSED, u_int8_t np)
{
	pb_stream sa_pbs;
	unsigned int pcn;
	bool ret = FALSE;
	bool ah_spi_generated = FALSE, esp_spi_generated = FALSE,
	     ipcomp_cpi_generated = FALSE;

	if (!extrapolate_v1_from_v2(sadb, st->st_policy, role)) {
		openswan_log(
			"can not derive IKEv1 policy from IKEv2 settings, failed");
		return_on(ret, FALSE);
	}
	...
	if (!phase_one_mode && ((st->st_policy) & POLICY_COMPRESS)) {
		...
		struct db_trans *ipcomp_trans =
			alloc_thing(struct db_trans, "ipcomp_trans");

		/* allocate space for 2 proposals */
		struct db_prop *ipcomp_prop = alloc_bytes(
			(sizeof(struct db_prop) * 2), "ipcomp_prop");
	...
	for (pcn = 0; pcn < sadb->prop_conj_cnt; pcn++) {
		...
		DBG(DBG_EMITTING,
		    DBG_log("out_sa pcn: %d has %d valid proposals", pcn,
			    valid_prop_cnt));
		...
		for (pn = 0; pn < pc->prop_cnt; pn++) {
			...
			for (tn = 0; tn != p->trans_cnt; tn++) {
				...
				/* automatically generate duration
                     ...
                     */
				if (phase_one_mode) {
					out_attr(OAKLEY_LIFE_TYPE,
						 OAKLEY_LIFE_SECONDS, attr_desc,
						 attr_val_descs, &trans_pbs);
				...
				for (an = 0; an < t->attr_cnt; an++) {
					...
					if (transmitted & LELEM(type))
						continue;
					transmitted |= LELEM(type);
					out_attr(type, a->val, attr_desc,
						 attr_val_descs, &trans_pbs);
				}
 ...
 */
static void emit_isakmp_sa_body(pb_stream *r_sa_pbs, uint32_t ipsecdoisit,
				struct isakmp_proposal proposal,
				struct isakmp_transform trans,
				u_char *attr_start, int attr_len)
{
	...
	if (!out_struct(&proposal, &isakmp_proposal_desc, r_sa_pbs,
			&proposal_pbs))
		impossible();
	...
	if (!out_struct(&trans, &isakmp_isakmp_transform_desc, &proposal_pbs,
			&trans_pbs))
		impossible();
	...
	struct state *st) /* current state object */
{
	...
	for (;;) {
		...
		size_t attr_len;
		struct isakmp_transform trans;
		lset_t seen_attrs = 0, seen_durations = 0;
		u_int16_t life_type;
		...
		ta.life_seconds =
			OAKLEY_ISAKMP_SA_LIFETIME_DEFAULT; /* When this SA expires (seconds) */
		...
		while (pbs_left(&trans_pbs) != 0) {
			...
			if ((a.isaat_af_type & ISAKMP_ATTR_RTYPE_MASK) >= 32) {
				...
				loglog(RC_LOG_SERIOUS,
				       "ignoring unknown attribute %u in Oakley Transform %u",
				       a.isaat_af_type, trans.isat_transnum);
				continue;
			}
			...
			case OAKLEY_ENCRYPTION_ALGORITHM | ISAKMP_ATTR_AF_TV:
				if (ikev1_alg_enc_ok(val, 0, c->alg_info_ike,
						     &ugh, ugh_buf,
						     sizeof(ugh_buf))) {
					ta.encrypter = ikev1_alg_get_encr(val);
					ta.encrypt =
						ta.encrypter->common.algo_v2id;
			...
			case OAKLEY_HASH_ALGORITHM | ISAKMP_ATTR_AF_TV:
				if (ikev1_alg_integ_present(val, 0)) {
					ta.integ_hasher =
						ikev1_crypto_get_hasher(val);
					ta.integ_hash = ta.integ_hasher->common
								.algo_v2id;
					ta.prf_hash = v2integ_to_prf(
						ta.integ_hash); /* returns IKEv2 number */
					ta.prf_hasher = ike_alg_get_prf(
						ta.prf_hash); /* so use v2 lookup */
...
rsasig:
			...
			case OAKLEY_LIFE_DURATION | ISAKMP_ATTR_AF_TV:
				...
				seen_attrs &= ~(LELEM(OAKLEY_LIFE_DURATION) |
			...
			if (ugh != NULL) {
				loglog(RC_LOG_SERIOUS, "%s.  Attribute %s", ugh,
				       enum_show(&oakley_attr_names, attr));
				break;
			}
		}
		...
		if (ugh == NULL) {
			...
			if (!spdb_v2_match_parent(sadb, trans.isat_transnum,
				...
				DBG(DBG_PARSING | DBG_CRYPT,
				    DBG_log("Oakley Transform %u unacceptable",
					    trans.isat_transnum));
				if (r_sa_pbs != NULL)
					close_output_pbs(r_sa_pbs);
			} else {
				...
				if (r_sa_pbs != NULL)
					emit_isakmp_sa_body(
						r_sa_pbs, ipsecdoisit, proposal,
						trans, attr_start, attr_len);
...
static bool parse_ipsec_transform(
	struct isakmp_transform *trans,
	struct ipsec_trans_attrs *attrs /* must be in IKEv2 number space */
	,
	pb_stream *prop_pbs, pb_stream *trans_pbs, struct_desc *trans_desc,
	...
	struct state *st) /* current state object */
{
	...
	*attrs = null_ipsec_trans_attrs;
	...
	while (pbs_left(trans_pbs) != 0) {
		...
		u_int32_t type;
		bool ipcomp_inappropriate =
			is_ipcomp; /* will get reset if OK */
		...
		case SA_LIFE_DURATION:
			...
			case SA_LIFE_TYPE_KBYTES:
				...
				break;
			default:
				loglog(RC_LOG_SERIOUS, "invalid life type: %u",
				       life_type);
				return FALSE;
			}
			break;
	...
	bool tunnel_mode) /* true for inner most tunnel SA */
{
	...
	if (r_proposal.isap_protoid == PROTO_IPCOMP) {
		...
		out_raw((u_char *)&pi->our_spi + IPSEC_DOI_SPI_SIZE -
				IPCOMP_CPI_SIZE,
			IPCOMP_CPI_SIZE, &r_proposal_pbs, "CPI");
	} else {
		get_ipsec_spi(pi,
	...
	if (!out_raw(trans_pbs->cur, pbs_left(trans_pbs), &r_trans_pbs,
		     "attributes"))
		impossible();
	...
	struct state *st) /* current state object */
{
	...
	while (next_full) {
		...
		do {
			if (next_proposal.isap_protoid == PROTO_IPCOMP) {
				...
				if (!in_raw((u_char *)&next_spi +
						    IPSEC_DOI_SPI_SIZE -
						    IPCOMP_CPI_SIZE,
					    IPCOMP_CPI_SIZE, &next_proposal_pbs,
					    "CPI"))
					return INVALID_SPI;
		...
		if (ah_seen) {
			...
			for (tn = 0; tn != ah_proposal.isap_notrans; tn++) {
				...
				if (!parse_ipsec_transform(
					    &ah_trans, &ah_attrs, &ah_prop_pbs,
					    &ah_trans_pbs,
					    &isakmp_ah_transform_desc,
					    previous_transnum, selection,
...
#else
			loglog(RC_LOG_SERIOUS,
			       "AH in IKEv1 mode is not supported. Proposal rejected");
			continue;
#endif
		}
		...
		if (ipcomp_seen) {
			...
			if (!can_do_IPcomp) {
				openswan_log(
					"compression proposed by %s, but KLIPS is not configured with IPCOMP",
					ip_str(&c->spd.that.host_addr));
				continue;
			}
			...
			for (tn = 0; tn != ipcomp_proposal.isap_notrans; tn++) {
				if (!parse_ipsec_transform(
					    &ipcomp_trans, &ipcomp_attrs,
					    &ipcomp_prop_pbs, &ipcomp_trans_pbs,
					    &isakmp_ipcomp_transform_desc,
					    previous_transnum, selection,
					    tn == ipcomp_proposal.isap_notrans -
							    1,
					    TRUE, st))
					return BAD_PROPOSAL_SYNTAX;
		...
		if (r_sa_pbs != NULL) {
			...
			/* Situation */
			if (!out_struct(&ipsecdoisit, &ipsec_sit_desc, r_sa_pbs,
					NULL))
				impossible();

			/* AH proposal */
			if (ah_seen)
				echo_proposal(st, ah_proposal, ah_trans,
					      esp_seen || ipcomp_seen ?
						      ISAKMP_NEXT_P :
						      ISAKMP_NEXT_NONE,
					      r_sa_pbs, &st->st_ah,
					      &isakmp_ah_transform_desc,
			...
			/* ESP proposal */
			if (esp_seen)
				echo_proposal(st, esp_proposal, esp_trans,
			...
			/* IPCOMP proposal */
			if (ipcomp_seen)
				echo_proposal(st, ipcomp_proposal, ipcomp_trans,
