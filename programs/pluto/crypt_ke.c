...
void calc_ke(struct pluto_crypto_req *r)
{
	...
	while (1) {
		privk = PK11_GenerateKeyPair(
			slot, CKM_DH_PKCS_KEY_PAIR_GEN, &dhp, &pubk, PR_FALSE,
			PR_TRUE, osw_return_nss_password_file_info());
		if (!privk) {
	...
	DBG(DBG_CRYPT, DBG_dump("NSS: Local DH secret:\n",
	...
	DBG(DBG_CRYPT, DBG_dump("NSS: Local DH public value (pointer):\n",
