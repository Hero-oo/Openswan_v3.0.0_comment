...
static int recalculate_mast_device_list(struct raw_iface *rifaces)
{
	...
	for (ifp = rifaces; ifp != NULL; ifp = ifp->next) {
		...
		if (strncmp(ifp->name, MASTDEVPREFIX,
			    sizeof(MASTDEVPREFIX) - 1))
			continue;

		if (sscanf(ifp->name, "mast%d", &mastno) == 1) {
			openswan_log("found %s device already present",
				     ifp->name);
			mastdevice[mastno] = MAST_AVAIL;
			if (!isunspecaddr(&ifp->addr)) {
				openswan_log("device %s already in use",
					     ifp->name);
	 ...
	 */
		 ...
		 */
				if (nat_traversal_support_port_floating &&
					...
					q->ip_addr = ifp->addr;
					setportof(htons(NAT_T_IKE_FLOAT_PORT),
						  &q->ip_addr);
...
static bool mast_do_command(struct connection *c, const struct spd_route *sr,
...
static bool mast_raw_eroute(
	const ip_address *this_host UNUSED, const ip_subnet *this_client UNUSED,
	const ip_address *that_host UNUSED, const ip_subnet *that_client UNUSED,
	ipsec_spi_t spi UNUSED, unsigned int proto UNUSED,
	unsigned int transport_proto UNUSED, unsigned int satype UNUSED,
	const struct pfkey_proto_info *proto_info UNUSED,
	time_t use_lifetime UNUSED, enum pluto_sadb_operations op UNUSED,
	const char *text_said UNUSED, char *policy_label UNUSED)
{
	...
	return pfkey_raw_eroute(this_host, this_client, that_host, that_client,
				spi, proto, transport_proto, satype, proto_info,
				use_lifetime, op, text_said, policy_label);
}
 ...
 */
static bool mast_sag_eroute_replace(struct state *st,
				    const struct spd_route *sr)
{
	...
	DBG_log("mast_sag_eroute_replace state #%d{ref=%d refhim=%d} "
		...
		(int)old_st->st_serialno, (int)old_st->st_ref,
		(int)old_st->st_refhim, (int)st->st_serialno, (int)st->st_ref,
		(int)st->st_refhim);
...
static bool mast_sag_eroute(struct state *st, const struct spd_route *sr,
			    ...
			    const char *opname UNUSED)
{
	...
	case ERO_ADD:
		return mast_do_command(st->st_connection, sr, "spdadd",
				       verb_suffix, st);

	case ERO_DELETE:
		return mast_do_command(st->st_connection, sr, "spddel",
				       verb_suffix, st);
