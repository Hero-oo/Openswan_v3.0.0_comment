...
void xfrm_kernel_alg_add(struct kernel_alg_info *kai)
{
	...
	if (!psa) {
		openswan_log(
			"not installing type=%s algo=%s, as sadb level is unaware of it",
	...
	while (p != NULL) {
		if (p->alg_type == alg_type) {
			...
			case IKEv2_TRANS_TYPE_COMPRESS:
				if (p->trans.compress == trans_num)
					return p;
				break;

			case IKEv2_TRANS_TYPE_ENCR:
				if (p->trans.encr == trans_num)
					return p;
				break;

			case IKEv2_TRANS_TYPE_INTEG:
				if (p->trans.integ == trans_num)
					return p;
				break;
...
sparse_names xfrm_type_names = {
	NE(NLMSG_NOOP),		NE(NLMSG_ERROR),	NE(NLMSG_DONE),
	NE(NLMSG_OVERRUN),

	NE(XFRM_MSG_NEWSA),	NE(XFRM_MSG_DELSA),	NE(XFRM_MSG_GETSA),

	NE(XFRM_MSG_NEWPOLICY), NE(XFRM_MSG_DELPOLICY), NE(XFRM_MSG_GETPOLICY),

	NE(XFRM_MSG_ALLOCSPI),	NE(XFRM_MSG_ACQUIRE),	NE(XFRM_MSG_EXPIRE),

	NE(XFRM_MSG_UPDPOLICY), NE(XFRM_MSG_UPDSA),
...
/** Compress Algs */
static sparse_names calg_list = { { SADB_X_CALG_DEFLATE, "deflate" },
				  { SADB_X_CALG_LZS, "lzs" },
				  { SADB_X_CALG_LZJH, "lzjh" },
				  { 0, sparse_end } };

static struct aead_alg aead_algs[] = {
	{ .id = SADB_X_EALG_AES_CCM_ICV8,
	  .icvlen = 8,
	  .name = "rfc4309(ccm(aes))" },
	{ .id = SADB_X_EALG_AES_CCM_ICV12,
	  .icvlen = 12,
	  .name = "rfc4309(ccm(aes))" },
	{ .id = SADB_X_EALG_AES_CCM_ICV16,
	  .icvlen = 16,
	  .name = "rfc4309(ccm(aes))" },
	{ .id = SADB_X_EALG_AES_GCM_ICV8,
	  .icvlen = 8,
	  .name = "rfc4106(gcm(aes))" },
	{ .id = SADB_X_EALG_AES_GCM_ICV12,
	  .icvlen = 12,
	  .name = "rfc4106(gcm(aes))" },
	{ .id = SADB_X_EALG_AES_GCM_ICV16,
	  .icvlen = 16,
	  .name = "rfc4106(gcm(aes))" },
};
...
static void linux_pfkey_add_aead(void)
{
	...
	if (kernel_alg_add(SADB_SATYPE_ESP, SADB_EXT_SUPPORTED_ENCRYPT, &alg) !=
	    1)
		loglog(RC_LOG_SERIOUS,
		       "Warning: failed to register algo_aes_gcm_8 for ESP");
	...
	if (kernel_alg_add(SADB_SATYPE_ESP, SADB_EXT_SUPPORTED_ENCRYPT, &alg) !=
	    1)
		loglog(RC_LOG_SERIOUS,
		       "Warning: failed to register algo_aes_gcm_12 for ESP");
	...
	if (kernel_alg_add(SADB_SATYPE_ESP, SADB_EXT_SUPPORTED_ENCRYPT, &alg) !=
	    1)
		loglog(RC_LOG_SERIOUS,
		       "Warning: failed to register algo_aes_gcm_16 for ESP");
	...
	if (kernel_alg_add(SADB_SATYPE_ESP, SADB_EXT_SUPPORTED_ENCRYPT, &alg) !=
	    1)
		loglog(RC_LOG_SERIOUS,
		       "Warning: failed to register algo_aes_ccm_8 for ESP");

	alg.sadb_alg_id = SADB_X_EALG_AES_CCM_ICV12;
	if (kernel_alg_add(SADB_SATYPE_ESP, SADB_EXT_SUPPORTED_ENCRYPT, &alg) !=
	    1)
		loglog(RC_LOG_SERIOUS,
		       "Warning: failed to register algo_aes_ccm_12 for ESP");

	alg.sadb_alg_id = SADB_X_EALG_AES_CCM_ICV16;
	if (kernel_alg_add(SADB_SATYPE_ESP, SADB_EXT_SUPPORTED_ENCRYPT, &alg) !=
	    1)
		loglog(RC_LOG_SERIOUS,
		       "Warning: failed to register algo_aes_ccm_16 for ESP");
 ...
 */
bool netlink_raw_eroute(
	const ip_address *this_host, const ip_subnet *this_client,
	...
	time_t use_lifetime UNUSED, enum pluto_sadb_operations sadb_op,
	const char *text_said, char *policy_label UNUSED)
{
	...
	if (DBGP(DBG_NETKEY)) {
		...
		DBG_log("creating SPD to %s->spi=%08x@%s proto=%u", sa_this,
			htonl(spi), sa_that, proto);
	}
	...
	if (policy_label != NULL) {
		...
		struct rtattr *attr =
			(struct rtattr *)((char *)&req + req.n.nlmsg_len);
 ...
 */
bool netlink_add_sa(struct kernel_sa *sa, bool replace)
{
	struct {
		struct nlmsghdr n;
	...
	if (DBGP(DBG_NETKEY)) {
		DBG_log("%s SA spi=%08x@%08x proto=%u family=%u",
			req.n.nlmsg_type == XFRM_MSG_UPDSA ? "updating" :
							     "creating",
			htonl(req.p.id.spi), htonl(req.p.id.daddr.a4),
			req.p.id.proto, req.p.family);
	}
	 ...
	 */
		...
		req.p.replay_window =
			sa->replay_window > 32 ? 32 : sa->replay_window;
		...
		if (sa->esp_info.authkeylen) {
			const char *name =
				sa->esp_info.auth_info->kernel_alg_info
					->kernel_alg_name;
			...
			switch (sa->esp_info.auth_info->kernel_alg_info
					->kernel_alg_id) {
			...
			case SADB_X_AALG_SHA2_512HMAC: {
				...
				memcpy((char *)RTA_DATA(attr) + sizeof(algo),
				       sa->authkey, sa->esp_info.authkeylen);
			...
			default: {
				...
				algo_old.alg_key_len =
					sa->esp_info.authkeylen * BITS_PER_BYTE;
				attr->rta_type = XFRMA_ALG_AUTH;
				attr->rta_len =
					RTA_LENGTH(sizeof(algo_old) +
						   sa->esp_info.authkeylen);
				strcpy(algo_old.alg_name, name);
				memcpy(RTA_DATA(attr), &algo_old,
				       sizeof(algo_old));
				memcpy((char *)RTA_DATA(attr) +
					       sizeof(algo_old),
				       sa->authkey, sa->esp_info.authkeylen);
		...
		if (sa->esatype == ET_IPCOMP) {
			...
			const char *name =
				sparse_name(calg_list, sa->esp_info.transid);
			...
			memcpy(RTA_DATA(attr), &algo, sizeof(algo));
			memcpy((char *)RTA_DATA(attr) + sizeof(algo),
			       sa->enckey, sa->esp_info.enckeylen);
			...
			const char *name =
				sa->esp_info.encr_info->kernel_alg_info
					->kernel_alg_name;
			if (name == NULL) {
				loglog(RC_LOG_SERIOUS,
		...
		if (sa->natt_type) {
			...
			attr = (struct rtattr
					*)((char *)attr +
					   attr->rta_len); /* ??? why is this not used? */
		}
 ...
 * @return bool True if successfull
 */
bool netlink_del_sa(const struct kernel_sa *sa)
{
	struct {
		struct nlmsghdr n;
...
void linux_pfkey_register_response(const struct sadb_msg *msg)
{
...
extern void netlink_acquire(struct nlmsghdr *n)
{
	...
	if (!found_sec_ctx) {
		...
		DBG(DBG_NETKEY, DBG_log("xfrm: remaining=%d , rta->len = %lu",
	...
	if (found_sec_ctx) {
		...
		DBG(DBG_NETKEY,
		    DBG_log("xfrm xuctx: exttype=%d, len=%d, ctx_doi=%d, ctx_alg=%d, ctx_len=%d",
			    xuctx->exttype, xuctx->len, xuctx->ctx_doi,
			    xuctx->ctx_alg, xuctx->ctx_len));
		DBG(DBG_NETKEY, DBG_log("xfrm xuctx security context: ",
					(xuctx + 1), xuctx->ctx_len));
		...
		if (xuctx->ctx_len <= MAX_SECCTX_LEN) {
			...
			if (uctx != NULL) {
				...
				uctx->ctx_len =
					xuctx->ctx_len; /*Length includes '\0'*/
...
void netlink_policy_expire(struct nlmsghdr *n)
{
...
ipsec_spi_t netlink_get_spi(const ip_address *src, const ip_address *dst,
...
retry:
	...
	DBG(DBG_NETKEY, DBG_log("netlink_get_spi: allocated 0x%x for %s",
...
bool netlink_sag_eroute(struct state *st, const struct spd_route *sr,
...
#define PROTO_INFO_COUNT 4
	...
	return eroute_connection(st, sr, inner_spi, inner_proto, inner_esatype,
				 proto_info + i, op, opname,
 ...
 */
bool netlink_eroute_idle(struct state *st, time_t idle_max)
{
	time_t idle_time;
...
bool netlink_shunt_eroute(struct connection *c, const struct spd_route *sr,
			  ...
			  const char *opname)
{
	...
	DBG(DBG_CONTROL,
	    DBG_log("request to %s a %s policy with netkey kernel --- experimental",
		    opname, enum_name(&routing_story, rt_kind)));
...

	{
		...
		return netlink_raw_eroute(&sr->this.host_addr, &sr->this.client,
					  fam->any, &sr->that.client,
					  htonl(spi), SA_INT, sr->this.protocol,
					  ET_INT, null_proto_info, 0, op, buf2,
					  c->policy_label);
	}
}

void netlink_process_raw_ifaces(struct raw_iface *rifaces)
{
...

		{
			...
			for (;;) {
				...
				if (q == NULL) {
					...
					q->ip_dev = id;
					id->id_rname = clone_str(
						ifp->name, "real device name");
					id->id_vname = clone_str(
						v->name,
					...
					openswan_log(
						"adding interface %s/%s %s:%d (%s)",
						q->ip_dev->id_vname,
						q->ip_dev->id_rname,
						ip_str(&q->ip_addr), q->port,
						q->socktypename);
 ...
 */
bool netlink_get_sa(const struct kernel_sa *sa, u_int *bytes)
{
	struct {
		struct nlmsghdr n;
		struct xfrm_usersa_id id;
