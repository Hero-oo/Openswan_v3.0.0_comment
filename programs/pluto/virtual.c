...
struct virtual_t *create_virtual(const struct connection *c, const char *string)
{
	...
	v = (struct virtual_t *)alloc_bytes(sizeof(struct virtual_t) +
...
fail:
	openswan_log("invalid virtual string [%s] - "
		     "virtual selection disabled for connection '%s'",
		     string, c->name);
	return NULL;
}
			     ...
			     const ip_address *his_addr)
{
	...
	if (c->spd.that.virt->flags & F_VIRTUAL_ALL) {
		...
		loglog(RC_LOG_SERIOUS,
		       ...
		       (c->spd.that.virt->flags & F_VIRTUAL_HOST) ? "host" :
								    "net");
		return NULL;
	}
...
void show_virtual_private()
{
	...
	whack_log(RC_COMMENT, "- allowed %d subnet%s: %s", private_net_ok_len,
		  (private_net_ok_len == 1) ? "" : "s", all_ok);

	whack_log(RC_COMMENT, "- disallowed %d subnet%s: %s",
		  private_net_ko_len, (private_net_ko_len == 1) ? "" : "s",
		  all_ko);
	if (truncok || truncko)
		whack_log(
			RC_COMMENT,
