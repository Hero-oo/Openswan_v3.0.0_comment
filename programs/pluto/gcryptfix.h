...
#define mpi_mulm(w, u, v, m)               \
	{                                  \
		mpz_mul((w), (u), (v));    \
		mpz_fdiv_r((w), (w), (m)); \
	}
#define mpi_fdiv_q(quot, dividend, divisor) \
	mpz_fdiv_q((quot), (dividend), (divisor))
#define mpi_fdiv_r(rem, dividend, divisor) \
	mpz_fdiv_r((rem), (dividend), (divisor))
#define mpi_fdiv_r_ui(rem, dividend, divisor) \
...
MPI generate_elg_prime(int mode, unsigned pbits, unsigned qbits, MPI g,
		       MPI **factors);
...
#define is_ELGAMAL(a) \
	((a) == PUBKEY_ALGO_ELGAMAL || (a) == PUBKEY_ALGO_ELGAMAL_E)
