...
bool doi_send_ikev2_certreq_thinking(struct state *st,
				     enum phase1_role role UNUSED)
{
	...
	if (!(st->st_connection->spd.that.ca.ptr != NULL)) {
		DBG(DBG_CONTROL, DBG_log("  no known CA for the other end"));
		unknown = FALSE;
	}
			   ...
			   enum phase1_role role, pb_stream *outpbs)
{
	...
	/*   send own (Initiator CERT) */
	{
		...
		if (!out_struct(&cert, &ikev2_certificate_desc, outpbs,
				&cert_pbs))
...
stf_status ikev2_send_certreq(struct state *st, struct msg_digest *md UNUSED,
			      enum phase1_role role UNUSED, pb_stream *outpbs)
{
	...
	if (that->cert_filename && that->cert.type == CERT_X509_SIGNATURE) {
		if (that->cert.u.x509->authKeyID.ptr) {
			...
			if (!ikev2_build_and_ship_CR(
				    CERT_X509_SIGNATURE,
				    that->cert.u.x509->authKeyID, outpbs,
				    ISAKMP_NEXT_NONE))
				return STF_INTERNAL_ERROR;
	...
	if (that_ca->ptr) {
		cacert = get_authcert(*that_ca, empty_chunk, empty_chunk,
				      AUTH_CA);
		if (cacert) {
			char buf[256];
			dntoa(buf, sizeof(buf), cacert->subject);
			DBG(DBG_CONTROL,
			    DBG_log("have CA '%s', send CERTREQ with CA's Key ID",
				    buf));
	...
	for (cacert = x509_get_authcerts_chain(); cacert;
	     cacert = cacert->next) {
	...
	pbs_set_np(outpbs, ISAKMP_NEXT_v2CERTREQ);
	success = ikev2_build_and_ship_CR(CERT_X509_SIGNATURE, allCAs, outpbs,
					  ISAKMP_NEXT_NONE);
...
bool doi_send_ikev2_cert_thinking(struct state *st)
{
	...
	send_cert = (c->policy & POLICY_RSASIG) && mycert.type != CERT_NONE &&
	...
	DBG(DBG_CONTROL, DBG_log(" my policy has %s RSASIG, the policy is : %s",
