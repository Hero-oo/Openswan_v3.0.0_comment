...
{
	DBG(DBG_CONTROL,
	    DBG_log("helper %d doing %s op id: %u", helpernum,
		    enum_show(&pluto_cryptoop_names, r->pcr_type), r->pcr_id));
#else
void pluto_do_crypto_op(struct pluto_crypto_req *r)
{
	DBG(DBG_CONTROL,
	    DBG_log("helper %d doing %s op id: %u", pc_helper_num,
		    enum_show(&pluto_cryptoop_names, r->pcr_type), r->pcr_id));
#endif
#ifdef DEBUG
	{
...
static void helper_passert_fail(const char *pred_str, const char *file_str,
				...
				unsigned long line_no)
{
	...
	if (chdir("helper") == -1) {
		...
		loglog(RC_LOG_SERIOUS,
		       "pluto: chdir() to 'helper' failed (%d %s)\n", e,
		       strerror(e));
	}
	osw_abort();
}
...
void pluto_crypto_helper(int fd, int helpernum)
{
	...
	DBG(DBG_CONTROL,
	    DBG_log("helper %d waiting on fd: %d", helpernum, fileno(in)));

	memset(reqbuf, 0, sizeof(reqbuf));
	while (fread((char *)reqbuf, sizeof(r->pcr_len), 1, in) == 1) {
		int restlen;
		int actnum;
		unsigned char *reqrest =
			((unsigned char *)reqbuf) + sizeof(r->pcr_len);
		...
		DBG(DBG_CONTROL,
		    DBG_log("helper %d read %d+4/%d bytes fd: %d", helpernum,
			    actnum, (int)r->pcr_len, fileno(in)));
	...
	/* probably normal EOF */
	loglog(RC_LOG_SERIOUS,
	       "pluto_crypto_helper: helper (%d) is exiting (eof, normal)\n",
	       helpernum);
...
void pluto_crypto_helper(int fd, int helpernum)
{
	...
	while (fread((char *)reqbuf, sizeof(r->pcr_len), 1, in) == 1) {
		...
		DBG(DBG_CONTROL,
		    DBG_log("helper %d read %d+4/%d bytesfd: %d", helpernum,
			    actnum, (int)r->pcr_len, fileno(in)));
	...
	fclose(out);
	loglog(RC_LOG_SERIOUS,
	       "pluto_crypto_helper: helper [nonnss] (%d) is exiting normally\n",
	       helpernum);
	exit(0);
}
#endif
				 ...
				 struct pluto_crypto_req *r)
{
	...
	DBG(DBG_CONTROL,
	    DBG_log("asking helper %d to do %s op on seq: %u (len=%u, pcw_work=%d)",
		    w->pcw_helpernum,
		    enum_show(&pluto_cryptoop_names, r->pcr_type), r->pcr_id,
		    (unsigned int)r->pcr_len, w->pcw_work + 1));
				 ...
				 bool *toomuch)
{
	...
	do {
		...
		if (w->pcw_dead == TRUE && w->pcw_reaped == TRUE) {
	...
	} while (((w->pcw_work >= w->pcw_maxbasicwork)) && --cnt > 0);

	if (cnt == 0 && r->pcr_pcim > pcim_ongoing_crypto) {
		cnt = pc_workers_cnt;
		while ((w->pcw_work >= w->pcw_maxcritwork) && --cnt > 0) {
			/* find an available worker */
			pc_worker_num++;
			if (pc_worker_num >= pc_workers_cnt) {
				pc_worker_num = 0;
	...
	if (cnt == 0 && r->pcr_pcim >= pcim_demand_crypto) {
		...
		if (pbs_offset(&reply_stream)) {
			cn->pcrc_reply_buffer = clone_bytes(
				reply_stream.start, pbs_offset(&reply_stream),
				"saved reply buffer");
		}
...
void handle_helper_comm(struct pluto_crypto_worker *w)
{
	...
	if (r->pcr_len > sizeof(reqbuf)) {
		loglog(RC_LOG_SERIOUS,
		       ...
		       w->pcw_helpernum, (unsigned long)w->pcw_pid,
		       (unsigned long)r->pcr_len,
...
killit:
	...
	DBG(DBG_CRYPT, DBG_log("calling callback function %p", cn->pcrc_func));
...
static void init_crypto_helper(struct pluto_crypto_worker *w, int n)
{
	...
	{
		int qlen =
			w->pcw_maxcritwork * sizeof(struct pluto_crypto_req) +
			10;
	...
	if (thread_status != 0) {
		...
		openswan_log("started helper (thread) pid=%ld (fd:%d)",
...
static void cleanup_crypto_helper(struct pluto_crypto_worker *w, int status)
{
	if (w->pcw_pipe) {
		loglog(RC_LOG_SERIOUS,
		       "closing helper(%lu) pid=%lu fd=%d exit=%d",
		       (unsigned long)w->pcw_helpernum,
		       (unsigned long)w->pcw_pid, w->pcw_pipe, status);
		close(w->pcw_pipe);
	}
