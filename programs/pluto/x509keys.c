		...
		if (cert->isacert_type == CERT_X509_SIGNATURE) {
			...
			if (parse_x509cert(blob, 0, &cert2)) {
				if (verify_x509cert(&cert2, strict_crl_policy,
					...
					add_x509_public_key_to_list(
						&st->st_keylist, NULL, &cert2,
		...
		if (v2cert->isac_enc == CERT_X509_SIGNATURE) {
			...
			if (parse_x509cert(blob, 0, &cert2)) {
				if (verify_x509cert(&cert2, strict_crl_policy,
					...
					dntoa(sbuf, ASN1_BUF_LEN,
					      cert2.subject);
					openswan_log(
						"%u: validated X509 certificate: '%s', added to trusted database",
						certnum, sbuf);
					DBG(DBG_X509 | DBG_PARSING,
					    DBG_log("Public key validated"));
			...
			if (parse_pkcs7_cert(blob, &cert2)) {
				...
				openswan_log(
					"%u: validated pkcs7 certificate: '%s', added to trusted database",
					certnum, sbuf);
				store_x509certs(&cert2, strict_crl_policy);
			} else
				plog("Syntax error in PKCS#7 wrapped X.509 certificates");
			...
			loglog(RC_LOG_SERIOUS,
			       "%u: ignoring %s certificate payload", certnum,
 ...
 */
void ikev1_decode_cr(struct msg_digest *md, generalName_t **requested_ca_names)
{
	struct payload_digest *p;
	...
	for (p = md->chain[ISAKMP_NEXT_CR]; p != NULL; p = p->next) {
		...
		if (cr->isacr_type == CERT_X509_SIGNATURE) {
			...
			DBG(DBG_PARSING | DBG_CONTROL, char buf[IDTOA_BUF];
 ...
 */
void ikev2_decode_cr(struct msg_digest *md, generalName_t **requested_ca_hashes)
{
	struct payload_digest *p;
	...
	for (p = md->chain[ISAKMP_NEXT_v2CERTREQ]; p != NULL; p = p->next) {
		...
		for (key.ptr = all_keys.ptr; key.ptr < end_keys;
		     key.ptr += key.len) {
			size_t remaining;
			generalName_t *gn;
...
bool ikev2_build_and_ship_CR(u_int8_t type, chunk_t keyIDs, pb_stream *outs,
			     u_int8_t np)
{
	pb_stream cr_pbs;
	...
	if (keyIDs.ptr == NULL) {
		DBG(DBG_X509,
		    DBG_log("failed to send CERTREQ, no CA keyIDs specified"));
		return FALSE;
	}
	...
	if (!out_struct(&cr_hd, &ikev2_certificate_req_desc, outs, &cr_pbs)) {
		DBG(DBG_X509,
		    DBG_log("failed to send CERTREQ, out_struct() failed"));
		return FALSE;
	}
	...
	if (!out_chunk(keyIDs, &cr_pbs, "CA")) {
		DBG(DBG_X509,
		    DBG_log("failed to send CERTREQ, out_chunk() failed"));
		return FALSE;
	}
...
bool collect_rw_ca_candidates(struct msg_digest *md, generalName_t **top)
{
	struct connection *d = find_host_connection(
		ANY_MATCH, &md->iface->ip_addr, pluto_port500, KH_ANY,
