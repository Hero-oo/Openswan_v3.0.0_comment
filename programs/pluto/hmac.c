...
void hmac_init(struct hmac_ctx *ctx, const struct ike_integ_desc *h,
	       const u_char *key, size_t key_len)
{
	...
	PK11SymKey *tkey2 = pk11_derive_wrapper_osw(
		tkey1, CKM_CONCATENATE_BASE_AND_DATA, hmac_pad,
...
void hmac_update(struct hmac_ctx *ctx, const u_char *data, size_t data_len)
{
#ifdef HAVE_LIBNSS
...
PK11SymKey *pk11_derive_wrapper_osw(PK11SymKey *base,
				    CK_MECHANISM_TYPE mechanism, chunk_t data,
			    ...
			    CK_ATTRIBUTE_TYPE operation, int keysize)
{
	...
	if (((mechanism == CKM_SHA256_KEY_DERIVATION) ||
		...
		default:
			DBG(DBG_CRYPT,
			    DBG_log("PK11_Derive_osw: Invalid NSS mechanism "));
		...
		PK11SymKey *tkey1 = pk11_derive_wrapper_osw(
			base, CKM_CONCATENATE_DATA_AND_BASE, dkey_chunk,
...
void nss_symkey_log(PK11SymKey *key, const char *msg)
{
	...
	if (!PK11_IsFIPS()) {
		if (key != NULL) {
			...
			DBG(DBG_CRYPT,
			    DBG_dump("value: ", keydata->data, keydata->len));
