...
static void key_add_ugh(const struct id *keyid, err_t ugh)
{
	...
	loglog(RC_NOKEY, "failure to fetch key for %s from DNS: %s", name, ugh);
}
...
static void key_add_request(const struct whack_message *msg)
{
	...
	if (ugh != NULL) {
		...
		if (msg->keyval.len == 0) {
			struct key_add_common *oc = alloc_thing(
				struct key_add_common, "key add common things");
			enum key_add_attempt kaa;
			...
			for (kaa = ka_TXT; kaa != ka_roof; kaa++) {
...
#ifdef USE_KEYRR
				case ka_KEY:
					ugh = start_adns_query(&keyid, NULL,
							       ns_t_key,
							       key_add_continue,
							       &kc->ac);
					break;
				...
				default:
					bad_case(
						kaa); /* suppress gcc warning */
				}
				if (ugh != NULL) {
					oc->diag[kaa] = clone_str(
						ugh, "early key add failure");
					oc->refCount--;
				}
			}
	     ...
	     */
		...
		} else {
			{
				...
				sha256_hash_buffer(msg->keyval.ptr,
						   msg->keyval.len, key_ckaid,
						   sizeof(key_ckaid));
...
void whack_process(int whackfd, struct whack_message msg)
{
	...
	if (msg.whack_initiate) {
		if (!listening)
			...
			initiate_connection(msg.name,
					    msg.whack_async ? NULL_FD :
							      dup_any(whackfd),
					    msg.debugging, pcim_demand_crypto);
	}
	...
	if (msg.whack_oppo_initiate) {
		if (!listening)
			...
			(void)initiate_ondemand(
				&msg.oppo_my_client, &msg.oppo_peer_client, 0,
				FALSE,
...
void whack_handle(int whackctlfd)
{
	...
	/* sanity check message */
	{
		...
		if ((size_t)n < offsetof(struct whack_message, whack_shutdown) +
					sizeof(msg.whack_shutdown)) {
			ugh = builddiag(
			...
			if (msg.magic == WHACK_BASIC_MAGIC) {
				...
				if (msg.whack_shutdown) {
					openswan_log("shutting down");
					exit_pluto(
						0); /* delete lock and leave, with 0 status */
				}
				ugh = ""; /* bail early, but without complaint */
			} else {
				ugh = builddiag(
					"ignoring message from whack with bad magic %08x; should be %08x; Mismatched versions of userland tools and PLUTO code.",
 ...
 */
bool whack_prompt_for(int whackfd, const char *prompt1, const char *prompt2,
		      bool echo, char *ansbuf, size_t ansbuf_len)
{
	...
	whack_log(echo ? RC_XAUTHPROMPT : RC_ENTERSECRET,
		  "%s prompt for %s:", prompt1, prompt2);
