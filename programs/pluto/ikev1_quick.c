...
static bool emit_subnet_id(struct end *e, u_int8_t np, ip_address endpoint,
			   u_int8_t protoid, u_int16_t port, pb_stream *outs)
{
				 ...
				 const char *satypename)
{
	...
	case PROTO_IPSEC_ESP:
		if ((needed_len = kernel_alg_esp_enc_keylen(
			     pi->attrs.transattrs.encrypt)) > 0) {
...
#endif
		needed_space =
			needed_len + pad_up(needed_len, ctx_me.hmac_digest_len);
	...
	DBG(DBG_CRYPT, DBG_log("%s KEYMAT\n", satypename);
			  ...
			  ip_subnet *net, const char *which)
{
	...
	default:
		/* XXX support more */
		loglog(RC_LOG_SERIOUS, "unsupported ID type %s", idtypename);
	...
	case ID_IPV6_ADDR: {
		...
		ughmsg = initaddr(id_pbs->cur, pbs_left(id_pbs), afi->af,
				  &temp_address);
	...
	case ID_IPV6_ADDR_SUBNET: {
		...
		ughmsg = initaddr(id_pbs->cur, afi->ia_sz, afi->af,
				  &temp_address);
		if (ughmsg == NULL)
			ughmsg = initaddr(id_pbs->cur + afi->ia_sz, afi->ia_sz,
					  afi->af, &temp_mask);
		if (ughmsg == NULL)
			ughmsg = initsubnet(&temp_address,
					    masktocount(&temp_mask), '0', net);
	...
	case ID_IPV6_ADDR_RANGE: {
		...
		DBG(DBG_PARSING | DBG_CONTROL, {
			...
			subnettot(net, 0, temp_buff, sizeof(temp_buff));
			DBG_log("%s is subnet %s (received as range)", which,
				temp_buff);
		});
		break;
	}
...
static bool check_net_id(struct isakmp_ipsec_id *id, pb_stream *id_pbs,
			 u_int8_t *protoid, u_int16_t *port, struct end *end,
			 ip_address *endip, const char *which)
{
	ip_subnet net_temp;
	...
	if (end->has_client && !samesubnet(&end->client, &net_temp)) {
		loglog(RC_LOG_SERIOUS,
		       "%s subnet returned does not match my proposal - us:%s vs them:%s",
		       which, subxmt, subrec);
		...
		loglog(RC_LOG_SERIOUS,
		       "%s subnet returned does not match my self-proposal - us:%s vs them:%s",
		       which, subxmt, subrec);
#ifdef ALLOW_MICROSOFT_BAD_PROPOSAL
		loglog(RC_LOG_SERIOUS,
		       "Allowing questionable self-proposal anyway [ALLOW_MICROSOFT_BAD_PROPOSAL]");
		bad_proposal = FALSE;
#else
		bad_proposal = TRUE;
#endif
 ...
 */
size_t quick_mode_hash12(u_char *dest, const u_char *start, const u_char *roof,
...
#define hmac_update(ctx, ptr, len)                    \
	...
	if (hash2)
		hmac_update_chunk(&ctx,
				  st->st_ni); /* include Ni_b in the hash */
	hmac_update(&ctx, start, roof - start);
	hmac_final(dest, &ctx);

	DBG(DBG_CRYPT, DBG_log("HASH(%d) computed:", hash2 + 1);
...
void init_phase2_iv(struct state *st, const msgid_t *msgid)
{
	...
	DBG_cond_dump(DBG_CRYPT, "last Phase 1 IV:", st->st_ph1_iv,
		      st->st_ph1_iv_len);

	st->st_new_iv_len = h->hash_digest_len;
	passert(st->st_new_iv_len <= sizeof(st->st_new_iv));

	DBG_cond_dump(DBG_CRYPT, "current Phase 1 IV:", st->st_iv,
		      st->st_iv_len);
	...
	DBG_cond_dump(DBG_CRYPT, "computed Phase 2 IV:", st->st_new_iv,
		      st->st_new_iv_len);
}
				 ...
				 struct pluto_crypto_req *r, err_t ugh)
{
	...
	struct state *const st =
		state_with_serialno(qke->qke_pcrc.pcrc_serialno);
	stf_status e;
		       ...
		       struct xfrm_user_sec_ctx_ike *uctx UNUSED)
{
	...
	if (uctx != NULL) {
		...
		DBG(DBG_CONTROL,
		    DBG_log("pending phase 2 with security context %s, %d",
			    st->sec_ctx->sec_ctx_value, st->sec_ctx->ctx_len));
	}
#endif
	...
	if (st->st_connection) {
		st->st_pfs_group =
			ike_alg_pfsgroup(st->st_connection, st->st_policy);
	}
	...
	if (!st->st_pfs_group)
		st->st_pfs_group =
			policy & POLICY_PFS ? isakmp_sa->st_oakley.group : NULL;

	if (policy & POLICY_PFS && st->st_pfs_group) {
		pfsgroupname =
			enum_name(&oakley_group_names, st->st_pfs_group->group);
	}

	{
		...
		openswan_log(
			...
			prettypolicy(policy), replacestr,
...
static stf_status quick_outI1_tail(struct pluto_crypto_req_cont *pcrc,
				   struct pluto_crypto_req *r, struct state *st)
{
	struct qke_continuation *qke = (struct qke_continuation *)pcrc;
     ...
     */
	{
		...
		if (!out_sa(&rbody, st->st_sadb, st, /*oakley_mode*/ FALSE,
			    INITIATOR, /* aggr */ FALSE, ISAKMP_NEXT_NONCE)) {
			reset_cur_state();
			return STF_INTERNAL_ERROR;
		}
	}

	{
		...
		/* Ni out */
		if (!ship_nonce(&st->st_ni, r, &rbody, np, "Ni")) {
			reset_cur_state();
			return STF_INTERNAL_ERROR;
		}
	}
	...
	/* [ KE ] out (for PFS) */
	if (st->st_pfs_group != NULL) {
		if (!ship_KE(st, r, &st->st_gi, &rbody,
	...
	if (has_client) {
		...
		if (!emit_subnet_id(&c->spd.this, ISAKMP_NEXT_ID,
				    st->st_localaddr, st->st_myuserprotoid,
				    st->st_myuserport, &rbody) ||
		    !emit_subnet_id(&c->spd.that, ISAKMP_NEXT_NONE,
				    st->st_remoteaddr, st->st_peeruserprotoid,
	...
	(void)quick_mode_hash12(r_hashval, r_hash_start, rbody.cur, st,
				&st->st_msgid, FALSE);
	...
	if (qke->replacing == SOS_NOBODY)
		whack_log(RC_NEW_STATE + STATE_QUICK_I1, "%s: initiate",
...
static const char *const verify_step_name[] = { "vos_fail",	  "vos_start",
						"vos_our_client", "vos_our_txt",
#ifdef USE_KEYRR
						"vos_our_key",
#endif /* USE_KEYRR */
						"vos_his_client", "vos_done" };
...
stf_status quick_inI1_outR1(struct msg_digest *md)
{
	...
	CHECK_QUICK_HASH(md,
			 quick_mode_hash12(hash_val, hash_pbs->roof,
					   md->message_pbs.roof, p1st,
					   &md->hdr.isa_msgid, FALSE),
			 "HASH(1)", "Quick I1");
...
static void report_verify_failure(struct verify_oppo_bundle *b, err_t ugh)
{
	...
	char fgwb[ADDRTOT_BUF], cb[ADDRTOT_BUF];
	ip_address client;
	err_t which;
					       ...
					       enum verify_oppo_step next_step)
{
	...
	struct connection *c = p1st->st_connection;
	struct verify_oppo_continuation *vc = alloc_thing(
		struct verify_oppo_continuation, "verify continuation");
	...
	case vos_our_client:
		...
		ugh = start_adns_query(&id, our_id, ns_t_txt,
				       quick_inI1_outR1_continue, &vc->ac);
		break;

	case vos_our_txt:
		vc->b.failure_ok = b->failure_ok = TRUE;
		ugh = start_adns_query(our_id, our_id /* self as SG */
				       ,
				       ns_t_txt, quick_inI1_outR1_continue,
				       &vc->ac);
		break;
	...
	case vos_our_key:
		vc->b.failure_ok = b->failure_ok = FALSE;
		ugh = start_adns_query(our_id, NULL, ns_t_key,
				       quick_inI1_outR1_continue, &vc->ac);
		break;
#endif
	...
	case vos_his_client:
		...
		iptoid(&client, &id);
		vc->b.failure_ok = b->failure_ok = FALSE;
		ugh = start_adns_query(&id, &c->spd.that.id, ns_t_txt,
				       quick_inI1_outR1_continue, &vc->ac);
		break;
				...
				struct state *p1st)
{
	...
	DBG(DBG_CONTROL, {
		...
		subnettot(&c->spd.this.client, 0, ours, sizeof(ours));
		subnettot(&c->spd.that.client, 0, his, sizeof(his));
		DBG_log("responding on demand from %s to %s state: %s", ours,
			his, verify_step_name[b->step]);
	});
		...
		next_step = vos_his_client;
		{
			...
			}
			{
				...
				for (gwp = ac->gateways_from_dns; gwp != NULL;
					...
					ugh = NULL;
#else
					ugh = "our client delegation depends on our " RRNAME
	...
	case vos_our_key:
		next_step = vos_his_client;
		{
			...
			}
			{
				...
				ugh = "our client delegation depends on our missing " RRNAME
				      " record";
				for (kp = ac->keys_from_dns; kp != NULL;
				     kp = kp->next) {
					ugh = "our client delegation depends on our " RRNAME
					      " record, but it has the wrong public key";
					if (same_RSA_public_key(
						    &pks->pub->u.rsa,
						    &kp->key->u.rsa)) {
...
/* forward definitions */
static stf_status quick_inI1_outR1_cryptotail(struct dh_continuation *dh,
					      struct pluto_crypto_req *r);

static void quick_inI1_outR1_cryptocontinue2(struct pluto_crypto_req_cont *pcrc,
					     struct pluto_crypto_req *r,
					     err_t ugh);

static void quick_inI1_outR1_cryptocontinue1(struct pluto_crypto_req_cont *pcrc,
					     struct pluto_crypto_req *r,
					     err_t ugh);
					    ...
					    struct adns_continuation *ac)
{
	...
	struct connection *c = p1st->st_connection;
	ip_subnet *our_net = &b->my.net, *his_net = &b->his.net;
	struct end our, peer;
	struct hidden_variables hv;
	...
	} else if (is_virtual_net_allowed(c, &peer.client,
					  &p1st->st_remoteaddr)) {
		peer.client_is_self = TRUE;
	}

	{
		...
		openswan_log("the peer proposed: %s -> %s (self=%s)", s1, d1,
			     peer.client_is_self ? "true" : "false");
	}
     ...
     */
	{
	     ...
	     */
			struct end me = c->spd.this, he = c->spd.that;
			char buf[2 * SUBNETTOT_BUF + 2 * ADDRTOT_BUF +
				 2 * IDTOA_BUF + 2 * ADDRTOT_BUF +
				 12]; /* + 12 for separating */
			size_t l;
		 ...
		 */
		     ...
		     */
					p = oppo_instantiate(
						p, &c->spd.that.host_addr,
						&c->spd.that.id, NULL,
						&our_client, &his_client);
				} else {
					/* Plain Road Warrior:
	...
	if (is_virtual_connection(c)) {
		...
		format_end(cthat, sizeof(cthat), &c->spd.that, NULL, TRUE,
			   LEMPTY);
		DBG(DBG_CONTROLMORE,
		    DBG_log("setting phase 2 virtual values to %s", cthat));
	}
	 ...
	 */
	     ...
	     */
			...
			RETURN_STF_FAILURE(parse_ipsec_sa_body(
				&in_pbs, &sapd->payload.sa, NULL, FALSE, st));
		}
		...
		/* [ KE ] in (for PFS) */
		RETURN_STF_FAILURE(
			accept_PFS_KE(md, &st->st_gi, "Gi", "Quick Mode I1"));
...

		{
			struct qke_continuation *qke = alloc_thing(
				struct qke_continuation, "quick_outI1 KE");
					     ...
					     err_t ugh)
{
	...
	if (st->st_pfs_group != NULL) {
		struct dh_continuation *dh =
			alloc_thing(struct dh_continuation, "quick outR1 DH");
		...
		e = start_dh_secret(&dh->dh_pcrc, st, st->st_import, RESPONDER,
					      ...
					      struct pluto_crypto_req *r)
{
	...
	RETURN_STF_FAILURE(parse_ipsec_sa_body(&sapd->pbs, &sapd->payload.sa,
					       &r_sa_pbs, FALSE, st));
	...
	openswan_log("responding to Quick Mode proposal {msgid:%08x}",
		     st->st_msgid);
	{
		...
		format_end(instbuf, sizeof(instbuf), &sr->this, &sr->that, TRUE,
			   LEMPTY);
		openswan_log("    us: %s", instbuf);

		format_end(instbuf, sizeof(instbuf), &sr->that, &sr->this,
			   FALSE, LEMPTY);
		openswan_log("  them: %s", instbuf);
	}
...

	{
		...
		if (padstr) {
			...
			openswan_log("inserting fake VID payload of %u size",
				     padsize);
			...
			if (!out_generic(np, &isakmp_vendor_id_desc, &md->rbody,
					 &vid_pbs))
				return STF_INTERNAL_ERROR;
	...
	if (st->st_pfs_group != NULL && r != NULL) {
		if (!justship_KE(&st->st_gr, &md->rbody,
				 id_pd != NULL ? ISAKMP_NEXT_ID :
						 ISAKMP_NEXT_NONE))
			return STF_INTERNAL_ERROR;
	...
	(void)quick_mode_hash12(r_hashval, r_hash_start, md->rbody.cur, st,
				&st->st_msgid, TRUE);
 ...
 */
static stf_status quick_inR1_outI2_cryptotail(struct dh_continuation *dh,
					      struct pluto_crypto_req *r);
...
stf_status quick_inR1_outI2(struct msg_digest *md)
{
	...
	/* SA in */
	{
		...
		RETURN_STF_FAILURE(parse_ipsec_sa_body(
			&sa_pd->pbs, &sa_pd->payload.sa, NULL, TRUE, st));
	}
	...
	/* [ KE ] in (for PFS) */
	RETURN_STF_FAILURE(
		accept_PFS_KE(md, &st->st_gr, "Gr", "Quick Mode R1"));

	if (st->st_pfs_group) {
		struct dh_continuation *dh =
			alloc_thing(struct dh_continuation, "quick outI2 DH");
		...
		return start_dh_secret(&dh->dh_pcrc, st, st->st_import,
				       INITIATOR, st->st_pfs_group->group);
	} else {
...
stf_status quick_inR1_outI2_cryptotail(struct dh_continuation *dh,
				       struct pluto_crypto_req *r)
{
	struct msg_digest *md = dh->md;
	struct state *st = md->st;
	     ...
	     */
			...
			if (!subnetisaddr(&c->spd.this.client,
			    ...
			    !subnetisaddr(&c->spd.that.client,
					  &c->spd.that.host_addr)) {
				loglog(RC_LOG_SERIOUS,
				       "IDci, IDcr payloads missing in message"
	...
	/* HASH(3) out -- sometimes, we add more content */
	{
...
#ifdef TPM
		{
			pb_stream *pbs = &md->rbody;
			size_t enc_len =
				pbs_offset(pbs) - sizeof(struct isakmp_hdr);
		...
		r_hash_start =
			r_hash_start; /* otherwise complaint about never used */
	}
...

	{
		DBG(DBG_CONTROLMORE,
		    DBG_log("inR1_outI2: instance %s[%ld], setting newest_ipsec_sa to #%ld (was #%ld) (spd.eroute=#%ld)",
			    ...
			    st->st_connection->instance_serial, st->st_serialno,
...
stf_status quick_inI2(struct msg_digest *md)
{
	...
	/* HASH(3) in */
	CHECK_QUICK_HASH(md, quick_mode_hash3(hash_val, st), "HASH(3)",
			 "Quick I2");
