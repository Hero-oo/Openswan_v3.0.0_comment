...
struct traffic_selector ikev2_end_to_ts(struct end *e, ip_address endpoint)
{
	...
	case AF_INET:
		...
		ts.low.u.v4.sin_addr.s_addr &=
			bitstomask(clientnet.maskbits).s_addr;
...
stf_status ikev2_emit_ts(struct msg_digest *md UNUSED, pb_stream *outpbs,
			 unsigned int np, struct traffic_selector *ts)
{
	struct ikev2_ts its;
	...
	case IKEv2_TS_IPV4_ADDR_RANGE:
		...
		its1.isat1_sellen =
			2 * 4 +
			8; /* See RFC 5669 SEction 13.3.1, 8 octet header plus 2 ip addresses */
		break;
...
stf_status ikev2_calc_emit_ts(struct msg_digest *md, pb_stream *outpbs,
			      enum phase1_role role, struct connection *c0,
...
#else
#define SET_SIN_LEN(x) \
	do {           \
	} while (0)
#define SET_SIN6_LEN(x) \
	do {            \
	} while (0)
#endif
...
int ikev2_parse_ts(struct payload_digest *const ts_pd,
		   struct traffic_selector *array, unsigned int array_max)
{
				...
				const char *which, int index)
{
	...
	DBG(DBG_CONTROL,
	    DBG_log("protocol %d and %s[%d].ipprotoid %d: %s match", proto,
		    which, index, ts_proto, m));
	return f;
}
					   ...
					   int *best_tsi_i, int *best_tsr_i)
{
	...
	for (tsi_ni = 0; tsi_ni < tsi_n; tsi_ni++) {
		...
		int fitrange_i = ikev2_match_protocol(
			ei->protocol, tsi[tsi_ni].ipprotoid,
			role == RESPONDER && narrowing,
			role == INITIATOR && narrowing, "tsi", tsi_ni);

		if (fitrange_i == 0)
			continue; /* save effort! */

		for (tsr_ni = 0; tsr_ni < tsr_n; tsr_ni++) {
			int fitrange_r = ikev2_match_protocol(
				er->protocol, tsr[tsr_ni].ipprotoid,
				role == RESPONDER && narrowing,
				role == INITIATOR && narrowing, "tsr", tsr_ni);
			...
			matchiness =
				fitrange_i +
				fitrange_r; /* ??? arbitrary objective function */
				  ...
				  const char *which, int index)
{
	...
	DBG(DBG_CONTROL,
	    DBG_log("   %s[%d] %u-%u: %s port match with %u.  fitness %d",
		    which, index, ts.startport, ts.endport, m, port, f));
	return f;
}
...
int ikev2_evaluate_connection_port_fit(const struct connection *d,
				       ...
				       const struct traffic_selector *tsi,
				       const struct traffic_selector *tsr,
				       int tsi_n, int tsr_n, int *best_tsi_i,
				       int *best_tsr_i)
{
	int tsi_ni;
	int bestfit_p = -1;
	...
	for (tsi_ni = 0; tsi_ni < tsi_n; tsi_ni++) {
		int tsr_ni;
		int fitrange_i = ikev2_match_port_range(
			ei->port, tsi[tsi_ni], role == RESPONDER && narrowing,
			role == INITIATOR && narrowing, "tsi", tsi_ni);
		...
		for (tsr_ni = 0; tsr_ni < tsr_n; tsr_ni++) {
			int fitrange_r = ikev2_match_port_range(
				er->port, tsr[tsr_ni],
				role == RESPONDER && narrowing,
				role == INITIATOR && narrowing, "tsr", tsr_ni);
			...
			DBG(DBG_CONTROL,
			    DBG_log("      evaluating_connection_port_fit tsi_n[%d] tsr_n[%d], range=%d/%d best=%d",
				    tsi_ni, tsr_ni, fitrange_i, fitrange_r,
				    bestfit_p));
...
static int ikev2_validate_transport_proposal(
	struct connection *d, struct state *st, enum phase1_role role,
	struct traffic_selector *tsi, struct traffic_selector *tsr,
	unsigned int tsi_n, unsigned int tsr_n)
{
	 ...
	 * with a transport mode connection */
		...
		loglog(RC_LOG_SERIOUS,
		       "received TSi[%d] selector with range %s~%s, "
	 ...
	 * with a transport mode connection */
		...
		loglog(RC_LOG_SERIOUS,
		       "received TSr[%d] selector with %s~%s, "
		       "incompatible with TRANSPORT mode -- refusing",
		       tsr_ni, a0, a1);
		return v2N_TS_UNACCEPTABLE;
	}
...
int ikev2_evaluate_connection_fit(struct connection *d, struct state *st,
				  struct spd_route *sr, enum phase1_role role,
				  ...
				  unsigned int tsi_n, unsigned int tsr_n)
{
	...
	DBG(DBG_CONTROLMORE, {
		...
		DBG_log("  ikev2_evaluate_connection_fit evaluating our "
			...
			d->name, ei3, ei->protocol, ei->port, er3, er->protocol,
			er->port, is_virtual_connection(d) ? "(virt)" : "");
	});
		...
		for (tsr_ni = 0; tsr_ni < tsr_n; tsr_ni++) {
			...
			DBG(DBG_CONTROLMORE, {
				...
				DBG_log("    tsi[%u]=%s/%s proto=%d portrange %d-%d",
					tsi_ni, lbi, hbi, tsi[tsi_ni].ipprotoid,
					tsi[tsi_ni].startport,
					tsi[tsi_ni].endport);
				DBG_log("    tsr[%u]=%s/%s proto=%d portrange %d-%d",
					tsr_ni, lbr, hbr, tsr[tsr_ni].ipprotoid,
		 ...
		 */
				...
				DBG(DBG_CONTROLMORE, {
					DBG_log("      has ts_range1=%u maskbits1=%u ts_range2=%u maskbits2=%u fitbits=%d <> %d",
						ts_range1, maskbits1, ts_range2,
						maskbits2, fitbits, bestfit);
				});
...
stf_status ikev2_child_ts_evaluate(struct traffic_selector tsi[16],
				   ...
				   unsigned int tsr_n, enum phase1_role role,
				   struct state *pst, struct connection *c,
				   ...
				   struct spd_route **best_sr)
{
     ...
     */
	{
		...
		DBG(DBG_CONTROLMORE,
		    DBG_log("ikev2_child_ts_evaluate, evaluating base fit for %s against tsi=%u,tsr=%u traffic selectors",
			    c->name, tsi_n, tsr_n));
		for (sra = &c->spd; sra != NULL; sra = sra->next) {
			int bfit_n = ikev2_evaluate_connection_fit(
				c, pst, sra, role, tsi, tsr, tsi_n, tsr_n);
			if (bfit_n > bestfit_n) {
				DBG(DBG_CONTROLMORE,
		...
		for (sra = &c->spd; hp == NULL && sra != NULL;
		     sra = sra->next) {
			hp = find_ID_host_pair(sra->this.id, sra->that.id);
			...
			for (d = hp->connections; d != NULL; d = d->IDhp_next) {
				...
				for (sr = &d->spd; sr != NULL; sr = sr->next) {
					newfit = ikev2_evaluate_connection_fit(
						d, pst, sr, role, tsi, tsr,
						tsi_n, tsr_n);
					if (newfit >
					    bestfit_n) { /// will complicated this with narrowing
						int bfit_p;
						...
						if (bfit_p > bestfit_p) {
							DBG(DBG_CONTROLMORE,
							    DBG_log("ikev2_evaluate_connection_port_fit found better fit d %s, tsi[%d],tsr[%d]",
								    d->name,
								    best_tsi_i,
								    best_tsr_i));
							bestfit_p = bfit_p;
						...
						DBG(DBG_CONTROLMORE,
						    DBG_log("prefix range fit d %s d->name was rejected by connection fit: %d > %d",
							    d->name, newfit,
							    bestfit_n));
					}
				}
	...
	if (bestfit_n > 0 && b != NULL) {
		DBG(DBG_CONTROLMORE,
		    DBG_log("ikev2_evaluate_connection_fit, concluded with %s",
			    b->name));
...
stf_status ikev2_child_sa_respond(struct msg_digest *md, struct state *st1,
				  pb_stream *outpbs)
{
...

	{
		...
		ret = ikev2_child_ts_evaluate(tsi, tsi_n, tsr, tsr_n, RESPONDER,
					      pst, c, &b, &bsr);
	 ...
	 */
		...
		if (b->kind == CK_TEMPLATE || b->kind == CK_GROUP) {
			...
			b = rw_instantiate(b, &pst->st_remoteaddr, NULL,
		...
		if (b != c) {
			...
			openswan_log("switched from \"%s\" to \"%s\"%s",
				     c->name, b->name,
				     fmt_connection_inst_name(
					     b, instance, sizeof(instance)));
		...
		st1->st_ts_this =
			ikev2_end_to_ts(&bsr->this, pst->st_localaddr);
		st1->st_ts_that =
			ikev2_end_to_ts(&bsr->that, pst->st_remoteaddr);
	...
	/* start of SA out */
	{
		...
		if (rn != NOTHING_WRONG) {
			...
			event_schedule(EVENT_SO_DISCARD,
				       EVENT_HALF_OPEN_TIMEOUT, st1);
			return STF_FAIL + rn;
		}
	}

	/* role is always RESPONDER, since we are replying to a request */
	ret = ikev2_calc_emit_ts(md, outpbs, RESPONDER, c, c->policy);
	if (ret != STF_OK) {
		return ret;
	}
 ...
 */
stf_status ipsec_outI1(int whack_sock, struct state *isakmp_sa,
		       ...
		       struct xfrm_user_sec_ctx_ike *uctx UNUSED)
{
...
#ifdef IKEV1
		return quick_outI1(whack_sock, isakmp_sa, c, policy, try,
				   replacing, uctx);
#else
	...
	return ikev2child_outC1(whack_sock, isakmp_sa, c, policy, try,
				replacing, uctx);
}

static void ikev2child_outC1_continue(struct pluto_crypto_req_cont *pcrc,
				      struct pluto_crypto_req *r, err_t ugh);

static stf_status ikev2child_outC1_tail(struct pluto_crypto_req_cont *pcrc,
					struct pluto_crypto_req *r);

stf_status ikev2child_outC1(
	int whack_sock, struct state *parentst, struct connection *c,
	lset_t policy, unsigned long try /* how many attempts so far */ UNUSED,
	so_serial_t replacing, struct xfrm_user_sec_ctx_ike *uctx UNUSED)
{
	struct state *st;
	...
	/* now. we need to go calculate the g^xy, if we want PFS (almost always do!!!) */
	{
		...
		ke->md->st = st;
		ke->md->transition_state =
			st; /* this have it's state transitioned by
					...
					struct pluto_crypto_req *r)
{
	...
	openswan_log(
		"starting rekey of CHILD SA for state=#%lu (expired) using PARENT SA #%lu as %s",
		st->st_replaced, st->st_clonedfrom,
	...
	/* HDR out */
	{
         ...
         * the parent SA.
         */
		r_hdr.isa_flags = ISAKMP_FLAGS_E |
				  IKEv2_ORIG_INITIATOR_FLAG(st);
	...
	if (c->policy & POLICY_PFS) {
		/* send KE */
		if (!justship_v2KE(st, &st->st_gi, st->st_oakley.groupnum,
				   &e_pbs_cipher, ISAKMP_NEXT_v2Ni))
			return STF_INTERNAL_ERROR;
	}
	...
	if (c0) {
		...
		ikev2_emit_ipsec_sa(md, &e_pbs_cipher, ISAKMP_NEXT_v2TSi, c0,
				    policy);

		st->st_ts_this =
			ikev2_end_to_ts(&c0->spd.this, st->st_localaddr);
		st->st_ts_that =
			ikev2_end_to_ts(&c0->spd.that, st->st_remoteaddr);
		...
		if (!(st->st_connection->policy & POLICY_TUNNEL)) {
			...
			ship_v2N(ISAKMP_NEXT_NONE, ISAKMP_PAYLOAD_NONCRITICAL,
				 0, &child_spi, v2N_USE_TRANSPORT_MODE,
				 &notify_data, &e_pbs_cipher);
		}
	}
...

	{
		...
		if (authloc == NULL || authloc < encstart)
			return STF_INTERNAL_ERROR;
		...
		ret = ikev2_encrypt_msg(md, role, authstart, iv, encstart,
...
static stf_status ikev2child_inCI1_tail(struct msg_digest *md, struct state *st,
					bool dopfs);
...
static stf_status ikev2child_inCI1_pfs(struct msg_digest *md)
{
	...
	if (is_suspended(st)) {
		openswan_log(
			"%s: already processing a suspended cyrpto operation "
			"on this SA, duplicate will be dropped.",
			__func__);
		return STF_TOOMUCHCRYPTO;
	}

	loglog(RC_COMMENT,
	       "msgid=%u CHILD_SA PFS rekey message received from %s:%u on %s (port=%d)",
	       md->msgid_received, ip_str(&md->sender),
	       (unsigned)md->sender_port, md->iface->ip_dev->id_rname,
	       md->iface->port);
...
static stf_status ikev2child_inCI1_nopfs(struct msg_digest *md)
{
	...
	loglog(RC_COMMENT,
	       "msgid=%u CHILD_SA no-PFS rekey message received from %s:%u on %s (port=%d)",
	       md->msgid_received, ip_str(&md->sender),
	       (unsigned)md->sender_port, md->iface->ip_dev->id_rname,
	       md->iface->port);
...
stf_status ikev2child_inCI1(struct msg_digest *md)
{
	...
	DBG(DBG_CONTROLMORE,
	    DBG_log("decrypting payload as %s",
				       ...
				       struct pluto_crypto_req *r, err_t ugh)
{
	...
	/* now. we need to go calculate the g^xy */
	{
		struct dh_continuation *dh =
			alloc_thing(struct dh_continuation, "ikev2_inCI1 KE");
		dh->md = md;
		...
		e = start_dh_v2(&dh->dh_pcrc, st, pcim_known_crypto, RESPONDER,
...
stf_status ikev2child_inCI1_tail(struct msg_digest *md, struct state *st,
				 bool dopfs)
{
	unsigned char *authstart;
	...
	/* send response */
	{
		...
		if (e.isag_np != ISAKMP_NEXT_NONE) {
			...
			if (!justship_v2Nonce(st, &e_pbs_cipher, &st->st_nr,
					      0)) {
				return STF_INTERNAL_ERROR;
			}
...

		{
			...
			if (IKEv2_IS_ORIG_INITIATOR(md->pst)) {
				DBG(DBG_CONTROLMORE,
				    DBG_log("encrypting payload as INITIATOR"));
				ret = ikev2_encrypt_msg(md, INITIATOR,
							authstart, iv, encstart,
							authloc, &e_pbs,
							&e_pbs_cipher);
			} else {
				DBG(DBG_CONTROLMORE,
				    DBG_log("encrypting payload as RESPONDER"));
...
static void ikev2child_inCR1_continue(struct pluto_crypto_req_cont *pcrc,
				      struct pluto_crypto_req *r, err_t ugh);
static stf_status ikev2child_inCR1_tail(struct msg_digest *md,
					struct state *st);
...
static stf_status ikev2child_inCR1_pfs(struct msg_digest *md)
{
	...
	{
		struct dh_continuation *dh =
			alloc_thing(struct dh_continuation, "ikev2_inCR1 KE");
		dh->md = md;
		set_suspended(st, dh->md);
		...
		e = start_dh_v2(&dh->dh_pcrc, st, pcim_known_crypto, INITIATOR,
				st->st_oakley.groupnum);
		if (e != STF_SUSPEND && e != STF_INLINE) {
			loglog(RC_CRYPTOFAILED,
			       "system too busy..? but we initiated?");
			delete_state(st);
		}
		reset_globals();
		return e;
...
stf_status ikev2child_inCR1_ntf(struct msg_digest *md)
{
	...
	for (p = md->chain[ISAKMP_NEXT_v2N]; p != NULL; p = p->next) {
		...
		openswan_log("received notification %u: %s",
						   ...
						   struct state *st)
{
	...
	if (!(c->policy & POLICY_TUNNEL)) {
		int err = ikev2_validate_transport_proposal(
			c, st, INITIATOR, tsi, tsr, tsi_n, tsr_n);
...

	{
		...
		sra = &c->spd;
		int bfit_n = ikev2_evaluate_connection_fit(
			c, st, sra, INITIATOR, tsi, tsr, tsi_n, tsr_n);
		if (bfit_n > bestfit_n) {
			DBG(DBG_CONTROLMORE,
			    DBG_log(" prefix fitness found a better match c %s",
				    c->name));
			int bfit_p = ikev2_evaluate_connection_port_fit(
				c, sra, INITIATOR, tsi, tsr, tsi_n, tsr_n,
				&best_tsi_i, &best_tsr_i);
			if (bfit_p > bestfit_p) {
				DBG(DBG_CONTROLMORE,
	...
	if ((bestfit_n > 0) && (bestfit_p > 0)) {
		...
		rangetosubnet(&st->st_ts_this.low, &st->st_ts_this.high,
			      &tmp_subnet_i);
		rangetosubnet(&st->st_ts_that.low, &st->st_ts_that.high,
			      &tmp_subnet_r);
		...
		c->spd.this.has_client = !(subnetishost(&c->spd.this.client) &&
		...
		c->spd.that.has_client = !(subnetishost(&c->spd.that.client) &&
...
stf_status ikev2_child_notify_process(struct msg_digest *md, struct state *st)
{
	struct payload_digest *p;
	...
	for (p = md->chain[ISAKMP_NEXT_v2N]; p != NULL; p = p->next) {
         ...
         */
		if (enum_name(&ikev2_notify_names, p->payload.v2n.isan_type) ==
		    NULL) {
...
static stf_status ikev2child_inCR1_tail(struct msg_digest *md, struct state *st)
{
	...
	if (md->chain[ISAKMP_NEXT_v2SA] == NULL ||
		...
		DBG(DBG_CONTROLMORE,
		    DBG_log("no v2SA, v2TSi or v2TSr received child trying to rekey CHILD_SA."));
	...
	DBG(DBG_CONTROLMORE,
	    DBG_log(" checking narrowing - responding to CR1"));
