...
MPI mpi_alloc(unsigned nlimbs UNUSED)
{
...
void mpi_set_buffer(MPI a, const u_char *buffer, unsigned nbytes, int sign)
{
...
u_char *get_random_bits(size_t nbits, int level UNUSED, int secure UNUSED)
{
...
static int build_index(MPI *exparray, int k, int i, int t)
{
	int j, bitno;
	int index = 0;
...
void mpi_mulpowm(MPI res, MPI *basearray, MPI *exparray, MPI m)
{
	...
	for (i = 1; i <= t; i++) {
		...
		barrett_mulm(res, tmp, G[idx], m, barrett_y, barrett_k,
...
void log_mpidump(const char *text UNUSED, MPI a)
{
	...
	for (i = len - 1; i >= 0; i--) {
		buf[i] = mpz_mdivmod_ui(&temp2, NULL, &temp1,
					1 << BITS_PER_BYTE);
		mpz_set(&temp1, &temp2);
	}
