...
bool logged_txt_warning = FALSE; /* should we complain about finding KEY? */

/* should we complain when we find no local id */
bool logged_myid_fqdn_txt_warning = FALSE, logged_myid_ip_txt_warning = FALSE,
     logged_myid_fqdn_key_warning = FALSE, logged_myid_ip_key_warning = FALSE;
...
struct connection *cur_connection =
	NULL; /* current connection, for diagnostics */
...
static void fmt_log(char *buf, size_t buf_len, const char *fmt, va_list ap)
{
	...
	if (reproc)
		...
		snprintf(buf, buf_len, "packet from %s:%u: ", ab,
...
static bool ensure_writeable_parent_directory(char *path)
{
	 ...
	 */
		...
		*e = '\0'; /* carve off dirname part of path */
	     ...
	     */
			syslog(LOG_CRIT, "can not write to %s: %s", path,
			       strerror(errno));
			happy = FALSE;
		} else {
...
static void open_peerlog(struct connection *c)
{
	...
	if (c->log_file_name == NULL) {
		...
		lf_len = peernamelen * 2 + strlen(base_perpeer_logdir) +
			 sizeof("//.log") + 1;
		c->log_file_name =
...
int openswan_log(const char *message, ...)
{
	va_list args;
...
void openswan_log_errno_routine(int e, const char *message, ...)
{
	va_list args;
	char m[LOG_WIDTH]; /* longer messages will be truncated */
	...
	whack_log(RC_LOG_SERIOUS, "~ERROR: %s. Errno %d: %s", m, e,
		  strerror(e));
}
...
void openswan_exit_log(const char *message, ...)
{
	va_list args;
	char m[LOG_WIDTH]; /* longer messages will be truncated */
...
void openswan_exit_log_errno_routine(int e, const char *message, ...)
{
	va_list args;
	char m[LOG_WIDTH]; /* longer messages will be truncated */
	...
	if (log_to_stderr)
		fprintf(stderr, "FATAL ERROR: %s. Errno %d: %s\n", m, e,
			strerror(e));
	if (log_to_syslog)
		syslog(LOG_ERR, "FATAL ERROR: %s. Errno %d: %s", m, e,
		       strerror(e));
	if (log_to_perpeer)
		peerlog(strerror(e), m);

	whack_log(RC_LOG_SERIOUS, "~FATAL ERROR: %s. Errno %d: %s", m, e,
		  strerror(e));

	exit_pluto(1);
}

void openswan_log_abort(const char *file_str, int line_no)
{
...
void extra_debugging(const struct connection *c)
{
	...
	if (cur_debugging) {
		...
		DBG_log("processing connection %s%s", c->name, b1);
	}
}
...
void openswan_DBG_dump(const char *label, const void *p, size_t len)
{
...
void daily_log_event(void)
{
	...
	ltime = localtime(&n);
	interval = (24 * 60 * 60) -
		   (ltime->tm_sec + ltime->tm_min * 60 + ltime->tm_hour * 3600);
...
void log_state(struct state *st, enum state_kind new_state)
{
	...
	if (!st || !st->st_connection || !st->st_connection->name) {
		DBG(DBG_CONTROLMORE,
		    DBG_log("log_state() called without state"));
		return;
	}
	...
	DBG(DBG_CONTROLMORE,
	    DBG_log("log_state calling openswan-statsd for connection %s with tunnel(%s) phase1(%s) phase2(%s)",
		    conn->name, tun, p1, p2));
	...
	snprintf(buf, sizeof(buf),
		 ...
		 st->st_ref == IPSEC_SAREF_NA ?
			 IPSEC_SAREF_NA :
		 st->st_ref == IPSEC_SAREF_NULL ?
			 0u :
			 IPsecSAref2NFmark(st->st_ref) |
				 IPSEC_NFMARK_IS_SAREF_BIT,
		 st->st_refhim == IPSEC_SAREF_NA ?
			 IPSEC_SAREF_NA :
		 st->st_refhim == IPSEC_SAREF_NULL ?
			 0u :
			 IPsecSAref2NFmark(st->st_refhim) |
				 IPSEC_NFMARK_IS_SAREF_BIT);
	system(buf);
	DBG(DBG_CONTROLMORE,
