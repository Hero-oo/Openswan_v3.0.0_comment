...
#define false FALSE
		...
		Tcl_Obj *
		tpm_StateToInstanceObj(struct state * st){ char result[512];
...
% include "systypes.i" % include "cstring.i" % include "mycdata.i" %
	include "cpointer.i" % include "openswantypes.i" %
	include "ietf_constants.h" % include "pluto_constants.h" %
...
int pbs_append(pb_stream *dest, int destoffset, pb_stream *src, int offset,
	       int length);
...
#define PAYLIMIT 20
	struct payload_digest digest[PAYLIMIT], *digest_roof,
