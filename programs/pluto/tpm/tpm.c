...
stf_status tpm_call_it(Tcl_Obj **objv, int objc)
{
	...
	DBG(DBG_CONTROLMORE,
	    DBG_log("TPM call %s %s %s %s %s", Tcl_GetString(objv[0]),
	...
	DBG(DBG_CONTROL, DBG_log("TPM %s(%s,%s,%s,%s) => %s",
				 ...
				 objc > 4 ? Tcl_GetString(objv[4]) : "", res));

	if (strcmp(res, "ignore") == 0 || strcmp(res, "nothing") == 0 ||
	    res[0] == '\0') {
		/* just quietly return */
		return STF_OK;
	}
...
stf_status tpm_call_out(const char *name, struct state *st,
...
stf_status tpm_call_out_crypt(const char *name, struct state *st,
...
stf_status tpm_call_out_notify(const char *name, struct state *st,
			       pb_stream *pbs, struct isakmp_hdr *hdr)
{
	Tcl_Obj **objv;
	int objc = 0, ret;
	char *res;
