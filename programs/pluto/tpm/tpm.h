...
extern stf_status tpm_call_out(const char *name, struct state *st,
			       struct connection *conn, struct msg_digest *md);

struct packet_byte_stream;
extern stf_status tpm_call_out_crypt(const char *name, struct state *st,
				     struct packet_byte_stream *pbs, int offset,
				     int length);
...
struct isakmp_hdr;
extern stf_status tpm_call_out_notify(const char *name, struct state *st,
...
#ifdef TPM
#define TCLCALLOUT(name, st, conn, md)                          \
	do                                                      \
		if (tpm_enabled) {                              \
			stf_status ret;                         \
...
#define TCLCALLOUT_notify(name, st, pbs, hdr)                          \
	do                                                             \
		if (tpm_enabled) {                                     \
			stf_status ret;                                \
	...
	while (0)
#define TCLCALLOUT_crypt(name, st, pbs, off, len)                    \
	do                                                           \
		if (tpm_enabled) {                                   \
			tpm_call_out_crypt(name, st, pbs, off, len); \
		}                                                    \
	while (0)
#else
