 ...
 * be kicked off.
 */
void host_pair_enqueue_pending(const struct connection *c, struct pending *p,
 ...
 *
 */
struct IPhost_pair *find_host_pair(bool exact, const ip_address *myaddr,
				   u_int16_t myport, enum keyword_host histype,
				   const ip_address *hisaddr, u_int16_t hisport)
{
     ...
     */
	if (myport == pluto_port4500)
		myport = pluto_port500;
	if (hisport == pluto_port4500)
		hisport = pluto_port500;

	DBG(DBG_CONTROLMORE, char b1[ADDRTOT_BUF]; char b2[ADDRTOT_BUF];
	    char himtypebuf[KEYWORD_NAME_BUFLEN];
	    DBG_log("find_host_pair: looking for me=%s:%d %s him=%s:%d %s\n",
		    myaddr ? (addrtot(myaddr, 0, b1, sizeof(b1)), b1) :
			     "<none>",
		    myport, keyword_name(&kw_host_list, histype, himtypebuf),
		    hisaddr ? (addrtot(hisaddr, 0, b2, sizeof(b2)), b2) :
			      "<none>",
	...
	for (prev = NULL, p = IPhost_pairs; p != NULL; prev = p, p = p->next) {
		DBG(DBG_CONTROLMORE, char b1[ADDRTOT_BUF]; char b2[ADDRTOT_BUF];
		    ...
		    DBG_log("find_host_pair: comparing to me=%s:%d %s him=%s:%d\n",
			    ...
			    p->me.host_port,
			    keyword_name(&kw_host_list, p->him.host_type,
					 himtypebuf),
...
#define FAIL_TO_MATCH_IF(cond)                                  \
	do                                                      \
		if (cond) {                                     \
			DBG_log("     failed to match " #cond); \
			continue;                               \
		}                                               \
	while (0)
#else
#define FAIL_TO_MATCH_IF(cond)    \
	...
	while (0)
		...
		FAIL_TO_MATCH_IF(p->me.host_port_specific &&
				 p->me.host_port != myport);
	...
	DBG(DBG_CONTROLMORE, DBG_log("find_host_pair: concluded with %s",
				     bestpair && bestpair->connections ?
					     bestpair->connections->name :
					     "<none>"));
	return bestpair;
}
...
void clear_IPhost_pair(struct connection *c)
{
	...
	if (IPhp == NULL) {
		...
		list_rm(struct connection, IPhp_next, c,
...
struct connection *find_host_pair_connections(const char *func, bool exact,
					      ...
					      u_int16_t hisport)
{
	struct IPhost_pair *hp = find_host_pair(exact, myaddr, myport, histype,
						hisaddr, hisport);
...
{
	if (oriented(*c)) {
		struct IPhost_pair *hp = find_host_pair(
			EXACT_MATCH, &c->spd.this.host_addr,
			c->spd.this.host_port, c->spd.that.host_type,
		...
		DBG(DBG_CONTROLMORE, char b1[ADDRTOT_BUF]; char b2[ADDRTOT_BUF];
		    ...
		    DBG_log("connect_to_host_pair: %s:%d %s %s:%d -> hp:%s\n",
			    ...
			    keyword_name(&kw_host_list, c->spd.that.host_type,
					 himtypebuf),
			    (addrtot(&c->spd.that.host_addr, 0, b2, sizeof(b2)),
			     b2),
			    c->spd.that.host_port,
			    (hp && hp->connections) ? hp->connections->name :
						      "none"));
...
#ifdef NAT_TRAVERSAL
			hp->me.host_port = nat_traversal_enabled ?
						   pluto_port500 :
						   c->spd.this.host_port;
			hp->him.host_port = nat_traversal_enabled ?
						    pluto_port500 :
						    c->spd.that.host_port;
#else
...
struct IDhost_pair *find_ID_host_pair_exact(const struct id me,
					    const struct id him)
{
	...
	for (p = IDhost_pairs; p != NULL; p = p->next) {
		DBG(DBG_CONTROLMORE, idtoa(&p->me_who, mewho, sizeof(mewho));
	...
	DBG(DBG_CONTROLMORE,
	    DBG_log("  concluded with %s", pbest && pbest->connections ?
						   pbest->connections->name :
						   "<none>"));
	return pbest;
}

struct IDhost_pair *find_ID_host_pair(const struct id me, const struct id him)
{
	struct IDhost_pair *p, *pbest = NULL;
	char mebuf[IDTOA_BUF], himbuf[IDTOA_BUF];
	char mewho[IDTOA_BUF], himwho[IDTOA_BUF];
	...
	for (p = IDhost_pairs; p != NULL; p = p->next) {
		DBG(DBG_CONTROLMORE, idtoa(&p->me_who, mewho, sizeof(mewho));
		    ...
		    DBG_log("                  comparing to me=%s him=%s (%s)\n",
			    mewho, himwho,
			    (p && p->connections) ? p->connections->name :
						    "<none>"));
		...
		if ((exactmatch == TRUE && pbestexact == FALSE) ||
		    pbest == NULL) {
			pbest = p;
...
void connect_to_IDhost_pair(struct connection *c)
{
	struct IDhost_pair *hp =
		find_ID_host_pair_exact(c->spd.this.id, c->spd.that.id);
...
void hostpair_list(void)
{
	...
	for (pi = IPhost_pairs; pi != NULL; pi = pi->next) {
		...
		if (c) {
			...
			whack_log(RC_LOG, "  IPpair: %s:%d %s %s:%d", b1,
				  c->spd.this.host_port, himtypebuf, b2,
				  c->spd.that.host_port);
			while (c != NULL) {
				fmt_connection_inst_name(c, instance,
							 sizeof(instance));
				whack_log(RC_LOG, "     %s%s", c->name,
					  instance[0] ? instance : "");
				c = c->IPhp_next;
			}
		}
	...
	for (pd = IDhost_pairs; pd != NULL; pd = pd->next) {
		...
		whack_log(RC_LOG, "  IDpair: %20s/%20s", b1, b2);
		while (c != NULL) {
