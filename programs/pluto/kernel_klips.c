...
#endif
{
	...
	for (;;) {
		...
		if (q == NULL) {
			...
			id->id_vname =
				clone_str(v->name, "virtual device name klips");
			id->id_count++;
...
static bool klips_do_command(struct connection *c, const struct spd_route *sr,
