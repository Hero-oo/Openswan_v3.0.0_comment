...
#define MAX_TXT_RDATA \
	((MAX_KEY_BYTES * 8 / 6) + 40) /* somewhat arbitrary overkill */
...
int adns_qfd =
	    NULL_FD, /* file descriptor for sending queries to adns (O_NONBLOCK) */
	adns_afd =
		NULL_FD; /* file descriptor for receiving answers from adns */
...
void init_adns(void)
{
...
#ifdef HAVE_NO_FORK
	adns_pid =
		vfork(); /* for better, for worse, in sickness and health..... */
#else
	adns_pid = fork();
#endif
	switch (adns_pid) {
	...
	case 0:
	     ...
	     */
			if (afds[1] == 0)
				afds[1] = dup(
					afds[1]); /* avoid being overwritten */
			if (qfds[0] != 0) {
				dup2(qfds[0], 0);
				close(qfds[0]);
			}
	...
	if (adns_pid != 0) {
		...
		if (p == -1) {
			...
			if (WEXITSTATUS(status) != 0)
				openswan_log(
					"ADNS process exited with status %d",
					(int)WEXITSTATUS(status));
			adns_pid = 0;
		} else if (WIFSIGNALED(status)) {
			openswan_log("ADNS process terminated by signal %d",
				     (int)WTERMSIG(status));
			adns_pid = 0;
		} else {
			openswan_log(
...
static err_t decode_iii(char **pp, struct id *gw_id)
{
	char *p = *pp + strspn(*pp, " \t");
	char *e = p + strcspn(p, " \t");
	char under = *e;
	...
	if (p == e)
		return "TXT " our_TXT_attr_string
		       " badly formed (no gateway specified)";
	...
	*e = '\0';
	if (*p == '@') {
		...
		if (ugh != NULL)
			return builddiag(
				"malformed FQDN in TXT " our_TXT_attr_string
				": %s",
				ugh);
	} else {
		...
		ip_address ip;
		err_t ugh = tnatoaddr(
			p, e - p, strchr(p, ':') == NULL ? AF_INET : AF_INET6,
			&ip);

		if (ugh != NULL)
			return builddiag(
				"malformed IP address in TXT " our_TXT_attr_string
				": %s",
				ugh);
...
static err_t process_txt_rr_body(char *str,
				 ...
				 struct adns_continuation *const cr)
{
	...
	if (doit) {
	     ...
	     */
			...
			err_t ugh = ttodatav(p, 0, 64, (char *)kb, sizeof(kb),
					     &kbc.len, diag_space,
					     sizeof(diag_space),
...
static const char *rr_typename(int type)
{
	switch (type) {
	case ns_t_txt:
		return "TXT";
...
static struct_desc qr_header_desc = { "Query Response Header", qr_header_fields,
...
static err_t eat_name(pb_stream *pbs)
{
...
static err_t eat_name_helpfully(pb_stream *pbs, const char *context)
{
	err_t ugh = eat_name(pbs);

	return ugh == NULL ?
		       ugh :
		       builddiag("malformed name within DNS record of %s: %s",
				 context, ugh);
}
...
static struct_desc qs_fixed_desc = { "Question Section entry fixed part",
				     qs_fixed_fields, sizeof(struct qs_fixed) };
...
static struct_desc rr_fixed_desc = {
	"Resource Record fixed part", rr_fixed_fields,
...
static struct_desc key_rdata_desc = { "KEY RR RData fixed part",
				      key_rdata_fields,
...
static struct_desc sig_rdata_desc = { "SIG RR RData fixed part",
				      sig_rdata_fields,
...
#ifdef USE_KEYRR
static err_t process_key_rr(u_char *ptr, size_t len,
			    bool doit /* should we capture information? */
			    ,
			    enum dns_auth_level dns_auth_level,
			    struct adns_continuation *const cr)
{
	    ...
	    && (kr.flags & 0x2CF0) == 0) /* must be zero */
	{
		...
		if (doit) {
			...
			TRY(add_public_key(&cr->id, dns_auth_level,
					   PUBKEY_ALG_RSA, &k,
					   &cr->keys_from_dns));
		}
	}
 ...
 * We concatenate them.
 */
static err_t unpack_txt_rdata(char *d, size_t dlen, const u_char *s,
			      size_t slen)
{
	size_t i = 0, o = 0;
...
static err_t process_txt_rr(u_char *rdata, size_t rdlen,
			    bool doit /* should we capture information? */
			    ,
			    enum dns_auth_level dns_auth_level,
			    struct adns_continuation *const cr)
{
	char str[RSA_MAX_ENCODING_BYTES * 8 / 6 +
		 200]; /* space for unpacked RDATA */

	TRY(unpack_txt_rdata(str, sizeof(str), rdata, rdlen));
	return process_txt_rr_body(str, doit, dns_auth_level, cr);
}

static err_t process_answer_section(
	pb_stream *pbs, bool doit /* should we capture information? */
	,
	enum dns_auth_level *dns_auth_level,
	...
	struct adns_continuation *const cr)
{
...
#ifdef USE_KEYRR
			case ns_t_key:
				ugh = process_key_rr(pbs->cur, tail, doit,
						     *dns_auth_level, cr);
				break;
#endif /* USE_KEYRR */
			case ns_t_txt:
				ugh = process_txt_rr(pbs->cur, tail, doit,
						     *dns_auth_level, cr);
				break;
			case ns_t_sig:
	...
	return doit && cr->gateways_from_dns == NULL
#ifdef USE_KEYRR
			       && cr->keys_from_dns == NULL
#endif /* USE_KEYRR */
		       ?
		       builddiag("no suitable %s record found in DNS",
				 rr_typename(type)) :
		       NULL;
}
...
static err_t process_dns_rr_answer(struct adns_continuation *const cr,
				   u_char ans[], int anslen)
{
	...
	if (((qr_header.stuff >> QRS_OPCODE_SHIFT) & QRS_OPCODE_MASK) !=
	    QRSO_QUERY)
	...
	if (r != 0)
		return r < (int)elemsof(rcode_text) ? rcode_text[r] :
						      "unknown rcode";
 ...
 */
static err_t process_dns_sockaddr_answer(struct adns_continuation *const cr,
					 u_char ans[], int anslen)
{
...
static err_t process_dns_answer(struct adns_continuation *const cr,
				u_char ans[], int anslen)
{
	switch (cr->type) {
	case ns_t_txt:
			    ...
			    const char *gwname USED_BY_DEBUG)
{
	...
	case ID_IPV6_ADDR: {
		...
		for (bl = addrbytesptr(&id->ip_addr, &b); bl-- != 0;) {
			if (op + 4 + sizeof(suffix) >=
			    name_buf + NS_MAXDNAME + 1)
	...
	DBG(DBG_CONTROL | DBG_DNS,
	    DBG_log("DNS query %lu for %s for %s (gw: %s)", serial, typename,
		    name_buf, gwname));
	return NULL;
}
...
void gw_addref(struct gw_info *gw)
{
	if (gw != NULL) {
		DBG(DBG_DNS,
		    DBG_log("gw_addref: %p refcnt: %d++", gw, gw->refcnt))
		gw->refcnt++;
	}
}
...
void gw_delref(struct gw_info **gwp)
{
	...
	if (gw != NULL) {
		DBG(DBG_DNS,
		    DBG_log("gw_delref: %p refcnt: %d--", gw, gw->refcnt));
...
static struct adns_continuation *continuation_for_qtid(unsigned long qtid)
{
	...
	if (qtid != 0)
		for (cr = continuations; cr != NULL && cr->qtid != qtid;
		     cr = cr->previous)
			;
	return cr;
}
...
static void release_all_continuations()
{
	struct adns_continuation *cr = NULL;
...
static void init_generic_adns_query(struct adns_continuation *cr)
{
	if (adns_pid == 0 && adns_restart_count < ADNS_RESTART_MAX) {
		openswan_log(
			"ADNS helper was not running. Restarting attempt %d",
			adns_restart_count);
		init_adns();
	}
...
static void start_generic_adns_query(int type, struct adns_continuation *cr,
...
err_t start_adns_hostname(sa_family_t addr_family, const char *hostname,
			  cont_fn_t cont_fn, struct adns_continuation *cr)
{
	/* link it in so it's all sane */
	init_generic_adns_query(cr);
...
void send_unsent_ADNS_queries(void)
{
	...
	for (;;) {
		if (buf_cur != buf_end) {
			...
			if (r == -1) {
				...
				case EAGAIN:
					DBG(DBG_DNS,
					    DBG_log("EAGAIN writing to ADNS"));
					break; /* try again later */
				default:
					try++;
					log_errno((e,
						   "error %d writing DNS query",
						   try));
					break; /* try again later */
				}
				unsent_ADNS_queries = TRUE;
...
void handle_adns_answer(void)
{
	...
	if (n < 0) {
		...
		if (adns_in_flight != 0) {
			openswan_log(
				"EOF from ADNS with %d queries outstanding (restarts %d)",
				adns_in_flight, adns_restart_count);
			recover_adns_die();
		}
		if (buflen != 0) {
			openswan_log(
				"EOF from ADNS with %lu bytes of a partial answer outstanding"
				"(restarts %d)",
				(unsigned long)buflen, adns_restart_count);
			recover_adns_die();
		}
		stop_adns();
		return;
	...
	while (buflen >= offsetof(struct adns_answer, ans) &&
		...
		struct adns_continuation *cr =
			continuation_for_qtid(buf.serial); /* assume it works */
	     ...
	     */
			...
			case EAI_NODATA:
				ugh = builddiag("no %s record for %s", typename,
						name_buf);
				break;
			case EAI_NONAME:
				ugh = builddiag("no host %s for %s record",
						name_buf, typename);
				break;
			default:
				ugh = builddiag(
					"failure querying DNS for %s of %s: %s",
					typename, name_buf,
					gai_strerror(buf.h_errno_val));
				break;
			}
		} else if (buf.result > (int)sizeof(buf.ans)) {
			ugh = builddiag(
				"(INTERNAL ERROR) answer too long (%ld) for buffer",
				(long)buf.result);
		} else {
			ugh = process_dns_answer(cr, buf.ans, buf.result);
			if (ugh != NULL)
				ugh = builddiag(
					"failure processing %s record of DNS answer for %s: %s",
		...
		DBG(DBG_RAW | DBG_CRYPT | DBG_PARSING | DBG_CONTROL | DBG_DNS,
		    ...
		    else DBG_log("async DNS answer %lu %s", cr->query.serial,
				 ugh););
		...
		buflen -= buf.len;
		memmove((unsigned char *)&buf, (unsigned char *)&buf + buf.len,
			buflen);
	}
}
 ...
 *
 */
static bool advance_end_dns_list(struct connection *c, struct end *end)
{
	...
	case AF_INET: {
		...
		const unsigned char *bytes =
			(unsigned char *)&ai4->sin_addr.s_addr;
		...
		if (bytes[0] == 127) {
			...
			openswan_log(
				"DNS lookup for '%s' rejected address %s: loopback address",
				end->host_addr_name, addrbuf);
			valid = FALSE;
		} else if (bytes[0] > 224 && bytes[0] <= 239) {
			/* multicast addresses now allowed, but maybe class E permitted */
			openswan_log(
				"DNS lookup for '%s' rejected address %s: multicast address",
				end->host_addr_name, addrbuf);
			valid = FALSE;
		} else if (bytes[0] == 255) {
			/* 255/8 is reserved */
			openswan_log(
				"DNS lookup for '%s' rejected rejected address %s: 255/8 is reserved",
				end->host_addr_name, addrbuf);
			valid = FALSE;
		} else if (bytes[0] == 0) {
			/* 0/8 is reserved */
			openswan_log(
				"DNS lookup for '%s' rejected address %s: 0/8 is reserved",
				end->host_addr_name, addrbuf);
			valid = FALSE;
		} else {
			valid = TRUE;
	...
	case AF_INET6: {
		...
		if (bytes[0] == 0 && bytes[1] == 0 && bytes[2] == 0 &&
			...
			valid = FALSE;
		} else if (bytes[0] == 0 && bytes[1] == 0 && bytes[2] == 0 &&
			   bytes[3] == 0 && bytes[4] == 0 && bytes[5] == 0 &&
			   bytes[6] == 0 && bytes[7] == 0 && bytes[8] == 0 &&
			   bytes[9] == 0 && bytes[10] == 0 && bytes[11] == 0) {
			/* section 2.5.5.1: IPv4 compatible IPv6 addresses, e.g.  ::1.2.3.4 */
			openswan_log(
				"DNS lookup for '%s' rejected address %s: IPv4-compatible IPv6 address",
				end->host_addr_name, addrbuf);
			valid = FALSE;
		} else if (bytes[0] == 0 && bytes[1] == 0 && bytes[2] == 0 &&
			   bytes[3] == 0 && bytes[4] == 0 && bytes[5] == 0 &&
			   bytes[6] == 0 && bytes[7] == 0 && bytes[8] == 0 &&
			   bytes[9] == 0 && bytes[10] == 0xff &&
			   bytes[11] == 0xff) {
			/* section 2.5.5.6: IPv4-mapped IPv6 addresses, e.g.  ::ffff:1.2.3.4 */
			/* XXX rejected for now: could become IPv4 address */
			openswan_log(
				"DNS lookup for '%s' rejected address %s: IPv4-mapped IPv6 address",
				end->host_addr_name, addrbuf);
			valid = FALSE;
			...
			openswan_log(
				"DNS lookup for '%s' rejected address %s: Link Local address returned",
				end->host_addr_name, addrbuf);
			valid = FALSE;
			...
			openswan_log(
				"DNS lookup for '%s' found address %s: Site Local address returned (okay)",
				end->host_addr_name, addrbuf);
			valid = FALSE;
			...
			/* rejected */
			openswan_log(
				"DNS lookup for '%s' found address %s: multicast address returned",
				end->host_addr_name, addrbuf);
			valid = FALSE;
		} else {
			valid = TRUE;
	...
	if (valid) {
		...
		DBG(DBG_DNS, DBG_log("advancing DNS to: %s (next: %s)", newaddr,
				     ai->ai_next ? "more" : "last"));
...
void iphostname_continuation(struct adns_continuation *cr, err_t ugh)
{
	struct iphostname_continuation *iph_c =
		(struct iphostname_continuation *)cr;
	struct addrinfo *ai;

	DBG(DBG_DNS, DBG_log("iphostname_continuation: %s %s", iph_c->c->name,
			     ugh ? ugh : "no-error"));
	if (ugh) {
...
static int ai_compare(const void *a, const void *b)
{
	...
	if (!bi->ai_addr)
		return -1;
	if (ai->ai_addrlen < bi->ai_addrlen)
		return -1;
	if (ai->ai_addrlen > bi->ai_addrlen)
		return 1;
...
struct addrinfo *sort_addr_info(struct addrinfo *ai)
{
	...
	for (ai_count = 0, ai1 = ai; ai1 != NULL;
	     ai1 = ai1->ai_next, ai_count++)
		;

	/* now make an array */
	array = (struct addrinfo **)alloca(sizeof(struct addrinfo *) *
					   ai_count);
...
void dump_addr_info(struct addrinfo *ans)
{
	...
	while (ans) {
		...
		DBG_log("ans %03u canonname=%s protocol=%u family=%u len=%u\n",
			ansnum, ans->ai_canonname, ans->ai_protocol,
			ans->ai_family, ans->ai_addrlen);
...
bool kick_adns_connection_lookup(struct connection *c, struct end *end,
				 bool newlookup)
{
	struct iphostname_continuation *iph_c;
	...
	while (valid == FALSE && end->host_address_list.address_list != NULL &&
	...
	if (e) {
		openswan_log("failed to initiate DNS lookup on %s: %s",
			     end->host_addr_name, e);
		return FALSE;
	}
