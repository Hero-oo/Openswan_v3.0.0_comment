...
const struct pfkey_proto_info null_proto_info[2] = {
	...
	{ proto: 0, encapsulation: 0, reqid: 0 }
};
...
void DBG_bare_shunt_log(const char *op, const struct bare_shunt *bs)
{
	DBG(DBG_KLIPS | DBG_OPPOINFO, {
		...
		DBG_log("%s bare shunt %p %s:%d --%d--> %s:%d => %s %s    %s    (%lds)",
			...
			(bs)->transport_proto, hist, hisport, sat, prio,
			(bs)->why, age);
	});
}
#endif
				       ...
				       const char *why)
{
	...
	/* actually initiate opportunism / ondemand */
	{
		...
		networkof(ours, &src);
		networkof(his, &dst);
		if (initiate_ondemand(&src, &dst, transport_proto, TRUE,
				      NULL_FD, uctx, "acquire") == 0) {
			/* if we didn't do any ondemand stuff the shunt is not needed */
			struct bare_shunt **bspp =
				bare_shunt_ptr(ours, his, transport_proto);
			if (bspp) {
 ...
 */
bool get_ipsec_spi(struct ipsec_proto_info *pi, int proto, struct state *st,
		   bool tunnel)
{
	...
	if (kernel_ops->get_spi) {
		...
		pi->our_spi = kernel_ops->get_spi(&st->st_remoteaddr,
						  &st->st_localaddr, proto,
						  tunnel, get_proto_reqid(),
						  IPSEC_DOI_SPI_OUR_MIN,
						  0xffffffff, text_said);
		return pi->our_spi != 0;
	}
...
ipsec_spi_t get_my_cpi(struct state *st, bool tunnel)
{
	...
	if (kernel_ops->get_spi) {
		...
		return kernel_ops->get_spi(&st->st_remoteaddr,
					   &st->st_localaddr, IPPROTO_COMP,
					   tunnel, get_proto_reqid(),
					   IPCOMP_FIRST_NEGOTIATED,
					   IPCOMP_LAST_NEGOTIATED, text_said);
	}
...
{
	int result;
	char me_str[ADDRTOT_BUF], myid_str2[IDTOA_BUF],
		srcip_str[ADDRTOT_BUF + sizeof("PLUTO_MY_SOURCEIP=") + 4],
		myclient_str[SUBNETTOT_BUF], myclientnet_str[ADDRTOT_BUF],
		myclientmask_str[ADDRTOT_BUF], peer_str[ADDRTOT_BUF],
		peerid_str[IDTOA_BUF], metric_str[sizeof("PLUTO_METRIC") + 5],
		connmtu_str[sizeof("PLUTO_MTU") + 5 + 1],
		peerclient_str[SUBNETTOT_BUF], peerclientnet_str[ADDRTOT_BUF],
	...
	if (addrbytesptr(&sr->this.host_nexthop, NULL) &&
		...
		addrtot(&sr->this.host_nexthop, 0, n,
			sizeof(nexthop_str) - strlen(nexthop_str));
...

	{
		...
		for (p = pluto_pubkeys; p != NULL; p = p->next) {
			...
			if (key->alg == PUBKEY_ALG_RSA &&
			    ...
			    trusted_ca_by_name(key->issuer, sr->that.ca,
					       &pathlen)) {
	...
	result = snprintf(
		...
		,
		c->name, c->interface->ip_dev->id_vname, nexthop_str, me_str,
		secure_myid_str, myclient_str, myclientnet_str,
		myclientmask_str, sr->this.port, sr->this.protocol, peer_str,
		secure_peerid_str, peerclient_str, peerclientnet_str,
		peerclientmask_str, sr->that.port, sr->that.protocol,
		secure_peerca_str, kernel_ops->kern_name, metric_str,
		connmtu_str, prettypolicy(c->policy),
...
#ifdef XAUTH
		,
		c->remotepeertype, c->cisco_dns_info ? c->cisco_dns_info : "",
	...
	/* figure out which verb suffix applies for logging purposes */
	{
		...
		default:
			loglog(RC_LOG_SERIOUS,
			       "unknown address family (do_command)");
			return FALSE;
		}
		verb_suffix = subnetisaddr(&sr->this.client, local) ? hs : cs;
	}
	return verb_suffix;
}

bool do_command(struct connection *c, const struct spd_route *sr,
		const char *verb, struct state *st)
{
	const char *verb_suffix = kernel_command_verb_suffix(st, sr);

	DBG(DBG_CONTROL, DBG_log("command executing %s%s", verb, verb_suffix));
...
static enum routability could_route(struct connection *c, struct spd_route *dsr)
{
	...
	DBG(DBG_CONTROL, DBG_log("could_route called for %s (kind=%s)", c->name,
     ...
     */
	if (!c->spd.that.has_client && c->kind == CK_TEMPLATE &&
	...
	if (ero != NULL && ero != c) {
		...
		if (outside && outside->kind == CK_PERMANENT && inside &&
		    inside->kind == CK_TEMPLATE) {
...
static bool shunt_eroute(struct connection *c, const struct spd_route *sr,
			 enum routing_t rt_kind, enum pluto_sadb_operations op,
			 const char *opname)
{
...
static bool sag_eroute(struct state *st, const struct spd_route *sr,
...
void unroute_connection(struct connection *c)
{
	...
	for (sr = &c->spd; sr; sr = sr->next) {
		...
		sr->routing =
			RT_UNROUTED; /* do now so route_owner won't find us */
...
{
	if (pp == NULL) {
		DBG(DBG_CONTROL, DBG_log("delete bare shunt: null pointer"))
	} else {
		struct bare_shunt *p = *pp;
...
void show_shunt_status(void)
{
	...
	for (bs = bare_shunts; bs != NULL; bs = bs->next) {
		...
		whack_log(RC_COMMENT,
			  "%s:%d -%d-> %s:%d => %s %s    %s    (%lds)", ourst,
			  ourport, bs->transport_proto, hist, hisport, sat,
			  prio, bs->why, age);
	}
}
...
raw_eroute(const ip_address *this_host, const ip_subnet *this_client,
	   ...
	   enum eroute_type esatype, const struct pfkey_proto_info *proto_info,
	   time_t use_lifetime, enum pluto_sadb_operations op,
	   const char *opname USED_BY_DEBUG, char *policy_label)
{
	char text_said[SATOT_BUF];
	bool result;
	...
	result = kernel_ops->raw_eroute(this_host, this_client, that_host,
					that_client, spi, proto,
					transport_proto, esatype, proto_info,
		   ...
		   int transport_proto)
{
	...
	return bspp != NULL && (*bspp)->said.proto == SA_INT &&
 ...
 * routed
 */
static void clear_narrow_holds(const ip_subnet *ours, const ip_subnet *his,
			       int transport_proto)
{
	struct bare_shunt *p, **pp;
	...
	for (pp = &bare_shunts; (p = *pp) != NULL;) {
		...
		if (samesubnet(ours, &po) && samesubnet(his, &ph) &&
			...
			(void)replace_bare_shunt(
				&p->ours.addr, &p->his.addr, BOTTOM_PRIO,
				SPI_PASS /* not used */
				,
	...
	if (transport_proto != 0) {
		...
		if (repl) {
			struct bare_shunt **bs_pp = bare_shunt_ptr(
				&this_broad_client, &that_broad_client, 0);
		...
		if (raw_eroute(null_host, &this_client, null_host, &that_client,
			       htonl(shunt_spi), SA_INT, transport_proto,
			       ET_INT, null_proto_info, SHUNT_PATIENCE, ERO_ADD,
			       why, NULL_POLICY)) {
			struct bare_shunt **bs_pp = bare_shunt_ptr(
				&this_client, &that_client, transport_proto);
		...
		DBG(DBG_KLIPS, DBG_log("%s specific host-to-host bare shunt",
				       repl ? "replacing" : "removing"));
		if (raw_eroute(null_host, &this_client, null_host, &that_client,
			       htonl(shunt_spi), SA_INT, 0 /* transport_proto */
			       ,
			       ET_INT, null_proto_info, SHUNT_PATIENCE, op, why,
			       NULL_POLICY)) {
			struct bare_shunt **bs_pp =
				bare_shunt_ptr(&this_client, &that_client, 0);
...
bool delete_bare_shunt_ptr(struct bare_shunt **bs_pp, const char *why)
{
	struct bare_shunt *bs = *bs_pp;
	ip_subnet this_client, that_client;
	int af;
	...
	DBG(DBG_KLIPS | DBG_OPPOINFO,
	    DBG_log("removing specific host-to-host bare shunt"));
	if (raw_eroute(null_host, &this_client, null_host, &that_client, spi,
		       proto, transport_proto, ET_INT, null_proto_info,
...
bool eroute_connection(struct state *st, const struct spd_route *sr,
		       ipsec_spi_t spi, unsigned int proto,
		       enum eroute_type esatype,
		       const struct pfkey_proto_info *proto_info,
		       unsigned int op, const char *opname, char *policy_label)
{
	const ip_address *null_host =
		aftoinfo(addrtypeof(&st->st_remoteaddr))->any;
	...
	if (DBGP(DBG_KLIPS)) {
		...
		DBG_log("eroute_connection: between %s<->%s", sa_src, sa_dst);
	}

	return raw_eroute(this, &sr->this.client, that, &sr->that.client, spi,
			  proto, sr->this.protocol, esatype, proto_info, 0, op,
			  buf2, policy_label);
}
...
bool assign_hold(struct connection *c USED_BY_DEBUG, struct spd_route *sr,
		 int transport_proto, const ip_address *src,
		 const ip_address *dst)
{
	...
	DBG(DBG_CONTROL, DBG_log("assign hold, routing was %s, needs to be %s",
	...
	if (eclipsable(sr)) {
         ...
         */
		free_bare_shunt(bare_shunt_ptr(
			&sr->this.client, &sr->that.client, sr->this.protocol));
	} else {
		...
		if (rn != ro) {
			...
			if (!eroute_connection(NULL_STATE, sr, htonl(SPI_HOLD),
					       SA_INT, ET_INT, null_proto_info,
					       op, reason, c->policy_label)) {
				return FALSE;
			}
		}
...
ipsec_spi_t shunt_policy_spi(struct connection *c, bool prospective)
{
	...
	return prospective ? shunt_spi[(c->policy & POLICY_SHUNT_MASK) >>
				       POLICY_SHUNT_SHIFT] :
			     fail_spi[(c->policy & POLICY_FAIL_MASK) >>
				      POLICY_FAIL_SHIFT];
}

static bool del_spi(ipsec_spi_t spi, int proto, const ip_address *src,
 ...
 */
static err_t setup_esp_sa(struct connection *c, struct state *st,
			  unsigned int encapsulation, bool inbound,
			  bool outgoing_ref_set, const char *inbound_str,
			  struct kernel_sa *said_next, u_int8_t natt_type,
			  ip_address src, u_int16_t natt_sport, ip_address dst,
			  u_int16_t natt_dport, ip_subnet src_client,
			  ip_subnet dst_client)
{
	...
	if (DBGP(DBG_KLIPS)) {
		...
		DBG_log("looking for %s alg with transid: %d keylen: %d auth: %d for spi=%08x [%s->%s]\n",
			inbound_str, st->st_esp.attrs.transattrs.encrypt,
			st->st_esp.attrs.transattrs.enckeylen,
			st->st_esp.attrs.transattrs.integ_hash, esp_spi, sa_src,
			sa_dst);
	}
	...
	if ((said_next->esp_info.auth == IKEv2_AUTH_HMAC_SHA2_256_128) &&
		...
		if (kernel_ops->sha2_truncbug_support) {
			...
			said_next->esp_info.auth =
				IKEv2_AUTH_HMAC_SHA2_256_128_TRUNCBUG;
		} else {
			loglog(RC_LOG_SERIOUS,
 ...
 *
 */
static bool setup_half_ipsec_sa(struct state *parent_st, struct state *st,
				struct spd_route *sr, bool inbound)
{
	...
	if (DBGP(DBG_KLIPS)) {
		...
		DBG_log("state #%lu(%s): setup %s %s-ipsec between %s%s<->%s%s for %s...%s",
			st->st_serialno, c->name, inbound_str, nattype_str,
			sa_src, srcport_thing, sa_dst, dstport_thing, tun_src,
			tun_dst);
	}
	...
	if (kernel_ops->inbound_eroute) {
		...
		set_text_said(text_said, &sr->that.host_addr, ipip_spi,
			      SA_IPIP);
		...
		time((inbound) ? &st->st_esp.our_lastused :
				 &st->st_esp.peer_lastused);
	 ...
	 */
		if (new_refhim == IPSEC_SAREF_NULL && !inbound) {
			DBG(DBG_KLIPS, DBG_log("recorded ref=%u as refhim",
					       said_next->ref));
	...
	if (st->st_esp.present) {
		err = setup_esp_sa(c, st, encapsulation, inbound,
				   outgoing_ref_set, inbound_str, said_next,
				   natt_type, src, srcport, dst, dstport,
	...
	if (kernel_ops->grp_sa && said_next > &said[1]) {
		...
		for (s = said; s < said_next - 1; s++) {
			char text_said0[SATOT_BUF], text_said1[SATOT_BUF];

			/* group s[1] and s[0], in that order */

			set_text_said(text_said0, s[0].dst, s[0].spi,
				      s[0].proto);
...
fail: {
	...
	while (said_next-- != said) {
		if (said_next->proto) {
			(void)del_spi(said_next->spi, said_next->proto, &src,
				      said_next->dst);
		}
	}
	return FALSE;
...
static bool teardown_half_ipsec_sa(struct state *st, struct end *that,
				   bool inbound)
{
	...
	if (kernel_ops->inbound_eroute && inbound &&
		...
		(void)raw_eroute(&that->host_addr, &that->client,
				 ...
				 256, IPSEC_PROTO_ANY, c->spd.this.protocol,
				 ET_UNSPEC, null_proto_info, 0, ERO_DEL_INBOUND,
				 "delete (half) inbound", c->policy_label);
	}
...
static void look_for_replacement_state(struct state *st)
{
	...
	DBG(DBG_CONTROL, DBG_log("checking if this is a replacement state");
	    DBG_log("  st->serialno=#%lu ost->serialno=#%lu ", st->st_serialno,
...
static void build_desired_sr(struct state *st, struct spd_route *desired_sr)
{
	/*
	...
	if (desired_sr->that.has_client == FALSE &&
		...
		openswan_log("using peer address %s as peer subnet proposal",
			     abuf);
	}
}
 ...
 */
bool install_inbound_ipsec_sa(struct state *parent_st, struct state *st)
{
	...
	if (c->kind != CK_PERMANENT && c->kind != CK_INSTANCE) {
		openswan_log(
			"%s: policy of type: %s can not be installed into kernel",
	...
	if (him->has_client) {
		for (;;) {
			...
			struct connection *o =
				route_owner(c, &desired_sr, &esr, NULL, NULL);
		...
		if (!st->st_connection->loopback) {
			...
			DBG(DBG_CONTROL,
			    DBG_log("installing outgoing SA now as refhim=%u",
				    st->st_refhim));
			if (!setup_half_ipsec_sa(parent_st, st, &desired_sr,
						 FALSE)) {
		      ...
		      struct state *st USED_BY_KLIPS)
{
	...
	bool eroute_installed = FALSE, firewall_notified = FALSE,
	...
	DBG(DBG_CONTROLMORE,
	    DBG_log("route_and_eroute with c: %s (next: %s) ero:%s esr:{%p} ro:%s rosr:{%p} and state: %lu",
		    c->name, (c->policy_next ? c->policy_next->name : "none"),
		    ero ? ero->name : "null", esr, ro ? ro->name : "null", rosr,
		    st ? st->st_serialno : 0));
	...
	if (bspp != NULL || ero != NULL) {
		...
		if (st == NULL)
			...
			eroute_installed =
				sag_eroute(st, sr, ERO_REPLACE, "replace");
		...
		/* if no state provided, then install a shunt for later */
		if (st == NULL)
			eroute_installed = shunt_eroute(
				c, sr, RT_ROUTED_PROSPECTIVE, ERO_ADD, "add");
		else
	...
	DBG(DBG_CONTROL, DBG_log("route_and_eroute: firewall_notified: %s",
				 firewall_notified ? "true" : "false"));
	if (!firewall_notified) {
		/* we're in trouble -- don't do routing */
	} else if (ro == NULL) {
		/* a new route: no deletion required, but preparation is */
		(void)do_command(c, sr, "prepare",
				 st); /* just in case; ignore failure */
	...
	if (route_installed) {
		...
		if (st == NULL) {
			...
			DBG(DBG_CONTROL, char cib[CONN_INST_BUF];
			    DBG_log("route_and_eroute: instance \"%s\"%s, setting eroute_owner to #%ld (was #%ld) (newest_ipsec_sa=#%ld)",
				    ...
				    (fmt_conn_instance(st->st_connection, cib),
				     cib),
				    st->st_serialno, sr->eroute_owner,
		...
		if (eroute_installed) {
			...
			if (bspp != NULL) {
				...
				(void)raw_eroute(
					...
					0 /* transport_proto */
					,
					ET_INT, null_proto_info, SHUNT_PATIENCE,
					ERO_REPLACE, "restore",
				...
				if (esr->eroute_owner == SOS_NOBODY) {
                     ...
                     */
					struct state *ost = state_with_serialno(
						esr->eroute_owner);
				...
				if (st == NULL)
					(void)shunt_eroute(c, sr, sr->routing,
							   ERO_DELETE,
							   "delete");
				else
					(void)sag_eroute(st, sr, ERO_DELETE,
							 "delete");
			}
		}
...
bool install_ipsec_sa(struct state *parent_st, struct state *st,
		      bool inbound_also USED_BY_KLIPS)
{
	...
	DBG(DBG_CONTROL,
	    DBG_log("state #%ld: install_ipsec_sa() for %s", st->st_serialno,
	...
	if (!st->st_outbound_done
		...
		if (!setup_half_ipsec_sa(parent_st, st, sr, FALSE)) {
			loglog(RC_LOG_SERIOUS,
			       "state #%lu: failed to setup outgoing SA",
			       st->st_serialno);
			return FALSE;
		}
		DBG(DBG_KLIPS,
		    DBG_log("state #%lu: set up outgoing SA, ref=%u/%u",
			    st->st_serialno, st->st_ref, st->st_refhim));
		st->st_outbound_done = TRUE;
	}

	DBG(DBG_KLIPS,
	    DBG_log("state #%lu: now setting up incoming SA", st->st_serialno));
	/* now setup inbound SA */
	if (st->st_ref == IPSEC_SAREF_NULL && inbound_also) {
		if (!setup_half_ipsec_sa(parent_st, st, sr, TRUE)) {
			loglog(RC_LOG_SERIOUS,
			       "state #%lu: failed to setup incoming SA",
			       st->st_serialno);
			return FALSE;
		}
		DBG(DBG_KLIPS,
		    DBG_log("state #%lu: set up incoming SA, ref=%u/%u",
			    st->st_serialno, st->st_ref, st->st_refhim));
	}
	...
	for (; sr != NULL; sr = sr->next) {
		...
		if (sr->eroute_owner != st->st_serialno &&
			...
			if (!route_and_eroute(st->st_connection, sr, orig_sr,
					      st)) {
	...
	if (st->st_connection->remotepeertype == CISCO) {
		...
		if (!st->st_connection->newest_ipsec_sa) {
			if (!do_command(st->st_connection,
					...
					"updateresolvconf", st)) {
				DBG(DBG_CONTROL,
				    DBG_log("Updating resolv.conf failed, you may need to update it manually"));
			}
		}
	}
		     ...
		     bool inbound_only USED_BY_KLIPS)
{
	...
	case USE_NETKEY:
	     ...
	     */
			...
			if (!st->st_connection->loopback) {
#endif
				(void)teardown_half_ipsec_sa(st, &c->spd.that,
							     FALSE);
#ifdef HAVE_LABELED_IPSEC
			}
#endif
		...
		if (st->st_connection->remotepeertype == CISCO &&
			...
			if (!do_command(st->st_connection,
				...
				DBG(DBG_CONTROL,
				    DBG_log("Restoring resolv.conf failed, you may need to do it manually"));
			}
		}
...
bool get_sa_info(struct state *st, bool inbound, time_t *ago)
{
	...
	DBG(DBG_KLIPS, DBG_log("get %s", text_said))
