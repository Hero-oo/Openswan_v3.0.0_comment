 ...
 */
stf_status start_dh_secretiv(struct pluto_crypto_req_cont *cn, struct state *st,
			     ...
			     u_int16_t oakley_group2)
{
	...
	pluto_crypto_copychunk(&dhq->thespace, dhq->space, &dhq->secret,
			       st->st_sec_chunk);
	...
	memcpy(wire_chunk_ptr(dhq, &dhq->rcookie), st->st_rcookie, COOKIE_SIZE);
...
void finish_dh_secretiv(struct state *st, struct pluto_crypto_req *r)
{
...
stf_status start_dh_secret(struct pluto_crypto_req_cont *cn, struct state *st,
			   enum crypto_importance importance,
			   enum phase1_role init, u_int16_t oakley_group2)
{
	struct pluto_crypto_req r;
	struct pcr_skeyid_q *dhq;
	const chunk_t *pss = get_preshared_secret(st->st_connection);
	...
	pluto_crypto_allocchunk(&dhq->thespace, &dhq->icookie, COOKIE_SIZE);
	memcpy(wire_chunk_ptr(&r.pcr_d.dhq, &dhq->icookie), st->st_icookie,
	       COOKIE_SIZE);

	pluto_crypto_allocchunk(&dhq->thespace, &dhq->rcookie, COOKIE_SIZE);
	memcpy(wire_chunk_ptr(&r.pcr_d.dhq, &dhq->rcookie), st->st_rcookie,
	       COOKIE_SIZE);
...
void finish_dh_secret(struct state *st, struct pluto_crypto_req *r)
{
	struct pcr_skeyid_r *dhr = &r->pcr_d.dhr;
 ...
 * invoke helper to do DH work.
 */
stf_status start_dh_v2(struct pluto_crypto_req_cont *cn, struct state *st,
		       enum crypto_importance importance,
		       enum phase1_role init /* TRUE=g_init,FALSE=g_r */
		       ,
		       u_int16_t oakley_group2)
...
void finish_dh_v2(struct state *st, struct pluto_crypto_req *r)
{
