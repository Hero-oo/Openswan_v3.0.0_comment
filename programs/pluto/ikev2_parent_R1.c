...
stf_status ikev2parent_inI1outR1(struct msg_digest *md)
{
	...
	c = find_host_connection(ANY_MATCH, &md->iface->ip_addr,
				 md->iface->port, KH_IPADDR, &md->sender,
				 md->sender_port, POLICY_IKEV2_ALLOW, LEMPTY,
				 &policy_hint);
	if (c == NULL) {
		if (policy_hint & POLICY_IKEV2_ALLOW) {
			...
			openswan_log(
				"connection refused, IKEv2 not authorized");
	...
	if (force_busy == TRUE) {
		...
		if (md->chain[ISAKMP_NEXT_v2KE] && md->chain[ISAKMP_NEXT_v2N] &&
			...
			if (memcmp(blob.ptr, dcookie, SHA1_DIGEST_SIZE) != 0) {
				...
				SEND_V2_NOTIFICATION_DATA(md, st, v2N_COOKIE,
							  &dc);
	...
	if (md->chain[ISAKMP_NEXT_v2N] &&
		...
		const u_int16_t isan_type =
			md->chain[ISAKMP_NEXT_v2N]->payload.v2n.isan_type;
		openswan_log("%s: received %s", from_state_name,
