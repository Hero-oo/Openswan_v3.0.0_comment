...
bool ikev2_out_attr(unsigned int type, unsigned long val,
		    ...
		    enum_names **attr_val_descs USED_BY_DEBUG, pb_stream *pbs)
{
	...
	DBG(
		DBG_EMITTING, if (type < ikev2_trans_attr_val_descs_size &&
...
bool ikev2_out_sa(pb_stream *outs, unsigned int protoid, struct db_sa *sadb,
		  struct state *st, bool parentSA, u_int8_t np)
{
	pb_stream sa_pbs;
	bool ret = FALSE;
	unsigned int pc_cnt;
	...
	for (pc_cnt = 0; pc_cnt < sadb->prop_disj_cnt; pc_cnt++) {
		...
		for (pr_cnt = 0; pr_cnt < vp->prop_cnt; pr_cnt++) {
			...
			if (!vp->props) {
				openswan_log(
					"%s: FATAL: prop_cnt=%d, but props=NULL",
			...
			if (pr_cnt + 1 < vp->prop_cnt ||
			    pc_cnt + 1 < sadb->prop_disj_cnt) {
 ...
 */
bool spdb_v2_match_parent(struct db_sa *sadb, unsigned propnum,
			  unsigned encr_transform, int encr_keylen,
			  unsigned integ_transform, unsigned prf_transform,
			  unsigned dh_transform)
{
	...
	for (pd_cnt = 0; pd_cnt < sadb->prop_disj_cnt; pd_cnt++) {
		 ...
		 */
				...
				if (dh_matched && prf_matched &&
				    integ_matched && encr_matched) {
					if (DBGP(DBG_CONTROL)) {
		 ...
		 *       statement is dangerous */
				DBG_log("proposal %u %6s encr= (policy:%20s[%d] vs offered:%s[%d]) [%u,%u]",
					...
					enum_show(&trans_type_encr_names,
						  encr_transform),
					encr_keylen_policy, pd_cnt, attempt++);
				...
				DBG_log("proposal %u %6s dh=   (policy:%20s vs offered:%s)",
					propnum, dh_matched ? "succ" : "failed",
				...
				for (itl->dh_i = 0;
					...
					if (spdb_v2_match_parent(
						...
						if (winning) {
							...
							if (itl->encr_keylens[itl->encr_i] ==
								    -1 ||
					   ...
					   struct ikev2_transform_list *itl)
{
	...
	while (prop->isap_numtrans-- > 0) {
		...
		if (!in_struct(&trans, &ikev2_trans_desc, prop_pbs, &trans_pbs))
		...
		DBG(DBG_PARSING,
		    DBG_log("collect encr: %u<=%u integ: %u<=%u prf: %u<=%u dh: %u<=%u",
			    encr_x, itl->encr_trans_next, integ_x,
			    itl->integ_trans_next, prf_x, itl->prf_trans_next,
					       ...
					       struct ikev2_prop winning_prop)
{
	...
	if (parentSA) {
		...
		get_ipsec_spi(&st->st_esp, IPPROTO_ESP, st, TRUE /* tunnel */);
	}
	...
	if (!out_struct(&r_proposal, &ikev2_prop_desc, r_sa_pbs,
			&r_proposal_pbs))
		impossible();
...
v2_notification_t ikev2_parse_parent_sa_body(
...
)
{
	...
	if (!st->st_sadb) {
		st->st_sadb =
			alginfo2parent_db2(st->st_connection->alg_info_ike);
	}
	 ...
	 */
		...
		{
			stf_status ret = ikev2_process_transforms(
				&proposal, &proposal_pbs, itl);
			if (ret != STF_OK)
				return ret;
		}
		...
		if (ikev2_match_transform_list_parent(
			    st->st_sadb, proposal.isap_propnum, itl, &ta)) {
     ...
     * winning value.
     */
	ta.encrypter = (struct ike_encr_desc *)ike_alg_ikev2_find(
		IKEv2_TRANS_TYPE_ENCR, ta.encrypt, ta.enckeylen);
	passert(ta.encrypter != NULL);
	if (ta.enckeylen <= 0)
		ta.enckeylen = ta.encrypter->keydeflen;

	ta.integ_hasher = (struct ike_integ_desc *)ike_alg_ikev2_find(
		IKEv2_TRANS_TYPE_INTEG, ta.integ_hash, 0);
	passert(ta.integ_hasher != NULL);

	ta.prf_hasher = (struct ike_prf_desc *)ike_alg_ikev2_find(
		IKEv2_TRANS_TYPE_PRF, ta.prf_hash, 0);
	...
	if (r_sa_pbs != NULL) {
		return ikev2_emit_winning_sa(st, r_sa_pbs, ta,
					     /*parentSA*/ TRUE, winning_prop);
	}
	return NOTHING_WRONG;
}

bool spdb_v2_match_child(struct db_sa *sadb, unsigned propnum,
			 unsigned encr_transform, int encr_keylen,
			 unsigned integ_transform, unsigned esn_transform)
{
	struct db_v2_prop *pd;
	unsigned int pd_cnt;
	...
	if (encr_keylen <= 0)
		...
		encr_keylen = kernel_alg_esp_enc_keylen(encr_transform) *
			      BITS_PER_BYTE;
...
v2_notification_t ikev2_parse_child_sa_body(
	pb_stream *sa_pbs, /* body of input SA Payload */
	const struct ikev2_sa *sa_prop UNUSED, /* header of input SA Payload */
	pb_stream *r_sa_pbs, /* if non-NULL, where to emit winning SA */
	struct state *st, /* current state object */
...
)
{
	 ...
	 */
		...
		case PROTO_IPSEC_ESP:
			if (proposal.isap_spisize == 4) {
				...
				DBG(DBG_PARSING, DBG_log("SPI received: %08x",
							 ntohl(spival)));
		...
		if (ikev2_match_transform_list_child(
			    p2alg, proposal.isap_propnum, itl, &ta)) {
			gotmatch = TRUE;
			winning_prop = proposal;
		}
		...
		if (gotmatch && selection && np == ISAKMP_NEXT_P) {
			openswan_log(
				"More than 1 proposal received from responder, ignoring rest");
			break;
		}
	}
	...
	if (r_sa_pbs != NULL) {
		return ikev2_emit_winning_sa(st, r_sa_pbs, ta,
					     /*parentSA*/ FALSE, winning_prop);
	}

	return NOTHING_WRONG;
}

stf_status ikev2_emit_ipsec_sa(struct msg_digest *md, pb_stream *outpbs,
			       ...
			       lset_t policy UNUSED)
{
	...
	p2alg = alginfo2child_db2(c->alg_info_esp);
	passert(p2alg != NULL);
	ikev2_out_sa(outpbs, proto, p2alg, md->st, FALSE, np);
