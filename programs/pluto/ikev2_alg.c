...
void kernel_alg_show_status(void)
{
	...
	ESP_EALG_FOR_EACH(sadb_id)
	{
		...
		whack_log(RC_COMMENT,
			  "algorithm ESP encrypt: id=%d, name=%s, "
			  "ivlen=%d, keysizemin=%d, keysizemax=%d",
			  id, enum_name(&trans_type_encr_names, alg_p->encr_id),
	...
	ESP_AALG_FOR_EACH(sadb_id)
	{
		...
		whack_log(RC_COMMENT,
			  "algorithm ESP auth attr: id=%d, name=%s, "
...
static const char *pfs_group_from_state(struct state *st)
{
	return st->st_pfs_group ?
		       enum_show(&oakley_group_names, st->st_pfs_group->group) :
		       "phase1";
}
...
void kernel_alg_show_connection(struct connection *c, const char *instance)
{
	...
	if (c->alg_info_esp) {
		...
		whack_log(RC_COMMENT, "\"%s\"%s:   %s algorithms wanted: %s",
			  c->name, instance, satype, buf);
	}

	if (c->alg_info_esp) {
		alg_info_snprint_phase2(buf, sizeof(buf),
					(struct alg_info_esp *)c->alg_info_esp);
		whack_log(RC_COMMENT, "\"%s\"%s:   %s algorithms loaded: %s",
			  c->name, instance, satype, buf);
	}
	...
	if (st && st->st_esp.present)
		whack_log(RC_COMMENT,
			  ...
			  c->policy & POLICY_PFS ? pfs_group_from_state(st) :
						   "nopfs");

	if (st && st->st_ah.present)
		whack_log(RC_COMMENT, "\"%s\"%s:   %s algorithm newest: %s-%s",
			  c->name, instance, satype,
			  enum_show(&trans_type_integ_names,
				    st->st_esp.attrs.transattrs.integ_hash),
			  c->policy & POLICY_PFS ? pfs_group_from_state(st) :
