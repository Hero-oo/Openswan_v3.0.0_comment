...
int createdelete_virtual(int createdelete, char *virtname)
{
	...
	if ((error = pfkey_msg_hdr_build(&extensions[0], createdelete, 0, 0,
					 ++pfkey_seq, getpid()))) {
		fprintf(stderr,
...
int main(int argc, char *argv[])
{
	...
	if (ioctl(s, shc.cf_cmd, &ifr) == -1) {
		switch (shc.cf_cmd) {
		case IPSEC_SET_DEV:
			fprintf(stderr, "%s: Socket ioctl failed on attach -- ",
				progname);
			switch (errno) {
			case EINVAL:
				fprintf(stderr,
		...
		case IPSEC_DEL_DEV:
			fprintf(stderr, "%s: Socket ioctl failed on detach -- ",
				progname);
			switch (errno) {
			case EINVAL:
				fprintf(stderr,
