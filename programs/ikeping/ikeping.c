 ...
 *
 */
static void send_ping(int afamily, int s, ip_address *raddr, int rport)
{
	...
	ih.isa_xchg = (exchange_number ? exchange_number :
	...
	if (sendto(s, &ih, sizeof(ih), 0, (struct sockaddr *)raddr, raddrlen) <
	    0) {
		perror("sendto");
		exit(5);
	}
 ...
 *
 */
static void reply_packet(int afamily, int s, ip_address *dst_addr, int dst_len,
...
static void receive_ping(int afamily, int s, int reply, int natt)
{
	...
	printf("\trcookie=%08x_%08x icookie=%08x_%08x msgid=%08x\n", tmp_ic[0],
	       tmp_ic[1], tmp_rc[0], tmp_rc[1], ih.isa_msgid);
	printf("\tnp=%03d  version=%d.%d    xchg=%s(%d)\n", ih.isa_np,
	       ih.isa_version >> ISA_MAJ_SHIFT, ih.isa_version & ISA_MIN_MASK,
	       xchg_name, ih.isa_xchg);
...
int main(int argc, char **argv)
{
	...
	while ((c = getopt_long(argc, argv, "hVvsp:b:46E:w:", long_opts, 0)) !=
	       EOF) {
		switch (c) {
		case 'h': /* --help */
			help();
		...
		case 'b':
			errstr = ttoaddr(optarg, strlen(optarg), afamily,
					 &laddr);
			if (errstr != NULL) {
				fprintf(stderr,
	...
	case AF_INET:
		...
		if (bind(s, (struct sockaddr *)&laddr.u.v4,
			 sizeof(laddr.u.v4)) < 0) {
			fprintf(stderr,
				"warning, unable to bind v4 socket port %u: %s",
	...
	case AF_INET6:
		...
		if (bind(s, (struct sockaddr *)&laddr.u.v6,
			 sizeof(laddr.u.v6)) < 0) {
			fprintf(stderr,
				"warning, unable to bind v6 socket to port %u: %s",
				lport, strerror(errno));
		}
		break;
	}
	...
	if (!listen_only) {
		while (optind < argc) {
			...
			errstr = ttoaddr(host, strlen(host), afamily, &raddr);
			if (errstr != NULL) {
				fprintf(stderr,
