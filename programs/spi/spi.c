...
enum life_severity { life_soft = 0, life_hard = 1, life_maxsever = 2 };
enum life_type {
	life_alloc = 0,
	life_bytes = 1,
	life_addtime = 2,
	...
	do {
		...
		if (strncmp(optargp, "soft", sizeof("soft") - 1) == 0) {
			...
			fprintf(stderr,
				...
				progname, optargt, optargt, optargp,
				(int)sizeof("soft"));
		...
		if (debug) {
			fprintf(stdout,
				...
				progname, life_severity, optargt, optargt,
				optargp, optargp, (int)sizeof("soft"));
		}
		if (*(optargp++) != '-') {
			fprintf(stderr,
		...
		if (debug) {
			fprintf(stdout,
				...
				progname, optargt, optargt, optargp, optargp,
				(int)strlen(optargt), (int)strlen(optargp),
				strncmp(optargp, "addtime",
					sizeof("addtime") - 1));
		}
		if (strncmp(optargp, "allocations",
			    sizeof("allocations") - 1) == 0) {
			life_type = life_alloc;
			optargp += sizeof("allocations") - 1;
		} else if (strncmp(optargp, "bytes", sizeof("bytes") - 1) ==
			   0) {
			life_type = life_bytes;
			optargp += sizeof("bytes") - 1;
		} else if (strncmp(optargp, "addtime", sizeof("addtime") - 1) ==
			   0) {
			life_type = life_addtime;
			optargp += sizeof("addtime") - 1;
		} else if (strncmp(optargp, "usetime", sizeof("usetime") - 1) ==
			   0) {
			life_type = life_usetime;
			optargp += sizeof("usetime") - 1;
		} else if (strncmp(optargp, "packets", sizeof("packets") - 1) ==
			   0) {
		...
		if (debug) {
			fprintf(stdout,
				...
				progname, optargt, optargt + strlen(optargt),
				optargp, (int)strlen(optargp));
		}
		if (strlen(optargp) == 0) {
			fprintf(stderr,
		...
		if (!((endptr == optargp + strlen(optargp)) ||
		      (endptr == optargp + strcspn(optargp, ", ")))) {
			fprintf(stderr,
				...
				progname, *endptr, (int)(endptr - myoptarg),
				myoptarg, (int)strlen(myoptarg),
...
int pfkey_register(uint8_t satype)
{
	...
	error = pfkey_msg_hdr_build(&extensions[0], SADB_REGISTER, satype, 0,
				    ++pfkey_seq, getpid());
	if (error != 0) {
		fprintf(stderr,
...
int decode_esp(char *algname)
{
	...
	if (!strcmp(algname, "3des-md5-96")) {
		...
		if (alg_info->alg_info_cnt > 1) {
			fprintf(stderr,
				"%s: Invalid encryption algorithm '%s' "
		...
		if (debug) {
			fprintf(stdout,
				...
				progname, alg_info->alg_info_cnt,
				esp_info->encryptalg, esp_info->authalg);
		}
		...
		if (kernel_alg_proc_read() == 0) {
			...
			if (ugh != NULL) {
				fprintf(stderr,
					"%s: ESP encryptalg=%d (\"%s\") "
					"not present - %s\n",
					progname, esp_ealg_id,
...
int main(int argc, char *argv[])
{
	...
	edst_opt = spi_opt = proto_opt = af_opt = said_opt = dst_opt = src_opt =
		NULL;
	{
		int i, j;
	...
	while ((c = getopt_long(argc, argv,
		...
		case 'b': /* set the SAref to use */
			...
			if (!(endptr == optarg + strlen(optarg))) {
				fprintf(stderr,
					"%s: Invalid character in SAREFi parameter: %s\n",
					progname, optarg);
				exit(1);
			}
			argcount--;
		...
		case 'B': /* set the SAref to use for outgoing packets */
			...
			if (!(endptr == optarg + strlen(optarg))) {
				fprintf(stderr,
					"%s: Invalid character in SAREFo parameter: %s\n",
					progname, optarg);
				exit(1);
			}
			argcount--;
		...
		case 'O': /* set interface from which packet should arrive */
			...
			if (!(endptr == optarg + strlen(optarg))) {
				fprintf(stderr,
					"%s: Invalid character in outif parameter: %s\n",
					progname, optarg);
				exit(1);
			}
			argcount--;
		...
		case 'l': {
			char *toolname = malloc(
				strlen(argv[0]) +
				10 /* update this when changing the sprintf() */
				+ strlen(optarg));
			sprintf(toolname, "%s --label %s", argv[0], optarg);
			progname = toolname;
		}
			tool_close_log();
			tool_init_log();
		...
		case 'A':
			if (optarg[0] == '0') {
				...
				default:
					fprintf(stderr,
						"%s: Authentication key must have a '0x', '0t' or '0s' prefix to select the format: %s\n",
						progname, optarg);
					exit(1);
				}
			}
			...
			if (!authkeylen) {
				fprintf(stderr,
					"%s: unknown format or syntax error in authentication key: %s\n",
					progname, optarg);
				exit(1);
			}
			...
			authkeylen =
				atodata(optarg, 0, (char *)authkey, authkeylen);
			break;
		case 'E':
			if (optarg[0] == '0') {
				switch (optarg[1]) {
				...
				default:
					fprintf(stderr,
						"%s: Encryption key must have a '0x', '0t' or '0s' prefix to select the format: %s\n",
						progname, optarg);
					exit(1);
				}
			}
			...
			if (!enckeylen) {
				fprintf(stderr,
					"%s: unknown format or syntax error in encryption key: %s\n",
					progname, optarg);
				exit(1);
			}
			...
			enckeylen =
				atodata(optarg, 0, (char *)enckey, enckeylen);
			break;
		case 'w':
		...
		case 'i':
			if (optarg[0] == '0') {
				...
				default:
					fprintf(stderr,
						"%s: IV must have a '0x', '0t' or '0s' prefix to select the format, found '%c'.\n",
						progname, optarg[1]);
					exit(1);
				}
			}
			...
			if (!ivlen) {
				fprintf(stderr,
					"%s: unknown format or syntax error in IV: %s\n",
					progname, optarg);
				exit(1);
			}
			iv = malloc(ivlen);
		...
		case 'F': /* src port */
			...
			if (!(endptr == optarg + strlen(optarg))) {
				fprintf(stderr,
					"%s: Invalid character in source parameter: %s\n",
					progname, optarg);
				exit(1);
			}
			break;
		...
		case '+': /* optionsfrom */
			...
			break;
		case 'f':
			if (parse_life_options(life, life_opt, optarg) != 0) {
				exit(1);
			};
			break;
		default:
	...
	if (argcount == 1) {
		...
		if ((stat("/proc/net/ipsec_spi", &sts)) != 0) {
			...
			ret = ret != -1 && WIFEXITED(ret) ? WEXITSTATUS(ret) :
							    1;
		}
		exit(ret);
	}
			...
			*/
			...
			if (minbits > keylen || maxbits < keylen) {
				fprintf(stderr,
					...
					progname, (int)keylen, (int)minbits,
					(int)maxbits);
				exit(1);
			}
	...
	if ((error = pfkey_msg_hdr_build(&extensions[0],
					 (alg == XF_DEL ? SADB_DELETE :
					  alg == XF_CLR ? SADB_FLUSH :
							  SADB_ADD),
					 proto2satype(proto), 0, ++pfkey_seq,
					 mypid))) {
		fprintf(stderr,
			"%s: Trouble building message header, error=%d.\n",
			progname, error);
	...
	if (!(alg == XF_CLR /* IE: pfkey_msg->sadb_msg_type == SADB_FLUSH */)) {
		...
		if (saref_me || saref_him) {
			error = pfkey_saref_build(
				&extensions[K_SADB_X_EXT_SAREF], saref_me,
				saref_him);
			if (error) {
				fprintf(stderr,
		...
		if (outif != 0) {
			if ((error = pfkey_outif_build(
				     &extensions[SADB_X_EXT_PLUMBIF], outif))) {
				fprintf(stderr,
		...
		if (life_opt[life_soft][life_alloc] != NULL ||
			...
			if (debug) {
				fprintf(stdout,
					"%s: lifetime_s extension assembled.\n",
					progname);
			}
		}
		...
		if (life_opt[life_hard][life_alloc] != NULL ||
			...
			if (debug) {
				fprintf(stdout,
					"%s: lifetime_h extension assembled.\n",
					progname);
			}
		}
		...
		if ((error = pfkey_address_build(
			     &extensions[SADB_EXT_ADDRESS_SRC],
			     SADB_EXT_ADDRESS_SRC, 0, 0, sockaddrof(&src)))) {
			addrtot(&src, 0, ipaddr_txt, sizeof(ipaddr_txt));
			fprintf(stderr,
		...
		if ((error = pfkey_address_build(
			     ...
			     SADB_EXT_ADDRESS_DST, 0, 0, sockaddrof(&edst)))) {
		...
		if ((error = pfkey_address_build(
			     &extensions[SADB_EXT_ADDRESS_PROXY],
			     SADB_EXT_ADDRESS_PROXY, 0, 0,
			     sockaddrof(&pfkey_address_p_ska)))) {
			fprintf(stderr,
				"%s: Trouble building address_p extension, error=%d.\n",
				progname, error);
			pfkey_extensions_free(extensions);
			exit(1);
		}
		...
		case XF_AHHMACSHA1:
		case XF_ESP3DESSHA196:
			if ((error = pfkey_key_build(
				     &extensions[SADB_EXT_KEY_AUTH],
				     SADB_EXT_KEY_AUTH, authkeylen * 8,
				     authkey))) {
				fprintf(stderr,
...
#endif /* KERNEL_ALG */
			if ((error = pfkey_key_build(
				     &extensions[SADB_EXT_KEY_ENCRYPT],
				     SADB_EXT_KEY_ENCRYPT, enckeylen * 8,
				     enckey))) {
				fprintf(stderr,
		...
		if ((pfkey_ident_build(
			    &extensions[SADB_EXT_IDENTITY_SRC],
			    SADB_EXT_IDENTITY_SRC, SADB_IDENTTYPE_PREFIX, 0,
			    strlen(pfkey_ident_s_ska), pfkey_ident_s_ska))) {
			fprintf(stderr,
				"%s: Trouble building ident_s extension, error=%d.\n",
				progname, error);
			pfkey_extensions_free(extensions);
			exit(1);
		}
		...
		if ((error = pfkey_ident_build(
			     &extensions[SADB_EXT_IDENTITY_DST],
			     SADB_EXT_IDENTITY_DST, SADB_IDENTTYPE_PREFIX, 0,
			     strlen(pfkey_ident_d_ska), pfkey_ident_d_ska))) {
			fprintf(stderr,
				"%s: Trouble building ident_d extension, error=%d.\n",
				progname, error);
			pfkey_extensions_free(extensions);
			exit(1);
		}
	...
	if (natt != 0) {
		...
		err = pfkey_x_nat_t_type_build(
			&extensions[K_SADB_X_EXT_NAT_T_TYPE], natt);
		success = pfkey_build(err, "pfkey_nat_t_type Add ESP SA",
		...
		if (sport != 0) {
			err = pfkey_x_nat_t_port_build(
				...
				K_SADB_X_EXT_NAT_T_SPORT, sport);
		...
		if (dport != 0) {
			err = pfkey_x_nat_t_port_build(
				...
				K_SADB_X_EXT_NAT_T_DPORT, dport);
			success = pfkey_build(err,
	...
	io_error = write(pfkey_sock, pfkey_msg,
			 pfkey_msg->sadb_msg_len * IPSEC_PFKEYv2_ALIGN);
	if (io_error < 0) {
		fprintf(stderr, "%s: pfkey write failed (errno=%d): ", progname,
			errno);
		pfkey_extensions_free(extensions);
		pfkey_msg_free(&pfkey_msg);
		switch (errno) {
		...
		while ((readlen = read(pfkey_sock, pfkey_buf,
			...
			if (debug) {
				printf("%s: pfkey v%d msg received. type=%d(%s) seq=%d len=%d pid=%d errno=%d satype=%d(%s)\n",
				       progname, pfkey_msg->sadb_msg_version,
			...
			if (readlen != (ssize_t)(pfkey_msg->sadb_msg_len *
						 IPSEC_PFKEYv2_ALIGN)) {
				if (debug) {
			...
			if ((pid_t)pfkey_msg->sadb_msg_pid == mypid) {
				if (saref_me || dumpsaref) {
					struct sadb_x_saref *s =
						(struct sadb_x_saref *)extensions
							[K_SADB_X_EXT_SAREF];
					if (s) {
						printf("%s: saref=%d/%d\n",
						       progname,
