...
static int open_udp_sock(const unsigned short port)
{
...
static int open_tcp_sock(const unsigned short port UNUSED)
{
	return -1;
}

static int udp_recv_loop(int udpsock)
{
	int packetcount = 0;
	int err;
	...
	do {
		...
		packetcount++;
		 ...
		 (maxpacketcount == 0 || packetcount < maxpacketcount));

	return 0;
}

static void dump_policyreply(struct ipsec_policy_cmd_query *q)
{
	...
	if (q->credential_count > 0) {
		...
		for (c = 0; c < q->credential_count; c++) {
			...
			default:
				printf("\tUnknown identity type %d",
				       q->credentials[c].ii_format);
				break;
			}
		}
	}
...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt_long(argc, argv, "hVighc:v:p:s:", long_opts, 0)) !=
	       EOF) {
		switch (c) {
		default:
		case 'h': /* --help */
