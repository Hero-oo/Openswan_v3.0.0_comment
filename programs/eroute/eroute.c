...
static struct option const longopts[] = { { "dst", 1, 0, 'D' },
					  { "src", 1, 0, 'S' },
					  { "eraf", 1, 0, 'f' },
					  { "add", 0, 0, 'a' },
					  { "addin", 0, 0, 'A' },
...
int main(int argc, char **argv)
{
	...
	while ((c = getopt_long(argc, argv, "" /*"acdD:e:i:hprs:S:f:vl:+:g"*/,
				longopts, 0)) != EOF) {
		switch (c) {
		case 'g':
			debug = 1;
		...
		case 'P':
			if (transport_proto_opt) {
				fprintf(stderr,
					"%s: Error, --transport-proto"
					" parameter redefined:%s, "
					"already defined as:%s\n",
					progname, optarg, transport_proto_opt);
				exit(1);
			}
	...
	case EMT_INREPLACEROUTE:
		if (!(said_af_opt && edst_opt && spi_opt && proto_opt) &&
		    !(said_opt)) {
			fprintf(stderr,
...
sa_build:
	...
	case EMT_DELEROUTE:
		...
		if ((error = pfkey_address_build(
			     ...
			     SADB_X_EXT_ADDRESS_SRC_FLOW, 0, 0,
		...
		if ((error = pfkey_address_build(
			     ...
			     SADB_X_EXT_ADDRESS_DST_FLOW, 0, 0,
		...
		if ((error = pfkey_address_build(
			     ...
			     SADB_X_EXT_ADDRESS_SRC_MASK, 0, 0,
		...
		if ((error = pfkey_address_build(
			     ...
			     SADB_X_EXT_ADDRESS_DST_MASK, 0, 0,
	...
	if (transport_proto != 0) {
		if ((error = pfkey_x_protocol_build(
			     &extensions[SADB_X_EXT_PROTOCOL],
			     transport_proto))) {
			fprintf(stderr,
				"%s: Trouble building transport"
	...
	if ((error = write(pfkey_sock, pfkey_msg,
